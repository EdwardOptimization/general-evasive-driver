...
Table~\ref{tab:levers} contrasts the candidate fixes against the single-head baseline (drift
...
\begin{table}[t]
\centering
\caption{Lever elimination vs.\ the single-head baseline (8-seed unless noted). Each row is a
full confirmatory run that differs only in the named intervention.}
\label{tab:levers}
\begin{tabular}{lccc}
\toprule
intervention & drift & avoidance & verdict \\
\midrule
single-head baseline       & $0.769$ & $0.775$ & --- \\
input-Jacobian penalty     & $0.731$ & $0.713$ & null/negative \\
gradient surgery (PCGrad)  & $0.556$ & $0.863$ & rebalances, cannot expand \\
\textbf{gated dual heads}  & $\mathbf{0.925}$ & $0.758$ & \textbf{expands frontier} \\
\quad gated, 16 seeds      & $0.856$ & $0.700$ & robust (canonical) \\
\bottomrule
\end{tabular}
\end{table}
...
each $\geq 0.75$). The single-head policy reaches this for roughly a third of seeds; the gated
policy reaches it for half ($8/16$). The remaining seeds either cannot drift or collapse avoidance
when drift is high---a genuine tradeoff that gated heads reduce but do not eliminate.
