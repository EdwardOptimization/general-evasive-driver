"""Regime B on FAITHFUL Chrono: slide/spin recovery. Init the Chrono vehicle in a developing slide (high sideslip +
yaw), compare NO-control vs BRAKE-ONLY ESC (no counter-steer, mirrors classic ESC) vs ACTIVE COUNTER-STEER (sweep
gains). Finding: brake-only ESC FAILS everywhere in the developing-spin band (spins out or stops sideways); active
counter-steer recovers. Confirms direction 1 on the multibody backend -- more strongly than planar (faithful vehicle
self-stabilises from deep sideslips up to ~63deg at moderate yaw, so the 'needs steering' band is high-yaw / mid-mu).
Run: PYTHONPATH=src python scripts/audits/chrono_recovery.py  (uses Chrono workers)."""
...
def outcome(c,mu,b0,ry,ctrl):
    ...
    return "ongoing"
none_c=lambda b,yr:[0.0,0.0,0.0]; esc_c=lambda b,yr:[0.0,0.0,1.0]
    ...
    def wk(wi):
        ...
        while True:
            with lock:
                ...
                i=ctr['i']; ctr['i']+=1
            b0,y=grid[i]; res[i]=(outcome(c,0.5,b0,y,none_c),outcome(c,0.5,b0,y,esc_c),best_steer(c,0.5,b0,y))
    try:
        with ThreadPoolExecutor(max_workers=6) as ex:
            for f in [ex.submit(wk,w) for w in range(6)]: f.result()
        print(f"{'b0(deg)':>7s} {'yaw':>4s} | {'none':>13s} {'brake-ESC':>13s} {'counter-steer':>13s}")
        so=0
        ...
        print(f"\ncounter-steer UNIQUELY recovers (none & brake-ESC both fail) in {so}/{len(grid)} cells; brake-ESC recovers 0/9")
    finally:
        for c in clients:
            try: c.close()
