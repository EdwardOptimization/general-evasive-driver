...
| `chrono_native_oracle_pricing.py` | `experiments/feasibility_audit/chrono_native_oracle_pricing_prereg.json`, `chrono_native_oracle_pricing_quick.json`; `runs/feasibility_audit/chrono_native_oracle_pricing/candidate_rows_quick.csv`, `progress_quick.jsonl` | `docs/m3230-d1b-chrono-native-oracle-pricing-smoke.md` | Can the D1b native Chrono oracle-search protocol execute on the preregistered Sedan/BMW variants before a full managed pricing run? Yes: quick gates passed in 276.6 s, both variants exercised v4_pertuned plus structured and CEM native-search candidates; quick mode gives no D1b direction verdict and does not satisfy CP-2. |
