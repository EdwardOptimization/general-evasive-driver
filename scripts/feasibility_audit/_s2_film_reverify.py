...
ck=torch.load(RUN/'distill_3vehicle_film_policy.pt', map_location='cpu')
...
clients=[ChronoWorkerClient(stderr_log=RUN/f'_film_rev_w{w}_stderr.log', read_timeout_s=600.0) for w in range(12)]
try:
...
print('\n=== RE-VERIFY FiLM driver per-(vehicle,regime) ===')
