"""Open-loop divergence gate for the EXACT-TMeasy physics model (faithful rewrite).

Identical gate to ``surrogate_physics_gate.py`` (beta-div @24 p90/mean, vx RMSE on the SAME
held-out split of ``surrogate_drift_data.npz``), but it replays the saved Chrono drift
actions through ``autodrift.gpu_physics_tmeasy`` — the model whose tyre is Chrono's EXACT
tabulated TMeasy force curves (extracted by ``extract_chrono_tmeasy_curves.py``) with the
grip fudge factors set to 1.0 and NO Pacejka calibration.

It prints the exact-TMeasy numbers alongside the published baselines:
    ...
    EXACT-TMeasy (no fudge)  : measured here

Usage:
    python scripts/feasibility_audit/surrogate_physics_tmeasy_gate.py
    python scripts/feasibility_audit/surrogate_physics_tmeasy_gate.py --calibrate-nontyre
...
from autodrift.gpu_physics_tmeasy import (  # noqa: E402
...
def _report(Vt, sur, label, quiet=False):
    ...
    if not quiet:
        print("[%-18s] beta_div @24: mean=%.4f p90=%.4f | max-over-traj mean=%.4f | vx_rmse=%.3f" % (
...
def gate_physics(A, V, init, mu, phys: PhysParams, label="EXACT-TMeasy", quiet=False):
    R, T, _ = A.shape
...
def main():
    ...
    print("=== EXACT-TMeasy model, NO fudge (held-out split) ===")
    p90, bmean, vr = gate_physics(A[va], V[va], init[va], mu, phys, label="EXACT-TMeasy(nofudge)")
    print("\nGATE wants p90 beta_div@24 <= 0.03. EXACT-TMeasy p90 = %.4f -> %s" % (
        p90, "PASS" if p90 <= 0.03 else "above 0.03"))
    print("vs calibrated-Pacejka 0.0435: %s" % (
        "BEATS" if p90 < 0.0435 else "worse"))
    ...
    if args.calibrate_nontyre:
        ...
        print("=== EXACT-TMeasy + non-tyre calibration (held-out split) ===")
        gate_physics(A[va], V[va], init[va], mu, ph, label="EXACT-TMeasy(+nontyre)")
