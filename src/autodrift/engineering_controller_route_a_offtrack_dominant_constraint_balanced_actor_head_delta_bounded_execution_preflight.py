"""Run M2960 actor-head delta bounded diagnostic execution preflight.

M2960 consumes the M2956/M2957/M2958/M2959 actor-head-delta admission chain and
runs one bounded current-sim diagnostic rollout, or records one failure row, for
each M2956 admitted row. The rollout loads parent checkpoints read-only and
wraps them with a zero-residual actor-head delta adapter so the actor-head delta
contract is executed without claiming a trained repair effect.
"""
...
MILESTONE_ID = (
    "m2960-engineering-controller-route-a-offtrack-dominant-constraint-balanced-"
    "actor-head-delta-bounded-execution-preflight"
)
NEXT_ID = (
    "m2961-engineering-controller-route-a-offtrack-dominant-constraint-balanced-"
    "actor-head-delta-bounded-execution-result-audit"
)
DEFAULT_M2956_DIR = Path(
...
DEFAULT_M2957_AUDIT = Path(
    "docs/m2957-engineering-controller-route-a-offtrack-dominant-constraint-balanced-actor-head-delta-"
    "execution-admission-materialization-result-audit.md"
)
DEFAULT_M2958_SYNTHESIS = Path(
    "docs/m2958-engineering-controller-route-a-offtrack-dominant-constraint-balanced-actor-head-delta-"
    "admission-branch-synthesis.md"
)
DEFAULT_M2959_DESIGN = Path(
    "docs/m2959-engineering-controller-route-a-offtrack-dominant-constraint-balanced-actor-head-delta-"
    "bounded-execution-design.md"
)
...
DEFAULT_DOC_PATH = Path(
    "docs/m2960-engineering-controller-route-a-offtrack-dominant-constraint-balanced-actor-head-delta-"
    "bounded-execution-preflight.md"
)
DEFAULT_FOLLOW_UP_MANIFEST = Path(
    "experiments/manifests/m2961-engineering-controller-route-a-offtrack-dominant-constraint-balanced-"
    "actor-head-delta-bounded-execution-result-audit.json"
)
DEFAULT_EVAL_SEED_BASE = 296000
DEFAULT_RESIDUAL_LIMIT = 0.05
...
EXPECTED_TOTAL_SOURCE_COUNT = 67
...
CLAIM_SCOPE = (
    "M2960 Route A actor-head delta bounded diagnostic execution preflight only; "
    "read-only parent checkpoint loading, zero-residual actor-head delta wrapper "
    "execution, reset, step, rollout, and policy action may be recorded for the "
    "56 M2956 admitted rows, while 11 stale fixed-source rows remain guardrails. "
    "No replay, validation, training, PPO, dependency execution, checkpoint "
    "mutation, checkpoint save, ranking, winner selection, promotion, repair "
    "success, driver-performance, paper, finite-window-vs-GRU, current-sim "
    "verdict, high-fidelity validation, full ideal driver, or self-ID claim is "
    "made"
)
FORBIDDEN_INTERPRETATION = (
    "repair success, implementation readiness, driver performance, validation "
    "readiness or result, controller/source/task/profile/checkpoint/candidate "
    ...
    "high-fidelity validation readiness or result, full ideal driver completion, "
...
CANDIDATE_FIELDNAMES = [
    ...
    "parent_checkpoint_exists",
    "parent_profile_config_exists",
    "claim_boundary",
]
...
CONTRACT_EXECUTION_FIELDNAMES = [
    "actor_head_delta_contract_execution_id",
    "resolution_id",
    "execution_candidate_id",
    "actor_head_delta_candidate_id",
    "source_execution_admission_candidate_id",
...
EXECUTION_FIELDNAMES = [
    ...
    "source_family",
    "source_row_id",
    "actor_head_delta_bounded_execution_preflight",
    "zero_residual_identity_mode",
    "residual_delta_abs_max",
    "residual_trace_count",
...
REQUIRED_ARTIFACT_KEYS = [
    ...
    "actor_head_delta_contract_execution_rows",
...
class _ZeroResidualHead(nn.Module):
    ...
    def forward(self, observation: torch.Tensor) -> torch.Tensor:
        return torch.zeros(observation.shape[:-1] + (self.action_dim,), dtype=observation.dtype, device=observation.device)


class ZeroResidualActorHeadDeltaAdapter:
    """ActorPolicy-compatible wrapper that executes the actor-head delta contract."""

    actor_head_delta_contract_mode = "zero_residual_identity"
    ...
    def __init__(
        ...
        *,
        ...
        self._scaffold = ConstraintBalancedActorHeadDeltaScaffold(
            self._parent_echo,
            _ZeroResidualHead(self.act_dim),
...
def run_actor_head_delta_bounded_execution_preflight(
    *,
    m2956_dir: Path | str = DEFAULT_M2956_DIR,
    m2957_audit: Path | str = DEFAULT_M2957_AUDIT,
    m2958_synthesis: Path | str = DEFAULT_M2958_SYNTHESIS,
    m2959_design: Path | str = DEFAULT_M2959_DESIGN,
    ...
    source = load_source_artifacts(
        m2956_dir=Path(m2956_dir),
        m2957_audit=Path(m2957_audit),
        m2958_synthesis=Path(m2958_synthesis),
        m2959_design=Path(m2959_design),
        executable_workload=Path(executable_workload),
        follow_up_manifest=Path(follow_up_manifest),
    )

    candidate_rows = build_execution_candidate_rows(source["m2956_candidate_rows"])
    ...
    guardrail_rows = build_guardrail_context_rows(
        source["m2956_source_guardrail_rows"],
        source["m2956_rejection_rows"],
    )
    ...
    contract_rows = artifact_rows["actor_head_delta_contract_execution_rows"]
    ...
    claim_rows = build_claim_boundary_rows(
        ...
        contract_rows_present=bool(contract_rows),
    )
...
def artifact_paths(output_dir: Path, *, doc_path: Path) -> dict[str, Path]:
    return {
        ...
        "actor_head_delta_contract_execution_rows": output_dir / "actor_head_delta_contract_execution_rows.csv",
...
def load_source_artifacts(
    *,
    ...
    paths = {
        ...
        "m2956_summary": m2956_dir / "summary.json",
        "m2956_candidate_rows": m2956_dir / "actor_head_delta_execution_admission_candidate_rows.csv",
        "m2956_rejection_rows": m2956_dir / "actor_head_delta_execution_admission_rejection_rows.csv",
        "m2956_source_guardrail_rows": m2956_dir / "source_guardrail_rows.csv",
        "m2956_actor_delta_contract_guard_rows": m2956_dir / "actor_delta_contract_guard_rows.csv",
        "m2956_claim_boundary_rows": m2956_dir / "claim_boundary_rows.csv",
        "m2956_gate_matrix": m2956_dir / "gate_matrix.csv",
    ...
    return {
        ...
        "m2957_audit_text": paths["m2957_audit"].read_text(encoding="utf-8")
        if source_exists["m2957_audit"]
        else "",
        "m2958_synthesis_text": paths["m2958_synthesis"].read_text(encoding="utf-8")
        if source_exists["m2958_synthesis"]
        else "",
        "m2959_design_text": paths["m2959_design"].read_text(encoding="utf-8")
        if source_exists["m2959_design"]
        else "",
        "m2956_summary": read_json(paths["m2956_summary"]) if source_exists["m2956_summary"] else {},
        "m2956_candidate_rows": read_csv_rows(paths["m2956_candidate_rows"]),
        "m2956_rejection_rows": read_csv_rows(paths["m2956_rejection_rows"]),
        "m2956_source_guardrail_rows": read_csv_rows(paths["m2956_source_guardrail_rows"]),
        "m2956_actor_delta_contract_guard_rows": read_csv_rows(paths["m2956_actor_delta_contract_guard_rows"]),
        "m2956_claim_boundary_rows": read_csv_rows(paths["m2956_claim_boundary_rows"]),
        "m2956_gate_matrix": read_csv_rows(paths["m2956_gate_matrix"]),
...
def build_execution_candidate_rows(candidate_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    admitted = [row for row in candidate_rows if str(row.get("execution_admission_status", "")) == ADMITTED_STATUS]
        ...
        candidate.update(
            {
                ...
                "parent_profile_config_path": row.get("parent_profile_config_path", ""),
                "execution_scheduled_in_m2960": True,
                ...
                "checkpoint_load_scheduled": True,
                ...
                "evaluator_labels_actor_visible": _bool(row.get("evaluator_labels_actor_visible", False)),
                ...
                "parent_checkpoint_exists": Path(str(row.get("parent_checkpoint_path", ""))).exists(),
                "parent_profile_config_exists": Path(str(row.get("parent_profile_config_path", ""))).exists(),
...
def build_resolution_rows(
    ...
    for index, candidate in enumerate(candidate_rows, start=1):
        ...
        failure_reason = ""
        if str(candidate.get("execution_admission_status", "")) != ADMITTED_STATUS:
            failure_reason = "m2956_candidate_not_admitted"
        ...
        elif candidate_contract_violation(candidate):
            failure_reason = "actor_or_claim_contract_violation"
        ...
        elif not _bool(candidate.get("parent_checkpoint_exists", False)):
            failure_reason = "parent_checkpoint_path_missing"
        elif not _bool(candidate.get("parent_profile_config_exists", False)):
            failure_reason = "parent_profile_config_path_missing"
        ...
        if execution_admitted:
            resolved_workload.update(
                {
                    "checkpoint_path": candidate.get("parent_checkpoint_path", ""),
                    "profile_config_path": candidate.get("parent_profile_config_path", ""),
                    "config_exists": True,
        ...
        row = {
            ...
            "actor_head_delta_candidate_id": candidate.get("actor_head_delta_candidate_id", ""),
            "source_execution_admission_candidate_id": candidate.get("source_execution_admission_candidate_id", ""),
            ...
            "parent_checkpoint_path": candidate.get("parent_checkpoint_path", ""),
            "parent_profile_config_path": candidate.get("parent_profile_config_path", ""),
...
def run_candidate_execution(
    *,
    ...
    if not resume:
        for name in (
            "bounded_execution_rows.csv",
            "bounded_execution_failure_rows.csv",
    ...
    profile_cache: dict[tuple[str, str, str], tuple[dict[str, Any], ZeroResidualActorHeadDeltaAdapter, dict[str, str]]] = {}
    ...
    for index, resolution in enumerate(resolution_rows):
        ...
        adapter: ZeroResidualActorHeadDeltaAdapter | None = None
        try:
            ...
            if cache_key not in profile_cache:
                ...
                parent_model, _checkpoint = load_actor_critic_checkpoint(checkpoint_path, device=device)
                adapter = ZeroResidualActorHeadDeltaAdapter(parent_model, residual_limit=float(residual_limit))
            ...
            before_trace_count = adapter.trace_count
            ...
            row.update(execution_metadata(resolution, eval_seed=eval_seed, adapter=adapter))
            episode_rows.append(row)
            contract_rows.append(
                contract_execution_row(
                    resolution,
                    adapter=adapter,
                    execution_row_present=True,
                    failure_row_present=False,
                    error_type="",
                    error_message="",
                    contract_execution_index=index + 1,
                    trace_count_delta=adapter.trace_count - before_trace_count,
                )
            )
            ...
            contract_rows.append(
                contract_execution_row(
                    resolution,
                    adapter=adapter,
                    execution_row_present=False,
                    failure_row_present=True,
                    error_type=type(exc).__name__,
                    error_message=str(exc),
                    contract_execution_index=index + 1,
                    trace_count_delta=0,
        ...
        write_run_state(
            output_dir / "run_state.json",
            {
                ...
                "contract_execution_count": len(contract_rows),
    ...
    write_csv_rows(
        output_dir / "actor_head_delta_contract_execution_rows.csv",
        contract_rows,
        fieldnames=CONTRACT_EXECUTION_FIELDNAMES,
    )
    ...
    status_pass = bool(
        ...
        and len(contract_rows) == len(resolution_rows)
        and bool(episode_rows)
        and all_metrics_finite
        and all(_bool(row.get("status_pass", False)) for row in contract_rows)
        and not any(forbidden_execution_flag(row) for row in episode_rows + failure_rows + contract_rows)
    )
    write_run_state(
        output_dir / "run_state.json",
        {
    ...
    return {
        "result_class": (
            "engineering_controller_route_a_offtrack_dominant_actor_head_delta_bounded_execution_preflight_pass"
            if status_pass
            else "engineering_controller_route_a_offtrack_dominant_actor_head_delta_bounded_execution_preflight_incomplete_or_fail"
        ),
...
def execution_metadata(
    ...
    *,
    ...
    contract = adapter.contract_summary()
    return {
        "m2960_eval_seed": int(eval_seed),
        "resolution_id": resolution.get("resolution_id", ""),
        "execution_candidate_id": resolution.get("execution_candidate_id", ""),
        "actor_head_delta_candidate_id": resolution.get("actor_head_delta_candidate_id", ""),
        "source_execution_admission_candidate_id": resolution.get("source_execution_admission_candidate_id", ""),
        ...
        "parent_checkpoint_path": resolution.get("parent_checkpoint_path", ""),
        "parent_profile_config_path": resolution.get("parent_profile_config_path", ""),
        "actor_head_delta_bounded_execution_preflight": True,
        "zero_residual_identity_mode": True,
        "residual_delta_abs_max": contract["residual_delta_abs_max"],
        "residual_trace_count": contract["residual_trace_count"],
...
def contract_execution_row(
    resolution: Mapping[str, Any],
    *,
    adapter: ZeroResidualActorHeadDeltaAdapter | None,
    execution_row_present: bool,
    failure_row_present: bool,
    error_type: str,
    error_message: str,
    contract_execution_index: int,
    trace_count_delta: int,
) -> dict[str, Any]:
    summary = adapter.contract_summary() if adapter is not None else {}
    residual_limit = float(summary.get("residual_limit", DEFAULT_RESIDUAL_LIMIT))
    residual_abs_max = float(summary.get("residual_delta_abs_max", 0.0))
    status_pass = bool(
        adapter is not None
        and _bool(summary.get("wrapper_instantiated", False))
        and _bool(summary.get("parent_checkpoint_loaded_read_only", False))
        and _bool(summary.get("zero_residual_identity_mode", False))
        and residual_abs_max <= residual_limit + 1e-9
        and int(trace_count_delta) > 0
        and not failure_row_present
    )
    return {
        "actor_head_delta_contract_execution_id": f"m2960-actor-head-delta-contract-execution-{contract_execution_index:04d}",
        "resolution_id": resolution.get("resolution_id", ""),
        "execution_candidate_id": resolution.get("execution_candidate_id", ""),
        "actor_head_delta_candidate_id": resolution.get("actor_head_delta_candidate_id", ""),
        "source_execution_admission_candidate_id": resolution.get("source_execution_admission_candidate_id", ""),
        ...
        "parent_checkpoint_path": resolution.get("parent_checkpoint_path", ""),
        "parent_profile_config_path": resolution.get("parent_profile_config_path", ""),
        "wrapper_instantiated": bool(summary.get("wrapper_instantiated", False)),
        "parent_checkpoint_loaded_read_only": bool(summary.get("parent_checkpoint_loaded_read_only", False)),
        "zero_residual_identity_mode": bool(summary.get("zero_residual_identity_mode", False)),
        "residual_limit": residual_limit,
        "residual_delta_norm_max": summary.get("residual_delta_norm_max", ""),
        "residual_delta_abs_max": residual_abs_max,
        "residual_trace_count": int(trace_count_delta),
        "parent_action_abs_max": summary.get("parent_action_abs_max", ""),
        "bounded_action_abs_max": summary.get("bounded_action_abs_max", ""),
        "action_low": summary.get("action_low", ""),
        "action_high": summary.get("action_high", ""),
        ...
        "ranking_run": False,
        "checkpoint_promoted": False,
        "execution_row_present": bool(execution_row_present),
        "failure_row_present": bool(failure_row_present),
...
def build_aggregate_rows(
    *,
    ...
    for index, value in enumerate(values, start=1):
        ...
        rows.append(
            {
                "aggregate_id": f"m2960-{aggregate_family}-aggregate-{index:04d}",
...
def build_guardrail_context_rows(
    source_guardrail_rows: list[dict[str, str]],
    rejection_rows: list[dict[str, str]],
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for index, row in enumerate(source_guardrail_rows, start=1):
        rows.append(
            {
                "guardrail_context_id": f"m2960-guardrail-context-{index:04d}",
    ...
    for index, row in enumerate(rejection_rows, start=1):
        rows.append(
            {
                "guardrail_context_id": f"m2960-guardrail-context-{offset + index:04d}",
                "guardrail_source": "m2956_actor_head_delta_execution_admission_rejection_rows",
                "guardrail_family": row.get("rejection_type", ""),
                "source_milestone": row.get("source_milestone", ""),
                "source_row_id": row.get("source_row_id", "") or row.get("source_execution_admission_candidate_id", ""),
...
def build_actor_contract_guard_rows(
    *,
    ...
    combined = candidate_rows + resolution_rows + episode_rows + failure_rows + guardrail_rows + contract_rows
    return [
        actor_guard("actor_shape", "observation_dim", P0_OBSERVATION_DIM, 72),
        actor_guard("actor_shape", "action_dim", ACTION_DIM, 3),
        actor_guard("actor_shape", "deployed_action_mapping", "[steer, throttle, brake]", "[steer, throttle, brake]"),
        actor_guard("delta_wrapper", "contract_execution_rows", len(contract_rows), len(resolution_rows)),
        actor_guard("delta_wrapper", "zero_residual_identity_mode", all_flag(contract_rows, "zero_residual_identity_mode"), True),
        actor_guard("delta_wrapper", "residual_delta_abs_max_within_limit", residuals_within_limit(contract_rows), True),
        actor_guard("delta_wrapper", "contract_execution_rows_pass", all_flag(contract_rows, "status_pass"), True),
        actor_guard("checkpoint", "parent_checkpoint_loaded_read_only", all_flag(contract_rows, "parent_checkpoint_loaded_read_only"), True),
        actor_guard("checkpoint", "checkpoint_save_scheduled", any_flag(combined, "checkpoint_save_scheduled"), False),
        actor_guard("checkpoint", "checkpoint_mutation_scheduled", any_flag(combined, "checkpoint_mutation_scheduled"), False),
        actor_guard("contract", "actor_input_contract_changed", any_flag(combined, "actor_input_contract_changed"), False),
        actor_guard("contract", "hidden_oracle_actor_input_required", any_flag(combined, "hidden_oracle_actor_input_required"), False),
        actor_guard("contract", "future_target_actor_input_required", any_flag(combined, "future_target_actor_input_required"), False),
        actor_guard("contract", "route_labels_actor_visible", any_flag(combined, "route_labels_actor_visible"), False),
        actor_guard("contract", "source_labels_actor_visible", any_flag(combined, "source_labels_actor_visible"), False),
        actor_guard("contract", "evaluator_labels_actor_visible", any_flag(combined, "evaluator_labels_actor_visible"), False),
        actor_guard("contract", "diagnostic_labels_actor_visible", any_flag(combined, "diagnostic_labels_actor_visible"), False),
        actor_guard("contract", "success_progress_labels_actor_visible", any_flag(combined, "success_progress_labels_actor_visible"), False),
        actor_guard("contract", "verdict_labels_actor_visible", any_flag(combined, "verdict_labels_actor_visible"), False),
        actor_guard("guardrail", "blocked_stale_guard_execution", any_flag(combined, "blocked_stale_guard_execution"), False),
        actor_guard("guardrail", "guardrail_rows_in_success_denominator", any_flag(combined, "guardrail_rows_in_success_denominator"), False),
        actor_guard("execution", "profile_specific_tuning", any_flag(combined, "profile_specific_tuning"), False),
        actor_guard("execution", "active_config_overwritten", any_flag(combined, "active_config_overwritten"), False),
        actor_guard("execution", "dependency_execution_performed", any_flag(combined, "dependency_execution_performed"), False),
        actor_guard("execution", "ranking_run", any_flag(combined, "ranking_run"), False),
    ]


def actor_guard(family: str, field: str, observed: Any, expected: Any) -> dict[str, Any]:
    return {
        "guard_id": f"m2960-actor-guard-{field}",
...
def build_claim_boundary_rows(
    *,
    ...
    allowed = [
        ...
        ("zero_residual_actor_head_delta_wrapper_execution", "execution", contract_rows_present, "contract execution rows"),
        ...
        ("contract_execution_rows_materialized", "artifact", artifacts_present, "actor_head_delta_contract_execution_rows.csv"),
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M2961 audit manifest"),
    ]
    blocked = [
        ("blocked_stale_guard_execution", "execution", "11 stale fixed-source rows remain guardrail only"),
        ...
        ("checkpoint_save_or_mutation", "checkpoint", "future training or promotion route"),
        ("controller_source_task_profile_checkpoint_candidate_ranking", "ranking", "future audited comparison route"),
        ...
        ("implementation_readiness", "verdict", "future implementation audit"),
...
def claim(claim_id: str, family: str, allowed: bool, made: bool, evidence: str) -> dict[str, Any]:
    return {
        "claim_id": f"m2960_{claim_id}",
        "claim_family": family,
        "allowed_in_m2960": allowed,
        "claim_made": made,
...
def build_gate_matrix_rows(
    *,
    ...
    failure_rows = artifact_rows["bounded_execution_failure_rows"]
    contract_rows = artifact_rows["actor_head_delta_contract_execution_rows"]
    source_status_counts = Counter(str(row.get("execution_admission_status", "")) for row in source["m2956_candidate_rows"])
    admitted_source_counts = Counter(str(row.get("source_milestone", "")) for row in candidate_rows)
    blocked_rejection_count = sum(
        str(row.get("source_milestone", "")) == "m2877"
        and str(row.get("rejection_type", "")) == BLOCKED_STALE_STATUS
        for row in source["m2956_rejection_rows"]
    )
    accounted_ids = {
    ...
    allowed_claims = [row for row in claim_rows if _bool(row["allowed_in_m2960"])]
    blocked_claims = [row for row in claim_rows if not _bool(row["allowed_in_m2960"])]
    gates = [
        (
            ...
            "M2956/M2957/M2958/M2959/M1690/follow-up artifacts present",
            "lineage_invalid",
        ),
        (
            "m2957_accepts_m2956",
            "lineage",
            "accepts M2956" in source["m2957_audit_text"],
            "accepts M2956" in source["m2957_audit_text"],
            True,
            "lineage_invalid",
        ),
        (
            "m2958_routes_to_m2959",
            "lineage",
            "M2958 synthesizes" in source["m2958_synthesis_text"],
            "M2958 synthesizes" in source["m2958_synthesis_text"],
            True,
            "lineage_invalid",
        ),
        (
            "m2959_admits_m2960",
            "lineage",
            MILESTONE_ID in source["m2959_design_text"],
            MILESTONE_ID in source["m2959_design_text"],
            True,
            "lineage_invalid",
        ),
        (
            "m2956_status_pass",
            "lineage",
            _bool(source["m2956_summary"].get("status_pass", False))
            and _bool(source["m2956_summary"].get("gate_matrix_pass", False)),
            {
                "status_pass": source["m2956_summary"].get("status_pass"),
                "gate_matrix_pass": source["m2956_summary"].get("gate_matrix_pass"),
            },
            "both true",
            "lineage_invalid",
        ),
        (
            "m2956_candidate_rows_loaded",
            "candidate_resolution",
            len(source["m2956_candidate_rows"]) == EXPECTED_ADMITTED_COUNT,
            len(source["m2956_candidate_rows"]),
            EXPECTED_ADMITTED_COUNT,
            "metric_artifact",
        ),
        (
            "m2956_admitted_candidate_count",
            "candidate_resolution",
            source_status_counts.get(ADMITTED_STATUS, 0) == EXPECTED_ADMITTED_COUNT
            and len(candidate_rows) == EXPECTED_ADMITTED_COUNT,
            {"source": dict(source_status_counts), "m2960_candidates": len(candidate_rows)},
            EXPECTED_ADMITTED_COUNT,
            "scenario_sampling_failure",
        ),
        (
            "blocked_stale_guard_count",
            "guardrail",
            blocked_rejection_count == EXPECTED_BLOCKED_STALE_GUARD_COUNT,
            blocked_rejection_count,
            EXPECTED_BLOCKED_STALE_GUARD_COUNT,
            "metric_artifact",
        ),
        (
        ...
        (
            "actor_head_delta_contract_rows_present",
            "contract",
            len(contract_rows) == len(candidate_rows),
            len(contract_rows),
            len(candidate_rows),
            "contract_violation",
        ),
        (
            "actor_head_delta_contract_rows_pass",
            "contract",
            bool(contract_rows) and all(_bool(row.get("status_pass", False)) for row in contract_rows),
            f"rows={len(contract_rows)} pass={sum(_bool(row.get('status_pass', False)) for row in contract_rows)}",
            "all contract execution rows pass",
            "contract_violation",
        ),
        (
            "all_selected_metrics_finite",
        ...
        (
            "blocked_stale_guard_rows_not_executed",
            "guardrail",
        ...
        (
            ...
            "execution_guardrail",
            not any(forbidden_execution_flag(row) for row in episode_rows + failure_rows + contract_rows),
    ...
    *,
    ...
    return {
        ...
        "result_class": (
            "engineering_controller_route_a_offtrack_dominant_actor_head_delta_bounded_execution_preflight_pass"
            if status_pass
            else "engineering_controller_route_a_offtrack_dominant_actor_head_delta_bounded_execution_preflight_fail"
        ),
        "generated_at_utc": utc_timestamp(),
        "output_dir": str(output_dir),
        "next_blocker": next_blocker,
        ...
        "m2956_status_pass": _bool(source["m2956_summary"].get("status_pass", False)),
        "m2956_gate_matrix_pass": _bool(source["m2956_summary"].get("gate_matrix_pass", False)),
        "m2956_candidate_row_count": len(source["m2956_candidate_rows"]),
        "m2956_rejection_guardrail_row_count": len(source["m2956_rejection_rows"]),
        "candidate_count": len(candidate_rows),
        "expected_candidate_count": EXPECTED_ADMITTED_COUNT,
        "blocked_stale_guard_row_count": len(source["m2956_rejection_rows"]),
        ...
        "policy_action_run": bool(episode_rows),
        "policy_rollout_run": bool(episode_rows),
        "actor_head_delta_bounded_execution_preflight": bool(episode_rows),
        "zero_residual_identity_mode": True,
        "blocked_stale_guard_execution": False,
        ...
        "hidden_oracle_actor_input_detected": any_flag(episode_rows + failure_rows + contract_rows, "hidden_oracle_actor_input_required"),
        "future_target_actor_input_required": any_flag(episode_rows + failure_rows + contract_rows, "future_target_actor_input_required"),
        "route_labels_actor_visible": any_flag(episode_rows + failure_rows + contract_rows, "route_labels_actor_visible"),
        "source_labels_actor_visible": any_flag(episode_rows + failure_rows + contract_rows, "source_labels_actor_visible"),
        "evaluator_labels_actor_visible": any_flag(episode_rows + failure_rows + contract_rows, "evaluator_labels_actor_visible"),
        "diagnostic_labels_actor_visible": any_flag(episode_rows + failure_rows + contract_rows, "diagnostic_labels_actor_visible"),
        "success_progress_labels_actor_visible": any_flag(episode_rows + failure_rows + contract_rows, "success_progress_labels_actor_visible"),
        "verdict_labels_actor_visible": any_flag(episode_rows + failure_rows + contract_rows, "verdict_labels_actor_visible"),
        "guardrail_rows_in_success_denominator": any_flag(episode_rows + failure_rows + contract_rows, "guardrail_rows_in_success_denominator"),
        "ranking_run": False,
        "winner_selected": False,
...
def render_milestone_doc(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- actor-head delta contract execution rows: {summary['actor_head_delta_contract_execution_row_count']}",
            ...
            f"- blocked stale guard rows excluded: {summary['blocked_stale_guard_row_count']}",
    ...
    return {
        ...
        "lineage": {
            ...
            "parent_dataset": [
                ...
                str(output_dir / "actor_head_delta_contract_execution_rows.csv"),
                ...
                str(doc_path),
                "docs/m2959-engineering-controller-route-a-offtrack-dominant-constraint-balanced-actor-head-delta-bounded-execution-design.md",
            ],
            "parent_config": [
                "experiments/manifests/m2960-engineering-controller-route-a-offtrack-dominant-constraint-balanced-actor-head-delta-bounded-execution-preflight.json",
                "experiments/manifests/m2959-engineering-controller-route-a-offtrack-dominant-constraint-balanced-actor-head-delta-bounded-execution-design.json",
            ],
            ...
            "blocked_by": [
                "M2960 diagnostics require a result audit before any verdict or continuation decision",
                "11 blocked stale fixed-source rows must remain protected guardrails",
                "zero-residual contract execution must not be interpreted as repair success",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "route_a_dependency_facing_offtrack_dominant_actor_head_delta_bounded_execution_result_audit",
            "evidence_increment": "audits bounded actor-head delta diagnostic execution artifacts from M2960",
            ...
            "stop_condition": [
                "stop if M2960 artifacts are missing or gate matrix fails",
                "stop if actor-head delta contract actor or claim boundaries were violated",
                "stop if blocked stale fixed-source guardrails entered execution or denominators",
            ],
            "fallback_plan": [
            ...
            "synthesis_trigger": "M2960 completes bounded actor-head delta diagnostic execution preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M2960 bounded diagnostic execution preflight artifacts",
            "admission_evidence": [
                "M2960 summary and gate matrix",
                "M2960 execution candidate resolution actor-head delta contract execution failure aggregate guard actor claim artifacts",
            ],
            "blocked_shortcuts": [
                "no validation ranking promotion performance verdict paper high-fidelity full ideal driver or self-ID claim",
                "no training replay PPO checkpoint mutation or checkpoint promotion",
                "no hidden/oracle/future-target actor input",
            ],
            "allowed_updates": [
                f"docs/{NEXT_ID}.md",
                f"docs/reviews/{NEXT_ID}.md",
                "M2961 status queue scoreboard research log and review",
                "one follow-up manifest only if M2961 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M2961 audit accepts or rejects M2960 as complete and claim-safe",
                "next route or stop state is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M2961 audits Route A engineering diagnostics and cannot infer history necessity or self-ID.",
            "history_necessity_tests": [
                "None in M2961; no wrong-history reset-hidden zero-history finite-window or GRU comparison verdict is run."
            ],
            "temporal_evidence_window": "M2960 Route A actor-head delta diagnostic execution only.",
            ...
            "allowed_claims": [
                "M2960 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits newly generated Route A actor-head delta closed-loop diagnostics",
            "paper_verdict_delta": "no paper verdict; audit may inform Route A engineering continuation only",
            "must_synthesize_if": [
                "M2961 cannot accept M2960 as complete and claim-safe",
                "M2961 would claim validation readiness driver performance repair success paper current-sim high-fidelity or self-ID",
                "M2961 would continue static design without new data or synthesis",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M2961 audits M2960 artifacts and selects one next route or stop state while preserving actor guardrail and claim boundaries without overclaiming.",
...
def residuals_within_limit(rows: Iterable[Mapping[str, Any]]) -> bool:
    ...
    for row in rows:
        try:
            residual = float(row.get("residual_delta_abs_max", "nan"))
            limit = float(row.get("residual_limit", "nan"))
        except (TypeError, ValueError):
            return False
        if not np.isfinite(residual) or not np.isfinite(limit) or residual > limit + 1e-9:
            return False
    return True
...
def all_flag(rows: Iterable[Mapping[str, Any]], key: str) -> bool:
    rows = list(rows)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m2956-dir", type=Path, default=DEFAULT_M2956_DIR)
    parser.add_argument("--m2957-audit", type=Path, default=DEFAULT_M2957_AUDIT)
    parser.add_argument("--m2958-synthesis", type=Path, default=DEFAULT_M2958_SYNTHESIS)
    parser.add_argument("--m2959-design", type=Path, default=DEFAULT_M2959_DESIGN)
    ...
    parser.add_argument("--residual-limit", type=float, default=DEFAULT_RESIDUAL_LIMIT)
...
def main() -> None:
    args = parse_args()
    summary = run_actor_head_delta_bounded_execution_preflight(
        m2956_dir=args.m2956_dir,
        m2957_audit=args.m2957_audit,
        m2958_synthesis=args.m2958_synthesis,
        m2959_design=args.m2959_design,
    ...
    print(f"contract_execution_rows={summary['actor_head_delta_contract_execution_row_count']}")
