"""PWR2-TMeasy: gpu_physics_pwr + the TWO MEASURED avoid-vx fixes the gap decomposition pinned.

This is an exact copy of ``autodrift.gpu_physics_pwr`` (the L1 RELAX-TMeasy faithful rewrite with
the front-axle drive traction cap + measured cruise resistance) with TWO additional changes, both
MEASURED from the real isolated ``veh.Sedan()``, that the faithful-rewrite gap decomposition
(docs/gpu-surrogate-design-2026-06.md "FAITHFUL-REWRITE GAPS DECOMPOSED TO ROOT") attributed the
residual avoid-vx 0.90 gap to:

  (1) GEAR SHIFT MAP (MEASURED, extract_chrono_shiftmap.py): the per-gear shift-up/down engine-rpm
      bands the running ``veh.Sedan()`` AutomaticTransmissionSimpleMap actually uses, measured
      empirically by instrumenting GetCurrentGear()+GetMotorSpeed() through a full-throttle accel
      (upshifts) and a lift+brake decel (downshifts). FINDING: the measured map is essentially
      IDENTICAL to the model's existing SHIFT_UP=(4000,4500,...)/SHIFT_DOWN=(1000,1200,...) (the
      shipped Sedan_AutomaticTransmissionSimpleMap.json IS the true map). We install the EXACT
      measured values here (3994,4492,4498,4500,4500,4500 / 1000,1203,1401,1600,1800,2000) for
      faithfulness, but this fix is a near-no-op: tracing the avoid replay, the model's gear at
      8 m/s is idx1 (ratio 0.489) = Chrono gear 2, matching, and the high-mu accel rollouts already
      track Chrono to <0.11 m/s. The "gearbox stuck one gear too low" hypothesis does NOT reproduce.

  (2) FRONT BRAKE SHARE (MEASURED, extract_chrono_brake.py): the Sedan brakes ALL FOUR wheels at
      2000 N.m/wheel (BrakeSimple, axle0=front and axle1=rear both 2000, NO front/rear bias).
      ``gpu_physics_pwr`` applies brake torque only to the two REAR wheels (omega_rl/rr), so on
      low-mu BRAKING the rear axle saturates at ~half the available friction -> measured model decel
      -2.33 m/s^2 vs Chrono's -3.6 (ratio ~0.66) at mu=0.363. THIS is the dominant residual: the 12
      braking-heavy avoid rollouts under-decelerate, while the 108 accel rollouts already match.
      THE FIX (this module): add the FRONT-axle brake as a longitudinal braking FORCE at the front
      contact, opposing front motion and friction-circle-limited by the front load
      (sqrt((mu*Fz_front)^2 - Fy_front^2)), so the model brakes all four wheels like Chrono. The
      front brake force enters the body longitudinal balance directly (the front wheels are not
      spin-state-integrated in this RWD-omega model, so the front brake is applied as the
      friction-limited contact force, exactly as a locked/ABS front wheel would deliver). At low
      brake / drift (tiny brake command) the term is ~0 so the drift regime is untouched.

Everything else (the EXACT TMeasy tyre, sigma, front drive cap, RWD wheel-spin states, load
transfer, sub-stepped integration) is IDENTICAL to gpu_physics_pwr. Both gates are run side-by-side
against gpu_physics_pwr by ``scripts/feasibility_audit/gpu_pwr2_gate.py``.

----------------------------------------------------------------------------------------------------
Original gpu_physics_pwr docstring (unchanged, retained for reference):
...
# FIX (1): MEASURED from the running veh.Sedan() (extract_chrono_shiftmap.py) — empirical engine
# rpm at each up/down shift of the AutomaticTransmissionSimpleMap. The measured map is essentially
# identical to the shipped JSON (the prior approximation), confirming the gear thresholds were NOT
# the bug; these are the exact measured values.
#   measured UP   = (3994, 4492, 4498, 4500, 4500, 4500)   [gear idx 5 never reached top -> JSON 4500]
#   measured DOWN = (1000, 1203, 1401, 1600, 1800, 2000)   [gear idx 0 never down-shifted -> JSON 1000]
SHIFT_DOWN = (1000.0, 1203.0, 1401.0, 1600.0, 1800.0, 2000.0)
SHIFT_UP = (3994.0, 4492.0, 4498.0, 4500.0, 4500.0, 4500.0)
...
class PhysParams:
    ...
    # FIX (2) ablation knob ONLY: 1.0 = front brake ON (the measured 4-wheel brake); 0.0 = front brake
    # OFF (= the rear-only pwr behaviour, for the gear-fix-only side of the gate breakdown). NOT a
    # calibration: the measured value is 1.0 (the Sedan brakes all 4 wheels equally).
...
def _accel_from_forces(vx, vy, yaw_rate, steer, Fx_f, Fy_f, Fx_r, Fy_r, P: PhysParamBatch,
    ...
    frame. ``Fx_brake_front_body`` (PWR2 FIX 2) is the front-axle BRAKE force already expressed in
    the BODY longitudinal frame: a locked/sliding brake force opposes the wheel's GROUND velocity
    ...
    drift steer, overshooting vy). It is therefore added directly to Fx_body, below.
...
def _continuous_derivs(vx, vy, yaw_rate, steer, omega_rl, omega_rr, throttle, brake, gear,
    ...
    # ---- FIX (2): MEASURED FRONT-AXLE BRAKE FORCE (the low-mu brake deficit fix) ----
    # The Chrono Sedan brakes ALL FOUR wheels at 2000 N.m/wheel (extract_chrono_brake.py: BrakeSimple,
    # axle0=front + axle1=rear both 2000, no front/rear bias). gpu_physics_pwr brakes ONLY the two
    # REAR wheels (via the omega_rl/rr spin states below), so on low-mu braking the rear axle alone
    # saturates at ~half the available friction -> measured model decel -2.33 m/s^2 vs Chrono's -3.6
    # (ratio ~0.66 at mu=0.363). The front wheels carry NO spin state in this RWD-omega model (they are
    # free-rolling, sx=0), so the front brake cannot act through a wheel ODE; we add it directly as a
    # longitudinal CONTACT braking force at each front wheel, opposing the front wheel's longitudinal
    # motion and friction-circle-limited by the front load (after the front lateral force):
    #   demand   = brake * max_brake_torque / r_eff           [N]  (the commanded brake force)
    #   budget   = sqrt((mu*Fz_front)^2 - Fy_front^2)         [N]  (friction circle, lateral-reserved)
    #   Fx_brake = -sign(vx_front) * min(demand, budget)
    # This is exactly the locked/ABS front-wheel contact force a brake delivers: at low mu the demand
    # exceeds the budget so the front brake is friction-limited (Chrono's locked-wheel decel); at the
    # tiny brake commands of the drift regime the term is ~0 so drift is untouched. The force is added
    # to each front tyre's longitudinal force (same wheel frame), then rotated by steer in
    # _accel_from_forces like the rest of the front-axle force. NOTHING is tuned: max_brake_torque and
    # the 4-wheel braking are both measured; the friction circle is the same mu*Fz budget used by the
    # drive cap above.
    brk = brake.clamp(0.0, 1.0) * P["front_brake_scale"]   # front_brake_scale=1.0 measured (4-wheel); 0.0 = ablate
    fb_demand = brk * P["max_brake_torque"] / P["r_eff"].clamp_min(1e-3)   # commanded front brake force [N]
    fmu_fl_b = (mu_scale * P["mu0"]) * fz_fl * gsf
    ...
    # TOTAL 4-WHEEL FRICTION CAP (measured, NOT tuned): with all four wheels braking, the whole-car
    # braking force is bounded by the total friction mu*(Fz_f+Fz_r) = mu*m*g (4 locked wheels deliver
    # at most the full friction ellipse, no more). gpu_physics_pwr's rear-only model already delivers
    # ~2.33 m/s^2 -- ABOVE the rear-axle steady friction alone, because the locked-rear lagged-slip
    # transient transiently rides the tyre curve; adding the FULL front friction budget on top would
    # double-count and over-brake (4.3 vs Chrono 3.6). The physically faithful constraint is that the
    # SUM of the rear longitudinal brake force already produced (|Fx_r| in the braking direction) and
    # the added front brake cannot exceed the measured total friction mu*m*g. So the front brake is
    # given only the HEADROOM up to the total budget, matching Chrono's all-4-locked decel. This is
    # the same mu*Fz friction budget used everywhere else -- the only addition is summing it over all
    # four wheels, which is exactly what "brake all four wheels" means.
    fz_tot = fz_fl + fz_fr + fz_rl + fz_rr                  # = m*g
    fmu_total = (mu_scale * P["mu0"]) * fz_tot              # mu*m*g total friction budget [N]
    # rear longitudinal force already opposing motion (braking direction is -sign(vx)); take the part
    # that is actually decelerating the car (negative w.r.t. vx).
    vx_brk_sign = torch.where(vx.abs() > 1e-3, torch.sign(vx), torch.ones_like(vx))
    fx_r_sum = fx_rl + fx_rr
    fx_r_braking = (-vx_brk_sign * fx_r_sum).clamp_min(0.0)  # rear force magnitude opposing motion [N]
    ...
    fb_l = fb_l * head_scale                                # per-wheel front brake after total-friction cap
    fb_r = fb_r * head_scale
    fx_brake_front = -vx_brk_sign * (fb_l + fb_r)

    # COMBINED-SLIP COUPLING (the drift-faithfulness term): a braking front wheel spends part of its
    # friction circle on the longitudinal brake force, so its LATERAL force must drop accordingly --
    # otherwise the front keeps full cornering grip WHILE braking and the drift-entry sideslip
    # overshoots (vy too high -> beta@24 too high, which broke the drift gate when the front brake was
    # added without this coupling). Reduce each front wheel's lateral force to the remaining lateral
    # budget after the brake: |Fy| <= sqrt((mu*Fz)^2 - Fx_brake^2). This is the same friction ellipse
    # already used for the budget; it just feeds the consumed longitudinal share back into Fy. At the
    # small front brake of the avoidance cruise it is ~a no-op; in the brake-heavy drift entry it
    # correctly de-grips the front, restoring the drift.
    fy_lat_budget_l = torch.sqrt((fmu_fl_b * fmu_fl_b - fb_l * fb_l).clamp_min(0.0) + 1.0)
    fy_lat_budget_r = torch.sqrt((fmu_fr_b * fmu_fr_b - fb_r * fb_r).clamp_min(0.0) + 1.0)
    fy_fl = torch.sign(fy_fl) * torch.minimum(fy_fl.abs(), fy_lat_budget_l)
    fy_fr = torch.sign(fy_fr) * torch.minimum(fy_fr.abs(), fy_lat_budget_r)

    # NOTE: fx_brake_front is already in the BODY longitudinal frame (it opposes body vx); it is fed
    # to _accel_from_forces as the body-frame brake force (NOT rotated by steer -- a sliding brake
    # opposes ground velocity, not wheel heading), so it is NOT added into the steered-frame Fx_f.
    Fx_f = fx_fl + fx_fr
    Fx_r = fx_rl + fx_rr
    Fy_f = fy_fl + fy_fr
    Fy_r = fy_rl + fy_rr
...
def init_state(vx0: torch.Tensor, vy0: torch.Tensor, yaw0: torch.Tensor, P: PhysParamBatch):
    ...
    # FAITHFUL cruise-entry seed (pwr3 FIX #1): Chrono enters these scenarios having approached at speed,
    # so its gearbox HOLDS the HIGHEST gear whose motor rpm is still inside the hysteresis band [down,up]
    # (it only downshifts once rpm < down). The old "lowest gear under up-threshold" rule modelled
    # accelerate-from-rest and seeded one gear too LOW at cruise (gear 2 vs Chrono gear 3 @ 8.8 m/s ->
    # 1.6x driveshaft over-torque -> avoid-vx over-accel). docs/.../"RESOLVED 2026-06-17".
    in_band = (motor_rpm_g >= down) & (motor_rpm_g <= up)
