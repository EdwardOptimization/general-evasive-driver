"""Reset-only validation for comparison-support executable specs."""
...
DEFAULT_OUTPUT_DIR = Path("runs/m2121_paper_route_outcome_supported_decisive_comparison_support_reset_validation_preflight")
DEFAULT_EVAL_SEED_BASE = 212100
...
EXPECTED_MATERIALIZATION_SEMANTICS = "comparison_support_smoke_proxy"
...
CONTRACT_FIELDNAMES = [
    *METADATA_FIELDS,
    ...
    "materialization_semantics_comparison_support_smoke_proxy",
    "paper_validity_claim_false",
    "generated_source_row_proxy_semantics",
    "actor_input_contract_human_view",
...
def contract_row_for_spec(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "generated_source_row_proxy_semantics": generated
        and materialization_semantics == EXPECTED_MATERIALIZATION_SEMANTICS,
        "actor_input_contract_human_view": actor_input_contract == "human_view_no_privileged_inputs",
    }
    return {
            ...
            rows.append(
                {
                    ...
                    "comparison_support_intent": metadata["comparison_support_intent"],
...
def reset_comparison_support_spec(
    *,
...
def claim_boundary_rows(*, reset_validity_admissible: bool) -> list[dict[str, Any]]:
    return [
        {
            "claim": "comparison_support_reset_validity",
            "admissible": reset_validity_admissible,
            "reason": "reset validity is admissible only if all reset gates pass and after M2122 audit",
        },
        {
        ...
        },
        {
            ...
            "admissible": False,
            "reason": "comparison-support smoke proxies need later measured support and task semantics evidence",
        },
        {
...
def run_comparison_support_reset_validation_preflight(
    *,
    ...
    intent_counts = _count_by(reset_rows, "comparison_support_intent")
    expected_intent_counts = _count_by(specs, "comparison_support_intent")
    ...
    write_csv_rows(output / "reset_distribution_by_intent.csv", _aggregate_rows(intent_counts), AGGREGATE_FIELDNAMES)
    write_csv_rows(
    ...
    summary = {
        "result_class": (
            "comparison_support_reset_validation_preflight_pass"
            if passes
            else "comparison_support_reset_validation_preflight_fail"
        ),
        ...
        "artifacts": {
            ...
            "reset_distribution_by_intent": str(output / "reset_distribution_by_intent.csv"),
