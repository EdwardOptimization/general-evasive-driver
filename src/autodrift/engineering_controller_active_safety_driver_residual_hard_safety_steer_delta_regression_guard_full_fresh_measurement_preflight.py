"""Run M3181 steer-delta regression guard full-fresh measurement."""
...
from autodrift.engineering_controller_active_safety_driver_residual_hard_safety_steer_delta_regression_guard_materialization_preflight import (
    ACTION_COMPONENTS,
    OUTPUT_SEMANTICS,
    POLICY_ID,
    steer_delta_regression_guard_direct_action,
)
...
NEXT_ID = (
    "m3182-engineering-controller-active-safety-driver-residual-hard-safety-"
    "steer-delta-regression-guard-full-fresh-measurement-result-audit"
)
M3180_ID = (
...
DEFAULT_M3180_AUDIT = Path(f"docs/{M3180_ID}.md")
DEFAULT_M3179_DIR = Path(
...
BASELINE_IDS = ("m3105", "m3172")
CLAIM_SCOPE = (
    "M3181 Active Safety Driver residual hard-safety steer-delta regression guard full-fresh "
    "measurement preflight only; the complete M3172 fresh denominator may be executed through "
    "the M3179 direct-action function as the full obs72-to-action3 action source and measured "
    "against same-row M3105 and M3172 artifacts. No validation, ranking, winner selection, "
    "checkpoint mutation, checkpoint promotion, public driver default mutation, driver-performance "
...
def load_sources(
    *,
    ...
    paths = {
        "m3180_audit": m3180_audit,
        "m3179_summary": m3179_dir / "summary.json",
        "m3179_gate_rows": m3179_dir / "gate_matrix.csv",
    ...
    return {
        ...
        "m3180_audit_text": paths["m3180_audit"].read_text(encoding="utf-8") if exists["m3180_audit"] else "",
        "m3179_summary": read_json(paths["m3179_summary"]) if exists["m3179_summary"] else {},
        "m3179_gate_rows": read_csv_rows(paths["m3179_gate_rows"]),
...
def full_fresh_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    rows = sorted(source["m3172_measurement_rows"], key=lambda row: str(row.get("runtime_smoke_episode_id", "")))
        ...
        plan.append(
            {
                **dict(source_row),
                **dict(workload),
                "runtime_smoke_episode_id": f"m3181-measurement-episode-{len(plan) + 1:04d}",
...
class M3181MeasurementPolicy:
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        del info
        action = steer_delta_regression_guard_direct_action(observation)
        self.step_count += 1
...
def normalize_episode_row(plan: Mapping[str, Any], row: Mapping[str, Any], telemetry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    item.update(
        {
            "runtime_driver_id": POLICY_ID,
            "policy": "active_safety_driver_steer_delta_regression_guard_measurement",
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3181 plan row failed pre-execution guards")
            ...
            try:
                ...
                row = m3088.run_episode_with_policy(
                    env,
                    policy,
                    "active_safety_driver_steer_delta_regression_guard_measurement",
...
def same_row_comparison_rows(
    ...
    *,
    ...
    m3172_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    baselines = {
        "m3105": {str(row.get("source_measurement_episode_id", "")): row for row in m3105_rows},
        "m3172": {str(row.get("source_measurement_episode_id", "")): row for row in m3172_rows},
    }
    ...
    for episode in episodes:
        ...
        for baseline_id, table in baselines.items():
            ...
            row = {
                "comparison_id": f"m3181-comparison-{len(rows) + 1:04d}",
                ...
                "m3181_success": _success(episode),
                "baseline_success": _success(baseline),
                "m3181_collision": _bool(episode.get("collision", False)),
                "baseline_collision": _bool(baseline.get("collision", False)),
                "m3181_offtrack": _offtrack(episode),
                "baseline_offtrack": _offtrack(baseline),
                "m3181_speed_too_low": _speed_too_low(episode),
                "baseline_speed_too_low": _speed_too_low(baseline),
                "m3181_min_clearance_margin": _float(episode.get("min_clearance_margin")),
                "baseline_min_clearance_margin": _float(baseline.get("min_clearance_margin")),
                "m3181_return": _float(episode.get("return")),
                "baseline_return": _float(baseline.get("return")),
                "m3181_speed_mean": _float(episode.get("speed_mean")),
            ...
            row.update(
                {
                    "success_delta": int(row["m3181_success"]) - int(row["baseline_success"]),
                    "collision_delta": int(row["m3181_collision"]) - int(row["baseline_collision"]),
                    "offtrack_delta": int(row["m3181_offtrack"]) - int(row["baseline_offtrack"]),
                    "speed_too_low_delta": int(row["m3181_speed_too_low"]) - int(row["baseline_speed_too_low"]),
                    "clearance_margin_delta": _float(row["m3181_min_clearance_margin"]) - _float(row["baseline_min_clearance_margin"]),
                    "return_delta": _float(row["m3181_return"]) - _float(row["baseline_return"]),
                    "speed_mean_delta": _float(row["m3181_speed_mean"]) - _float(row["baseline_speed_mean"]),
                }
            )
            rows.append(row)
    return rows
...
def _with_scope(rows: list[dict[str, Any]], *, id_prefix: str = "m3181") -> list[dict[str, Any]]:
    updated = []
    for row in rows:
        item = dict(row)
        ...
        if "policy" in item:
            item["policy"] = "active_safety_driver_steer_delta_regression_guard_measurement"
...
def contract_guard_rows(source: Mapping[str, Any], plan_rows: list[dict[str, Any]], episodes: list[dict[str, Any]], failures: list[dict[str, Any]]) -> list[dict[str, Any]]:
    sample = steer_delta_regression_guard_direct_action(np.zeros(P0_OBSERVATION_DIM, dtype=np.float32))
    ...
    rows = [
        ...
        m3088.guard("m3179_status_pass", "lineage", _bool(source["m3179_summary"].get("status_pass", False)), True),
        m3088.guard("m3179_gate_matrix_pass", "lineage", _bool(source["m3179_summary"].get("gate_matrix_pass", False)), True),
    ]
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    claims = [
        ...
        ("self_id", "forbidden", False, False, "history necessity tests outside M3181"),
    ]
    return [
        {
            "claim_id": f"m3181-{claim_id}",
            "claim_family": family,
            "allowed_in_m3181": allowed,
            "claim_made": made,
...
def _m3180_selects_m3181(text: str) -> bool:
    return (
        "m3181-engineering-controller-active-safety-driver-residual-hard-safety-steer-delta-regression-guard-full-fresh-measurement-preflight"
        in text
...
def gate_matrix_rows(
    *,
    ...
    return [
        ...
        gate("m3180_selects_m3181_route", "lineage", _m3180_selects_m3181(source["m3180_audit_text"]), "route marker", "present", "lineage_invalid"),
        gate("m3179_status_pass", "lineage", _bool(source["m3179_summary"].get("status_pass", False)), source["m3179_summary"].get("status_pass"), True, "lineage_invalid"),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject M3181 full-fresh measurement artifacts before validation or stop.",
        "lineage": {
            ...
            "derived_from": [MILESTONE_ID, M3180_ID, M3179_ID, M3172_ID, M3105_ID],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "behavior_negative_steer_delta_guard_full_fresh_measurement_result_audit",
            "evidence_increment": "audits full-fresh measurement evidence for M3179 guard candidate",
            ...
            "stop_condition": ["stop if M3181 is behavior-negative or contract-unsafe", "route to validation planning only after audit acceptance"],
            "fallback_plan": ["route to M3181 artifact repair if rows or gates fail", "route to synthesis if behavior-negative"],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3181 completes full-fresh measurement",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3181 full-fresh measurement artifacts",
            "admission_evidence": ["M3181 summary measurement comparison guard claim and gate artifacts"],
            ...
            "allowed_updates": [f"docs/{NEXT_ID}.md", f"docs/reviews/{NEXT_ID}.md", f"experiments/reviews/{NEXT_ID}.json", "M3182 status queue scoreboard research log and review"],
            "next_stage_criteria": ["M3182 accepts or rejects M3181 and selects next route"],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3182 audits engineering measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3182; self-ID and GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3181 measurement artifacts only.",
            ...
            "allowed_claims": ["M3181 artifact completeness and claim-safety audit", "no driver-performance verdict paper-level result finite-window-vs-GRU result high-fidelity validation result full ideal driver completion repair-success robustness-result feasibility-proof or level3 self-identification claim"],
        },
        "local_search_guard": {
            ...
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3182 audits engineering measurement evidence",
            "must_synthesize_if": ["M3182 cannot select validation planning synthesis artifact-repair or stop", "M3182 would claim repair-success validation driver-performance current-sim verdict robustness-result or self-ID evidence"],
        },
        "success_criteria": [f"docs/{NEXT_ID}.md exists", "M3182 audits M3181 row counts gates actor contract and claim boundaries", "M3182 selects exactly one next route or stop state"],
        "failure_criteria": ["M3182 hides missing M3181 artifacts or failed gates", "M3182 treats M3181 measurement as validation or repair success", "M3182 leaves next route ambiguous"],
        "decision_rule": "Pass only if M3182 audits M3181 artifacts and selects one next route or stop state while preserving claim boundaries without overclaiming.",
...
def run_full_fresh_measurement_preflight(
    *,
    ...
    source = load_sources(m3180_audit=m3180_audit, m3179_dir=m3179_dir, m3172_dir=m3172_dir, m3105_dir=m3105_dir, m3012_dir=m3012_dir)
    ...
    comparisons = same_row_comparison_rows(episodes, m3105_rows=source["m3105_measurement_rows"], m3172_rows=source["m3172_measurement_rows"])
    ...
    summary: dict[str, Any] = {
        ...
        "result_class": (
            "active_safety_driver_steer_delta_regression_guard_full_fresh_measurement_pass"
            if status_pass
            else "active_safety_driver_steer_delta_regression_guard_full_fresh_measurement_fail"
        ),
        "status_pass": status_pass,
        ...
        "success_count_delta_vs_m3172": _sum_delta("m3172", "success_delta"),
        "collision_count_delta_vs_m3172": _sum_delta("m3172", "collision_delta"),
        "offtrack_count_delta_vs_m3172": _sum_delta("m3172", "offtrack_delta"),
        "speed_too_low_count_delta_vs_m3172": _sum_delta("m3172", "speed_too_low_delta"),
        ...
        "decision": "active_safety_driver_steer_delta_regression_guard_full_fresh_measurement_route_to_m3182_result_audit",
        "next_blocker": NEXT_ID,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3180-audit", type=Path, default=DEFAULT_M3180_AUDIT)
    parser.add_argument("--m3179-dir", type=Path, default=DEFAULT_M3179_DIR)
...
def main() -> None:
    ...
    summary = run_full_fresh_measurement_preflight(
        m3180_audit=args.m3180_audit,
        m3179_dir=args.m3179_dir,
