"""Run M3105 v4 v2-fallback no-regression hard-safety direct-action repair full-fresh measurement."""
...
M3104_ID = (
    "m3104-engineering-controller-active-safety-driver-v4-v2-fallback-no-regression-hard-safety-"
...
DEFAULT_M3104_AUDIT = Path(f"docs/{M3104_ID}.md")
...
CLAIM_SCOPE = (
    "M3105 Active Safety Driver v4 v2-fallback no-regression hard-safety direct-action repair full-fresh "
    "measurement preflight only; the complete M3084 fresh current-sim denominator may "
    "be executed through the M3103 v4 action function as the full obs72-to-action3 "
    "action source and measurement, same-row comparison against M3095 M3100 and M3090, "
    "contract, claim, gate, doc, and M3106 audit artifacts may be written. No validation, ranking, winner "
...
class V4RepairMeasurementPolicy:
    """Policy adapter that calls the M3103 v4 direct-action repair function."""
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        del info
        action = v4_v2_fallback_no_regression_hard_safety_direct_action(observation, V4_POLICY_CONFIG)
        self.step_count += 1
...
def load_sources(
    *,
    ...
    paths = {
        ...
        "m3103_no_regression_rows": m3103_dir / "no_regression_guard_rows.csv",
    ...
    return {
        ...
        "m3104_audit_text": paths["m3104_audit"].read_text(encoding="utf-8") if exists["m3104_audit"] else "",
        ...
        "m3103_no_regression_rows": read_csv_rows(paths["m3103_no_regression_rows"]),
...
def full_fresh_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(plan, start=1):
        row["runtime_smoke_episode_id"] = f"m3105-measurement-episode-{index:04d}"
    return plan


def _with_scope(rows: list[dict[str, Any]], *, id_prefix: str = "m3105") -> list[dict[str, Any]]:
    updated = []
    for row in rows:
        item = dict(row)
        ...
        if "policy" in item:
            item["policy"] = "active_safety_driver_v4_v2_fallback_no_regression_hard_safety_direct_action_repair_measurement"
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3105 plan row failed pre-execution guards")
            ...
            policy = V4RepairMeasurementPolicy()
            try:
                ...
                row = m3088.run_episode_with_policy(
                    env,
                    policy,
                    "active_safety_driver_v4_v2_fallback_no_regression_hard_safety_repair_measurement",
                    int(plan["eval_seed"]),
                )
            finally:
                env.close()
...
def same_row_comparison_rows(
    ...
    for episode in episodes:
        ...
        for baseline_id in ("m3095", "m3100", "m3090"):
            ...
            rows.append(
                {
                    "comparison_id": f"m3105-same-row-comparison-{len(rows) + 1:04d}",
                    ...
                    "m3105_outcome_bucket": episode.get("outcome_bucket", ""),
                    ...
                    "m3105_min_clearance_margin": episode.get("min_clearance_margin", ""),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3105 v4 v2-fallback no-regression hard-safety full-fresh measurement artifacts before any validation ranking promotion driver-performance current-sim verdict high-fidelity paper full-driver repair-success robustness-result or self-ID claim.",
        "lineage": {
            "parent_checkpoint": [
                str(doc_path),
                "runs/m3103_engineering_controller_active_safety_driver_v4_v2_fallback_no_regression_hard_safety_direct_action_repair_materialization_preflight/direct_action_policy_config.json",
            ],
            "parent_dataset": [
            ...
            "blocked_by": [
                "M3105 full-fresh measurement rows require audit before any validation or repair-success route",
                "same-row comparison against M3095 M3100 and M3090 is measurement context and not a performance verdict before M3106",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "v4_no_regression_full_fresh_measurement_result_audit",
            "evidence_increment": "audits full-fresh current-sim measurement rows and same-row comparison from the v4 v2-fallback no-regression hard-safety direct-action repair",
            ...
            "stop_condition": [
                "stop if M3105 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "synthesize if M3106 cannot select one next route",
            ],
            "fallback_plan": [
                "route to v4 materialization repair if artifacts are incomplete or contract-unsafe",
                "route to behavior repair synthesis if measurement is behavior-negative but contract-safe",
                "route to validation planning only after M3106 accepts artifact completeness and claim boundaries",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3105 completes full-fresh v4 no-regression hard-safety repair measurement preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3105 full-fresh v4 v2-fallback no-regression hard-safety repair measurement artifacts",
            "admission_evidence": ["M3105 summary gate matrix comparison measurement metric contract and claim artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3106 status queue scoreboard research log and review",
                "one follow-up manifest only if M3106 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3106 accepts or rejects M3105 as complete and claim-safe",
                "next behavior synthesis validation-planning stop or repair route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3106 audits engineering runtime-measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3106; self-ID/GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3105 full-fresh v4 repair measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3105 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits complete fresh-denominator environment-loop evidence through the v4 direct-action repair",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3106 audits engineering runtime measurement evidence",
            "must_synthesize_if": [
                "M3106 cannot accept M3105 as complete and claim-safe",
                "M3106 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict robustness-result or self-ID evidence",
                "M3106 cannot select behavior synthesis validation planning stop or next repair route",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M3106 audits M3105 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
        "commands": [{"name": "active_safety_driver_v4_v2_fallback_no_regression_hard_safety_repair_full_fresh_measurement_result_audit_doc", "command": "true"}],
...
def gate_matrix_rows(
    *,
    ...
    config = source["m3103_policy_config"]
    audit_accepts = "accept_m3103_materialization_route_to_m3105_full_fresh_measurement" in str(
        source.get("m3104_audit_text", "")
    )
    baseline_counts = Counter(str(row.get("baseline_id", "")) for row in comparison_rows)
    expected_comparison_rows = EXPECTED_FULL_ROWS * 3
    ...
    return [
        ...
        gate("m3104_accepts_m3105_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        ...
        gate("m3103_policy_config_status", "lineage", str(config.get("policy_id", "")) == POLICY_ID, config.get("policy_id"), POLICY_ID, "lineage_invalid"),
        gate("m3103_gate_rows_pass", "lineage", all(_bool(row.get("status_pass", False)) for row in source.get("m3103_gate_rows", [])), len(source.get("m3103_gate_rows", [])), "all pass", "lineage_invalid"),
        gate("m3103_no_regression_rows_pass", "lineage", all(_bool(row.get("status_pass", False)) for row in source.get("m3103_no_regression_rows", [])), len(source.get("m3103_no_regression_rows", [])), "all pass", "lineage_invalid"),
        ...
        gate("m3100_gate_matrix_pass", "lineage", _bool(source["m3100_summary"].get("gate_matrix_pass", False)), source["m3100_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
        ...
        gate("same_row_baseline_counts", "comparison", all(baseline_counts.get(baseline_id, 0) == EXPECTED_FULL_ROWS for baseline_id in ("m3095", "m3100", "m3090")), dict(sorted(baseline_counts.items())), "64 rows per baseline", "metric_artifact"),
...
def render_doc(summary: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- success count delta vs M3100: {summary['success_count_delta_vs_m3100']}",
            f"- collision count delta vs M3100: {summary['collision_count_delta_vs_m3100']}",
            f"- offtrack count delta vs M3100: {summary['offtrack_count_delta_vs_m3100']}",
            f"- speed-too-low count delta vs M3100: {summary['speed_too_low_count_delta_vs_m3100']}",
...
def run_full_fresh_measurement_preflight(
    *,
    ...
    summary = {
        ...
        "result_class": (
            "active_safety_driver_v4_v2_fallback_no_regression_hard_safety_direct_action_repair_full_fresh_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_v4_v2_fallback_no_regression_hard_safety_direct_action_repair_full_fresh_measurement_preflight_fail"
        ),
        ...
        "same_row_exact_seed_match_count_m3095": _exact_seed_count("m3095"),
        "same_row_exact_seed_match_count_m3100": _exact_seed_count("m3100"),
        "same_row_exact_seed_match_count_m3090": _exact_seed_count("m3090"),
        "success_count_delta_vs_m3095": _sum_delta("m3095", "success_delta"),
        "success_count_delta_vs_m3100": _sum_delta("m3100", "success_delta"),
        "success_count_delta_vs_m3090": _sum_delta("m3090", "success_delta"),
        "collision_count_delta_vs_m3095": _sum_delta("m3095", "collision_delta"),
        "collision_count_delta_vs_m3100": _sum_delta("m3100", "collision_delta"),
        "collision_count_delta_vs_m3090": _sum_delta("m3090", "collision_delta"),
        "offtrack_count_delta_vs_m3095": _sum_delta("m3095", "offtrack_delta"),
        "offtrack_count_delta_vs_m3100": _sum_delta("m3100", "offtrack_delta"),
        "offtrack_count_delta_vs_m3090": _sum_delta("m3090", "offtrack_delta"),
        "speed_too_low_count_delta_vs_m3095": _sum_delta("m3095", "speed_too_low_delta"),
        "speed_too_low_count_delta_vs_m3100": _sum_delta("m3100", "speed_too_low_delta"),
        "speed_too_low_count_delta_vs_m3090": _sum_delta("m3090", "speed_too_low_delta"),
        "clearance_margin_delta_mean_vs_m3095": _mean_delta("m3095", "clearance_margin_delta"),
        "clearance_margin_delta_mean_vs_m3100": _mean_delta("m3100", "clearance_margin_delta"),
        "clearance_margin_delta_mean_vs_m3090": _mean_delta("m3090", "clearance_margin_delta"),
        "return_delta_mean_vs_m3095": _mean_delta("m3095", "return_delta"),
        "return_delta_mean_vs_m3100": _mean_delta("m3100", "return_delta"),
        "return_delta_mean_vs_m3090": _mean_delta("m3090", "return_delta"),
        "speed_mean_delta_mean_vs_m3095": _mean_delta("m3095", "speed_mean_delta"),
        "speed_mean_delta_mean_vs_m3100": _mean_delta("m3100", "speed_mean_delta"),
        "speed_mean_delta_mean_vs_m3090": _mean_delta("m3090", "speed_mean_delta"),
        ...
        "required_artifacts_present": present,
        "m3103_status_pass": _bool(source["m3103_summary"].get("status_pass", False)),
        "m3103_gate_matrix_pass": _bool(source["m3103_summary"].get("gate_matrix_pass", False)),
        "m3100_status_pass": _bool(source["m3100_summary"].get("status_pass", False)),
        "m3100_gate_matrix_pass": _bool(source["m3100_summary"].get("gate_matrix_pass", False)),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3104-audit", type=Path, default=DEFAULT_M3104_AUDIT)
...
def main() -> None:
    ...
    print(f"success_delta_vs_m3100={summary['success_count_delta_vs_m3100']}")
...
def contract_guard_rows(
    *,
    ...
    sample_action = v4_v2_fallback_no_regression_hard_safety_direct_action(
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3106 audit manifest"),
    ]
    blocked = [
    ...
    rows = [
        {
            "claim_id": f"m3105-{claim_id}",
            "claim_family": family,
            "allowed_in_m3105": True,
            "claim_made": made,
            "status_pass": made,
