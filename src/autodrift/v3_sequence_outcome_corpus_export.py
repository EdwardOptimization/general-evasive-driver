"""Export M743 v3 sequence-outcome rows as an auditable corpus."""
...
V3_ROLLOUT_FIELDS = [
    *ROLLOUT_FIELDS,
    *[field for field in V3_EXTRA_FIELDS if field not in ROLLOUT_FIELDS],
    *V3_ENRICHED_FIELDS,
]

POSITIVE_FIELDS = ["contrast_group_id", "proof_positive", *V3_ROLLOUT_FIELDS]
CONTRAST_FIELDS = ["contrast_group_id", "contrast_role", "proof_positive", *V3_ROLLOUT_FIELDS]
...
def _enrich_row(row: dict[str, Any], fault_specs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    enriched = {field: row.get(field, "") for field in V3_ROLLOUT_FIELDS}
    enriched.update(
        {
...
def _has_required_v3_metadata(row: dict[str, Any]) -> bool:
    return all(str(row.get(field, "")).strip() for field in REQUIRED_V3_FIELDS)
...
def _with_contrast(row: dict[str, Any], *, group_id: str, role: str, proof_positive: bool) -> dict[str, Any]:
    output = {field: row.get(field, "") for field in V3_ROLLOUT_FIELDS}
    output.update(
        {
...
def classify_v3_sequence_outcome_corpus(
    *,
    positive_rows: int,
    ...
    if (
        ...
        or int(positive_rows_missing_v3_metadata) > 0
        or int(positive_rows_missing_fidelity_metadata) > 0
    ):
        return "v3_sequence_outcome_corpus_artifact"
    if int(positive_rows) < int(min_positive_rows):
        return "v3_sequence_outcome_corpus_sparse"
    if (
        ...
        return "v3_sequence_outcome_corpus_unbalanced"
    if int(hard_negative_rows) < int(positive_rows):
        return "v3_sequence_outcome_corpus_hard_negative_sparse"
    return "v3_sequence_outcome_corpus_exported"


def export_v3_sequence_outcome_corpus(
    *,
    summary_path: Path,
    rollouts_path: Path,
    ...
    positive_rows_missing_v3_metadata = sum(1 for row in positives if not _has_required_v3_metadata(row))
    ...
    result_class = classify_v3_sequence_outcome_corpus(
        ...
        positive_rows_missing_v3_metadata=positive_rows_missing_v3_metadata,
    ...
    positive_output_rows = [
        {
            **{field: row.get(field, "") for field in V3_ROLLOUT_FIELDS},
    ...
    write_csv_rows(run_dir / "hard_negative_rows.csv", hard_negative_rows, fieldnames=V3_ROLLOUT_FIELDS)
    write_csv_rows(run_dir / "excluded_sentinel_rows.csv", excluded_sentinel_rows, fieldnames=V3_ROLLOUT_FIELDS)
    ...
    summary = {
        "run_type": "v3_sequence_outcome_corpus_export",
        ...
        "positive_rows_missing_v3_metadata": int(positive_rows_missing_v3_metadata),
        ...
        "v3_metadata_gate_pass": bool(
            positive_rows_missing_v3_metadata == 0 and positive_rows_missing_fidelity_metadata == 0
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export a sentinel-filtered v3 sequence-outcome corpus.")
    ...
    summary = export_v3_sequence_outcome_corpus(
