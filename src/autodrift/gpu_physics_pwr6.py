"""PWR6-TMeasy: pwr3 + the FRONT-wheel longitudinal-slip degree of freedom (option B, MINIMAL).

This is gpu_physics_pwr3 with ONE surgical addition: the FRONT wheels' omitted longitudinal slip.
pwr3 hard-codes the FRONT longitudinal slip to ZERO (free-rolling front, _continuous_derivs ~L613-617:
``_wheel_forces(zero, alpha_f_lag, ...)``), so its combined-slip friction circle never robs the front
Fy that Chrono's does in the braking/drive-laden drift entry. Verified from the per-wheel telemetry
(drift_heldout_lateral_chrono.npz): corr(|front sx|, pwr3 front-Fy over-production) = -0.84; the worst-
beta cells carry |front sx| up to 0.47. The same omitted DOF is the leading hypothesis for the avoid
down-ramp ax-gap (Chrono front kappa +0.22 on release, unmodelled).

WHY OPTION B (measured): the PRINCIPLED FWD restructure (option A: drive real front spin states, make
the rear non-driven) was BUILT AND GATED and it BREAKS THE DRIFT -- the planar drift saddle (sigma_scale
0.165 + the whole pwr3 calibration) is entangled with the RWD-omega drive structure; moving the drive to
the front and passivating the rear regresses the drift honest beta@24 to ~0.07 even with the front-Fy
COUPLING DISABLED (the structural change alone, not the Fy fix, breaks it). So pwr6 takes the LOW-RISK
variant: keep pwr3's structure verbatim (RWD-omega drive + front-traction cap + rear spin states, the
drift is preserved) and ONLY add the front-Fy-robbing.

THE ADDITION: a FRONT longitudinal slip-lag state (sx_fl_lag/sx_fr_lag) whose EMERGENT target is the
slip that the front longitudinal FORCE the car already carries (pwr3's capped FWD drive force + the
front brake) implies through the EXACT front Fx curve -- so the front sx is NOT imposed-measured but
emerges from the carried front force. It is relaxed over the measured sigma, and (scaled by the MEASURED
FRONT_SX_COUPLING ~= 0.40) fed into the FRONT combined-slip _wheel_forces so the friction circle robs
front Fy. CRITICAL CAVEAT (measured): piping the FULL emergent front sx through pwr3's CURRENT ellipse
OVER-corrects (front Fy -487 vs Chrono -1283 at the worst -- the ellipse is too aggressive at high sx);
the front Fy(alpha,sx) coupling is re-extracted from Chrono (the SAME validation the rear law passed) and
softened to ~0.40 (tail Fy RMSE bottoms there; meanErr -79 N vs +223 N at 1.0). The rear law is UNTOUCHED.

----------------------------------------------------------------------------------------------------
Original pwr3 docstring (UNCHANGED below this line -- pwr6 only ADDS the front Fy-robbing):
...
#         ax_f, ay_f, alpha_f_lag, alpha_r_lag, sx_rl_lag, sx_rr_lag, sx_fl_lag, sx_fr_lag]
# PWR6 (option B, MINIMAL/non-structural): pwr3 is UNCHANGED (RWD-omega drive, front-traction cap,
# rear spin states) -- it works for the drift. We ONLY ADD two FRONT longitudinal slip-lag states
# (sx_fl_lag/sx_fr_lag). The front sx EMERGES from the front longitudinal FORCE the car already
# carries (pwr3's capped FWD drive force + the front brake): we solve the EXACT front Fx curve for
# the slip that produces that force, relax it over sigma, and feed (FRONT_SX_COUPLING * sx_f_lag)
# into the FRONT combined-slip _wheel_forces so the friction circle robs front Fy (the omitted DOF).
# This touches ONLY the front Fy; the longitudinal dynamics, drift saddle and rear are unchanged.
# alpha_f_lag/alpha_r_lag/sx_*_lag are the relaxation-length-lagged tyre SLIPS: the TMeasy SLIP
# transient that holds the tyre force (hence sideslip) up during a drift entry.
PHYS_STATE_DIM = 19
IDX = dict(
    ...
    sx_fl_lag=17, sx_fr_lag=18,
)
ACT_DIM = 3

# MEASURED front combined-slip softening (re-extracted from drift_heldout_lateral_chrono.npz the SAME
# way the rear law was validated; NOT tuned to the gate). Piping the FULL front sx through pwr3's
# friction-circle ellipse over-corrects (ellipse too aggressive at high sx); the measured front Fy
# tail-RMSE bottoms when the sx entering the ellipse is scaled by ~0.40 (tail meanErr -79 N at 0.40
# vs +223 N at 1.0, -352 N at 0.0). Only the lateral-coupling magnitude uses the softened value.
FRONT_SX_COUPLING = 0.40
...
class PhysParams:
    ...
    # --- PWR6 FRONT longitudinal-slip DOF ---
    # front_sx_coupling: MEASURED softening of the emergent front sx into the combined-slip ellipse
    # (default FRONT_SX_COUPLING=0.40, re-extracted from Chrono; sensitivity knob only).
    front_sx_coupling: float = FRONT_SX_COUPLING
...
def _front_sx_target(fx_target, alpha_lag, fz, fz_nom, mu_scale, P, grip):
    """Solve the EXACT front Fx curve for the longitudinal slip that produces ``fx_target`` (the
    front longitudinal force the car already carries: pwr3's capped FWD drive force + front brake).

    The front sx EMERGES from the carried front FORCE -- it is NOT an imposed-measured slip. We
    invert the combined-slip Fx law by a few fixed-point Newton steps on the per-wheel longitudinal
    slip stiffness k_long = dfx/dsx (read off the EXACT curve at the current iterate), branchless and
    batched. The target is then relaxed over sigma by the caller and fed (scaled by the measured
    FRONT_SX_COUPLING) into the front _wheel_forces, where the friction circle robs front Fy."""
    sx = (fx_target / (fz * mu_scale).clamp_min(1.0) * 0.10).clamp(-0.6, 0.6)  # cheap first guess
    for _ in range(3):
        fx_i, _ = _wheel_forces(sx, alpha_lag, fz, fz_nom, mu_scale, P, grip)
        ds = 0.02
        fxp, _ = _wheel_forces(sx + ds, alpha_lag, fz, fz_nom, mu_scale, P, grip)
        fxm, _ = _wheel_forces(sx - ds, alpha_lag, fz, fz_nom, mu_scale, P, grip)
        k = ((fxp - fxm) / (2.0 * ds))
        k = torch.where(k.abs() < 50.0, torch.full_like(k, 50.0) * torch.sign(k + 1e-9), k)
        sx = (sx + (fx_target - fx_i) / k).clamp(-0.8, 0.8)
    return sx


def _continuous_derivs(vx, vy, yaw_rate, steer, omega_rl, omega_rr, throttle, brake, gear,
                       ax_f, ay_f, alpha_f_lag, alpha_r_lag, sx_rl_lag, sx_rr_lag,
                       sx_fl_lag, sx_fr_lag, P: PhysParamBatch, h: float):
    """Continuous-time derivatives with SLIP relaxation (the L1 transient) + the PWR6 front-Fy DOF.

    pwr3's structure is UNCHANGED (RWD-omega drive, front-traction cap, rear spin states). PWR6 ADDS
    two FRONT longitudinal slip-lag states (sx_fl_lag/sx_fr_lag) whose EMERGENT target is the slip
    that the carried front longitudinal force (capped FWD drive + front brake) implies through the
    EXACT front Fx curve; relaxed over sigma and (scaled by front_sx_coupling) fed into the FRONT
    combined-slip _wheel_forces so the friction circle robs front Fy -- the omitted DOF.

    alpha_*_lag/sx_*_lag are the relaxation-lagged tyre SLIPS; the EXACT curve is evaluated at the
    LAGGED slip, so the force builds over the measured physical relaxation length sigma. The lag is
    advanced SEMI-IMPLICITLY by the sub-step ``h`` BEFORE the force eval. All inputs [N]. Returns
    (vx_dot, vy_dot, yaw_dot, orl_dot, orr_dot, alpha_f_lag, alpha_r_lag, sx_rl_lag, sx_rr_lag,
    sx_fl_lag, sx_fr_lag, ax_body, ay_body)."""
    mu_scale = P.mu / P["mu0"]
    ...
    c_fs = P["front_sx_coupling"]

    # ---- PWR6: the FRONT longitudinal FORCE the car carries, and the EMERGENT front slip ----
    # The Sedan is FWD: the front wheels carry the (capped) drive force AND the front brake. We
    # compute that carried per-wheel front longitudinal force HERE (the same capped FWD drive force
    # pwr3's cap kludge uses, recomputed identically below, plus the front brake), invert the EXACT
    # front Fx curve for the slip that produces it (_front_sx_target), relax that slip over sigma,
    # and (scaled by the measured front_sx_coupling) feed it into the front _wheel_forces so the
    # combined-slip friction circle robs front Fy. The front sx EMERGES from the carried force.
    ...
    T_wheel = 0.5 * (T_eng / ratio.clamp_min(1e-3)) / P["final_drive"].clamp_min(1e-3)
    # front friction budget + cap (IDENTICAL to pwr3's cap, so the longitudinal drive is unchanged).
    # The front Fy used in the cap is from the LAGGED alpha at the current (pre-coupling) front slip;
    # use the previous-step front sx lag for the Fy estimate (a one-substep-old value is sufficient
    # for the friction-circle budget and avoids an algebraic loop).
    _, fy_fl_cap = _wheel_forces(c_fs * sx_fl_lag, alpha_f_lag, fz_fl, fz_nom_f, mu_scale, P, gsf)
    _, fy_fr_cap = _wheel_forces(c_fs * sx_fr_lag, alpha_f_lag, fz_fr, fz_nom_f, mu_scale, P, gsf)
    fmu_fl0 = (mu_scale * P["mu0"]) * fz_fl * gsf
    fmu_fr0 = (mu_scale * P["mu0"]) * fz_fr * gsf
    fx_cap_l0 = torch.sqrt((fmu_fl0 * fmu_fl0 - fy_fl_cap * fy_fl_cap).clamp_min(0.0) + 1.0).clamp_min(0.5 * fmu_fl0)
    fx_cap_r0 = torch.sqrt((fmu_fr0 * fmu_fr0 - fy_fr_cap * fy_fr_cap).clamp_min(0.0) + 1.0).clamp_min(0.5 * fmu_fr0)
    T_wheel_l0 = torch.clamp(T_wheel, -fx_cap_l0 * P["r_eff"], fx_cap_l0 * P["r_eff"])
    T_wheel_r0 = torch.clamp(T_wheel, -fx_cap_r0 * P["r_eff"], fx_cap_r0 * P["r_eff"])
    # carried per-wheel FRONT longitudinal force = capped drive force - front brake (opposes motion).
    T_brake_f = brake * P["max_brake_torque"]
    fx_front_l = T_wheel_l0 / P["r_eff"] - (T_brake_f / P["r_eff"]) * torch.sign(vx_safe)
    fx_front_r = T_wheel_r0 / P["r_eff"] - (T_brake_f / P["r_eff"]) * torch.sign(vx_safe)
    sx_fl_inst = _front_sx_target(fx_front_l, alpha_f_lag, fz_fl, fz_nom_f, mu_scale, P, gsf)
    sx_fr_inst = _front_sx_target(fx_front_r, alpha_f_lag, fz_fr, fz_nom_f, mu_scale, P, gsf)
    ...
    # FRONT wheels (PWR6): real emergent longitudinal slip from the carried front force; the sx
    # entering the EXACT combined-slip ellipse is scaled by the MEASURED front_sx_coupling (pwr3's
    # ellipse is too aggressive at high sx; the re-extracted Chrono coupling softens it to ~0.40).
    _, fy_fl = _wheel_forces(c_fs * sx_fl_lag, alpha_f_lag, fz_fl, fz_nom_f, mu_scale, P, gsf)
    _, fy_fr = _wheel_forces(c_fs * sx_fr_lag, alpha_f_lag, fz_fr, fz_nom_f, mu_scale, P, gsf)
    ...
    # PWR6 keeps pwr3's LONGITUDINAL balance EXACTLY: the front drive force is carried by pwr3's
    # rear-omega cap path (unchanged below), so the front-axle longitudinal contribution to the body
    # stays ZERO as in pwr3 (the emergent front sx is used ONLY to rob the front Fy via combined
    # slip, NOT to re-inject a front longitudinal force -- that would double-count the drive). The
    # ONLY change vs pwr3 is the reduced front Fy (fy_fl/fy_fr now carry the friction-circle robbing).
    Fx_f = torch.zeros_like(vx)
    Fx_r = fx_rl + fx_rr
    Fy_f = fy_fl + fy_fr
    Fy_r = fy_rl + fy_rr
    ...
    return (vx_dot, vy_dot, yaw_dot, omega_rl_dot, omega_rr_dot,
            alpha_f_lag, alpha_r_lag, sx_rl_lag, sx_rr_lag, sx_fl_lag, sx_fr_lag,
            ax_body, ay_body)
...
def physics_step(state: torch.Tensor, action: torch.Tensor, gear: torch.Tensor,
    ...
    for _ in range(nsub):
        ...
        (vx_dot, vy_dot, yaw_dot, orl_dot, orr_dot,
         alpha_f_lag, alpha_r_lag, sx_rl_lag, sx_rr_lag, sx_fl_lag, sx_fr_lag,
         ax_body, ay_body) = _continuous_derivs(
    ...
    out[:, IDX["sx_fl_lag"]] = sx_fl_lag
    out[:, IDX["sx_fr_lag"]] = sx_fr_lag
...
def init_state(vx0: torch.Tensor, vy0: torch.Tensor, yaw0: torch.Tensor, P: PhysParamBatch):
    ...
    st[:, IDX["sx_fl_lag"]] = 0.0   # front in slip equilibrium at init (no drive/brake carried yet)
    st[:, IDX["sx_fr_lag"]] = 0.0
    return st, gear
