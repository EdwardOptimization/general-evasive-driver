"""Run M3144 residual trajectory-timing speed-envelope full-fresh measurement."""
...
from autodrift.engineering_controller_active_safety_driver_residual_trajectory_timing_speed_envelope_materialization_preflight import (
    ACTION_COMPONENTS,
    OUTPUT_SEMANTICS,
    POLICY_CONFIG,
    POLICY_ID,
    residual_trajectory_timing_speed_envelope_action,
)
...
NEXT_ID = (
    "m3145-engineering-controller-active-safety-driver-residual-trajectory-timing-"
    "speed-envelope-full-fresh-measurement-result-audit"
)
M3143_ID = (
...
DEFAULT_M3143_AUDIT = Path(f"docs/{M3143_ID}.md")
...
CLAIM_SCOPE = (
    "M3144 Active Safety Driver residual trajectory-timing speed-envelope full-fresh "
    "measurement preflight only; the complete M3084 fresh current-sim denominator may be "
    "executed through the M3142 direct-action function as the full obs72-to-action3 action "
    "source and measurement, same-row comparison against M3105 M3095 M3100 and M3090, "
    "contract, claim, gate, doc, and M3145 audit artifacts may be written. No validation, "
...
class M3144RepairMeasurementPolicy:
    """Policy adapter that calls the M3142 direct-action repair function."""
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        del info
        action = residual_trajectory_timing_speed_envelope_action(observation, POLICY_CONFIG)
        self.step_count += 1
...
def load_sources(
    *,
    ...
    paths = {
        ...
        "m3142_speed_envelope_rule_rows": m3142_dir / "speed_envelope_rule_rows.csv",
        "m3142_runtime_contract_rows": m3142_dir / "runtime_contract_rows.csv",
    ...
    return {
        ...
        "m3143_audit_text": paths["m3143_audit"].read_text(encoding="utf-8") if exists["m3143_audit"] else "",
        ...
        "m3142_speed_envelope_rule_rows": read_csv_rows(paths["m3142_speed_envelope_rule_rows"]),
        "m3142_runtime_contract_rows": read_csv_rows(paths["m3142_runtime_contract_rows"]),
...
def full_fresh_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(plan, start=1):
        row["runtime_smoke_episode_id"] = f"m3144-measurement-episode-{index:04d}"
    return plan


def _with_scope(rows: list[dict[str, Any]], *, id_prefix: str = "m3144") -> list[dict[str, Any]]:
    updated = []
    for row in rows:
        item = dict(row)
        ...
        if "policy" in item:
            item["policy"] = "active_safety_driver_residual_trajectory_timing_speed_envelope_measurement"
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3144 plan row failed pre-execution guards")
            ...
            policy = M3144RepairMeasurementPolicy()
            try:
                ...
                row = m3088.run_episode_with_policy(
                    env,
                    policy,
                    "active_safety_driver_residual_trajectory_timing_speed_envelope_measurement",
                    int(plan["eval_seed"]),
                )
            finally:
                env.close()
...
def same_row_comparison_rows(
    ...
    for episode in episodes:
        ...
        for baseline_id in BASELINE_IDS:
            ...
            rows.append(
                {
                    "comparison_id": f"m3144-same-row-comparison-{len(rows) + 1:04d}",
                    ...
                    "m3144_outcome_bucket": episode.get("outcome_bucket", ""),
                    ...
                    "m3144_min_clearance_margin": episode.get("min_clearance_margin", ""),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3144 residual trajectory-timing speed-envelope full-fresh measurement artifacts before any validation ranking promotion driver-performance current-sim high-fidelity full-driver repair-success robustness-result or self-ID claim.",
        "lineage": {
            "parent_checkpoint": [
                str(doc_path),
                "runs/m3142_engineering_controller_active_safety_driver_residual_trajectory_timing_speed_envelope_materialization_preflight/direct_action_policy_config.json",
            ],
            "parent_dataset": [
            ...
            "parent_objective": [
                "audit full-fresh M3142 residual trajectory-timing speed-envelope measurement before broader interpretation"
            ],
            "derived_from": [MILESTONE_ID, M3143_ID, M3142_ID, M3105_ID, M3095_ID, M3100_ID],
            "blocked_by": [
                "M3144 full-fresh measurement rows require audit before any validation or repair-success route",
                "same-row comparison against M3105 M3095 M3100 and M3090 is measurement context and not a performance verdict before M3145",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "residual_trajectory_timing_speed_envelope_full_fresh_measurement_result_audit",
            "evidence_increment": "audits full-fresh current-sim measurement rows and same-row comparison from the M3142 residual trajectory-timing speed-envelope direct-action candidate",
            ...
            "stop_condition": [
                "stop if M3144 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "synthesize if M3145 cannot select one next route",
            ],
            "fallback_plan": [
                "route to M3142 artifact repair if artifacts are incomplete or contract-unsafe",
                "route to behavior repair synthesis if measurement is behavior-negative but contract-safe",
                "route to validation planning only after M3145 accepts artifact completeness and claim boundaries",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3144 completes full-fresh M3142 residual trajectory-timing speed-envelope measurement preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3144 full-fresh residual trajectory-timing speed-envelope measurement artifacts",
            "admission_evidence": ["M3144 summary gate matrix comparison measurement metric contract and claim artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3145 status queue scoreboard research log and review",
                "one follow-up manifest only if M3145 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3145 accepts or rejects M3144 as complete and claim-safe",
                "next behavior synthesis validation-planning stop or repair route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3145 audits engineering runtime-measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3145; self-ID/GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3144 full-fresh residual repair measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3144 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits complete fresh-denominator environment-loop evidence through the M3142 residual trajectory-timing speed-envelope direct-action candidate",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3145 audits engineering runtime measurement evidence",
            "must_synthesize_if": [
                "M3145 cannot accept M3144 as complete and claim-safe",
                "M3145 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict robustness-result or self-ID evidence",
                "M3145 cannot select behavior synthesis validation planning stop or next repair route",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M3145 audits M3144 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
        "commands": [
            {
...
def gate_matrix_rows(
    *,
    ...
    config = source["m3142_policy_config"]
    audit_accepts = "accept_m3142_materialization_route_to_full_fresh_measurement" in str(
        source.get("m3143_audit_text", "")
    )
    runtime_contracts = source.get("m3142_runtime_contract_rows", [])
    action_probes = source.get("m3142_action_probe_rows", [])
    ...
    return [
        ...
        gate("m3143_accepts_m3144_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        ...
        gate("m3142_policy_config_status", "lineage", str(config.get("policy_id", "")) == POLICY_ID, config.get("policy_id"), POLICY_ID, "lineage_invalid"),
        gate("m3142_gate_rows_pass", "lineage", all(_bool(row.get("status_pass", False)) for row in source.get("m3142_gate_rows", [])), len(source.get("m3142_gate_rows", [])), "all pass", "lineage_invalid"),
        gate(
            "m3142_speed_envelope_rule_rows_present",
            "lineage",
            len(source.get("m3142_speed_envelope_rule_rows", [])) >= 4,
            len(source.get("m3142_speed_envelope_rule_rows", [])),
            ">=4",
            "lineage_invalid",
        ),
        gate(
            "m3142_runtime_contract_rows_pass",
            "lineage",
...
def render_doc(summary: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            "# M3144 Residual Trajectory-Timing Speed-Envelope Full-Fresh Measurement Preflight",
            "",
            "## Summary",
            "",
            ...
            "## Interpretation",
            "",
            "M3144 records full-fresh current-sim rows through the M3142 residual trajectory-timing speed-envelope direct-action candidate and writes same-row deltas against M3105, M3095, M3100, and M3090. These are measurement and audit-input artifacts for M3145 only. They are not validation, ranking, promotion, repair-success, robustness-result, driver-performance, current-sim verdict, high-fidelity, paper, finite-window-vs-GRU, full-driver, or self-ID evidence.",
            "",
            "Rejected claims:",
            "",
            "```text",
...
def contract_guard_rows(
    *,
    ...
    sample_action = residual_trajectory_timing_speed_envelope_action(
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3145 audit manifest"),
    ]
    blocked = [
    ...
    rows = [
        {
            "claim_id": f"m3144-{claim_id}",
            "claim_family": family,
            "allowed_in_m3144": True,
            "claim_made": made,
            "status_pass": made,
...
def run_full_fresh_measurement_preflight(
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "result_class": (
            "active_safety_driver_residual_trajectory_timing_speed_envelope_full_fresh_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_residual_trajectory_timing_speed_envelope_full_fresh_measurement_preflight_fail"
        ),
        ...
        "decision": "active_safety_driver_residual_trajectory_timing_speed_envelope_full_fresh_measurement_route_to_m3145_result_audit",
        "next_blocker": NEXT_ID,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3143-audit", type=Path, default=DEFAULT_M3143_AUDIT)
