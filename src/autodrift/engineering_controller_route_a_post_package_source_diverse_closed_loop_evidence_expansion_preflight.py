"""Run M2828 post-package source-diverse closed-loop evidence expansion preflight.

M2828 consumes the M2827 design, M2824 package limitation surface, M1690
executable workload matrix, and M2737/M2807/M2816 prior execution surfaces. It
executes one bounded diagnostic rollout for each fixed non-same-surface
L3_online_gru candidate selected by M2827, while keeping package limitations,
prior surfaces, protected rows, and HF3 blocker rows outside execution and
...
DEFAULT_MILESTONE = (
    "m2828-engineering-controller-route-a-post-package-source-diverse-closed-loop-"
    "evidence-expansion-preflight"
)
DEFAULT_NEXT_BLOCKER = (
    "m2829-engineering-controller-route-a-post-package-source-diverse-closed-loop-"
    "evidence-expansion-result-audit"
)
DEFAULT_M2827_DESIGN = Path(
    "docs/m2827-engineering-controller-route-a-post-package-source-diverse-closed-loop-"
    "evidence-expansion-design.md"
)
DEFAULT_M2824_DIR = Path("runs/m2824_engineering_controller_route_a_post_recoverability_negative_limited_package")
...
DEFAULT_DOC_PATH = Path(
    "docs/m2828-engineering-controller-route-a-post-package-source-diverse-closed-loop-"
    "evidence-expansion-preflight.md"
)
DEFAULT_FOLLOW_UP_MANIFEST = Path(
    "experiments/manifests/m2829-engineering-controller-route-a-post-package-source-diverse-"
    "closed-loop-evidence-expansion-result-audit.json"
)
DEFAULT_EVAL_SEED_BASE = 282800
...
SELECTED_TASK_SOURCES: tuple[tuple[str, str, str, str, str, str, tuple[str, ...]], ...] = (
    (
        ...
        ("t4_actuator_delay_or_response", "t4_capability_or_staged_warmup"),
    ),
    (
        "m1680-spec-0009",
        "T4",
    ...
    (
        ...
        ("t5_loss_or_boundary", "t5_near_boundary_or_delay"),
    ),
    (
        "m1680-spec-0039",
        "T5",
        "curved_boundary_obstacle|drive_loss_proxy",
        "mapping_window_unspecified",
        "curved_boundary_obstacle",
        "t5_boundary_axis_retarget",
        ("t5_loss_or_boundary", "t5_curved_or_retargeted_obstacle"),
    ),
    (
        "m1680-spec-0042",
        "T5",
        "t5_boundary_axis_retarget|drive_loss_proxy",
        "mapping_window_unspecified",
        "t5_boundary_axis_retarget",
        "t5_boundary_axis_retarget",
        ("t5_loss_or_boundary", "t5_curved_or_retargeted_obstacle"),
...
EXPECTED_PRIOR_SURFACE_MIN_UNIQUE_COUNT = 21
EXPECTED_SOURCE_FAMILY_COUNT = 5

CLAIM_SCOPE = (
    "M2828 Route A post-package source-diverse closed-loop evidence expansion "
    "preflight only; reset, step, policy action, and rollout fields may be "
    "recorded for the fixed 16 selected M1690 L3_online_gru rows while M2737, "
    "M2807, M2816, same-recoverability, package-limitation, protected, and HF3 "
    "blocker rows remain guardrails outside execution and success denominators. "
    "No replay, validation, training, PPO, source build, adapter probe, external "
    "simulation, ranking, winner selection, promotion, package publication, "
...
FORBIDDEN_INTERPRETATION = (
    "package publication, repair success, recoverability success, driver "
    "performance, validation readiness or result, controller ranking, source "
    "ranking, task-family ranking, scenario-role ranking, stress-axis ranking, "
...
SOURCE_FAMILY_AGGREGATE_FIELDNAMES = [
    "aggregate_id",
    "source_family_tag",
    "candidate_count",
    "episode_count",
...
def run_post_package_source_diverse_closed_loop_evidence_expansion_preflight(
    *,
    m1690_workload: Path | str = DEFAULT_EXECUTABLE_WORKLOAD,
    m2827_design: Path | str = DEFAULT_M2827_DESIGN,
    m2824_dir: Path | str = DEFAULT_M2824_DIR,
    ...
    source = load_source_artifacts(
        ...
        m2827_design=Path(m2827_design),
        m2824_dir=Path(m2824_dir),
    ...
    candidate_rows = build_post_package_candidate_rows(source, prior_exclusion_rows)
    ...
    write_csv_rows(paths["post_package_candidate_rows"], candidate_rows, fieldnames=CANDIDATE_FIELDNAMES)
    ...
    source_family_aggregate_rows = build_source_family_aggregate_rows(
    ...
    gate_rows = build_gate_matrix_rows(
        ...
        source_family_aggregate_rows=source_family_aggregate_rows,
...
def artifact_paths(output_dir: Path, *, doc_path: Path) -> dict[str, Path]:
    return {
        ...
        "post_package_candidate_rows": output_dir / "post_package_candidate_rows.csv",
        ...
        "source_family_aggregate_rows": output_dir / "source_family_aggregate_rows.csv",
...
def load_source_artifacts(
    *,
    ...
    paths = {
        "m2827_design": m2827_design,
        "m2824_summary": m2824_dir / "summary.json",
        "m2824_known_blocker_disclosure_rows": m2824_dir / "known_blocker_disclosure_rows.csv",
        "m2824_recoverability_limitations_rows": m2824_dir / "recoverability_limitations_rows.csv",
        "m2824_actor_action_contract_rows": m2824_dir / "actor_action_contract_rows.csv",
        "m2824_claim_boundary_rows": m2824_dir / "claim_boundary_rows.csv",
    ...
    return {
        ...
        "m2827_design_text": paths["m2827_design"].read_text(encoding="utf-8")
        if source_exists["m2827_design"]
        else "",
        "m2824_summary": read_json(paths["m2824_summary"]) if source_exists["m2824_summary"] else {},
        "m2824_known_blocker_disclosure_rows": read_csv_rows(paths["m2824_known_blocker_disclosure_rows"]),
        "m2824_recoverability_limitations_rows": read_csv_rows(paths["m2824_recoverability_limitations_rows"]),
        "m2824_actor_action_contract_rows": read_csv_rows(paths["m2824_actor_action_contract_rows"]),
        "m2824_claim_boundary_rows": read_csv_rows(paths["m2824_claim_boundary_rows"]),
...
def build_prior_surface_exclusion_rows(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, ((source_panel, task_source_id), row_count) in enumerate(sorted(counts.items()), start=1):
        rows.append(
            {
                "exclusion_id": f"m2828-prior-surface-exclusion-{index:04d}",
                ...
                "execution_run": False,
                "same_recoverability_surface": source_panel == "m2816_instrumented_execution_rows",
...
def build_post_package_candidate_rows(
    ...
    for index, selected in enumerate(SELECTED_TASK_SOURCES, start=1):
        ...
        rows.append(
            {
                "candidate_id": f"m2828-post-package-candidate-{index:04d}",
                "task_source_id": task_source_id,
...
def build_resolution_rows(
    ...
    for index, candidate in enumerate(candidate_rows, start=1):
        resolution_id = f"m2828-resolution-{index:04d}"
...
def build_package_limitation_guard_rows(source: dict[str, Any]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for blocker in source["m2824_known_blocker_disclosure_rows"]:
        rows.append(
            package_guard(
    ...
    for limitation in source["m2824_recoverability_limitations_rows"]:
        rows.append(
            package_guard(
                source_file="recoverability_limitations_rows",
                source_row_id=str(limitation.get("limitation_id", "")),
                blocker_or_limitation=str(limitation.get("limitation_id", "")),
                source_milestone=str(limitation.get("source_milestone", "")),
                observed_value=str(limitation.get("observed_value", "")),
                blocked_interpretation=str(limitation.get("blocked_interpretation", "")),
                current_status="active_limitation",
                index=len(rows) + 1,
            )
        )
    return rows
...
def package_guard(
    *,
    ...
    return {
        "guard_id": f"m2828-package-limitation-guard-{index:04d}",
...
def run_candidate_execution(
    *,
    ...
    summary = {
        "result_class": (
            "engineering_controller_route_a_post_package_source_diverse_closed_loop_evidence_expansion_pass"
            if status_pass
            else "engineering_controller_route_a_post_package_source_diverse_closed_loop_evidence_expansion_incomplete_or_fail"
        ),
...
def candidate_execution_metadata(resolution: Mapping[str, Any], *, eval_seed: int) -> dict[str, Any]:
    return {
        "m2828_eval_seed": int(eval_seed),
        "resolution_id": resolution.get("resolution_id", ""),
        "candidate_id": resolution.get("candidate_id", ""),
        "bounded_post_package_source_diverse_closed_loop_evidence_expansion_preflight": True,
        ...
        "same_recoverability_execution": False,
...
def write_derived_outputs(
    paths: dict[str, Path],
    source_family_aggregate_rows: list[dict[str, Any]],
    ...
    write_csv_rows(
        paths["source_family_aggregate_rows"],
        source_family_aggregate_rows,
        fieldnames=SOURCE_FAMILY_AGGREGATE_FIELDNAMES,
    )
    write_csv_rows(
        paths["scenario_role_metric_rows"],
        scenario_role_metric_rows,
        fieldnames=SCENARIO_ROLE_METRIC_FIELDNAMES,
    )
...
def build_source_family_aggregate_rows(
    *,
    ...
    rows: list[dict[str, Any]] = []
    source_family_tags = sorted({str(row["source_family_tag"]) for row in candidate_rows})
    for index, tag in enumerate(source_family_tags, start=1):
        candidate_ids = [str(candidate["candidate_id"]) for candidate in candidate_rows if candidate["source_family_tag"] == tag]
        family_episodes = [row for candidate_id in candidate_ids for row in episodes_by_candidate.get(candidate_id, [])]
        family_failures = [row for candidate_id in candidate_ids for row in failures_by_candidate.get(candidate_id, [])]
        rows.append(
            {
                "aggregate_id": f"m2828-source-family-aggregate-{index:04d}",
                "source_family_tag": tag,
                "candidate_count": len(candidate_ids),
                "episode_count": len(family_episodes),
                "failure_count": len(family_failures),
                "accounted_count": len(family_episodes) + len(family_failures),
                "success_rate_diagnostic": mean_bool(family_episodes, "success"),
                "collision_rate_diagnostic": mean_bool(family_episodes, "collision"),
                "offtrack_rate_diagnostic": mean_eq(family_episodes, "termination_reason", "off_track"),
                "clearance_margin_mean": mean_float(family_episodes, "min_clearance_margin"),
                "return_mean": mean_float(family_episodes, "return"),
                "all_selected_metrics_finite": selected_metrics_are_finite(family_episodes) if family_episodes else False,
...
def build_scenario_role_metric_rows(
    *,
    ...
    for index, candidate in enumerate(candidate_rows, start=1):
        ...
        rows.append(
            {
                "metric_id": f"m2828-scenario-role-metric-{index:04d}",
...
def build_failure_taxonomy_rows(
    *,
    ...
    for episode in episode_rows:
        ...
        rows.append(
            {
                "failure_taxonomy_id": f"m2828-failure-taxonomy-{len(rows) + 1:04d}",
...
def build_actor_contract_guard_rows(
    *,
    ...
    return [
        ...
        actor_guard(
            "recoverability_labels_actor_visible",
            any_flag(candidate_rows + episode_rows + failure_rows, "recoverability_labels_actor_visible"),
            False,
        ),
        ...
        actor_guard(
            "scenario_role_labels_actor_visible",
            any_flag(candidate_rows + episode_rows + failure_rows, "scenario_role_labels_actor_visible"),
            False,
        ),
...
def actor_guard(field: str, observed: Any, expected: Any) -> dict[str, Any]:
    return {
        "guard_id": f"m2828-actor-guard-{field}",
        "guard_family": field,
        "observed": observed,
        "expected": expected,
...
def build_claim_boundary_rows(
    *,
    ...
    specs = [
        ("bounded_execution_artifact_completeness", "artifact", True, artifacts_present, "M2828 artifact set"),
        ("fresh_closed_loop_diagnostic_rows", "diagnostic_execution", True, episode_or_failure_rows_present, "candidate execution rows or failure rows"),
        ("candidate_accounting", "lineage", True, all_candidates_accounted, "all 16 selected candidates accounted"),
        ("follow_up_result_audit_registered", "follow_up_route", True, follow_up_manifest_registered, "M2829 result-audit manifest"),
...
def claim(claim_id: str, family: str, allowed: bool, made: bool, evidence: str) -> dict[str, Any]:
    return {
        "claim_id": f"m2828-claim-{claim_id}",
        "claim_family": family,
        "allowed_in_m2828": allowed,
        "claim_made": made,
...
def build_gate_matrix_rows(
    *,
    ...
    failure_rows = artifact_rows["candidate_execution_failure_rows"]
    checks = [
        ("source_artifacts_present", "lineage", all(source["source_exists"].values()), source["source_exists"], "all M2827/M2824/M1690/M2737/M2807/M2816/spec/follow-up artifacts present", "lineage_invalid"),
        ...
        ("prior_surface_exclusions_loaded", "candidate_surface", len({row["task_source_id"] for row in prior_exclusion_rows}) >= EXPECTED_PRIOR_SURFACE_MIN_UNIQUE_COUNT, len({row["task_source_id"] for row in prior_exclusion_rows}), f">={EXPECTED_PRIOR_SURFACE_MIN_UNIQUE_COUNT}", "lineage_invalid"),
        ...
        ("source_family_aggregate_rows", "metric", len(source_family_aggregate_rows) == EXPECTED_SOURCE_FAMILY_COUNT, len(source_family_aggregate_rows), EXPECTED_SOURCE_FAMILY_COUNT, "metric_artifact"),
        ...
        ("package_limitation_guard_rows", "boundary", len(package_guard_rows) >= 12, len(package_guard_rows), ">=12", "lineage_invalid"),
        ...
        ("same_recoverability_execution_false", "boundary", not any_flag(episode_rows + failure_rows, "same_recoverability_execution"), any_flag(episode_rows + failure_rows, "same_recoverability_execution"), False, "objective_overfit"),
...
def build_summary(
    *,
    ...
    return {
        "milestone": milestone,
        "result_class": (
            "engineering_controller_route_a_post_package_source_diverse_closed_loop_evidence_expansion_preflight_pass"
            if status_pass
            else "engineering_controller_route_a_post_package_source_diverse_closed_loop_evidence_expansion_preflight_fail"
        ),
        "status_pass": status_pass,
        "generated_at_utc": utc_timestamp(),
        "output_dir": str(output_dir),
        ...
        "source_family_aggregate_row_count": len(source_family_aggregate_rows),
        ...
        "same_recoverability_execution": any_flag(episode_rows + failure_rows, "same_recoverability_execution"),
        ...
        "recoverability_labels_actor_visible": any_flag(candidate_rows + episode_rows + failure_rows, "recoverability_labels_actor_visible"),
        "stress_axis_labels_actor_visible": any_flag(candidate_rows + episode_rows + failure_rows, "stress_axis_labels_actor_visible"),
        "scenario_role_labels_actor_visible": any_flag(candidate_rows + episode_rows + failure_rows, "scenario_role_labels_actor_visible"),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m2827-design", type=Path, default=DEFAULT_M2827_DESIGN)
    parser.add_argument("--m2824-dir", type=Path, default=DEFAULT_M2824_DIR)
...
def main() -> None:
    args = parse_args()
    summary = run_post_package_source_diverse_closed_loop_evidence_expansion_preflight(
        m1690_workload=args.m1690_workload,
        m2827_design=args.m2827_design,
        m2824_dir=args.m2824_dir,
    ...
    print(
        "M2828 post-package source-diverse closed-loop evidence expansion preflight: "
