"""Materialize M3098 v3 high-speed obstacle/edge hard-safety direct-action repair."""
...
MILESTONE_ID = (
    "m3098-engineering-controller-active-safety-driver-v3-high-speed-obstacle-edge-"
    "hard-safety-direct-action-repair-materialization-preflight"
)
NEXT_ID = (
    "m3099-engineering-controller-active-safety-driver-v3-high-speed-obstacle-edge-"
    "hard-safety-direct-action-repair-materialization-result-audit"
)
M3097_ID = "m3097-engineering-controller-active-safety-driver-v2-residual-collision-offtrack-hard-safety-repair-synthesis"
M3095_ID = (
...
POLICY_ID = "m3098_high_speed_obstacle_edge_hard_safety_direct_action_repair_v3"

DEFAULT_M3097_SYNTHESIS = Path(f"docs/{M3097_ID}.md")
...
DEFAULT_OUTPUT_DIR = Path(
    "runs/m3098_engineering_controller_active_safety_driver_v3_high_speed_obstacle_edge_"
...
CLAIM_SCOPE = (
    "M3098 Active Safety Driver v3 high-speed obstacle/edge hard-safety direct-action "
    "repair materialization only; artifacts may define actor-visible obs72 to direct "
    "action3 [steer throttle brake] repair rules, config, guards, doc, and M3099 audit "
    "manifest. No reset, step, rollout, replay, fitting, PPO, training, validation, "
    "ranking, winner selection, checkpoint mutation, checkpoint promotion, driver-"
    "performance verdict, current-sim verdict, repair success, robustness-result, high-"
    "fidelity validation, paper evidence, finite-window-vs-GRU evidence, full ideal "
...
V3_POLICY_CONFIG: dict[str, Any] = deepcopy(V2_POLICY_CONFIG)
V3_POLICY_CONFIG.update(
    {
        "policy_id": POLICY_ID,
        "repair_route": "high_speed_obstacle_edge_hard_safety_direct_action_repair",
...
V3_POLICY_CONFIG["gains"].update(
    {
        "road_center_steer": 0.68,
        "edge_brake": 0.44,
        "obstacle_steer": 0.88,
        "obstacle_brake": 1.02,
        "yaw_damping": 0.28,
        "lateral_velocity_damping": 0.26,
        "steer_rate_damping": 0.12,
        "stability_brake": 0.22,
        "brake_to_throttle_suppression": 0.78,
        "edge_to_throttle_suppression": 0.34,
        "stability_to_throttle_suppression": 0.16,
        "speed_floor_throttle_boost": 0.42,
        "speed_floor_brake_release": 0.32,
        "high_speed_obstacle_brake": 0.40,
        "high_speed_edge_brake": 0.30,
        "high_speed_throttle_suppression": 0.52,
    }
)
V3_POLICY_CONFIG["thresholds"].update(
    {
        "base_throttle_normalized": 0.02,
        "obstacle_lateral_window_m": 7.0,
        "road_edge_warning_margin_m": 2.25,
        "speed_floor_mps": 6.0,
        "hard_safety_speed_mps": 12.0,
        "hard_safety_full_speed_mps": 18.0,
        "speed_floor_recovery_obstacle_urgency_cap": 0.26,
        "speed_floor_recovery_edge_urgency_cap": 0.26,
    }
)
...
def _config_value(config: Mapping[str, Any], section: str, key: str) -> float:
    default = _float(V3_POLICY_CONFIG[section][key])
    return _float(config.get(section, {}).get(key), default)


def high_speed_obstacle_edge_hard_safety_direct_action(
    observation: np.ndarray,
    config: Mapping[str, Any] | None = None,
) -> np.ndarray:
    """Compute v3 direct [steer, throttle, brake] from actor-visible obs72 only."""

    cfg: Mapping[str, Any] = config or V3_POLICY_CONFIG
    ...
    hard_speed = _config_value(cfg, "thresholds", "hard_safety_speed_mps")
    full_speed = _config_value(cfg, "thresholds", "hard_safety_full_speed_mps")
    speed_risk = _clip01((vx_body - hard_speed) / max(full_speed - hard_speed, 1e-6))
    stability_urgency = _clip01((abs(vy_body) / 4.0 + abs(yaw_rate) / 1.5 + abs(ay_body) / 8.0) / 3.0)
    hard_safety_urgency = max(obstacle_urgency, edge_urgency, stability_urgency)
    ...
    brake_physical = _clip01(
        ...
        + _float(gains.get("high_speed_obstacle_brake")) * speed_risk * obstacle_urgency
        + _float(gains.get("high_speed_edge_brake")) * speed_risk * edge_urgency
    )
    ...
    throttle = (
        ...
        - _float(gains.get("high_speed_throttle_suppression")) * speed_risk * hard_safety_urgency
    )
    if recovery_allowed:
...
def build_rule_rows() -> list[dict[str, Any]]:
    specs = [
        (
            "m3098-rule-high-speed-obstacle-brake",
            "high_speed_obstacle_braking_and_throttle_suppression",
            "p0",
            "ego_response;obstacle_slots",
            "brake;throttle;steer",
            "increase braking and suppress throttle when actor-visible obstacle urgency occurs at high speed",
            "high_speed_obstacle_brake",
            0.0,
            1.2,
        ),
        (
            "m3098-rule-high-speed-edge-brake",
            "high_speed_edge_braking_and_corridor_recovery",
            "p0",
            "ego_response;road_left_boundary;road_right_boundary",
            "brake;throttle;steer",
            "increase edge braking and road-centering pressure under high-speed road-edge urgency",
            "high_speed_edge_brake",
            0.0,
            1.0,
        ),
        (
            "m3098-rule-speed-floor-gated",
            "risk_gated_speed_floor_recovery",
            "p1",
            "ego_response;obstacle_slots;road_left_boundary;road_right_boundary",
            "throttle;brake",
            "allow speed-floor throttle only when obstacle and edge urgency remain low",
            "speed_floor_throttle_boost",
            0.0,
            1.0,
        ),
        (
            "m3098-rule-stability-damping",
            "stability_damping_under_hard_safety",
            "p1",
            "ego_response;actuator_state",
            "steer;brake;throttle",
            "damp yaw lateral velocity acceleration and steering pressure while preserving hard-safety braking",
            "stability_brake",
            0.0,
            0.8,
        ),
        (
            "m3098-rule-direct-action-bound",
...
def build_actor_input_exclusion_rows() -> list[dict[str, Any]]:
    ...
    return [
        {
            "exclusion_id": f"m3098-exclusion-{index:04d}",
...
def build_claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3099 audit manifest"),
    ]
    blocked = [
    ...
    rows = [
        {
            "claim_id": f"m3098-{claim_id}",
            "claim_family": family,
            "allowed_in_m3098": True,
            "claim_made": made,
            "status_pass": made,
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3098 v3 hard-safety repair materialization artifacts before any measurement validation ranking promotion driver-performance current-sim verdict high-fidelity paper full-driver repair-success robustness-result or self-ID claim.",
        "lineage": {
            ...
            "derived_from": [MILESTONE_ID, M3097_ID, M3095_ID, M3093_ID],
            "blocked_by": [
                "M3098 materialization artifacts require audit before measurement",
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "v3_hard_safety_materialization_result_audit",
            "evidence_increment": "audits the M3098 v3 direct-action repair materialization artifacts before measurement",
            ...
            "stop_condition": [
                "stop if M3098 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "route to measurement only if M3098 is complete and claim-safe",
            ],
            "fallback_plan": [
            ...
            "synthesis_trigger": "M3098 completes v3 hard-safety repair materialization",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3098 v3 hard-safety repair materialization artifacts",
            "admission_evidence": ["M3098 summary rule config exclusion claim and gate artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3099 status queue scoreboard research log and review",
                "one follow-up manifest only if M3099 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3099 accepts or rejects M3098 as complete and claim-safe",
                "next measurement, artifact-repair, synthesis, or stop route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3099 audits engineering materialization artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3099; self-ID/GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3098 materialization artifacts only.",
            "negative_result_policy": "Reject or repair M3098 artifacts rather than returning self-ID to the mainline objective.",
            "allowed_claims": [
                "M3098 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits the new v3 repair materialization before measurement",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3099 prepares a measurement route decision",
            "must_synthesize_if": [
                "M3099 cannot accept M3098 as complete and claim-safe",
                "M3099 would claim validation driver-performance paper high-fidelity current-sim verdict repair-success robustness-result or self-ID evidence",
                "M3099 cannot select exactly one measurement repair synthesis or stop route",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M3099 audits M3098 artifacts and selects one next route while preserving actor and claim boundaries.",
        "commands": [{"name": "active_safety_driver_v3_repair_materialization_result_audit_doc", "command": "true"}],
...
def _probe_rows() -> list[dict[str, Any]]:
    probes = [
        ...
        ("high_speed_obstacle", _probe_observation(speed_mps=17.0, obstacle=True)),
        ("high_speed_edge", _probe_observation(speed_mps=16.0, edge_urgent=True, lateral_velocity_norm=0.2)),
    ]
    rows = []
    for name, obs in probes:
        action = high_speed_obstacle_edge_hard_safety_direct_action(obs, V3_POLICY_CONFIG)
        rows.append(
            {
                "probe_id": f"m3098-probe-{name}",
...
def build_gate_rows(
    *,
    ...
    config = V3_POLICY_CONFIG
    termination_counts = Counter(str(row.get("termination_reason", "")) for row in residual_rows)
    rule_families = {str(row.get("rule_family", "")) for row in rule_rows}
    return [
        gate("m3097_synthesis_present", "source", bool(m3097_text), True, "lineage_invalid"),
        gate("m3097_route_marker", "source", "route_to_m3098_v3_high_speed_obstacle_edge_hard_safety_repair_materialization" in m3097_text, True, "lineage_invalid"),
        gate("m3095_status_pass", "source", m3095_summary.get("status_pass"), True, "lineage_invalid"),
        gate("m3095_gate_matrix_pass", "source", m3095_summary.get("gate_matrix_pass"), True, "lineage_invalid"),
        gate("m3095_collision_count", "source", m3095_summary.get("measurement_collision_count"), 5, "behavior_regression"),
        gate("m3095_offtrack_count", "source", m3095_summary.get("measurement_offtrack_count"), 2, "behavior_regression"),
        gate("m3095_speed_too_low_count", "source", m3095_summary.get("measurement_speed_too_low_count"), 0, "metric_artifact"),
        gate("residual_row_count", "source", len(residual_rows), 7, "metric_artifact"),
        gate("residual_collision_count", "source", termination_counts.get("obstacle_collision", 0), 5, "metric_artifact"),
        gate("residual_offtrack_count", "source", termination_counts.get("off_track", 0), 2, "metric_artifact"),
        ...
        gate("high_speed_obstacle_gain_present", "repair_config", "high_speed_obstacle_brake" in config.get("gains", {}), True, "metric_artifact"),
        gate("high_speed_edge_gain_present", "repair_config", "high_speed_edge_brake" in config.get("gains", {}), True, "metric_artifact"),
        gate("rule_rows_present", "artifact", len(rule_rows) >= 5, True, "metric_artifact"),
        gate("high_speed_obstacle_rule_present", "artifact", "high_speed_obstacle_braking_and_throttle_suppression" in rule_families, True, "metric_artifact"),
        gate("high_speed_edge_rule_present", "artifact", "high_speed_edge_braking_and_corridor_recovery" in rule_families, True, "metric_artifact"),
        ...
        gate("high_speed_obstacle_probe_brakes", "repair_config", probe_rows[1]["brake"] > 0.4, True, "behavior_regression"),
        gate("high_speed_obstacle_probe_suppresses_throttle", "repair_config", probe_rows[1]["throttle"] < 0.0, True, "behavior_regression"),
        gate("high_speed_edge_probe_brakes", "repair_config", probe_rows[2]["brake"] > 0.0, True, "behavior_regression"),
        gate("high_speed_edge_probe_suppresses_throttle", "repair_config", probe_rows[2]["throttle"] < 0.0, True, "behavior_regression"),
...
def write_doc(path: Path, summary: Mapping[str, Any]) -> None:
    text = f"""# M3098 Active Safety Driver v3 High-Speed Obstacle/Edge Hard-Safety Repair Materialization Preflight
...
def run_preflight(
    *,
    ...
    m3097_text = m3097_synthesis.read_text(encoding="utf-8") if m3097_synthesis.exists() else ""
    m3095_summary = read_json(m3095_dir / "summary.json") if (m3095_dir / "summary.json").exists() else {}
    m3095_rows = read_csv_rows(m3095_dir / "measurement_episode_rows.csv")
    m3093_summary = read_json(m3093_dir / "summary.json") if (m3093_dir / "summary.json").exists() else {}
    residual_rows = [row for row in m3095_rows if str(row.get("termination_reason", ""))]
    ...
    write_json(paths["direct_action_policy_config"], V3_POLICY_CONFIG)
    ...
    gate_rows = build_gate_rows(
        m3097_text=m3097_text,
        m3095_summary=m3095_summary,
    ...
    summary: dict[str, Any] = {
        ...
        "required_artifacts_present": required_artifacts_present,
        "result_class": "active_safety_driver_v3_high_speed_obstacle_edge_hard_safety_repair_materialization_preflight_pass",
        ...
        "m3095_status_pass": _bool(m3095_summary.get("status_pass", False)),
        "m3095_gate_matrix_pass": _bool(m3095_summary.get("gate_matrix_pass", False)),
        "m3095_residual_count": len(residual_rows),
        "m3095_collision_count": int(termination_counts.get("obstacle_collision", 0)),
        "m3095_offtrack_count": int(termination_counts.get("off_track", 0)),
        "m3095_speed_too_low_count": int(termination_counts.get("speed_too_low", 0)),
        "m3093_status_pass": _bool(m3093_summary.get("status_pass", False)),
        "low_speed_probe_throttle": probe_rows[0]["throttle"],
        "high_speed_obstacle_probe_throttle": probe_rows[1]["throttle"],
        "high_speed_obstacle_probe_brake": probe_rows[1]["brake"],
        "high_speed_edge_probe_throttle": probe_rows[2]["throttle"],
        "high_speed_edge_probe_brake": probe_rows[2]["brake"],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3097-synthesis", type=Path, default=DEFAULT_M3097_SYNTHESIS)
...
def main() -> None:
    ...
    summary = run_preflight(
        m3097_synthesis=args.m3097_synthesis,
    ...
    print(f"high_speed_obstacle_probe_brake={summary['high_speed_obstacle_probe_brake']}")
    print(f"high_speed_edge_probe_brake={summary['high_speed_edge_probe_brake']}")
