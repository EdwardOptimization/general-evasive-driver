"""Run M2838 post Route C HF3 stop source-diverse closed-loop preflight.

M2838 consumes the M2837 design and the current M1690 executable workload. It
executes one bounded diagnostic rollout for each fixed unused L3_online_gru row
selected by M2837 while keeping M2737, M2759, M2807, M2816, M2828, protected,
and HF3 blocker rows outside execution and outside ordinary success
denominators.
"""
...
DEFAULT_MILESTONE = "m2838-engineering-controller-post-route-c-hf3-stop-fresh-source-diverse-closed-loop-evidence-preflight"
DEFAULT_NEXT_BLOCKER = "m2839-engineering-controller-post-route-c-hf3-stop-fresh-source-diverse-closed-loop-evidence-result-audit"
DEFAULT_M2837_DESIGN = Path(
    "docs/m2837-engineering-controller-post-route-c-hf3-stop-evidence-producing-branch-selection-design.md"
)
...
DEFAULT_M2759_DIR = Path(
    ...
    "containment_probe_bounded_execution_preflight"
)
...
SELECTED_TASK_SOURCES: tuple[tuple[str, str, str, str, str, str, tuple[str, ...]], ...] = (
    ...
    (
        ...
        "t5_near_boundary_warmup",
        "t5_near_boundary_warmup",
        ("t5_near_boundary_or_delay", "t5_capability_step_down"),
    ),
    (
        "m1680-spec-0056",
        "T5",
        ...
        ("t5_curved_or_retargeted_obstacle",),
    ),
    (
        "m1680-spec-0057",
        "T5",
...
EXPECTED_PRIOR_SURFACE_MIN_UNIQUE_COUNT = 43

CLAIM_SCOPE = (
    "M2838 Route A post Route C HF3 stop fresh source-diverse closed-loop "
    "evidence preflight only; reset, step, policy action, and rollout fields "
    "may be recorded for the fixed 16 selected M1690 L3_online_gru rows while "
    "M2737, M2759, M2807, M2816, M2828, protected, and HF3 blocker rows remain "
...
FORBIDDEN_INTERPRETATION = (
    ...
    "controller ranking, source ranking, task-family ranking, scenario-role "
    "ranking, stress-axis ranking, profile ranking, winner selection, "
...
def run_post_route_c_hf3_stop_source_diverse_closed_loop_evidence_preflight(
    *,
    m1690_workload: Path | str = DEFAULT_EXECUTABLE_WORKLOAD,
    m2837_design: Path | str = DEFAULT_M2837_DESIGN,
    m2737_dir: Path | str = DEFAULT_M2737_DIR,
    m2759_dir: Path | str = DEFAULT_M2759_DIR,
    ...
    source = load_source_artifacts(
        ...
        m2837_design=Path(m2837_design),
        m2737_dir=Path(m2737_dir),
        m2759_dir=Path(m2759_dir),
    ...
    candidate_rows = build_selected_candidate_rows(source, prior_exclusion_rows)
    resolution_rows, resolved_sources = build_resolution_rows(candidate_rows, source["m1690_by_task_source"])

    write_csv_rows(paths["selected_candidate_rows"], candidate_rows, fieldnames=CANDIDATE_FIELDNAMES)
    ...
    for _ in range(2):
        ...
        if paths["summary"].exists() and paths["doc"].exists():
...
def artifact_paths(output_dir: Path, *, doc_path: Path, follow_up_manifest: Path) -> dict[str, Path]:
    return {
        ...
        "selected_candidate_rows": output_dir / "selected_candidate_rows.csv",
...
def load_source_artifacts(
    *,
    ...
    return {
        ...
        "m2837_design_text": paths["m2837_design"].read_text(encoding="utf-8")
        if source_exists["m2837_design"]
        else "",
...
def build_prior_surface_exclusion_rows(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for source_panel, key in (
        ...
        ("m2759_probe_execution_rows", "m2759_probe_execution_rows"),
        ...
        for row in source[key]:
            ...
            if task_source_id and row.get("profile_name", CANONICAL_PROFILE) == CANONICAL_PROFILE:
    ...
    for index, ((source_panel, task_source_id), row_count) in enumerate(sorted(counts.items()), start=1):
        rows.append(
            {
                "exclusion_id": f"m2838-prior-surface-exclusion-{index:04d}",
...
def build_selected_candidate_rows(
    ...
    for index, selected in enumerate(SELECTED_TASK_SOURCES, start=1):
        ...
        rows.append(
            {
                "candidate_id": f"m2838-selected-candidate-{index:04d}",
                "task_source_id": task_source_id,
...
def build_resolution_rows(
    ...
    for index, candidate in enumerate(candidate_rows, start=1):
        resolution_id = f"m2838-resolution-{index:04d}"
...
def run_candidate_execution(
    *,
    ...
    summary = {
        "result_class": (
            "engineering_controller_post_route_c_hf3_stop_source_diverse_closed_loop_evidence_preflight_pass"
            if status_pass
            else "engineering_controller_post_route_c_hf3_stop_source_diverse_closed_loop_evidence_preflight_incomplete_or_fail"
        ),
...
def candidate_execution_metadata(resolution: Mapping[str, Any], *, eval_seed: int) -> dict[str, Any]:
    return {
        "m2838_eval_seed": int(eval_seed),
        "resolution_id": resolution.get("resolution_id", ""),
        "candidate_id": resolution.get("candidate_id", ""),
        "bounded_post_route_c_hf3_stop_source_diverse_closed_loop_evidence_preflight": True,
...
def build_scenario_role_metric_rows(
    *,
    ...
    for index, candidate in enumerate(candidate_rows, start=1):
        ...
        rows.append(
            {
                "metric_id": f"m2838-scenario-role-metric-{index:04d}",
...
def build_failure_taxonomy_rows(
    *,
    ...
    for episode in episode_rows:
        ...
        rows.append(
            {
                "failure_taxonomy_id": f"m2838-failure-taxonomy-{len(rows) + 1:04d}",
...
def build_actor_contract_guard_rows(
    *,
    ...
    return [
        ...
        actor_guard("source_labels_actor_visible", any_flag(candidate_rows + episode_rows + failure_rows, "source_labels_actor_visible"), False),
...
def actor_guard(field: str, observed: Any, expected: Any) -> dict[str, Any]:
    return {
        "guard_id": f"m2838-actor-guard-{field}",
        "guard_family": field,
        "observed": observed,
        "expected": expected,
...
def build_claim_boundary_rows(
    *,
    ...
    specs = [
        ("bounded_execution_artifact_completeness", "artifact", True, artifacts_present, "M2838 artifact set"),
        ...
        ("follow_up_result_audit_registered", "follow_up_route", True, follow_up_manifest_registered, "M2839 result-audit manifest"),
...
def claim(claim_id: str, family: str, allowed: bool, made: bool, evidence: str) -> dict[str, Any]:
    return {
        "claim_id": f"m2838-claim-{claim_id}",
        "claim_family": family,
        "allowed_in_m2838": allowed,
        "claim_made": made,
...
def build_gate_matrix_rows(
    *,
    ...
    failure_rows = artifact_rows["candidate_execution_failure_rows"]
    checks = [
        ("source_artifacts_present", "lineage", all(source["source_exists"].values()), source["source_exists"], "all M2837/M1690/M2737/M2759/M2807/M2816/M2828/spec/follow-up artifacts present", "lineage_invalid"),
        ...
        ("ordinary_denominator_false", "boundary", not any_flag(episode_rows + failure_rows + prior_exclusion_rows, "ordinary_success_denominator_allowed"), any_flag(episode_rows + failure_rows + prior_exclusion_rows, "ordinary_success_denominator_allowed"), False, "proof_washout"),
    ...
    *,
    ...
    return {
        ...
        "result_class": (
            "engineering_controller_post_route_c_hf3_stop_source_diverse_closed_loop_evidence_preflight_pass"
            if status_pass
            else "engineering_controller_post_route_c_hf3_stop_source_diverse_closed_loop_evidence_preflight_fail"
        ),
        "status_pass": status_pass,
        "generated_at_utc": utc_timestamp(),
        "output_dir": str(output_dir),
        ...
        "ordinary_success_denominator_allowed": any_flag(episode_rows + failure_rows + prior_exclusion_rows, "ordinary_success_denominator_allowed"),
        ...
        "source_labels_actor_visible": any_flag(candidate_rows + episode_rows + failure_rows, "source_labels_actor_visible"),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m2837-design", type=Path, default=DEFAULT_M2837_DESIGN)
...
def main() -> None:
    args = parse_args()
    summary = run_post_route_c_hf3_stop_source_diverse_closed_loop_evidence_preflight(
        m1690_workload=args.m1690_workload,
        m2837_design=args.m2837_design,
    ...
    print(
        "M2838 post Route C HF3 stop source-diverse closed-loop evidence preflight: "
