"""Sequence-level interventions over M749 v4 reset-sensitive source rows."""
...
def _source_row(raw: dict[str, Any], *, role: str, source_pool: str) -> dict[str, Any]:
    ...
    row["source_kind"] = "v4_reset_source"
    row["claim_boundary_level"] = "current_model_or_proxy"
...
def load_v4_reset_source_rows(
    *,
    ...
    primary_candidates = [
        _source_row(row, role="primary", source_pool="m749_v4_reset_only")
    ...
    sentinel_candidates = [
        _source_row(row, role="sentinel", source_pool="m749_v4_history_insensitive")
...
def _v4_source_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def _row_for_v4_sequence_variant(
    *,
    source: dict[str, Any],
    variant: str,
    horizon: int,
    ...
    for key in V4_EXTRA_SOURCE_FIELDS:
        row[key] = source.get(key, "")
    row["source_kind"] = str(source.get("source_kind", "v4_reset_source"))
    return row


def classify_v4_reset_sequence_result(
    *,
    result_class: str,
) -> str:
    mapping = {
        "sequence_outcome_positive": "v4_reset_sequence_outcome_positive",
        "sequence_action_only": "v4_reset_sequence_action_only",
        "sequence_source_balance_blocked": "v4_reset_source_balance_blocked",
        "sequence_artifact": "v4_reset_sequence_artifact",
        "sequence_neutral": "v4_reset_sequence_neutral",
    }
    return mapping.get(str(result_class), f"v4_reset_{result_class}")


def run_v4_reset_source_sequence_intervention(
    *,
    ...
    if not model.is_online_recurrent:
        raise ValueError("v4 reset-source sequence intervention requires an online recurrent checkpoint")
    ...
    source_rows = load_v4_reset_source_rows(
    ...
    source_balance = _v4_source_summary(source_rows)
    ...
    base_result = classify_sequence_result(
        ...
        min_sequence_action_rows=400,
        min_sequence_action_seeds=12,
        min_sequence_outcome_rows=40,
        min_sequence_outcome_seeds=10,
        min_sequence_outcome_pairs=6,
        max_sequence_outcome_dominance=0.25,
    )
    result_class = classify_v4_reset_sequence_result(result_class=base_result)
    ...
    summary = {
        "run_type": "v4_reset_source_sequence_intervention",
        ...
        "future_only_fault_count": int(len(config.get("future_only_faults", []))),
        ...
        **source_balance,
        ...
        "sequence_outcome_positive": bool(result_class == "v4_reset_sequence_outcome_positive"),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run v4 reset-source sequence interventions.")
    ...
    parser.add_argument("--seed-start", type=int, default=76000)
    ...
    run_dir = args.run_dir or make_run_dir(prefix="v4_reset_source_sequence_intervention")
    summary = run_v4_reset_source_sequence_intervention(
