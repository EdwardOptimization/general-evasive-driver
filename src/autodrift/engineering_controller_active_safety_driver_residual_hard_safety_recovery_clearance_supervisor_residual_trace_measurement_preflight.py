"""Run M3210 recovery-clearance supervisor residual-trace measurement."""
...
from autodrift.engineering_controller_active_safety_driver_residual_hard_safety_recovery_clearance_supervisor_architecture_materialization_preflight import (
    ACTION_COMPONENTS,
    OUTPUT_SEMANTICS,
    POLICY_ID,
    SUPERVISOR_POLICY_CONFIG as POLICY_CONFIG,
    recovery_clearance_supervisor_candidate_action,
)
...
MILESTONE_ID = (
    "m3210-engineering-controller-active-safety-driver-residual-hard-safety-"
    "recovery-clearance-supervisor-residual-trace-measurement-preflight"
)
NEXT_ID = (
    "m3211-engineering-controller-active-safety-driver-residual-hard-safety-"
    "recovery-clearance-supervisor-residual-trace-measurement-result-audit"
)
M3209_ID = (
...
DEFAULT_M3209_AUDIT = Path(f"docs/{M3209_ID}.md")
DEFAULT_M3208_DIR = Path(
...
DEFAULT_OUTPUT_DIR = Path(
    "runs/m3210_engineering_controller_active_safety_driver_residual_hard_safety_"
    "recovery_clearance_supervisor_residual_trace_measurement_preflight"
)
...
CLAIM_SCOPE = (
    "M3210 Active Safety Driver recovery-clearance supervisor residual-trace "
    "measurement preflight only; the seven accepted residual blocker trace bindings "
    "may be executed through the M3208 deterministic obs72-to-action3 supervisor "
    "candidate and compared offline against same-trace M3205 prior candidate and "
    "M3189 incumbent evidence. Artifacts may include candidate trace rows, trace-step "
    "rows, same-trace comparison rows, guards, claims, gates, doc, and M3211 audit manifest. "
...
COMPARISON_FIELDNAMES = [
    ...
    "m3194_offtrack",
    "incumbent_offtrack",
    "m3210_termination_reason",
    "m3205_termination_reason",
    "m3194_termination_reason",
    "incumbent_termination_reason",
    "m3210_min_clearance_margin",
    "m3205_min_clearance_margin",
    "m3194_min_clearance_margin",
    "incumbent_min_clearance_margin",
    "m3210_vs_m3205_clearance_margin_delta",
    "m3210_vs_m3194_clearance_margin_delta",
    "m3210_vs_incumbent_clearance_margin_delta",
    "m3210_return",
    "m3205_return",
    "m3194_return",
    "incumbent_return",
    "m3210_vs_m3205_return_delta",
    "m3210_vs_m3194_return_delta",
    "m3210_vs_incumbent_return_delta",
    "m3210_speed_mean",
    "m3205_speed_mean",
    "m3194_speed_mean",
    "incumbent_speed_mean",
    "m3210_vs_m3205_speed_mean_delta",
    "m3210_vs_m3194_speed_mean_delta",
    "m3210_vs_incumbent_speed_mean_delta",
    "aligned_step_count_vs_m3205",
    "obs_hash_match_count_vs_m3205",
    "meaningful_action_delta_step_count_vs_m3205",
    "preterminal_action_delta_step_count_vs_m3205",
    "terminal_window_action_delta_step_count_vs_m3205",
    "mean_action_delta_l2_vs_m3205",
    "max_action_delta_l2_vs_m3205",
...
def load_sources(
    *,
    ...
    paths = {
        "m3209_audit": m3209_audit,
        "m3208_summary": m3208_dir / "summary.json",
        "m3208_gate_rows": m3208_dir / "gate_matrix.csv",
        "m3208_runtime_contract_rows": m3208_dir / "runtime_contract_rows.csv",
        "m3205_summary": m3205_dir / "summary.json",
        "m3205_candidate_trace_execution_rows": m3205_dir / "candidate_trace_execution_rows.csv",
        "m3205_candidate_trace_step_rows": m3205_dir / "candidate_trace_step_rows.csv",
    ...
    return {
        ...
        "m3209_audit_text": paths["m3209_audit"].read_text(encoding="utf-8") if exists["m3209_audit"] else "",
        "m3208_summary": read_json(paths["m3208_summary"]) if exists["m3208_summary"] else {},
        "m3208_gate_rows": read_csv_rows(paths["m3208_gate_rows"]),
        "m3208_runtime_contract_rows": read_csv_rows(paths["m3208_runtime_contract_rows"]),
        "m3205_summary": read_json(paths["m3205_summary"]) if exists["m3205_summary"] else {},
        "m3205_candidate_trace_execution_rows": read_csv_rows(paths["m3205_candidate_trace_execution_rows"]),
        "m3205_candidate_trace_step_rows": read_csv_rows(paths["m3205_candidate_trace_step_rows"]),
...
def trace_execution_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    m3205_by_binding = {
        str(row.get("trace_source_binding_id", "")): dict(row)
        for row in source["m3205_candidate_trace_execution_rows"]
    }
    rows = sorted(
    ...
    for index, incumbent in enumerate(rows, start=1):
        ...
        m3205 = m3205_by_binding.get(str(incumbent.get("trace_source_binding_id", "")), {})
        ...
        status_pass = bool(
            _bool(source["m3208_summary"].get("status_pass", False))
            and _bool(source["m3205_summary"].get("status_pass", False))
        ...
        plan.append(
            {
                **incumbent,
                **workload,
                "trace_execution_id": f"m3210-candidate-trace-execution-{index:04d}",
                "m3205_trace_execution_id": m3205.get("trace_execution_id", ""),
...
class M3210CandidateTraceDriver:
    def act(self, observation: np.ndarray) -> np.ndarray:
        return recovery_clearance_supervisor_candidate_action(observation, POLICY_CONFIG)
...
def failure_row(plan: Mapping[str, Any], *, error_type: str, error_message: str) -> dict[str, Any]:
    ...
    row.update(
        {
            "trace_failure_id": f"m3210-trace-failure-{plan.get('trace_execution_id', '')}",
    ...
    item.update(
        {
            ...
            "claim_boundary": CLAIM_SCOPE,
            "m3205_trace_execution_id": plan.get("m3205_trace_execution_id", ""),
...
def run_candidate_trace_execution_plan(
    *,
    ...
    driver = M3210CandidateTraceDriver()
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3210 trace execution plan row failed pre-execution guards")
...
def same_trace_comparison_rows(
    *,
    m3210_executions: list[dict[str, Any]],
    m3210_steps: list[dict[str, Any]],
    ...
    m3210_by_binding = {str(row.get("trace_source_binding_id", "")): dict(row) for row in m3210_executions}
    ...
    m3210_steps_by_binding = _steps_by_binding(m3210_steps)
    ...
    for binding_id in sorted(set(m3210_by_binding) | set(m3205_by_binding) | set(m3194_by_binding) | set(incumbent_by_binding)):
        candidate = m3210_by_binding.get(binding_id, {})
        m3205 = m3205_by_binding.get(binding_id, {})
        m3194 = m3194_by_binding.get(binding_id, {})
        incumbent = incumbent_by_binding.get(binding_id, {})
        context = candidate or m3205 or m3194 or incumbent
        stats_m3205 = _step_delta_stats(m3210_steps_by_binding.get(binding_id, []), m3205_steps_by_binding.get(binding_id, []))
        stats_m3194 = _step_delta_stats(m3210_steps_by_binding.get(binding_id, []), m3194_steps_by_binding.get(binding_id, []))
        stats_incumbent = _step_delta_stats(
            m3210_steps_by_binding.get(binding_id, []),
            incumbent_steps_by_binding.get(binding_id, []),
        )
        delta_vs_m3205 = _bad_count(m3205) - _bad_count(candidate)
        delta_vs_m3194 = _bad_count(m3194) - _bad_count(candidate)
        delta_vs_incumbent = _bad_count(incumbent) - _bad_count(candidate)
        rows.append(
            {
                "comparison_id": f"m3210-same-trace-comparison-{len(rows) + 1:04d}",
                ...
                "m3210_trace_execution_id": candidate.get("trace_execution_id", ""),
                "m3205_trace_execution_id": m3205.get("trace_execution_id", ""),
                ...
                "m3210_success": _bool(candidate.get("success", False)),
                "m3205_success": _bool(m3205.get("success", False)),
                "m3194_success": _bool(m3194.get("success", False)),
                "incumbent_success": _bool(incumbent.get("success", False)),
                "m3210_collision": _bool(candidate.get("collision", False)),
                "m3205_collision": _bool(m3205.get("collision", False)),
                "m3194_collision": _bool(m3194.get("collision", False)),
                "incumbent_collision": _bool(incumbent.get("collision", False)),
                "m3210_offtrack": _offtrack(candidate),
                "m3205_offtrack": _offtrack(m3205),
                "m3194_offtrack": _offtrack(m3194),
                "incumbent_offtrack": _offtrack(incumbent),
                "m3210_termination_reason": candidate.get("termination_reason", ""),
                "m3205_termination_reason": m3205.get("termination_reason", ""),
                "m3194_termination_reason": m3194.get("termination_reason", ""),
                "incumbent_termination_reason": incumbent.get("termination_reason", ""),
                "m3210_min_clearance_margin": _float(candidate.get("min_clearance_margin")),
                "m3205_min_clearance_margin": _float(m3205.get("min_clearance_margin")),
                ...
                "m3210_vs_m3205_clearance_margin_delta": _float(candidate.get("min_clearance_margin")) - _float(m3205.get("min_clearance_margin")),
                "m3210_vs_m3194_clearance_margin_delta": _float(candidate.get("min_clearance_margin")) - _float(m3194.get("min_clearance_margin")),
                "m3210_vs_incumbent_clearance_margin_delta": _float(candidate.get("min_clearance_margin")) - _float(incumbent.get("min_clearance_margin")),
                "m3210_return": _float(candidate.get("return")),
                "m3205_return": _float(m3205.get("return")),
                "m3194_return": _float(m3194.get("return")),
                "incumbent_return": _float(incumbent.get("return")),
                "m3210_vs_m3205_return_delta": _float(candidate.get("return")) - _float(m3205.get("return")),
                "m3210_vs_m3194_return_delta": _float(candidate.get("return")) - _float(m3194.get("return")),
                "m3210_vs_incumbent_return_delta": _float(candidate.get("return")) - _float(incumbent.get("return")),
                "m3210_speed_mean": _float(candidate.get("speed_mean")),
                "m3205_speed_mean": _float(m3205.get("speed_mean")),
                "m3194_speed_mean": _float(m3194.get("speed_mean")),
                "incumbent_speed_mean": _float(incumbent.get("speed_mean")),
                "m3210_vs_m3205_speed_mean_delta": _float(candidate.get("speed_mean")) - _float(m3205.get("speed_mean")),
                "m3210_vs_m3194_speed_mean_delta": _float(candidate.get("speed_mean")) - _float(m3194.get("speed_mean")),
                "m3210_vs_incumbent_speed_mean_delta": _float(candidate.get("speed_mean")) - _float(incumbent.get("speed_mean")),
                "aligned_step_count_vs_m3205": stats_m3205["aligned_step_count"],
                "obs_hash_match_count_vs_m3205": stats_m3205["obs_hash_match_count"],
                "meaningful_action_delta_step_count_vs_m3205": stats_m3205["meaningful_action_delta_step_count"],
                "preterminal_action_delta_step_count_vs_m3205": stats_m3205["preterminal_action_delta_step_count"],
                "terminal_window_action_delta_step_count_vs_m3205": stats_m3205["terminal_window_action_delta_step_count"],
                "mean_action_delta_l2_vs_m3205": stats_m3205["mean_action_delta_l2"],
                "max_action_delta_l2_vs_m3205": stats_m3205["max_action_delta_l2"],
                ...
                "outcome_changed_vs_m3205": _outcome_changed(candidate, m3205),
...
def guard(
    ...
    *,
    ...
    item["guard_id"] = str(item["guard_id"]).replace("m3189-", "m3210-", 1)
...
def contract_guard_rows(
    *,
    ...
    sample_action = recovery_clearance_supervisor_candidate_action(np.zeros(P0_OBSERVATION_DIM, dtype=np.float32), POLICY_CONFIG)
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    claims = [
        ...
        ("self_id", "forbidden", False, False, "history necessity tests outside M3210"),
    ]
    return [
        {
            "claim_id": f"m3210-{claim_id}",
            "claim_family": family,
            "allowed_in_m3210": allowed,
            "claim_made": made,
...
def _m3209_selects_m3210(text: str) -> bool:
    return (
        MILESTONE_ID
        in text
        or "M3210 may execute the M3208 candidate" in text
        or "ready for same-seven residual-trace measurement" in text
    )
...
def gate_matrix_rows(
    *,
    ...
    return [
        ...
        gate("m3209_selects_m3210_route", "lineage", _m3209_selects_m3210(source["m3209_audit_text"]), "route marker", "present", "lineage_invalid"),
        gate("m3208_status_pass", "lineage", _bool(source["m3208_summary"].get("status_pass")), source["m3208_summary"].get("status_pass"), True, "lineage_invalid"),
        gate("m3208_gate_matrix_pass", "lineage", _bool(source["m3208_summary"].get("gate_matrix_pass")), source["m3208_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
        ...
        gate("m3205_gate_matrix_pass", "lineage", _bool(source["m3205_summary"].get("gate_matrix_pass")), source["m3205_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject M3210 recovery-clearance supervisor residual-trace measurement artifacts before full-fresh measurement validation ranking promotion driver-performance current-sim high-fidelity full-driver repair-success robustness-result feasibility-proof or self-ID claim.",
        "lineage": {
            ...
            "parent_objective": ["audit M3210 recovery-clearance supervisor residual-trace measurement before full-fresh measurement or synthesis"],
            "derived_from": [MILESTONE_ID, M3209_ID, M3208_ID, M3205_ID, M3199_ID, M3189_ID, M3187_ID, M3012_ID],
            "blocked_by": [
                "M3210 measurement artifacts require audit before interpretation",
                "M3210 is measurement only and not validation repair success or public default replacement",
            ],
            "supersedes": ["unreviewed M3210 residual-trace measurement interpretation"],
            "invalidates": [],
        },
        "review_artifact": f"docs/reviews/{NEXT_ID}.md",
        "public_gates": [
            "M3211 must audit M3210 candidate trace rows step rows same-trace comparison rows guards claims and gates",
            "M3211 must preserve obs72-only actor runtime and direct [steer throttle brake] action contract",
            "M3211 must reject validation ranking promotion driver-performance current-sim high-fidelity full-driver repair-success robustness-result feasibility-proof and self-ID claims",
            "M3211 must select full-fresh measurement artifact-repair synthesis or stop as exactly one route",
        ],
        "private_holdout_policy": "not_used",
        "forbidden_shortcuts": [
            "do not rerun tune rank promote or mutate checkpoints in M3211",
            "do not convert M3210 residual-trace measurement into validation repair-success performance current-sim robustness-result high-fidelity paper or self-ID claims",
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "recovery_clearance_supervisor_residual_trace_measurement_result_audit",
            "evidence_increment": "audits same-seven residual-trace closed-loop measurement for the M3208 recovery-clearance supervisor candidate",
            ...
            "stop_condition": [
                "stop if M3210 artifacts are missing or gate matrix fails",
            ...
            "fallback_plan": [
                "route to M3210 artifact repair if rows or guards fail",
                "route to synthesis if residual-trace measurement is behavior-negative or inconclusive",
                "route to full-fresh measurement only if M3211 accepts a non-regressive residual-trace result versus M3205 and incumbent",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3210 materializes residual-trace measurement artifacts",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3210 residual-trace measurement artifacts",
            "admission_evidence": [
                "M3210 summary candidate trace rows step rows same-trace comparison rows contract guards claim rows and gate matrix"
            ],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3211 status queue scoreboard research log and review",
                "one follow-up manifest only if M3211 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3211 accepts or rejects M3210 as complete and claim-safe",
                "next full-fresh measurement synthesis artifact-repair or stop route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3211 audits engineering residual-trace measurement and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3211; self-ID and GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3210 residual-trace measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3210 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3211 audits engineering recovery-clearance supervisor measurement evidence",
            "must_synthesize_if": [
                "M3211 cannot select full-fresh measurement synthesis artifact-repair or stop",
                "M3211 would claim repair-success validation driver-performance current-sim verdict robustness-result or self-ID evidence",
            ],
        },
        "success_criteria": [
            f"docs/{NEXT_ID}.md exists",
            "M3211 audits M3210 row counts gates actor contract and claim boundaries",
            "M3211 selects exactly one next route or stop state",
        ],
        "failure_criteria": [
            "M3211 hides missing M3210 artifacts or failed gates",
            "M3211 treats M3210 traces as repair success or performance verdict",
            "M3211 changes actor input action contract or public driver default",
            "M3211 leaves next route ambiguous",
        ],
        "decision_rule": "Pass only if M3211 audits M3210 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
        "commands": [
            {
                "name": "active_safety_driver_recovery_clearance_supervisor_residual_trace_measurement_result_audit_doc",
                "command": "true",
            }
        ],
...
def render_doc(summary: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- M3210 success/collision/offtrack: {summary['m3210_success_count']}/{summary['m3210_collision_count']}/{summary['m3210_offtrack_count']}",
            ...
            f"- outcome changed vs M3205/M3194/incumbent: {summary['outcome_changed_vs_m3205_count']}/{summary['outcome_changed_vs_m3194_count']}/{summary['outcome_changed_vs_incumbent_count']}",
            f"- hard-safety improved vs M3205/M3194/incumbent: {summary['hard_safety_improved_vs_m3205_count']}/{summary['hard_safety_improved_vs_m3194_count']}/{summary['hard_safety_improved_vs_incumbent_count']}",
            f"- hard-safety regressed vs M3205/M3194/incumbent: {summary['hard_safety_regressed_vs_m3205_count']}/{summary['hard_safety_regressed_vs_m3194_count']}/{summary['hard_safety_regressed_vs_incumbent_count']}",
...
def run_residual_trace_measurement_preflight(
    *,
    ...
    comparisons = same_trace_comparison_rows(
        ...
        m3205_steps=source["m3205_candidate_trace_step_rows"],
    ...
    summary = {
        ...
        "outcome_changed_vs_m3205_count": sum(_bool(row.get("outcome_changed_vs_m3205")) for row in comparisons),
        ...
        "hard_safety_improved_vs_m3205_count": sum(_bool(row.get("hard_safety_improved_vs_m3205")) for row in comparisons),
        ...
        "hard_safety_regressed_vs_m3205_count": sum(_bool(row.get("hard_safety_regressed_vs_m3205")) for row in comparisons),
        ...
        "m3210_success_count": sum(_bool(row.get("success", False)) for row in executions),
        "m3210_collision_count": sum(_bool(row.get("collision", False)) for row in executions),
        "m3210_offtrack_count": sum(_offtrack(row) for row in executions),
        "m3205_success_count": sum(_bool(row.get("success", False)) for row in source["m3205_candidate_trace_execution_rows"]),
        "m3205_collision_count": sum(_bool(row.get("collision", False)) for row in source["m3205_candidate_trace_execution_rows"]),
        "m3205_offtrack_count": sum(_offtrack(row) for row in source["m3205_candidate_trace_execution_rows"]),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--m3209-audit", type=Path, default=DEFAULT_M3209_AUDIT)
    parser.add_argument("--m3208-dir", type=Path, default=DEFAULT_M3208_DIR)
...
def main(argv: list[str] | None = None) -> None:
    ...
    summary = run_residual_trace_measurement_preflight(
        m3209_audit=args.m3209_audit,
        m3208_dir=args.m3208_dir,
