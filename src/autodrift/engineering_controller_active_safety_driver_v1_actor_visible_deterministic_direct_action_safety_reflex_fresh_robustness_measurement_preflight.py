"""Run M3084 deterministic safety-reflex fresh robustness measurement preflight."""
...
DEFAULT_M3083_AUDIT = Path(f"docs/{M3083_ID}.md")
DEFAULT_M3082_DIR = Path(
...
CLAIM_SCOPE = (
    "M3084 Active Safety Driver v1 actor-visible deterministic direct-action safety-reflex "
    "fresh robustness measurement preflight only; fresh current-sim measurement rows and "
    "metric tables may be recorded for the M3078 deterministic direct-action safety-reflex "
    "candidate on the M3082 fresh panel. The candidate is executed as final_action = "
    "actor_visible_safety_reflex_action(obs72). No runtime base policy, hidden oracle, TTC, "
    "target, provenance, source, route, outcome, progress, or verdict actor input is used. "
    "No validation result, driver-performance verdict, current-sim verdict, ranking, winner "
    "selection, checkpoint promotion, checkpoint mutation, repair success, high-fidelity "
    "validation, paper evidence, finite-window-vs-GRU, full ideal driver, or self-ID claim "
    "is made"
)
...
MEASUREMENT_FIELDNAMES = [
    ...
    *PANEL_METADATA_FIELDNAMES,
    *[field for field in m3080.MEASUREMENT_FIELDNAMES if field != "measurement_episode_id"],
]
FAILURE_FIELDNAMES = [
    "measurement_episode_id",
    *PANEL_METADATA_FIELDNAMES,
    *[field for field in m3080.FAILURE_FIELDNAMES if field != "measurement_episode_id"],
]
GUARD_FIELDNAMES = m3080.GUARD_FIELDNAMES
GATE_FIELDNAMES = m3080.GATE_FIELDNAMES
CLAIM_FIELDNAMES = [
    "claim_id",
    "claim_family",
    "allowed_in_m3084",
...
def _bool(value: Any) -> bool:
    return m3080._bool(value)


def _float(value: Any) -> float:
    return m3080._float(value)


def _mean(values: Iterable[float]) -> float | str:
    return m3080._mean(values)


def _success(row: Mapping[str, Any]) -> bool:
    return m3080._success(row)


def _sum_bool(rows: Iterable[Mapping[str, Any]], key: str) -> int:
    return m3080._sum_bool(rows, key)


def _int_from_text(value: Any, default: int = 0) -> int:
    try:
...
def any_flag(rows: Iterable[Mapping[str, Any]], key: str) -> bool:
    return m3080.any_flag(rows, key)
...
def load_sources(
    *,
    ...
    m3078_dir: Path,
    m3012_dir: Path,
    m3080_dir: Path = DEFAULT_M3080_DIR,
) -> dict[str, Any]:
    paths = {
        "m3083_audit": m3083_audit,
        "m3082_summary": m3082_dir / "summary.json",
        "m3082_panel_rows": m3082_dir / "fresh_robustness_panel_rows.csv",
        "m3082_admission_rows": m3082_dir / "robustness_admission_guard_rows.csv",
        "m3082_actor_rows": m3082_dir / "actor_contract_guard_rows.csv",
        "m3082_claim_rows": m3082_dir / "claim_boundary_rows.csv",
        "m3082_gate_rows": m3082_dir / "gate_matrix.csv",
        ...
        "m3080_summary": m3080_dir / "summary.json",
        "m3080_measurement_rows": m3080_dir / "measurement_episode_rows.csv",
    }
    ...
    return {
        ...
        "m3083_audit_text": paths["m3083_audit"].read_text(encoding="utf-8") if exists["m3083_audit"] else "",
        "m3082_summary": read_json(paths["m3082_summary"]) if exists["m3082_summary"] else {},
        "m3082_panel_rows": read_csv_rows(paths["m3082_panel_rows"]),
        "m3082_admission_rows": read_csv_rows(paths["m3082_admission_rows"]),
        "m3082_actor_rows": read_csv_rows(paths["m3082_actor_rows"]),
        "m3082_claim_rows": read_csv_rows(paths["m3082_claim_rows"]),
        "m3082_gate_rows": read_csv_rows(paths["m3082_gate_rows"]),
        ...
        "m3080_summary": read_json(paths["m3080_summary"]) if exists["m3080_summary"] else {},
        "m3080_measurement_rows": read_csv_rows(paths["m3080_measurement_rows"]),
    }


def _workload_candidates_by_family(workload_rows: list[dict[str, str]]) -> dict[tuple[str, str], list[dict[str, str]]]:
    grouped: dict[tuple[str, str], list[dict[str, str]]] = defaultdict(list)
    for row in sorted(workload_rows, key=lambda item: item.get("executable_workload_id", "")):
        grouped[(str(row.get("task_family", "")), str(row.get("binding_role", "")))].append(row)
    return grouped


def _m3080_reference_rows(rows: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    return {str(row.get("measurement_episode_id", "")): row for row in rows}


def fresh_measurement_plan(source: dict[str, Any]) -> list[dict[str, Any]]:
    panel_rows = sorted(source["m3082_panel_rows"], key=lambda row: row.get("fresh_panel_row_id", ""))
    candidates = _workload_candidates_by_family(source["m3012_workload_rows"])
    references = _m3080_reference_rows(source["m3080_measurement_rows"])
    used_m3080_seeds = {_int_from_text(row.get("eval_seed")) for row in source["m3080_measurement_rows"]}
    rows: list[dict[str, Any]] = []
    family_counts: dict[tuple[str, str, str], int] = defaultdict(int)
    for index, panel in enumerate(panel_rows, start=1):
        task_family = str(panel.get("task_family", ""))
        binding_role = str(panel.get("binding_role", ""))
        axis_id = str(panel.get("axis_id", ""))
        key = (task_family, binding_role)
        available = candidates.get(key, [])
        count_key = (axis_id, task_family, binding_role)
        workload = available[family_counts[count_key] % len(available)] if available else {}
        family_counts[count_key] += 1
        reference = references.get(str(panel.get("m3080_reference_measurement_episode_id", "")), {})
        hidden_label_violation = any(
            _bool(panel.get(field, False))
            for field in (
        ...
        eval_seed = _int_from_text(panel.get("eval_seed"))
        status_pass = bool(
            available
            and _bool(workload.get("status_pass", False))
            and Path(config_path).exists()
            and _bool(panel.get("fresh_seed", False))
            and not _bool(panel.get("source_denominator_reused", True))
            and not _bool(panel.get("fixed_denominator_row_reused", True))
            and _bool(panel.get("measurement_admission_after_m3083", False))
            and eval_seed not in used_m3080_seeds
            and int(panel.get("actor_observation_dim", -1)) == P0_OBSERVATION_DIM
            and int(panel.get("actor_action_dim", -1)) == ACTION_DIM
            and str(panel.get("candidate_output_semantics", "")) == DIRECT_ACTION_SEMANTICS
            and not _bool(panel.get("runtime_base_policy_required", True))
        ...
        rows.append(
            {
                ...
                **dict(panel),
                ...
                "baseline_measurement_row_id": panel.get("m3080_reference_measurement_episode_id", ""),
                "baseline": reference,
                "base_profile_name": workload.get("profile_binding_name", panel.get("base_profile_name", "")),
                "direct_action_profile_name": (
                    f"{workload.get('profile_binding_name', panel.get('base_profile_name', ''))}"
                    f"+m3078_deterministic_safety_reflex_fresh_{axis_id}"
                ),
                ...
                "strata": ";".join(
                    ...
                    for value in (
                        ...
                        binding_role,
                        axis_id,
                        str(panel.get("fresh_scenario_distribution", "")),
...
def measurement_metadata(plan: Mapping[str, Any], telemetry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "panel_family": plan.get("panel_family", ""),
        "axis_id": plan.get("axis_id", ""),
        "axis_family": plan.get("axis_family", ""),
        "scenario_family": plan.get("scenario_family", ""),
        "fresh_scenario_distribution": plan.get("fresh_scenario_distribution", ""),
        "fresh_seed": _bool(plan.get("fresh_seed", False)),
        "m3080_reference_measurement_episode_id": plan.get("m3080_reference_measurement_episode_id", ""),
        "m3080_reference_eval_seed": plan.get("m3080_reference_eval_seed", ""),
        "m3080_reference_outcome_bucket": plan.get("m3080_reference_outcome_bucket", ""),
        "source_denominator_reused": _bool(plan.get("source_denominator_reused", False)),
        "fixed_denominator_row_reused": _bool(plan.get("fixed_denominator_row_reused", False)),
        "measurement_admission_after_m3083": _bool(plan.get("measurement_admission_after_m3083", False)),
...
def normalize_measurement_row(row: Mapping[str, Any]) -> dict[str, Any]:
    standard = m3080.normalize_measurement_row(row)
...
def failure_row(plan: Mapping[str, Any], *, error_type: str, error_message: str) -> dict[str, Any]:
    ...
    row.update({field: plan.get(field, "") for field in PANEL_METADATA_FIELDNAMES})
    row.update(
        {
            "measurement_episode_id": plan.get("measurement_episode_id", ""),
            "baseline_measurement_row_id": plan.get("baseline_measurement_row_id", ""),
...
def run_measurement_plan(
    *,
    ...
    contract_pass = m3080.policy_contract_pass(policy_config)
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("fresh measurement plan row failed pre-execution guards")
            ...
            policy = m3080.DeterministicSafetyReflexPolicy(policy_config=policy_config)
            try:
                ...
                row = run_episode_with_policy(env, policy, "m3078_deterministic_safety_reflex_fresh", int(plan["eval_seed"]))
            finally:
                env.close()
...
def metric_summary_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for key in ("axis_id", "axis_family", "fresh_scenario_distribution", "binding_role", "base_profile_name", "task_family"):
    ...
    for group, group_rows in sorted(groups.items()):
        summaries.append(
            {
                "metric_summary_id": f"m3084-metric-summary-{len(summaries) + 1:04d}",
                "group": group,
...
def actor_contract_guard_rows(
    *,
    ...
    sample_action = m3080.actor_visible_safety_reflex_action(
    ...
    return [
        ...
        guard("policy_config_contract_pass", "actor_contract", m3080.policy_contract_pass(policy_config), True),
        ...
        guard("scheduled_fresh_panel_rows", "denominator", len(plan_rows), EXPECTED_PANEL_ROWS),
        guard("accounted_measurement_rows", "denominator", len(accounted), len(plan_rows)),
        guard("m3080_seed_overlap_count", "denominator", m3080_seed_overlap_count, 0),
        guard("robustness_axis_count", "denominator", len({row.get("axis_id", "") for row in plan_rows}), EXPECTED_AXIS_COUNT),
        guard(
            "profile_binding_count",
            "denominator",
            len({row.get("binding_role", "") for row in plan_rows}),
            EXPECTED_BINDING_ROLE_COUNT,
        ),
...
def claim(claim_id: str, family: str, allowed: bool, made: bool, evidence: str) -> dict[str, Any]:
    return {
        "claim_id": f"m3084-{claim_id}",
        "claim_family": family,
        "allowed_in_m3084": allowed,
        "claim_made": made,
...
def claim_boundary_rows(*, measurement_rows_present: bool, artifacts_present: bool, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ("fresh_measurement_rows", "measurement", measurement_rows_present, "measurement_episode_rows.csv"),
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3085 audit manifest"),
    ]
    blocked = [
        ...
        ("robustness_verdict", "verdict", "M3085 audit and later validation route"),
    ]
...
def gate_matrix_rows(
    *,
    ...
    audit_accepts = "accept_m3082_fresh_robustness_panel_route_to_m3084_measurement_preflight" in source["m3083_audit_text"]
    ...
    axes = {row.get("axis_id", "") for row in plan_rows}
    return [
        gate("source_artifacts_present", "source", source_present, source["source_exists"], "all required sources", "lineage_invalid"),
        gate("m3083_accepts_m3084_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        gate("m3082_status_pass", "lineage", _bool(source["m3082_summary"].get("status_pass", False)), source["m3082_summary"].get("status_pass"), True, "lineage_invalid"),
        gate("m3082_gate_matrix_pass", "lineage", _bool(source["m3082_summary"].get("gate_matrix_pass", False)), source["m3082_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
        ...
        gate("policy_config_contract_pass", "contract", m3080.policy_contract_pass(policy_config), m3080.policy_contract_pass(policy_config), True, "contract_violation"),
        ...
        gate("fresh_panel_denominator", "denominator", len(plan_rows) == EXPECTED_PANEL_ROWS, len(plan_rows), EXPECTED_PANEL_ROWS, "scenario_sampling_failure"),
        gate("m3080_seed_overlap_zero", "denominator", m3080_seed_overlap_count == 0, m3080_seed_overlap_count, 0, "seed_fragility"),
        gate("axis_coverage", "denominator", len(axes) == EXPECTED_AXIS_COUNT, "|".join(sorted(axes)), EXPECTED_AXIS_COUNT, "scenario_sampling_failure"),
        gate("measurement_episode_rows", "execution", len(episodes) == EXPECTED_PANEL_ROWS, len(episodes), EXPECTED_PANEL_ROWS, "metric_artifact"),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path, summary_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3084 fresh robustness measurement artifacts before any validation, ranking, promotion, driver-performance, current-sim verdict, high-fidelity, paper, full-driver, repair-success, or self-ID claim.",
        "lineage": {
            ...
            "derived_from": [MILESTONE_ID, M3083_ID, M3082_ID, M3078_ID],
            "blocked_by": [
                "M3084 measurement rows require audit before any continuation or stop decision",
                "fresh robustness measurement rows are not validation or promotion evidence before M3085",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "fresh_robustness_measurement_result_audit",
            "evidence_increment": "audits fresh robustness closed-loop measurement rows for the deterministic safety-reflex candidate",
            ...
            "stop_condition": [
                "stop if M3084 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "synthesize if M3084 is negative on primary fresh robustness safety counts",
            ],
            "fallback_plan": [
                ...
                "route to branch synthesis or stop if fresh robustness behavior is clearly negative",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3084 completes deterministic safety-reflex fresh robustness measurement preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3084 deterministic direct-action safety-reflex fresh robustness measurement artifacts",
            "admission_evidence": ["M3084 summary and gate matrix", "M3084 measurement episode metric actor contract and claim artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3085 status queue scoreboard research log and review",
                "one follow-up manifest only if M3085 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3085 accepts or rejects M3084 as complete and claim-safe",
                "next route or stop state is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3085 audits engineering measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3085; finite-window and GRU comparison remains a later same-case engineering ablation."],
            "temporal_evidence_window": "M3084 deterministic safety-reflex fresh robustness measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3084 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits the first deterministic safety-reflex fresh robustness behavior measurement",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3085 audits engineering measurement evidence",
            "must_synthesize_if": [
                "M3085 cannot accept M3084 as complete and claim-safe",
                "M3085 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict or self-ID evidence",
                "M3085 cannot select a repair continuation synthesis or stop route",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M3085 audits M3084 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
        "commands": [{"name": "active_safety_driver_v1_deterministic_safety_reflex_fresh_robustness_measurement_result_audit_doc", "command": "true"}],
...
def render_doc(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- robustness axis count: {summary['robustness_axis_count']}",
...
def run_fresh_robustness_measurement_preflight(
    *,
    m3083_audit: Path,
    m3082_dir: Path,
    m3078_dir: Path,
    ...
    plan_rows = fresh_measurement_plan(source)
    m3080_seed_overlap_count = len(
        {_int_from_text(row.get("eval_seed")) for row in plan_rows}
        & {_int_from_text(row.get("eval_seed")) for row in source["m3080_measurement_rows"]}
    )
    ...
    axis_counts = Counter(str(row.get("axis_id", "")) for row in episodes)
    ...
    summary = {
        ...
        "result_class": (
            "active_safety_driver_v1_actor_visible_deterministic_direct_action_safety_reflex_fresh_robustness_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_v1_actor_visible_deterministic_direct_action_safety_reflex_fresh_robustness_measurement_preflight_fail"
        ),
        "status_pass": status_pass,
        ...
        "scheduled_measurement_row_count": len(plan_rows),
        "target_measurement_row_count": EXPECTED_PANEL_ROWS,
        "measurement_episode_row_count": len(episodes),
        "measurement_failure_row_count": len(failures),
        "recorded_row_count": len(episodes) + len(failures),
        "m3080_seed_overlap_count": m3080_seed_overlap_count,
        "robustness_axis_count": len({row.get("axis_id", "") for row in plan_rows}),
        ...
        "required_artifacts_present": present,
        "m3082_status_pass": _bool(source["m3082_summary"].get("status_pass", False)),
        "m3082_gate_matrix_pass": _bool(source["m3082_summary"].get("gate_matrix_pass", False)),
        ...
        "policy_config_contract_pass": m3080.policy_contract_pass(policy_config),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3083-audit", type=Path, default=DEFAULT_M3083_AUDIT)
    parser.add_argument("--m3082-dir", type=Path, default=DEFAULT_M3082_DIR)
...
def main() -> None:
    args = parse_args()
    summary = run_fresh_robustness_measurement_preflight(
        m3083_audit=args.m3083_audit,
        m3082_dir=args.m3082_dir,
