"""Measured runner for the outcome-supported comparison-support workload."""
...
DEFAULT_EXECUTABLE_TASK_SPECS = Path(
    "runs/m2118_paper_route_outcome_supported_decisive_comparison_support_materialization_preflight/"
...
DEFAULT_OUTPUT_DIR = Path("runs/m2125_paper_route_outcome_supported_decisive_comparison_support_measured_execution")
DEFAULT_EVAL_SEED_BASE = 212300
...
def load_executable_task_specs(path: Path | str = DEFAULT_EXECUTABLE_TASK_SPECS) -> list[dict[str, Any]]:
    ...
    if not isinstance(rows, list):
        raise ValueError("comparison-support measured runner specs must contain executable_task_specs")
...
def comparison_support_metadata_row(workload_row: Mapping[str, Any], spec: Mapping[str, Any]) -> dict[str, Any]:
...
def metadata_missing_rows(
    *,
    ...
    for index, workload_row in enumerate(workload_rows):
        ...
        metadata = comparison_support_metadata_row(workload_row, spec)
...
def validation_failure_rows(
    *,
    ...
    for index, row in enumerate(workload_rows):
        ...
        if str(row.get("materialization_semantics", "")) != "comparison_support_smoke_proxy":
            failures.append(
                {
                    "workload_id": workload_id,
                    "error_type": "unexpected_materialization_semantics",
                    "error_message": str(row.get("materialization_semantics", "")),
                }
            )
        for flag in (
            "training_scheduled",
...
def measured_episode_row(
    *,
    ...
    row.update(comparison_support_metadata_row(workload_row, executable_spec))
    row.update(
        {
            "eval_seed": int(eval_seed),
            "success": _episode_success(row),
            "comparison_support_measured_execution": True,
...
def measured_failure_row(
    *,
    ...
    return {
        **comparison_support_metadata_row(workload_row, executable_spec),
        "eval_seed": int(eval_seed),
...
def claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "claim": "comparison_support_measured_execution_completed",
            "admissible": True,
        ...
        },
        {
            ...
            "admissible": False,
            "reason": "generated comparison-support smoke proxies require audit and localization before paper claims",
        },
        {
            "claim": "paper_valid_generated_task_semantics",
            "admissible": False,
            "reason": "generated comparison-support rows have paper_validity_claim=false",
        },
        {
...
def finalize_outputs(
    *,
    ...
    validation_rows = [dict(row) for row in read_csv_rows(output_dir / "validation_failure_rows.csv")]
    if not (output_dir / "failure_rows.csv").exists():
        write_csv_rows(output_dir / "failure_rows.csv", [], fieldnames=FAILURE_FIELDNAMES)
    if not (output_dir / "validation_failure_rows.csv").exists():
        write_csv_rows(output_dir / "validation_failure_rows.csv", [], fieldnames=VALIDATION_FAILURE_FIELDNAMES)
    ...
    aggregate_paths = {
        ...
        "intent_aggregate": ("intent_aggregate.csv", "comparison_support_intent"),
        "target_support_tier_aggregate": ("target_support_tier_aggregate.csv", "target_support_tier"),
    ...
    expected_intent_counts = _count_by(workload_rows, "comparison_support_intent")
    target_support_tier_counts = _count_by(episode_rows, "target_support_tier")
    expected_target_support_tier_counts = _count_by(workload_rows, "target_support_tier")
    ...
    intent_quota_pass = intent_counts == expected_intent_counts
    target_support_tier_quota_pass = target_support_tier_counts == expected_target_support_tier_counts
    ...
    passes = (
        ...
        and intent_quota_pass
        and target_support_tier_quota_pass
    ...
    summary = {
        "result_class": (
            "comparison_support_measured_execution_pass"
            if passes
            else "comparison_support_measured_execution_incomplete_or_fail"
        ),
        ...
        "expected_intent_counts": expected_intent_counts,
        "intent_counts": intent_counts,
        "intent_quota_pass": intent_quota_pass,
        "expected_target_support_tier_counts": expected_target_support_tier_counts,
        "target_support_tier_counts": target_support_tier_counts,
        "target_support_tier_quota_pass": target_support_tier_quota_pass,
...
def run_comparison_support_measured_execution(
    *,
    ...
    target_spec_count: int = TARGET_SPEC_COUNT,
    target_profile_count: int = TARGET_PROFILE_COUNT,
    next_blocker: str = "m2126-paper-route-outcome-supported-decisive-comparison-support-measured-execution-result-audit",
...
def main() -> None:
    ...
    parser.add_argument(
        "--next-blocker",
        default="m2126-paper-route-outcome-supported-decisive-comparison-support-measured-execution-result-audit",
    )
    args = parser.parse_args()
    summary = run_comparison_support_measured_execution(
