"""No-rerun outcome localization for comparison-support measured artifacts."""
...
DEFAULT_SUMMARY = Path("runs/m2125_paper_route_outcome_supported_decisive_comparison_support_measured_execution/summary.json")
...
DEFAULT_OUTPUT_DIR = Path("runs/m2128_paper_route_outcome_supported_decisive_comparison_support_outcome_localization")
...
TARGET_INTENT_COUNT = 4
TARGET_SUPPORT_TIER_COUNT = 4
...
AGGREGATE_SPECS: dict[str, tuple[str, ...]] = {
    ...
    "outcome_by_intent": ("comparison_support_intent",),
    "outcome_by_target_support_tier": ("target_support_tier",),
    "outcome_by_source_kind": ("source_kind",),
    "outcome_by_proxy_template": ("proxy_template_family",),
    "outcome_by_profile_intent": ("profile_name", "comparison_support_intent"),
    "outcome_by_profile_target_support_tier": ("profile_name", "target_support_tier"),
    "outcome_by_profile_source_kind": ("profile_name", "source_kind"),
    "outcome_by_intent_source_kind": ("comparison_support_intent", "source_kind"),
    "outcome_by_source_profile": ("task_source_id", "profile_name"),
    "outcome_by_task_intent_kind": ("task_source_id", "comparison_support_intent", "source_kind"),
}
...
def comparison_ready_candidates(aggregate_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {field: row.get(field, "") for field in COMPARISON_CANDIDATE_FIELDNAMES}
        for row in comparison_support_candidates(aggregate_rows)
...
def claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "claim": "no_rerun_comparison_support_outcome_localization_completed",
            "admissible": True,
            "reason": "M2128 reads existing M2125 artifacts and writes diagnostic aggregates only",
        },
        {
        ...
        },
        {
            ...
            "admissible": False,
            "reason": "localization may find support slices but does not run a controlled finite-window-vs-GRU comparison",
        },
        {
            "claim": "paper_level_benchmark_result",
            "admissible": False,
            "reason": "generated comparison-support rows remain smoke proxies with paper_validity_claim=false",
        },
        {
...
def _required_artifacts(output: Path) -> dict[str, str]:
    artifacts = {
        ...
        "comparison_ready_candidates": output / "comparison_ready_candidates.csv",
...
def localize_comparison_support_outcomes(
    *,
    ...
    target_intent_count: int = TARGET_INTENT_COUNT,
    target_support_tier_count: int = TARGET_SUPPORT_TIER_COUNT,
    ...
    ready_candidates = comparison_ready_candidates(all_aggregate_rows)
    ...
    write_csv_rows(
        output / "comparison_ready_candidates.csv",
        ready_candidates,
        fieldnames=COMPARISON_CANDIDATE_FIELDNAMES,
    )
    ...
    intent_count = len({str(row.get("comparison_support_intent", "")) for row in episode_rows})
    observed_support_tier_count = len({str(row.get("target_support_tier", "")) for row in episode_rows})
    ...
    result_passes = (
        source_summary.get("result_class") == "comparison_support_measured_execution_pass"
        ...
        and intent_count == int(target_intent_count)
        and observed_support_tier_count == int(target_support_tier_count)
    ...
    summary = {
        "result_class": (
            "comparison_support_outcome_localization_pass"
            if result_passes
            else "comparison_support_outcome_localization_incomplete_or_fail"
        ),
        ...
        "target_intent_count": int(target_intent_count),
        "support_tier_count": observed_support_tier_count,
        "target_support_tier_count": int(target_support_tier_count),
        ...
        "intent_counts": _count_by(episode_rows, "comparison_support_intent"),
        "target_support_tier_counts": _count_by(episode_rows, "target_support_tier"),
        ...
        "dynamics_band_counts": _count_by(episode_rows, "dynamics_band"),
        "obstacle_timing_band_counts": _count_by(episode_rows, "obstacle_timing_band"),
        "road_width_band_counts": _count_by(episode_rows, "road_width_band"),
        "initial_speed_band_counts": _count_by(episode_rows, "initial_speed_band"),
...
def main() -> None:
    ...
    parser.add_argument("--target-intent-count", type=int, default=TARGET_INTENT_COUNT)
    parser.add_argument("--target-support-tier-count", type=int, default=TARGET_SUPPORT_TIER_COUNT)
    parser.add_argument("--next-blocker", default=DEFAULT_NEXT_BLOCKER)
    args = parser.parse_args()
    summary = localize_comparison_support_outcomes(
        ...
        target_intent_count=int(args.target_intent_count),
        target_support_tier_count=int(args.target_support_tier_count),
