"""No-rollout outcome localization for support-first measured execution."""
...
DEFAULT_SUMMARY = Path("runs/m1880_executable_v2_support_first_measured_runner_execution/summary.json")
DEFAULT_OUTPUT_DIR = Path("runs/m1882_executable_v2_support_first_outcome_localization")
DEFAULT_NEXT_BLOCKER = "m1883-executable-v2-support-first-success-semantics-task-quality-repair-design"
...
def _outcome_counts(episode_rows: list[Mapping[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
...
def _recommended_next_route(*, outcome_counts: Mapping[str, int], dominant_profile_count: int) -> str:
    success_count = int(outcome_counts.get("success_obstacle_pass", 0))
    if success_count == 0 and dominant_profile_count >= 8:
        return "success_metric_semantics_and_task_quality_repair_design"
    if int(outcome_counts.get("off_track_noncollision_noncompletion", 0)) > success_count:
        return "scenario_task_quality_repair_design"
    return "result_audit_before_ranking_design"


def localize_support_first_outcomes(
    *,
    ...
    min_dominant_episodes: int = MIN_DOMINANT_EPISODES,
    ...
    aggregate_specs: dict[str, tuple[str, ...]] = {
        ...
        "role_surface_aggregate": ("v2_role_surface_id",),
        "role_panel_profile_aggregate": ("role_panel_id", "controller_profile_name"),
        "role_surface_profile_aggregate": ("v2_role_surface_id", "controller_profile_name"),
        "profile_aggregate": ("controller_profile_name",),
        "profile_role_panel_aggregate": ("controller_profile_name", "role_panel_id"),
        "profile_role_surface_aggregate": ("controller_profile_name", "v2_role_surface_id"),
        ...
        "role_surface_hidden_bucket_aggregate": ("v2_role_surface_id", "hidden_dynamics_bucket"),
        "role_surface_road_bucket_aggregate": ("v2_role_surface_id", "road_boundary_bucket"),
        "role_surface_timing_bucket_aggregate": ("v2_role_surface_id", "obstacle_timing_bucket"),
        "role_surface_lateral_bucket_aggregate": ("v2_role_surface_id", "obstacle_lateral_bucket"),
        "role_surface_sampled_label_aggregate": ("v2_role_surface_id", "sampled_obstacle_label"),
        "scenario_profile_aggregate": ("scenario_profile_name",),
    }
    ...
    aggregate_rows["role_surface_outcome_aggregate"] = aggregate_outcome_rows(
        episode_rows,
        ("v2_role_surface_id", "outcome_bucket"),
    )
    aggregate_rows["profile_outcome_aggregate"] = aggregate_outcome_rows(
        episode_rows,
        ("controller_profile_name", "outcome_bucket"),
    )
    ...
    dominant_role_surface_count = len(
        {
            str(row.get("v2_role_surface_id"))
            for row in dominant_slices
            if row.get("v2_role_surface_id") not in (None, "")
        }
    )
    dominant_profile_count = len(
        {
            ...
            for row in dominant_slices
            if row.get("controller_profile_name") not in (None, "")
        }
    )
    ...
    outcome_counts = _outcome_counts(episode_rows)
    ...
    outcome_dominance_class = (
        "diffuse_support_first_outcome_dominance"
        if dominant_role_panel_count >= 3 and dominant_role_surface_count >= 6 and dominant_profile_count >= 8
        else "localized_support_first_outcome_dominance"
    )
    ranking_blocked = bool(dominant_slices)
    recommended_next_route = _recommended_next_route(
    ...
    result_passes = (
        ...
        and set(TARGET_LOCALIZATION_SLICE_TYPES).issubset(set(target_slice_types_present))
        and bool(recommended_next_route)
    )

    summary = {
        "result_class": (
            "support_first_outcome_localization_pass"
            if result_passes
            else "support_first_outcome_localization_incomplete_or_fail"
        ),
        ...
        **aggregate_row_counts,
        ...
        "dominant_role_surface_count": dominant_role_surface_count,
        "dominant_profile_count": dominant_profile_count,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Localize support-first outcome dominance from measured rows.")
    ...
    parser.add_argument("--min-dominant-episodes", type=int, default=MIN_DOMINANT_EPISODES)
    parser.add_argument("--next-blocker", default=DEFAULT_NEXT_BLOCKER)
    args = parser.parse_args()

    summary = localize_support_first_outcomes(
        episode_rows_path=args.episode_rows,
        summary_path=args.summary,
        output_dir=args.output_dir,
        target_episode_count=int(args.target_episode_count),
        min_dominant_episodes=int(args.min_dominant_episodes),
