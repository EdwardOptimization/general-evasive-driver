"""Run M3131 residual trajectory-level clearance/stability corridor reflex full-fresh measurement."""
...
MILESTONE_ID = (
    "m3131-engineering-controller-active-safety-driver-residual-hard-safety-trajectory-level-"
    "clearance-stability-corridor-reflex-full-fresh-measurement-preflight"
)
NEXT_ID = (
    "m3132-engineering-controller-active-safety-driver-residual-hard-safety-trajectory-level-"
    "clearance-stability-corridor-reflex-full-fresh-measurement-result-audit"
)
M3130_ID = (
...
DEFAULT_M3130_AUDIT = Path(f"docs/{M3130_ID}.md")
...
CLAIM_SCOPE = (
    "M3131 Active Safety Driver residual trajectory-level clearance/stability corridor reflex full-fresh "
    "measurement preflight only; the complete M3084 fresh current-sim denominator may be "
    "executed through the M3129 direct-action function as the full obs72-to-action3 action "
    "source and measurement, same-row comparison against M3105 M3095 M3100 and M3090, "
    "contract, claim, gate, doc, and M3132 audit artifacts may be written. No validation, "
...
class M3131RepairMeasurementPolicy:
    """Policy adapter that calls the M3129 direct-action repair function."""
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        del info
        action = trajectory_level_clearance_stability_corridor_action(observation, POLICY_CONFIG)
        self.step_count += 1
...
def load_sources(
    *,
    ...
    paths = {
        ...
        "m3129_trajectory_level_corridor_rule_rows": m3129_dir / "trajectory_level_corridor_rule_rows.csv",
        "m3129_runtime_contract_rows": m3129_dir / "runtime_contract_rows.csv",
        "m3129_actor_input_exclusion_rows": m3129_dir / "actor_input_exclusion_rows.csv",
    ...
    return {
        ...
        "m3130_audit_text": paths["m3130_audit"].read_text(encoding="utf-8") if exists["m3130_audit"] else "",
        ...
        "m3129_trajectory_level_corridor_rule_rows": read_csv_rows(paths["m3129_trajectory_level_corridor_rule_rows"]),
        "m3129_runtime_contract_rows": read_csv_rows(paths["m3129_runtime_contract_rows"]),
        "m3129_actor_input_exclusion_rows": read_csv_rows(paths["m3129_actor_input_exclusion_rows"]),
...
def full_fresh_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(plan, start=1):
        row["runtime_smoke_episode_id"] = f"m3131-measurement-episode-{index:04d}"
    return plan


def _with_scope(rows: list[dict[str, Any]], *, id_prefix: str = "m3131") -> list[dict[str, Any]]:
    updated = []
    for row in rows:
        item = dict(row)
        ...
        if "policy" in item:
            item["policy"] = "active_safety_driver_residual_hard_safety_trajectory_level_clearance_stability_corridor_reflex_measurement"
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3131 plan row failed pre-execution guards")
            ...
            policy = M3131RepairMeasurementPolicy()
            try:
                ...
                row = m3088.run_episode_with_policy(
                    env,
                    policy,
                    "active_safety_driver_residual_hard_safety_trajectory_level_clearance_stability_corridor_reflex_measurement",
                    int(plan["eval_seed"]),
                )
            finally:
                env.close()
...
def same_row_comparison_rows(
    ...
    for episode in episodes:
        ...
        for baseline_id in BASELINE_IDS:
            ...
            rows.append(
                {
                    "comparison_id": f"m3131-same-row-comparison-{len(rows) + 1:04d}",
                    ...
                    "m3131_policy": episode.get("policy", ""),
                    ...
                    "m3131_collision": _bool(episode.get("collision", False)),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3131 residual trajectory-level clearance/stability corridor reflex full-fresh measurement artifacts before any validation ranking promotion driver-performance current-sim high-fidelity full-driver repair-success robustness-result or self-ID claim.",
        "lineage": {
            "parent_checkpoint": [
                str(doc_path),
                "runs/m3129_engineering_controller_active_safety_driver_residual_hard_safety_trajectory_level_clearance_stability_corridor_reflex_materialization_preflight/direct_action_policy_config.json",
            ],
            "parent_dataset": [
            ...
            "parent_objective": [
                "audit full-fresh M3129 residual trajectory-level clearance/stability corridor-reflex repair measurement before broader interpretation"
            ],
            "derived_from": [MILESTONE_ID, M3130_ID, M3129_ID, M3105_ID, M3095_ID, M3100_ID],
            "blocked_by": [
                "M3131 full-fresh measurement rows require audit before any validation or repair-success route",
                "same-row comparison against M3105 M3095 M3100 and M3090 is measurement context and not a performance verdict before M3132",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "residual_hard_safety_trajectory_level_clearance_stability_corridor_reflex_full_fresh_measurement_result_audit",
            "evidence_increment": "audits full-fresh current-sim measurement rows and same-row comparison from the M3129 residual trajectory-level clearance/stability corridor-reflex direct-action repair",
            ...
            "stop_condition": [
                "stop if M3131 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "synthesize if M3132 cannot select one next route",
            ],
            "fallback_plan": [
                "route to M3129 artifact repair if artifacts are incomplete or contract-unsafe",
                "route to behavior repair synthesis if measurement is behavior-negative but contract-safe",
                "route to validation planning only after M3132 accepts artifact completeness and claim boundaries",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3131 completes full-fresh M3129 residual repair measurement preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3131 full-fresh residual trajectory-level clearance/stability corridor-reflex repair measurement artifacts",
            "admission_evidence": ["M3131 summary gate matrix comparison measurement metric contract and claim artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3132 status queue scoreboard research log and review",
                "one follow-up manifest only if M3132 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3132 accepts or rejects M3131 as complete and claim-safe",
                "next behavior synthesis validation-planning stop or repair route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3132 audits engineering runtime-measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3132; self-ID/GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3131 full-fresh residual repair measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3131 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits complete fresh-denominator environment-loop evidence through the M3129 residual trajectory-level clearance/stability corridor-reflex direct-action repair",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3132 audits engineering runtime measurement evidence",
            "must_synthesize_if": [
                "M3132 cannot accept M3131 as complete and claim-safe",
                "M3132 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict robustness-result or self-ID evidence",
                "M3132 cannot select behavior synthesis validation planning stop or next repair route",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M3132 audits M3131 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
...
def gate_matrix_rows(
    *,
    ...
    config = source["m3129_policy_config"]
    audit_accepts = "accept_m3129_materialization_route_to_m3131_full_fresh_measurement" in str(
        source.get("m3130_audit_text", "")
    )
    ...
    return [
        ...
        gate("m3130_accepts_m3131_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        gate("m3129_status_pass", "lineage", _bool(source["m3129_summary"].get("status_pass", False)), source["m3129_summary"].get("status_pass"), True, "lineage_invalid"),
        gate("m3129_gate_matrix_pass", "lineage", _bool(source["m3129_summary"].get("gate_matrix_pass", False)), source["m3129_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
        gate("m3129_policy_config_status", "lineage", str(config.get("policy_id", "")) == POLICY_ID, config.get("policy_id"), POLICY_ID, "lineage_invalid"),
        gate("m3129_gate_rows_pass", "lineage", all(_bool(row.get("status_pass", False)) for row in source.get("m3129_gate_rows", [])), len(source.get("m3129_gate_rows", [])), "all pass", "lineage_invalid"),
        gate(
            "m3129_trajectory_level_corridor_rule_rows_present",
            "lineage",
            len(source.get("m3129_trajectory_level_corridor_rule_rows", [])) >= 8,
            len(source.get("m3129_trajectory_level_corridor_rule_rows", [])),
            ">=8",
            "lineage_invalid",
        ),
        gate(
            "m3129_runtime_contract_rows_pass",
            "lineage",
            len(source.get("m3129_runtime_contract_rows", [])) >= 4
            and all(_bool(row.get("status_pass", False)) for row in source.get("m3129_runtime_contract_rows", [])),
            len(source.get("m3129_runtime_contract_rows", [])),
            ">=4 rows all pass",
            "lineage_invalid",
        ),
        gate(
            "m3129_actor_input_exclusion_rows_pass",
            "lineage",
            len(source.get("m3129_actor_input_exclusion_rows", [])) >= 10
            and all(_bool(row.get("status_pass", False)) for row in source.get("m3129_actor_input_exclusion_rows", [])),
            len(source.get("m3129_actor_input_exclusion_rows", [])),
            ">=10 rows all pass",
            "lineage_invalid",
        ),
...
def contract_guard_rows(
    *,
    ...
    sample_action = trajectory_level_clearance_stability_corridor_action(
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3132 audit manifest"),
    ]
    blocked = [
    ...
    rows = [
        {
            "claim_id": f"m3131-{claim_id}",
            "claim_family": family,
            "allowed_in_m3131": True,
            "claim_made": made,
            "status_pass": made,
...
def run_full_fresh_measurement_preflight(
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "result_class": (
            "active_safety_driver_residual_hard_safety_trajectory_level_clearance_stability_corridor_reflex_full_fresh_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_residual_hard_safety_trajectory_level_clearance_stability_corridor_reflex_full_fresh_measurement_preflight_fail"
        ),
        ...
        "required_artifacts_present": present,
        "m3129_status_pass": _bool(source["m3129_summary"].get("status_pass", False)),
        "m3129_gate_matrix_pass": _bool(source["m3129_summary"].get("gate_matrix_pass", False)),
        ...
        "checkpoint_model_required": False,
        "recurrent_hidden_state_required": False,
        "direct_action_formula": "action = trajectory_level_clearance_stability_corridor_action(obs72) -> [steer, throttle, brake]",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3130-audit", type=Path, default=DEFAULT_M3130_AUDIT)
