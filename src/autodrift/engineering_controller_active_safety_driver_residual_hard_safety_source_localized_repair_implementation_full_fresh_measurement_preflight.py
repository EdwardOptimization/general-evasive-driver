"""Run M3172 residual hard-safety source-localized repair implementation full-fresh measurement."""
...
DEFAULT_M3171_AUDIT = Path(f"docs/{M3171_ID}.md")
...
CLAIM_SCOPE = (
    "M3172 Active Safety Driver residual hard-safety source-localized repair implementation full-fresh "
    "measurement preflight only; the complete M3084 fresh current-sim denominator may be "
    "executed through the M3170 direct-action function as the full obs72-to-action3 action "
    "source and measurement, same-row comparison against M3105 M3095 M3100 and M3090, "
    "contract, claim, gate, doc, and M3173 audit artifacts may be written. No validation, "
...
class M3172RepairMeasurementPolicy:
    """Policy adapter that calls the M3170 direct-action repair function."""
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        del info
        action = source_localized_repair_direct_action(observation, POLICY_CONFIG)
        self.step_count += 1
...
def load_sources(
    *,
    ...
    paths = {
        ...
        "m3170_runtime_contract_rows": m3170_dir / "runtime_contract_rows.csv",
        "m3170_action_probe_rows": m3170_dir / "action_probe_rows.csv",
    ...
    return {
        ...
        "m3171_audit_text": paths["m3171_audit"].read_text(encoding="utf-8") if exists["m3171_audit"] else "",
        ...
        "m3170_runtime_contract_rows": read_csv_rows(paths["m3170_runtime_contract_rows"]),
        "m3170_action_probe_rows": read_csv_rows(paths["m3170_action_probe_rows"]),
...
def full_fresh_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(plan, start=1):
        row["runtime_smoke_episode_id"] = f"m3172-measurement-episode-{index:04d}"
    return plan


def _with_scope(rows: list[dict[str, Any]], *, id_prefix: str = "m3172") -> list[dict[str, Any]]:
    updated = []
    for row in rows:
        item = dict(row)
        ...
        if "policy" in item:
            item["policy"] = "active_safety_driver_residual_hard_safety_source_localized_repair_implementation_measurement"
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3172 plan row failed pre-execution guards")
            ...
            policy = M3172RepairMeasurementPolicy()
            try:
                ...
                row = m3088.run_episode_with_policy(
                    env,
                    policy,
                    "active_safety_driver_residual_hard_safety_source_localized_repair_implementation_measurement",
                    int(plan["eval_seed"]),
                )
            finally:
                env.close()
...
def same_row_comparison_rows(
    ...
    for episode in episodes:
        ...
        for baseline_id in BASELINE_IDS:
            ...
            rows.append(
                {
                    "comparison_id": f"m3172-same-row-comparison-{len(rows) + 1:04d}",
                    ...
                    "m3172_outcome_bucket": episode.get("outcome_bucket", ""),
                    ...
                    "m3172_min_clearance_margin": episode.get("min_clearance_margin", ""),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3172 residual hard-safety source-localized repair implementation full-fresh measurement artifacts before any validation ranking promotion driver-performance current-sim high-fidelity full-driver repair-success robustness-result or self-ID claim.",
        "lineage": {
            "parent_checkpoint": [
                str(doc_path),
                str(DEFAULT_M3170_DIR / "direct_action_policy_config.json"),
            ],
            "parent_dataset": [
            ...
            "parent_objective": [
                "audit full-fresh M3170 residual hard-safety source-localized repair implementation measurement before broader interpretation"
            ],
            "derived_from": [MILESTONE_ID, M3171_ID, M3170_ID, M3105_ID, M3095_ID, M3100_ID],
            "blocked_by": [
                "M3172 full-fresh measurement rows require audit before any validation or repair-success route",
                "same-row comparison against M3105 M3095 M3100 and M3090 is measurement context and not a performance verdict before M3173",
            ],
            "supersedes": ["direct interpretation of M3172 rows without audit"],
            "invalidates": [],
        },
        "review_artifact": f"docs/reviews/{NEXT_ID}.md",
        "public_gates": [
            "M3173 must audit M3172 summary measurement comparison metric guard claim and gate artifacts",
            "M3173 must preserve obs72/action3 direct [steer throttle brake] runtime contract and runtime_base_policy_required false",
            "M3173 must reject validation ranking promotion driver-performance current-sim verdict high-fidelity paper full-driver repair-success robustness-result and self-ID claims",
            "M3173 must select exactly one behavior synthesis validation-planning stop or next repair route",
        ],
        "private_holdout_policy": "not_used",
        "forbidden_shortcuts": [
            "do not rerun tune expand rank promote validate or mutate checkpoints",
            "do not convert M3172 same-row deltas into validation driver-performance current-sim robustness-result high-fidelity paper full-driver repair-success or self-ID claims",
            "do not change actor input or action contract",
        ],
        "workflow_synthesis": {
            "branch": "active_safety_driver_residual_hard_safety_source_localized_repair_measurement",
            "evidence_axis": "source_localized_repair_implementation_full_fresh_measurement_result_audit",
            "evidence_increment": "audits full-fresh current-sim measurement rows and same-row comparison from the M3170 residual hard-safety source-localized repair implementation direct-action candidate",
            ...
            "stop_condition": [
                "stop if M3172 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "synthesize if M3173 cannot select one next route",
            ],
            "fallback_plan": [
                "route to M3170 artifact repair if artifacts are incomplete or contract-unsafe",
                "route to behavior repair synthesis if measurement is behavior-negative but contract-safe",
                "route to validation planning only after M3173 accepts artifact completeness and claim boundaries",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3172 completes full-fresh M3170 residual hard-safety source-localized repair implementation measurement preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3172 full-fresh residual hard-safety source-localized repair implementation measurement artifacts",
            "admission_evidence": ["M3172 summary gate matrix comparison measurement metric contract and claim artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3173 status queue scoreboard research log and review",
                "one follow-up manifest only if M3173 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3173 accepts or rejects M3172 as complete and claim-safe",
                "next behavior synthesis validation-planning stop or repair route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3173 audits engineering runtime-measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3173; self-ID/GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3172 full-fresh residual repair measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3172 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits complete fresh-denominator environment-loop evidence through the M3170 residual hard-safety source-localized repair implementation direct-action candidate",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3173 audits engineering runtime measurement evidence",
            "must_synthesize_if": [
                "M3173 cannot accept M3172 as complete and claim-safe",
                "M3173 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict robustness-result or self-ID evidence",
                "M3173 cannot select behavior synthesis validation planning stop or next repair route",
            ],
        },
        "success_criteria": [
            f"docs/{NEXT_ID}.md exists",
            "M3173 audits M3172 row counts gates actor contract same-row comparison and claim boundaries",
            "M3173 rejects validation ranking promotion driver-performance high-fidelity paper full-driver repair-success robustness-result and self-ID claims",
            "M3173 selects exactly one next route or stop state",
        ],
        "failure_criteria": [
            "M3173 hides M3172 failures or missing artifacts",
            "M3173 treats M3172 runtime measurement as validation repair-success or performance verdict",
            "M3173 changes actor input or action contract",
            "M3173 leaves next route ambiguous",
        ],
        "decision_rule": "Pass only if M3173 audits M3172 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
        "commands": [
            {
                "name": "active_safety_driver_residual_hard_safety_source_localized_repair_implementation_full_fresh_measurement_result_audit_doc",
                "command": "true",
            }
        ],
...
def gate_matrix_rows(
    *,
    ...
    config = source["m3170_policy_config"]
    audit_accepts = (
        "accept_m3170_source_localized_repair_implementation_route_to_m3172_full_fresh_measurement_preflight"
        in str(source.get("m3171_audit_text", ""))
    )
    runtime_contracts = source.get("m3170_runtime_contract_rows", [])
    action_probes = source.get("m3170_action_probe_rows", [])
    ...
    return [
        ...
        gate("m3171_accepts_m3172_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        ...
        gate("m3170_gate_matrix_pass", "lineage", _bool(source["m3170_summary"].get("gate_matrix_pass", False)), source["m3170_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
        gate("m3170_policy_config_status", "lineage", str(config.get("policy_id", "")) == POLICY_ID, config.get("policy_id"), POLICY_ID, "lineage_invalid"),
        gate("m3170_gate_rows_pass", "lineage", all(_bool(row.get("status_pass", False)) for row in source.get("m3170_gate_rows", [])), len(source.get("m3170_gate_rows", [])), "all pass", "lineage_invalid"),
        gate(
            "m3170_source_localized_rule_rows_present",
            "lineage",
            len(source.get("m3170_source_localized_rule_rows", [])) >= 4,
            len(source.get("m3170_source_localized_rule_rows", [])),
            ">=4",
            "lineage_invalid",
        ),
        gate(
            "m3170_runtime_contract_rows_pass",
            "lineage",
            len(runtime_contracts) >= 1 and all(_bool(row.get("status_pass", False)) for row in runtime_contracts),
            len(runtime_contracts),
            ">=1 rows all pass",
            "lineage_invalid",
        ),
        gate(
            "m3170_action_probe_rows_pass",
            "lineage",
            len(action_probes) >= 6
            and all(
                _bool(row.get("action_finite", False))
                and _bool(row.get("action_bounded", False))
                and _bool(row.get("delta_limited", False))
                for row in action_probes
            ),
            len(action_probes),
            ">=6 rows finite bounded delta-limited",
            "lineage_invalid",
        ),
...
def contract_guard_rows(
    *,
    ...
    sample_action = source_localized_repair_direct_action(
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3173 audit manifest"),
    ]
    blocked = [
    ...
    rows = [
        {
            "claim_id": f"m3172-{claim_id}",
            "claim_family": family,
            "allowed_in_m3172": True,
            "claim_made": made,
            "status_pass": made,
...
def run_full_fresh_measurement_preflight(
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "result_class": (
            "active_safety_driver_residual_hard_safety_source_localized_repair_implementation_full_fresh_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_residual_hard_safety_source_localized_repair_implementation_full_fresh_measurement_preflight_fail"
        ),
        ...
        "m3170_gate_matrix_pass": _bool(source["m3170_summary"].get("gate_matrix_pass", False)),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3171-audit", type=Path, default=DEFAULT_M3171_AUDIT)
