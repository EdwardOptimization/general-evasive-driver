"""COAST-TMeasy: gpu_physics_relax with the MEASURED cruise longitudinal resistance.

This is an exact copy of ``autodrift.gpu_physics_relax`` (L1 RELAX-TMeasy: EXACT Chrono TMeasy
tyre + physical slip-relaxation, sigma measured) with ONE change: the two longitudinal-resistance
params ``drag_coeff`` and ``rolling_resist_coeff`` are set to the values MEASURED by a real
Chrono Sedan COASTDOWN (``scripts/feasibility_audit/extract_chrono_coastdown.py``), instead of the
values that were CALIBRATED on the drift saddle.

WHY: the relax model runs TOO FAST in the cruise/avoidance regime (avoid vx_rmse 1.21 vs the drift
floor 0.235; the vx error grows from ~0.03 at step 0 to +0.74 by step 50 under partial throttle;
vy/lateral is faithful at ~0.14). The calibrated drag_coeff=0.80 / rolling_resist_coeff=0.03 were
tuned on high-sideslip drift, where the dominant longitudinal sink is induced drag from the
sideslipping tyres -- NOT the low-sideslip cruise resistance. So we MEASURED the cruise resistance
directly with a coastdown.

WHAT THE COASTDOWN ACTUALLY SHOWS (this IS the measurement, reported honestly):
The Chrono Sedan has NO explicit aerodynamic body, so the coast deceleration does NOT grow with
v^2 -- it is roughly CONSTANT at ~0.25-0.30 m/s^2 across 9-13 m/s (a least-squares v^2 fit returns
a NEGATIVE, unphysical drag coefficient: the resistance is rolling + driveline/engine-brake drag in
gear, essentially speed-independent above ~5 m/s; there is a ~0.5 m/s^2 bump near 7-8 m/s where the
automatic gearbox downshifts during the coast). The faithful parameterisation is therefore
drag_coeff = 0 (no aero) and rolling_resist_coeff = the measured constant-decel Crr (~0.028).

Crucially, the measured cruise resistance (~0.28 m/s^2) is NOT higher than the calibrated model's
resistance (calibrated drag=0.80/Crr=0.03 already applies ~0.35-0.39 m/s^2 at cruise for the avoid
vehicle): the calibrated longitudinal RESISTANCE is not the cause of the over-speeding. This module
installs the honestly-measured coast resistance; the gate then reports whether that closes the
avoidance vx gap and, if a gap remains, localises it to the DRIVEN (throttle-on engine map / gear /
driveline) side rather than the passive resistance. Everything else (sigma_scale, the EXACT TMeasy
tyre, geometry, powertrain, integration) is IDENTICAL to gpu_physics_relax.

----------------------------------------------------------------------------------------------------
Original gpu_physics_relax docstring (unchanged model, retained for reference):
...
class PhysParams:
    ...
    # --- MEASURED cruise longitudinal resistance (Chrono Sedan COASTDOWN) ---
    # extract_chrono_coastdown.py: zero-throttle/zero-brake coast on flat mu=0.8 -> the coast
    # deceleration is ~constant ~0.28 m/s^2 across 9-13 m/s (NO aero body => no v^2 growth; a v^2
    # LS fit returns a negative, unphysical coefficient). So drag_coeff=0 (no aero) and Crr is the
    # measured constant-decel rolling+driveline drag. This REPLACES the drift-saddle-calibrated
    # drag_coeff=0.80 / rolling_resist_coeff=0.03 (see module docstring). MEASURED, not fit.
