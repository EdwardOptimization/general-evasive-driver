"""FWD-TMeasy (APPROACH A): PROPER front-wheel-drive structural model.

This is a copy of ``gpu_physics_pwr3`` (the gear-SEED-fixed RELAX-TMeasy) with ONE STRUCTURAL change:
the powertrain is made a physically-correct FRONT-wheel drive instead of pwr3's rear-drive kludge.

  RATIONALE (T2 probe, docs/multi-fidelity-gpu-rewrite-design-2026-06.md "T2 PROBE VERDICT"):
  pwr3 is a kludge -- it is nominally FWD (Sedan is ShaftsDriveline2WD, driven axle 0 = FRONT) but it
  applies the drive torque to the REAR spin states (omega_rl/rr) and merely CAPS that rear drive by a
  FRONT friction circle. The probe found that on the avoid down-ramp the model DRIVE force matches
  Chrono's driveshaft (+-14 N) yet Chrono accelerates ~660 N MORE -- attributed to the FWD-vs-rear-drive
  STRUCTURAL distribution. This variant BUILDS the directly-targeted test: apply the driveshaft torque to
  the FRONT wheel spin states (new omega_fl/fr), let the FRONT tyres carry the drive through their OWN
  combined-slip friction circle (during the avoid lane-change the front is steered + at a slip angle, so
  the front drive Fx is combined-slip-limited the way Chrono's is), and make the REAR wheels FREE-ROLLING
  (rear longitudinal slip -> 0, rear Fx ~ rolling resistance only, no drive torque). pwr3's front-axle
  friction CAP is removed (the front tyre's own combined-slip circle now does that job physically).

  State changes vs pwr3: PHYS_STATE_DIM 17 -> 21. Two new front wheel spin states (omega_fl, omega_fr)
  and two new front longitudinal slip-relaxation lags (sx_fl_lag, sx_fr_lag). The rear spin states are
  retained but carry NO drive torque (free-rolling). Everything else (EXACT TMeasy tyre, measured sigma,
  geometry, gearbox FSM, load transfer, sub-stepped integration, gear seed) is IDENTICAL to pwr3.

----------------------------------------------------------------------------------------------------
Original gpu_physics_pwr3 docstring (rear-drive kludge; retained for reference):
...
IDX = dict(
    ...
    # APPROACH A (FWD): front wheel spin states + front longitudinal slip lags.
...
def _continuous_derivs(vx, vy, yaw_rate, steer, omega_rl, omega_rr, throttle, brake, gear,
    ...
    """Continuous-time derivatives with SLIP relaxation (the L1 transient) -- PROPER FWD.

    APPROACH A: the driveshaft torque is applied to the FRONT wheel spin states (omega_fl/fr) and the
    FRONT longitudinal slip (sx_fl/fr) is derived from those, so the FRONT tyre carries the drive
    through its OWN combined-slip friction circle. The REAR wheels are FREE-ROLLING (rear long. slip
    relaxes toward 0 from the rolling-equilibrium omega_rl/rr; rear Fx ~ rolling resistance only, no
    drive torque). alpha_*_lag/sx_*_lag are the relaxation-lagged tyre SLIPS: the EXACT curve is
    evaluated at the LAGGED slip, so the force builds up over the measured physical relaxation length
    sigma. The lag is advanced SEMI-IMPLICITLY by the sub-step ``h`` BEFORE the force eval. All inputs
    [N]. Returns (vx_dot, vy_dot, yaw_dot, orl_dot, orr_dot,
    alpha_f_lag_new, alpha_r_lag_new, sx_rl_lag_new, sx_rr_lag_new,
    ofl_dot, ofr_dot, sx_fl_lag_new, sx_fr_lag_new, ax_body, ay_body)."""
    mu_scale = P.mu / P["mu0"]
    ...
    # ---- INSTANTANEOUS FRONT longitudinal slip from front wheel spin (FWD driven axle) ----
    # The front wheel rolls along its STEERED direction; the rolling-direction contact velocity is the
    # body velocity at the front axle rotated into the wheel frame: vx_wheel = vx*cos(d) + vy_f*sin(d).
    # (At the avoid lane-change steer is small so this ~ vx; carried exactly for combined-slip fidelity.)
    half_tf = 0.5 * P["track_f"]
    cs_f = torch.cos(steer)
    sn_f = torch.sin(steer)
    vx_fl_b = vx - yaw_rate * half_tf       # body-x contact vel at the front-left (track offset)
    vx_fr_b = vx + yaw_rate * half_tf
    vx_fl = vx_fl_b * cs_f + vy_f * sn_f     # rotate into the (steered) front-left wheel frame
    vx_fr = vx_fr_b * cs_f + vy_f * sn_f
    vx_fl_safe = torch.sign(vx_fl) * vx_fl.abs().clamp_min(0.75)
    vx_fr_safe = torch.sign(vx_fr) * vx_fr.abs().clamp_min(0.75)
    sx_fl_inst = (P["r_eff"] * omega_fl - vx_fl) / vx_fl_safe.abs().clamp_min(0.5)
    sx_fr_inst = (P["r_eff"] * omega_fr - vx_fr) / vx_fr_safe.abs().clamp_min(0.5)
    ...
    sig_k_fl = _sigma_at(fz_fl, P, "kappa")     # FRONT longitudinal relaxation (FWD driven axle)
    ...
    # APPROACH A (FWD): the FRONT wheels are DRIVEN, so their longitudinal slip (sx_fl/fr_lag, built up
    # by the front spin states absorbing the driveshaft torque) carries the drive Fx through the FRONT
    # combined-slip friction circle -- combined with the steered front slip angle alpha_f_lag, exactly
    # as Chrono's FWD front tyre is. The REAR wheels are FREE-ROLLING: their spin states track vx/r_eff
    # so sx_rl/rr_lag -> ~0 and the rear tyre Fx is ~0 (pure rolling); the rear longitudinal resistance
    # is the body rolling-resistance term in _accel_from_forces.
    ...
    # ---- powertrain torque to the FRONT wheels (PROPER FWD; driven axle = front) ----
    # The Chrono Sedan is FRONT-wheel drive (ShaftsDriveline2WD, driven axle 0 = front). The motor rpm
    # is read off the FRONT (driven) wheel speed through the final drive + gear ratio, and the full
    # engine force is applied to the FRONT wheel spin states. The front tyre then sheds whatever it
    # cannot put down as longitudinal slip through its OWN combined-slip friction circle (computed
    # above from the EXACT TMeasy curve at the front Fz + steered slip angle) -- which is exactly how
    # Chrono's FWD front tyre is traction-limited on the load-LOSING front axle. NO separate friction
    # cap is needed; the structural FWD distribution replaces pwr3's rear-drive-plus-cap kludge.
    omega_axle = 0.5 * (omega_fl + omega_fr)
    ...
    T_wheel = 0.5 * T_axle      # open diff splits equally between the two FRONT wheels

    # brake torque opposes each wheel's spin direction (all 4 wheels braked).
    T_brake = brake * P["max_brake_torque"]
    T_brake_fl = T_brake * torch.sign(omega_fl)
    T_brake_fr = T_brake * torch.sign(omega_fr)
    T_brake_rl = T_brake * torch.sign(omega_rl)
    T_brake_rr = T_brake * torch.sign(omega_rr)

    # FRONT (driven) wheel-spin dynamics: drive torque - tyre reaction - brake. The tyre reaction uses
    # the front longitudinal force in the WHEEL ROLLING direction (the combined-slip Fx magnitude along
    # +kappa), i.e. fx_*_wheel = |Fx0|*cx evaluated above -- which is exactly fx_fl/fx_fr (the wheel-
    # frame longitudinal force, BEFORE the steer rotation done in _accel_from_forces).
    omega_fl_dot = (T_wheel - P["r_eff"] * fx_fl - T_brake_fl) / P["i_wheel"]
    omega_fr_dot = (T_wheel - P["r_eff"] * fx_fr - T_brake_fr) / P["i_wheel"]

    # REAR (free-rolling) wheel-spin dynamics: NO drive torque; the rear wheel is driven only by the
    # tyre rolling reaction + brake, so it tracks vx/r_eff and the rear longitudinal slip relaxes to 0.
    omega_rl_dot = (-P["r_eff"] * fx_rl - T_brake_rl) / P["i_wheel"]
    omega_rr_dot = (-P["r_eff"] * fx_rr - T_brake_rr) / P["i_wheel"]
...
def physics_step(state: torch.Tensor, action: torch.Tensor, gear: torch.Tensor,
    ...
    # FWD: motor rpm derives from the FRONT (driven) wheel speed.
    omega_axle = 0.5 * (omega_fl + omega_fr)
    ...
    out[:, IDX["omega_fl"]] = omega_fl
    out[:, IDX["omega_fr"]] = omega_fr
...
def init_state(vx0: torch.Tensor, vy0: torch.Tensor, yaw0: torch.Tensor, P: PhysParamBatch):
    """Build an [N,21] initial state + gear from initial velocity. Wheel omega seeded to vx/r_eff,
    ...
    # rolling wheels: omega = vx / r_eff (no initial slip) -- all four (front DRIVEN, rear free-rolling)
    st[:, IDX["omega_rl"]] = vx0 / P["r_eff"]
    st[:, IDX["omega_rr"]] = vx0 / P["r_eff"]
    st[:, IDX["omega_fl"]] = vx0 / P["r_eff"]
    st[:, IDX["omega_fr"]] = vx0 / P["r_eff"]
    ...
    st[:, IDX["sx_fl_lag"]] = 0.0   # front (driven) wheels also start at rolling equilibrium
