"""Measured execution for the calibrated scale-up subset."""
...
from autodrift.controller_family_calibrated_scale_up_preflight import DEFAULT_OUTPUT_DIR as DEFAULT_M1712_OUTPUT_DIR
...
DEFAULT_SCALE_UP_CALIBRATION_SPECS = DEFAULT_M1712_OUTPUT_DIR / "scale_up_calibration_specs.json"
DEFAULT_SCALE_UP_MATRIX = DEFAULT_M1712_OUTPUT_DIR / "scale_up_matrix.csv"
DEFAULT_RUN_DIR = Path("runs/m1715_controller_family_calibrated_scale_up_execution")
DEFAULT_EVAL_SEED_BASE = 171500
TARGET_EPISODE_COUNT = 864
TARGET_PROFILE_COUNT = 12
TARGET_SCALE_UP_CALIBRATION_SPEC_COUNT = 72
TARGET_SELECTED_BASE_SPEC_COUNT = 18
TARGET_SCALE_UP_VARIANT_COUNT = 4
SCALE_UP_FAILURE_FIELDNAMES = [
    "workload_id",
...
def load_scale_up_calibration_specs(path: Path | str = DEFAULT_SCALE_UP_CALIBRATION_SPECS) -> list[dict[str, Any]]:
    payload = read_json(path)
    return list(payload["scale_up_calibration_specs"])


def scale_up_executable_specs(specs: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return calibration_executable_specs(specs)


def scale_up_workload_rows(path: Path | str = DEFAULT_SCALE_UP_MATRIX) -> list[dict[str, Any]]:
    rows = read_csv_rows(path)
    converted: list[dict[str, Any]] = []
    for row in rows:
        item = dict(row)
        item["workload_id"] = str(row["scale_up_workload_id"])
        item["task_source_id"] = str(row["calibration_spec_id"])
        item["strata"] = ";".join(
            [
                "calibrated_scale_up",
                f"task_family_{row['task_family']}",
                f"variant_{row['scale_up_variant_label']}",
...
def _run_scale_up_workload_cell(
    *,
    ...
    row.update(
        {
            "scale_up_workload_id": str(workload_row["scale_up_workload_id"]),
            "calibration_workload_id": str(workload_row["calibration_workload_id"]),
            "calibration_spec_id": str(workload_row["calibration_spec_id"]),
            "scale_up_variant_label": str(workload_row["scale_up_variant_label"]),
            ...
            "calibrated_scale_up_execution": True,
...
def _write_scale_up_aggregates(output_dir: Path, episode_rows: list[dict[str, Any]]) -> dict[str, int]:
    profile_aggregate = aggregate_outcome_rows(episode_rows, ("profile_name",))
    scale_up_variant_aggregate = aggregate_outcome_rows(episode_rows, ("scale_up_variant_label",))
    ...
    write_csv_rows(output_dir / "scale_up_variant_aggregate.csv", scale_up_variant_aggregate)
    ...
    return {
        ...
        "scale_up_variant_aggregate_rows": len(scale_up_variant_aggregate),
...
def finalize_scale_up_outputs(
    *,
    output_dir: Path,
    target_workload_count: int,
    next_blocker: str = "m1716-paper-route-controller-family-calibrated-scale-up-result-audit",
) -> dict[str, Any]:
    ...
    if not (output_dir / "failure_rows.csv").exists():
        write_csv_rows(output_dir / "failure_rows.csv", failure_rows, fieldnames=SCALE_UP_FAILURE_FIELDNAMES)
    aggregate_counts = _write_scale_up_aggregates(output_dir, episode_rows)
    ...
    result_passes = (
        ...
        and aggregate_counts["scale_up_variant_aggregate_rows"] == TARGET_SCALE_UP_VARIANT_COUNT
        and aggregate_counts["outcome_aggregate_rows"] > 0
        and aggregate_counts["termination_reason_aggregate_rows"] > 0
    )
    summary = {
        "result_class": (
            "controller_family_calibrated_scale_up_execution_pass"
            if result_passes
            else "controller_family_calibrated_scale_up_execution_incomplete_or_fail"
        ),
        ...
        "scale_up_calibration_spec_count": (
            len({row["calibration_spec_id"] for row in episode_rows}) if episode_rows else 0
        ),
        "target_scale_up_calibration_spec_count": TARGET_SCALE_UP_CALIBRATION_SPEC_COUNT,
        ...
        "scale_up_variant_count": len({row["scale_up_variant_label"] for row in episode_rows}) if episode_rows else 0,
        "target_scale_up_variant_count": TARGET_SCALE_UP_VARIANT_COUNT,
        ...
        **aggregate_counts,
        ...
        "artifacts": {
            ...
            "scale_up_variant_aggregate": str(output_dir / "scale_up_variant_aggregate.csv"),
...
def run_calibrated_scale_up_execution(
    *,
    output_dir: Path | str = DEFAULT_RUN_DIR,
    calibration_specs_path: Path | str = DEFAULT_SCALE_UP_CALIBRATION_SPECS,
    workload_path: Path | str = DEFAULT_SCALE_UP_MATRIX,
    ...
    executable_specs = scale_up_executable_specs(load_scale_up_calibration_specs(calibration_specs_path))
    spec_by_id = {str(spec["calibration_spec_id"]): spec for spec in executable_specs}
    workload_rows = scale_up_workload_rows(workload_path)
    ...
    if not resume:
        for path in (
            ...
            output / "scale_up_variant_aggregate.csv",
    ...
    if not (output / "failure_rows.csv").exists():
        write_csv_rows(output / "failure_rows.csv", [], fieldnames=SCALE_UP_FAILURE_FIELDNAMES)
    ...
    for cell_index, workload_row in enumerate(workload_rows):
        ...
        try:
            ...
            row = _run_scale_up_workload_cell(
                workload_row=workload_row,
            ...
            failure_row = {
                ...
                "scale_up_workload_id": str(workload_row.get("scale_up_workload_id", "")),
                ...
                "scale_up_variant_label": str(workload_row.get("scale_up_variant_label", "")),
    ...
    return finalize_scale_up_outputs(output_dir=output, target_workload_count=len(workload_rows), next_blocker=next_blocker)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run measured calibrated scale-up execution.")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_RUN_DIR)
    parser.add_argument("--calibration-specs", type=Path, default=DEFAULT_SCALE_UP_CALIBRATION_SPECS)
    parser.add_argument("--workload", type=Path, default=DEFAULT_SCALE_UP_MATRIX)
    ...
    parser.add_argument("--next-blocker", default="m1716-paper-route-controller-family-calibrated-scale-up-result-audit")
    args = parser.parse_args()

    summary = run_calibrated_scale_up_execution(
