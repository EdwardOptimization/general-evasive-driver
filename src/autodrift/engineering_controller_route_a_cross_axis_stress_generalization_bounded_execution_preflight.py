"""Run M2753 cross-axis stress generalization bounded execution preflight.

M2753 consumes the M2752 design and the M1690 executable workload matrix. It
selects exactly 12 fixed non-same-panel L3_online_gru workload rows, excludes
M2746/M2737 prior-panel task sources, runs one bounded closed-loop diagnostic
episode per resolved row, and carries protected/HF3 blockers as guardrails. It
does not rank, validate, train, promote, or claim driver performance.
"""
...
DEFAULT_MILESTONE = "m2753-engineering-controller-route-a-cross-axis-stress-generalization-bounded-execution-preflight"
DEFAULT_NEXT_BLOCKER = (
    "m2754-engineering-controller-route-a-cross-axis-stress-generalization-bounded-execution-result-audit"
)
DEFAULT_M2752_DESIGN = Path(
...
DEFAULT_EVAL_SEED_BASE = 275300
CANONICAL_PROFILE = "L3_online_gru"

SELECTED_TASK_SOURCES: tuple[tuple[str, str, str, str], ...] = (
    ("m1680-spec-0001", "T4", "actuator_delay_step|t4_capability_step_temporal", "actuator_delay_or_response"),
    ("m1680-spec-0003", "T4", "t4_actuator_delay_response|actuator_delay_step", "actuator_delay_or_response"),
    ("m1680-spec-0008", "T4", "actuator_delay_step|t4_capability_step_temporal", "actuator_delay_or_response"),
    ("m1680-spec-0010", "T4", "t4_actuator_delay_response|actuator_delay_step", "actuator_delay_or_response"),
    ("m1680-spec-0037", "T5", "brake_fade_or_loss_proxy|late_reveal_boundary", "brake_or_drive_authority"),
    ("m1680-spec-0039", "T5", "curved_boundary_obstacle|drive_loss_proxy", "brake_or_drive_authority"),
    ("m1680-spec-0042", "T5", "t5_boundary_axis_retarget|drive_loss_proxy", "brake_or_drive_authority"),
    ("m1680-spec-0043", "T5", "t5_near_boundary_warmup|t5_boundary_axis_retarget", "late_boundary_or_near_boundary"),
    ("m1680-spec-0044", "T5", "actuator_delay_step|t5_near_boundary_warmup", "actuator_delay_or_response"),
    ("m1680-spec-0045", "T5", "brake_fade_or_loss_proxy|late_reveal_boundary", "late_boundary_or_near_boundary"),
    ("m1680-spec-0046", "T5", "capability_step_down|t5_near_boundary_warmup", "late_boundary_or_near_boundary"),
    ("m1680-spec-0047", "T5", "curved_boundary_obstacle|drive_loss_proxy", "curved_or_retargeted_obstacle"),
)
EXPECTED_CANDIDATE_COUNT = len(SELECTED_TASK_SOURCES)
EXPECTED_PRIOR_PANEL_MIN_COUNT = 9

STRESS_AXIS_TAGS: dict[str, tuple[str, ...]] = {
    "m1680-spec-0001": ("actuator_delay_or_response",),
    "m1680-spec-0003": ("actuator_delay_or_response",),
    "m1680-spec-0008": ("actuator_delay_or_response",),
    "m1680-spec-0010": ("actuator_delay_or_response",),
    "m1680-spec-0037": ("brake_or_drive_authority", "late_boundary_or_near_boundary"),
    "m1680-spec-0039": ("brake_or_drive_authority", "curved_or_retargeted_obstacle"),
    "m1680-spec-0042": ("brake_or_drive_authority", "curved_or_retargeted_obstacle"),
    "m1680-spec-0043": ("late_boundary_or_near_boundary", "curved_or_retargeted_obstacle"),
    "m1680-spec-0044": ("actuator_delay_or_response", "late_boundary_or_near_boundary"),
    "m1680-spec-0045": ("brake_or_drive_authority", "late_boundary_or_near_boundary"),
    "m1680-spec-0046": ("late_boundary_or_near_boundary",),
    "m1680-spec-0047": ("brake_or_drive_authority", "curved_or_retargeted_obstacle"),
}

CLAIM_SCOPE = (
    "M2753 Route A cross-axis stress generalization bounded execution preflight "
    "only; reset, step, rollout, and policy actions may be recorded for the 12 "
    "selected non-same-panel M1690 L3_online_gru rows, while M2746/M2737 "
    "prior-panel rows, protected blocker rows, and HF3 blocker rows remain "
...
def run_cross_axis_stress_generalization_bounded_execution_preflight(
    *,
    ...
    m2749_dir: Path | str = DEFAULT_M2749_DIR,
    m2752_design: Path | str = DEFAULT_M2752_DESIGN,
    ...
    source = load_source_artifacts(
        ...
        m2749_dir=Path(m2749_dir),
        m2752_design=Path(m2752_design),
        executable_specs=Path(executable_specs),
        follow_up_manifest=Path(follow_up_manifest),
    )
    prior_exclusion_rows = build_prior_panel_exclusion_rows(source)
    candidate_rows = build_cross_axis_candidate_rows(source, prior_exclusion_rows)
    resolution_rows, resolved_sources = build_resolution_rows(candidate_rows, source["m1690_by_task_source"])

    write_csv_rows(paths["cross_axis_candidate_rows"], candidate_rows, fieldnames=CANDIDATE_FIELDNAMES)
    write_csv_rows(paths["prior_panel_exclusion_rows"], prior_exclusion_rows, fieldnames=PRIOR_EXCLUSION_FIELDNAMES)
...
def artifact_paths(output_dir: Path, *, doc_path: Path) -> dict[str, Path]:
    return {
        ...
        "cross_axis_candidate_rows": output_dir / "cross_axis_candidate_rows.csv",
        ...
        "prior_panel_exclusion_rows": output_dir / "prior_panel_exclusion_rows.csv",
...
def load_source_artifacts(
    *,
    ...
    paths = {
        ...
        "m2749_blocker_matrix": m2749_dir / "blocker_matrix.csv",
        "m2749_next_action_rows": m2749_dir / "next_action_admission_rows.csv",
    ...
    return {
        ...
        "m2752_design_text": paths["m2752_design"].read_text(encoding="utf-8") if source_exists["m2752_design"] else "",
        ...
        "blocker_rows": read_csv_rows(paths["m2749_blocker_matrix"]),
        "next_action_rows": read_csv_rows(paths["m2749_next_action_rows"]),
    }
...
def build_prior_panel_exclusion_rows(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, ((source_panel, task_source_id), row_count) in enumerate(sorted(counts.items()), start=1):
        rows.append(
            {
                "exclusion_id": f"m2753-prior-panel-exclusion-{index:04d}",
...
def build_cross_axis_candidate_rows(
    ...
    for index, (task_source_id, expected_task_family, expected_source_edge, primary_axis) in enumerate(
        ...
        rows.append(
            {
                "candidate_id": f"m2753-cross-axis-candidate-{index:04d}",
                "task_source_id": task_source_id,
...
def build_resolution_rows(
    ...
    for index, candidate in enumerate(candidate_rows, start=1):
        resolution_id = f"m2753-resolution-{index:04d}"
...
def run_candidate_execution(
    *,
    ...
    summary = {
        "result_class": (
            "engineering_controller_route_a_cross_axis_stress_generalization_bounded_execution_pass"
            if status_pass
            else "engineering_controller_route_a_cross_axis_stress_generalization_bounded_execution_incomplete_or_fail"
        ),
...
def build_stress_axis_aggregate_rows(
    *,
    ...
    for index, tag in enumerate(
        ...
        rows.append(
            {
                "aggregate_id": f"m2753-stress-axis-aggregate-{index:04d}",
                "stress_axis_tag": tag,
...
def build_blocker_guard_rows(blocker_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for index, blocker in enumerate(blocker_rows, start=1):
        rows.append(
            {
                "guard_id": f"m2753-blocker-guard-{index:04d}",
...
def actor_guard(field: str, observed: Any, expected: Any) -> dict[str, Any]:
    return {
        "guard_id": f"m2753-actor-guard-{field}",
        "guard_family": field,
        "observed": observed,
        "expected": expected,
...
def build_claim_boundary_rows(
    *,
    ...
    specs = [
        ("bounded_execution_artifact_completeness", "artifact", True, artifacts_present and episode_or_failure_rows_present, "M2753 artifacts"),
        ("follow_up_result_audit_registered", "follow_up_route", True, follow_up_manifest_registered, "M2754 result-audit manifest"),
...
def claim(claim_id: str, family: str, allowed: bool, made: bool, evidence: str) -> dict[str, Any]:
    return {
        "claim_id": f"m2753-claim-{claim_id}",
        "claim_family": family,
        "allowed_in_m2753": allowed,
        "claim_made": made,
...
def build_gate_matrix_rows(
    *,
    ...
    failure_rows = artifact_rows["candidate_execution_failure_rows"]
    checks = [
        ("source_artifacts_present", "lineage", all(source["source_exists"].values()), source["source_exists"], "all M2752/M1690/M2746/M2737/M2749/spec/follow-up artifacts present", "lineage_invalid"),
        ...
        ("prior_panel_exclusions_loaded", "candidate_surface", len({row["task_source_id"] for row in prior_exclusion_rows}) >= EXPECTED_PRIOR_PANEL_MIN_COUNT, len({row["task_source_id"] for row in prior_exclusion_rows}), f">={EXPECTED_PRIOR_PANEL_MIN_COUNT}", "lineage_invalid"),
...
def build_summary(
    *,
    ...
    return {
        "milestone": milestone,
        "result_class": (
            "engineering_controller_route_a_cross_axis_stress_generalization_bounded_execution_preflight_pass"
            if status_pass
            else "engineering_controller_route_a_cross_axis_stress_generalization_bounded_execution_preflight_fail"
        ),
        "status_pass": status_pass,
        "generated_at_utc": utc_timestamp(),
        "output_dir": str(output_dir),
...
def render_milestone_doc(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- prior-panel exclusion rows: {summary['prior_panel_exclusion_row_count']}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m2749-dir", type=Path, default=DEFAULT_M2749_DIR)
    parser.add_argument("--m2752-design", type=Path, default=DEFAULT_M2752_DESIGN)
...
def main() -> None:
    args = parse_args()
    summary = run_cross_axis_stress_generalization_bounded_execution_preflight(
        m1690_workload=args.m1690_workload,
        m2746_dir=args.m2746_dir,
        m2737_dir=args.m2737_dir,
        m2749_dir=args.m2749_dir,
        m2752_design=args.m2752_design,
    ...
    print(
        "M2753 cross-axis stress execution preflight: "
