"""M2848 response-predictive recurrent-belief bounded continuation preflight.

This runner continues from the audited M2846 candidate checkpoint with a small,
bounded PPO run. It writes continuation, proof, generalization, promotion,
actor-contract, and claim-boundary artifacts. It does not validate, rank,
...
DEFAULT_MILESTONE = (
    "m2848-engineering-controller-route-a-response-predictive-recurrent-belief-core-"
    "training-bounded-continuation-preflight"
)
DEFAULT_OUTPUT_DIR = Path(
    "runs/m2848_engineering_controller_route_a_response_predictive_recurrent_belief_core_"
    "training_bounded_continuation_preflight"
)
DEFAULT_M2847_AUDIT = Path(
    "docs/m2847-engineering-controller-route-a-response-predictive-recurrent-belief-core-"
    "training-implementation-preflight-result-audit.md"
)
DEFAULT_M2846_SUMMARY = Path(
    "runs/m2846_engineering_controller_route_a_response_predictive_recurrent_belief_core_"
    "training_implementation_preflight/summary.json"
)
DEFAULT_SOURCE_CHECKPOINT = Path(
    "runs/m2846_engineering_controller_route_a_response_predictive_recurrent_belief_core_"
    "training_implementation_preflight/checkpoints/m2846_response_predictive_recurrent_belief_candidate.pt"
)
DEFAULT_FOLLOW_UP_MANIFEST = Path(
    "experiments/manifests/m2849-engineering-controller-route-a-response-predictive-"
    "recurrent-belief-core-training-bounded-continuation-result-audit.json"
)
DEFAULT_DOC_PATH = Path(
    "docs/m2848-engineering-controller-route-a-response-predictive-recurrent-belief-core-"
    "training-bounded-continuation-preflight.md"
)
DEFAULT_NEXT_BLOCKER = (
    "m2849-engineering-controller-route-a-response-predictive-recurrent-belief-core-"
    "training-bounded-continuation-result-audit"
)

CLAIM_SCOPE = (
    "M2848 response-predictive recurrent-belief core training bounded continuation "
    "preflight only"
)
...
RESULT_CLASS_PASS = (
    ...
    "bounded_continuation_preflight_pass"
)
RESULT_CLASS_FAIL = (
    "engineering_controller_route_a_response_predictive_recurrent_belief_core_training_"
    "bounded_continuation_preflight_failed"
)

RESPONSE_TARGET_FIELDNAMES = m2846_base.RESPONSE_TARGET_FIELDNAMES
TRAINING_SEED_FIELDNAMES = m2846_base.TRAINING_SEED_FIELDNAMES
TRAINING_RUN_FIELDNAMES = m2846_base.TRAINING_RUN_FIELDNAMES
PARAMETER_GROUP_FIELDNAMES = m2846_base.PARAMETER_GROUP_FIELDNAMES
RESPONSE_PROBE_FIELDNAMES = m2846_base.RESPONSE_PROBE_FIELDNAMES
HIDDEN_INTERVENTION_FIELDNAMES = m2846_base.HIDDEN_INTERVENTION_FIELDNAMES
GATE_FIELDNAMES = m2846_base.GATE_FIELDNAMES
ACTOR_GUARD_FIELDNAMES = m2846_base.ACTOR_GUARD_FIELDNAMES
CLAIM_FIELDNAMES = m2846_base.CLAIM_FIELDNAMES
REQUIRED_PARAMETER_GROUPS = m2846_base.REQUIRED_PARAMETER_GROUPS
REQUIRED_NON_ACTOR_HEAD_GROUPS = m2846_base.REQUIRED_NON_ACTOR_HEAD_GROUPS
RESPONSE_CHANNELS = m2846_base.RESPONSE_CHANNELS
FALSE_CLAIM_FLAGS = m2846_base.FALSE_CLAIM_FLAGS


def run_response_predictive_recurrent_belief_core_training_bounded_continuation_preflight(
    output_dir: Path | str = DEFAULT_OUTPUT_DIR,
    *,
    m2847_audit: Path | str = DEFAULT_M2847_AUDIT,
    m2846_summary: Path | str = DEFAULT_M2846_SUMMARY,
    ...
    source_paths = _source_paths(Path(m2847_audit), Path(m2846_summary), Path(source_checkpoint))
    _require_sources(source_paths)

    m2846_summary_data = read_json(source_paths["m2846_summary"])
    m2838_summary = read_json(source_paths["m2838_summary"])
    env_config = build_m2848_env_config()
    ppo_config = build_m2848_ppo_config(
    ...
    proof_gate_rows = build_proof_gate_rows(
        ...
        m2846_summary=m2846_summary_data,
    )
    generalization_gate_rows = build_generalization_gate_rows(training_seed_rows, m2838_summary)
    ...
    follow_up = build_m2849_manifest(summary)
    ...
    summary = {
        **summary,
        ...
        "m2849_follow_up_manifest_registered": paths["registered_follow_up_manifest"].exists(),
    }
    summary["status_pass"] = bool(
        summary["status_pass"]
        and summary["required_artifacts_present"]
        and summary["m2849_follow_up_manifest_registered"]
    )
...
def _paths(output: Path, doc_path: Path, follow_up_manifest: Path) -> dict[str, Path]:
    return {
        ...
        "candidate_checkpoint": output / "checkpoints" / "m2848_response_predictive_recurrent_belief_continuation_candidate.pt",
...
def _source_paths(
    ...
    paths = {
        "m2847_audit": m2847_audit,
        "m2846_summary": m2846_summary,
        "source_checkpoint": source_checkpoint,
    }
    if m2846_summary.exists():
        summary = read_json(m2846_summary)
        paths["m2838_summary"] = Path(summary.get("m2838_summary", ""))
    else:
        paths["m2838_summary"] = Path("")
    return paths
...
def _require_sources(paths: dict[str, Path]) -> None:
    ...
    if missing:
        raise FileNotFoundError(f"M2848 missing required source artifacts: {missing}")


def build_m2848_env_config() -> DriftEnvConfig:
...
def build_m2848_ppo_config(
    *,
    ...
    if int(rollout_steps) <= 4:
        raise ValueError("M2848 rollout_steps must exceed response horizon 4")
    return PPOConfig(
...
def build_protocol_config_snapshot(source_paths: dict[str, Path], config: PPOConfig) -> dict[str, Any]:
    return {
        ...
        "bounded_continuation_preflight_only": True,
...
def build_training_seed_rows(seed: int) -> list[dict[str, Any]]:
    return [
        {
            "seed_row_id": "m2848_bounded_continuation_seed_000",
            "split": "bounded_continuation_preflight",
            "seed": int(seed),
            "training_smoke": True,
...
def build_checkpoint_metadata(
    *,
    ...
    return {
        "m2848_response_predictive_recurrent_belief_core_training_bounded_continuation_preflight": {
            "milestone": milestone,
...
def read_csv_rows(path: Path | str) -> list[dict[str, str]]:
    return m2846_base.read_csv_rows(path)
...
def build_training_run_rows(
    ...
    *,
    ...
    return [
        {
            "training_run_id": "m2848_bounded_continuation_000",
            "seed": int(seed),
...
def build_parameter_group_trace_rows(source_checkpoint: Path, candidate_checkpoint: Path) -> list[dict[str, Any]]:
    source_state = m2846_base._load_checkpoint_state(source_checkpoint) if source_checkpoint.exists() else {}
    candidate_state = m2846_base._load_checkpoint_state(candidate_checkpoint) if candidate_checkpoint.exists() else {}
    rows: list[dict[str, Any]] = []
    for group in REQUIRED_PARAMETER_GROUPS:
        source_group = m2846_base._state_group(source_state, group)
        candidate_group = m2846_base._state_group(candidate_state, group)
        comparable = m2846_base._comparable_tensors(source_group, candidate_group)
        delta_l2 = ""
        delta_max_abs = ""
        if comparable:
            squared = 0.0
        ...
        source_hash = m2846_base.hash_state_group(source_group)
        candidate_hash = m2846_base.hash_state_group(candidate_group)
        rows.append(
            {
...
def build_response_prediction_probe_rows(
    ...
    for row in response_target_rows:
        ...
        rows.append(
            {
                "probe_id": f"m2848_response_probe_target_{int(row['observation_index']):02d}",
...
def build_hidden_intervention_probe_rows() -> list[dict[str, Any]]:
    return [
        {
            "probe_id": f"m2848_hidden_intervention_{name}",
            "intervention": name,
            "executed": False,
            "validation_denominator": False,
            "self_id_claim_made": False,
            "routed_to_next_audit": True,
            "evidence": "not_collected_in_m2848_bounded_continuation_preflight",
            "claim_boundary": CLAIM_SCOPE,
        }
...
def build_checkpoint_manifest(
    ...
    *,
    ...
    return {
        "manifest_id": "m2848_checkpoint_manifest_v0",
        "milestone": DEFAULT_MILESTONE,
        "source_checkpoint": str(source_checkpoint),
...
def build_claim_boundary_rows(training_status: str) -> list[dict[str, Any]]:
    rows = [
        {
            "claim_id": "bounded_continuation_preflight_executed",
            "claim_family": "allowed_bounded_continuation_preflight",
...
def build_proof_gate_rows(
    *,
    ...
    m2846_admitted = (
        bool(m2846_summary.get("status_pass", False))
        and bool(m2846_summary.get("required_artifacts_present", False))
        and bool(m2846_summary.get("gate_matrix_pass", False))
    )
    gates = [
        (
            "proof_m2846_admitted_source_checkpoint",
            "lineage",
            m2846_admitted,
            str(m2846_admitted),
            "M2846 status/artifacts/gate matrix pass",
            1,
            "lineage_invalid",
        ),
        (
...
def build_generalization_gate_rows(
    ...
    gates = [
        (
            ...
            ">=1 bounded continuation seed row",
        ...
        (
            ...
            "failure_taxonomy",
            True,
            "contract_violation,lineage_invalid,metric_artifact,proof_washout,training_instability",
        ...
        (
            ...
            "claim_boundary",
            True,
            "bounded continuation preflight only",
...
def build_summary(
    *,
    ...
    return {
        ...
        "m2847_audit": str(source_paths["m2847_audit"]),
        "m2846_summary": str(source_paths["m2846_summary"]),
        "m2838_summary": str(source_paths["m2838_summary"]),
        "source_load_mode": source_load_mode,
        "m2846_source_status_pass": bool(m2846_summary.get("status_pass", False)),
        "m2846_source_required_artifacts_present": bool(m2846_summary.get("required_artifacts_present", False)),
        "m2846_source_gate_matrix_pass": bool(m2846_summary.get("gate_matrix_pass", False)),
        "training_status": training_status,
        "training_error": train_error,
        "bounded_continuation_run": training_status == "completed",
        ...
        "m2849_follow_up_manifest_registered": False,
    }
...
def build_m2849_manifest(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "lineage": {
            ...
            "parent_dataset": [
                ...
                summary["doc"],
                summary["m2847_audit"],
                summary["m2846_summary"],
                summary["m2838_summary"],
            ],
            "parent_config": [
                "experiments/manifests/m2848-engineering-controller-route-a-response-predictive-recurrent-belief-core-training-bounded-continuation-preflight.json",
                "experiments/manifests/m2847-engineering-controller-route-a-response-predictive-recurrent-belief-core-training-implementation-preflight-result-audit.json",
            ],
            "parent_objective": [
                "audit M2848 bounded response-predictive recurrent-belief continuation artifacts before interpretation"
            ],
            "derived_from": [DEFAULT_MILESTONE],
            "blocked_by": [
                "M2849 must audit M2848 summary status gates artifact completeness checkpoint lineage and claim boundaries",
                "M2849 must keep M2838 weak diagnostic rows visible and outside performance denominators",
                "M2849 must not promote the M2848 checkpoint or claim validation driver performance paper current-sim high-fidelity full-driver or self-ID evidence",
            ],
            "supersedes": ["unaudited bounded continuation preflight interpretation"],
            "invalidates": [],
        },
        "review_artifact": f"docs/reviews/{DEFAULT_NEXT_BLOCKER}.md",
        "public_gates": [
            "M2849 must audit whether M2848 wrote complete continuation-preflight artifacts",
            "M2849 must verify actor 72/action 3 and no hidden/oracle actor input were preserved",
            "M2849 must verify response target schema includes only observation indices 0-8 and excludes previous-command fields from targets",
            "M2849 must verify parameter trace shows recurrent/fusion or response-prediction mutation and rejects actor_mean.bias-only continuation",
            "M2849 must verify proof generalization promotion actor claim and gate rows before any next training decision",
            "M2849 must not validate rank promote compute success-rate verdict claim performance paper current-sim high-fidelity full-driver or self-ID result",
        ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "response_predictive_recurrent_belief_core_training_bounded_continuation_result_audit",
            "evidence_increment": "audits M2848 bounded continuation-preflight artifacts before any interpretation or further training",
            ...
            "stop_condition": [
                "stop if M2848 changed actor 72/action 3",
                ...
                "stop if M2848 claims validation readiness driver performance paper current-sim high-fidelity or self-ID evidence",
            ],
            "fallback_plan": [
                "route to implementation repair design only if a narrow artifact or runner issue is identified",
                "route to bounded follow-up training design only if M2848 passes and audit admits more training evidence",
                "route to limited-baseline freeze or branch stop if continuation cannot change evidence safely",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M2848 continuation preflight has produced executable artifacts requiring audit",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "audit M2848 response-predictive recurrent-belief bounded continuation artifacts",
            "admission_evidence": [
                "M2848 summary artifact is expected before M2849 runs",
                "M2848 checkpoint manifest parameter trace response target schema and gate rows require audit before interpretation",
            ],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M2849 status queue scoreboard and review",
                "one bounded follow-up manifest if audit accepts a next route",
            ],
            "next_stage_criteria": [
                "M2848 status and gate rows are accepted or rejected",
        ...
        "self_id_evidence_discipline": {
            ...
            "current_frame_substitution_risk": "M2849 audits continuation preflight only and does not run finite-window-vs-GRU or self-ID tests.",
            "history_necessity_tests": [
                "Hidden intervention rows from M2848 are diagnostic artifact rows only and cannot support a self-ID claim."
            ],
            "temporal_evidence_window": "M2848 continuation artifacts plus M2838 weak diagnostic accounting.",
            "negative_result_policy": "If M2848 failed, preserve the negative continuation evidence and route to repair design, freeze, or stop rather than weakening gates.",
            "allowed_claims": [
                "M2848 bounded continuation preflight accepted or rejected",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits new executable continuation-preflight artifacts from M2848",
            "paper_verdict_delta": "no paper verdict; audit governs Route A engineering training continuation",
            "must_synthesize_if": [
                "M2848 cannot preserve actor 72/action 3",
                "M2848 response targets require hidden/oracle fields",
                "M2848 produces only actor_mean.bias changes",
                "M2848 cannot write proof generalization promotion rows",
                "M2848 claims validation readiness driver performance paper current-sim high-fidelity or self-ID evidence",
            ],
        },
        "hypothesis": "A bounded result audit can accept or reject M2848 bounded continuation-preflight artifacts before any interpretation.",
        "success_criteria": [
            f"docs/{DEFAULT_NEXT_BLOCKER}.md exists",
            "audit checks M2848 summary checkpoint manifest parameter trace response target schema and gate rows",
        ...
        "failure_criteria": [
            "M2849 runs new training validation ranking promotion or success-rate verdict computation",
            "M2849 hides M2848 gate failures or weakens actor/claim boundaries",
            "M2849 claims repair success driver performance validation readiness/result high-fidelity validation paper finite-window-vs-GRU current-sim verdict full ideal driver completion or self-ID result",
        ],
        "decision_rule": "Pass only if M2849 audits M2848 artifacts under the unchanged actor and claim boundaries without new execution or overclaiming.",
...
def file_sha256(path: Path | str) -> str:
    return m2846_base.file_sha256(path)


def _float_or_none(value: Any) -> float | None:
    return m2846_base._float_or_none(value)


def _as_bool(value: Any) -> bool:
    return m2846_base._as_bool(value)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the M2848 bounded continuation preflight.")
    parser.add_argument("--m2847-audit", type=Path, default=DEFAULT_M2847_AUDIT)
    parser.add_argument("--m2846-summary", type=Path, default=DEFAULT_M2846_SUMMARY)
    ...
    parser.add_argument("--seed", type=int, default=284800)
    return parser.parse_args()


def main() -> None:
    args = _parse_args()
    summary = run_response_predictive_recurrent_belief_core_training_bounded_continuation_preflight(
        output_dir=args.output_dir,
        m2847_audit=args.m2847_audit,
        m2846_summary=args.m2846_summary,
