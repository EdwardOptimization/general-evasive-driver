"""Materialize Route A post-clearance non-same-repair offtrack localization.

This runner reanalyzes existing M2807/M2809 artifacts only. It writes
row-level failure localization, outcome bucket, offtrack containment, context,
guardrail, actor, claim, and gate artifacts. It does not execute environments,
policies, replay, validation, training, source builds, adapter probes,
ranking, promotion, or high-fidelity simulation.
"""
...
DEFAULT_MILESTONE = (
    "m2810-engineering-controller-route-a-post-clearance-negative-non-same-repair-"
    "cross-axis-offtrack-containment-localization-panel-materialization-preflight"
)
DEFAULT_NEXT_BLOCKER = (
    "m2811-engineering-controller-route-a-post-clearance-negative-non-same-repair-"
    "cross-axis-offtrack-containment-localization-panel-materialization-result-audit"
)
...
DEFAULT_M2809_SYNTHESIS = Path(
    "docs/m2809-engineering-controller-route-a-post-clearance-negative-non-same-repair-"
    "cross-axis-bounded-execution-result-synthesis.md"
)
DEFAULT_M2808_AUDIT = Path(
    "docs/m2808-engineering-controller-route-a-post-clearance-negative-non-same-repair-"
    "cross-axis-bounded-execution-result-audit.md"
)
...
DEFAULT_DOC_PATH = Path(
    "docs/m2810-engineering-controller-route-a-post-clearance-negative-non-same-repair-"
    "cross-axis-offtrack-containment-localization-panel-materialization-preflight.md"
)
DEFAULT_FOLLOW_UP_MANIFEST = Path(
    "experiments/manifests/m2811-engineering-controller-route-a-post-clearance-negative-"
    "non-same-repair-cross-axis-offtrack-containment-localization-panel-materialization-result-audit.json"
)

CLAIM_SCOPE = (
    "M2810 Route A post-clearance negative non-same-repair offtrack-containment "
    "localization panel materialization only; existing M2807/M2809 artifacts are "
    "reanalyzed into row-level diagnostic context while no reset, step, rollout, replay, "
...
OFFTRACK_CONTAINMENT_FIELDNAMES = [
    "containment_id",
    "localization_id",
    "candidate_id",
    "task_source_id",
...
CLAIM_CHECKS = (
    ("failure_localization_panel_materialized", True, True, "M2810 localization artifacts"),
    ("outcome_bucket_context_materialized", True, True, "M2810 outcome-bucket rows"),
    ("offtrack_containment_context_materialized", True, True, "M2810 offtrack-containment rows"),
    ("stress_axis_context_materialized", True, True, "M2810 stress-axis context rows"),
    ("source_edge_context_materialized", True, True, "M2810 source-edge context rows"),
    ("guardrail_context_preserved", True, True, "M2810 guardrail context rows"),
    ("actor_contract_preserved", True, True, "M2810 actor guard rows"),
    ("follow_up_result_audit_registered", True, True, "M2811 result-audit manifest"),
...
def materialize_post_clearance_negative_non_same_repair_offtrack_containment_localization_panel(
    output_dir: Path | str,
    *,
    m2807_dir: Path | str = DEFAULT_M2807_DIR,
    m2809_synthesis: Path | str = DEFAULT_M2809_SYNTHESIS,
    m2808_audit: Path | str = DEFAULT_M2808_AUDIT,
    ...
    source = load_source_artifacts(
        Path(m2807_dir),
        m2809_synthesis=Path(m2809_synthesis),
        m2808_audit=Path(m2808_audit),
    ...
    offtrack_rows = build_offtrack_containment_rows(localization_rows)
...
def artifact_paths(output_dir: Path, doc_path: Path) -> dict[str, Path]:
    return {
        ...
        "offtrack_containment_rows": output_dir / "offtrack_containment_rows.csv",
...
def write_outputs(
    ...
    write_csv_rows(paths["offtrack_containment_rows"], offtrack_rows, fieldnames=OFFTRACK_CONTAINMENT_FIELDNAMES)
...
def load_source_artifacts(
    m2807_dir: Path,
    *,
    m2809_synthesis: Path,
    m2808_audit: Path,
    route_plan: Path,
    follow_up_manifest: Path,
) -> dict[str, Any]:
    paths = {
        "m2809_synthesis": m2809_synthesis,
        "m2808_audit": m2808_audit,
        "m2807_summary": m2807_dir / "summary.json",
        "m2807_non_same_repair_candidate_rows": m2807_dir / "non_same_repair_candidate_rows.csv",
        "m2807_candidate_execution_rows": m2807_dir / "candidate_execution_rows.csv",
        "m2807_stress_axis_aggregate_rows": m2807_dir / "stress_axis_aggregate_rows.csv",
        "m2807_prior_panel_exclusion_rows": m2807_dir / "prior_surface_exclusion_rows.csv",
        "m2807_blocker_guard_rows": m2807_dir / "blocker_guard_rows.csv",
        "m2807_actor_contract_guard_rows": m2807_dir / "actor_contract_guard_rows.csv",
        "m2807_claim_boundary_rows": m2807_dir / "claim_boundary_rows.csv",
    ...
    return {
        ...
        "m2807_summary": read_json(paths["m2807_summary"]),
        "m2807_non_same_repair_candidate_rows": _read_csv_rows(paths["m2807_non_same_repair_candidate_rows"]),
        "m2807_candidate_execution_rows": _read_csv_rows(paths["m2807_candidate_execution_rows"]),
        "m2807_stress_axis_aggregate_rows": _read_csv_rows(paths["m2807_stress_axis_aggregate_rows"]),
        "m2807_prior_panel_exclusion_rows": _read_csv_rows(paths["m2807_prior_panel_exclusion_rows"]),
        "m2807_blocker_guard_rows": _read_csv_rows(paths["m2807_blocker_guard_rows"]),
        "m2807_actor_contract_guard_rows": _read_csv_rows(paths["m2807_actor_contract_guard_rows"]),
        "m2807_claim_boundary_rows": _read_csv_rows(paths["m2807_claim_boundary_rows"]),
...
def build_failure_localization_rows(source: dict[str, Any]) -> list[dict[str, Any]]:
    candidates = {row["candidate_id"]: row for row in source["m2807_non_same_repair_candidate_rows"]}
    rows = []
    for idx, row in enumerate(source["m2807_candidate_execution_rows"], start=1):
        candidate = candidates.get(row.get("candidate_id", ""), {})
        margin = _float(row.get("min_clearance_margin"))
        rows.append(
            {
                "localization_id": f"m2810-localization-{idx:04d}",
                ...
                "recoverability_window_success": _bool(row.get("recoverability_window_success")),
                "recoverability_window_success_available": _bool(row.get("recoverability_window_success_available")),
...
def build_outcome_bucket_rows(localization_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for idx, (failure_family, group) in enumerate(sorted(grouped.items()), start=1):
        ...
        rows.append(
            {
                "bucket_id": f"m2810-outcome-bucket-{idx:04d}",
...
def build_offtrack_containment_rows(localization_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = []
    offtrack_rows = [row for row in localization_rows if row["failure_family"] == "offtrack_positive_clearance"]
    for idx, row in enumerate(offtrack_rows, start=1):
        severity = _float(row.get("off_track_severity_proxy"))
        overshoot = _float(row.get("max_off_track_overshoot"))
        time_to_offtrack = _float(row.get("time_to_first_off_track_s"))
        rows.append(
            {
                "containment_id": f"m2810-offtrack-containment-{idx:04d}",
                "localization_id": row["localization_id"],
                "candidate_id": row["candidate_id"],
                "task_source_id": row["task_source_id"],
                "task_family": row["task_family"],
                "source_edge": row["source_edge"],
                "stress_axis_primary": row["stress_axis_primary"],
                "min_clearance_margin": row["min_clearance_margin"],
                "time_to_first_off_track_s": time_to_offtrack,
                "off_track_severity_proxy": severity,
                "max_off_track_overshoot": overshoot,
                "recoverability_window_success": _bool(row.get("recoverability_window_success")),
                "recoverability_window_success_available": _bool(row.get("recoverability_window_success_available")),
                "success": False,
                "collision": False,
                "offtrack_noncollision": True,
                "containment_family": classify_containment_family(severity, overshoot, time_to_offtrack),
                "diagnostic_only_no_verdict": True,
                "ranking_claim_made": False,
                "actor_visible_allowed": False,
                "claim_scope": CLAIM_SCOPE,
...
def classify_containment_family(
    severity: float | None,
    overshoot: float | None,
    time_to_offtrack: float | None,
) -> str:
    if severity is None and overshoot is None and time_to_offtrack is None:
        return "offtrack_containment_unmeasured"
    if severity is not None and severity <= 0.0 and (overshoot is None or overshoot <= 0.0):
        return "offtrack_containment_boundary_touch"
    if time_to_offtrack is not None and time_to_offtrack <= 1.5:
        return "early_offtrack_containment_failure"
    return "positive_clearance_offtrack_containment_failure"


def build_stress_axis_context_rows(source: dict[str, Any]) -> list[dict[str, Any]]:
    rows = []
    for idx, row in enumerate(source["m2807_stress_axis_aggregate_rows"], start=1):
        rows.append(
            {
                "context_id": f"m2810-stress-axis-context-{idx:04d}",
...
def build_source_edge_context_rows(localization_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for idx, (source_edge, group) in enumerate(sorted(grouped.items()), start=1):
        ...
        rows.append(
            {
                "context_id": f"m2810-source-edge-context-{idx:04d}",
...
def build_guardrail_context_rows(source: dict[str, Any]) -> list[dict[str, Any]]:
    rows = []
    for idx, row in enumerate(source["m2807_prior_panel_exclusion_rows"], start=1):
        rows.append(
            {
                "guardrail_context_id": f"m2810-guardrail-prior-panel-{idx:04d}",
    ...
    for idx, row in enumerate(source["m2807_blocker_guard_rows"], start=1):
        rows.append(
            {
                "guardrail_context_id": f"m2810-guardrail-blocker-{idx:04d}",
...
def build_actor_contract_guard_rows(
    ...
    return [
        actor_guard("m2810-actor-guard-observation-shape", "p0_observation_dim", P0_OBSERVATION_DIM, 72),
        actor_guard("m2810-actor-guard-action-shape", "action_dim", ACTION_DIM, 3),
        actor_guard("m2810-actor-guard-m2807-rows-pass", "m2807_actor_guard_rows_pass", m2807_actor_rows_pass(source), True),
        actor_guard(
            "m2810-actor-guard-hidden-oracle",
            "hidden_oracle_actor_input_required",
            any(_bool(row["hidden_oracle_actor_input_required"]) for row in localization_rows),
            False,
        ),
        actor_guard(
            "m2810-actor-guard-actor-contract-changed",
            "actor_input_contract_changed",
            any(_bool(row.get("actor_input_contract_changed")) for row in source["m2807_candidate_execution_rows"]),
            False,
        ),
        actor_guard("m2810-actor-guard-localization-labels", "localization_labels_actor_visible", False, False),
        actor_guard(
            "m2810-actor-guard-stress-axis-labels",
            "stress_axis_labels_actor_visible",
            any(_bool(row["stress_axis_labels_actor_visible"]) for row in localization_rows),
            False,
        ),
        actor_guard("m2810-actor-guard-source-edge-labels", "source_edge_labels_actor_visible", False, False),
        actor_guard(
            "m2810-actor-guard-success-progress-labels",
            "success_progress_labels_actor_visible",
            any(_bool(row["success_progress_labels_actor_visible"]) for row in localization_rows),
            False,
        ),
        actor_guard(
            "m2810-actor-guard-verdict-labels",
            "verdict_labels_actor_visible",
            any(_bool(row["verdict_labels_actor_visible"]) for row in localization_rows),
            False,
        ),
        actor_guard(
            "m2810-actor-guard-protected-denominator",
            "protected_rows_in_success_denominator",
            any(_bool(row["protected_rows_in_success_denominator"]) for row in guardrail_rows),
            False,
        ),
        actor_guard(
            "m2810-actor-guard-guardrail-actor-visible",
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "claim_id": f"m2810-claim-{claim_family}",
            "claim_family": claim_family,
            "allowed_in_m2810": allowed,
...
def build_gate_matrix_rows(
    ...
    *,
    ...
    return [
        gate_row("m2810-gate-source-artifacts-present", "artifact", all(source["source_exists"].values()), True),
        gate_row("m2810-gate-required-artifacts-present", "artifact", required_artifacts_present, True),
        gate_row("m2810-gate-m2807-status-pass", "lineage", _bool(source["m2807_summary"].get("status_pass")), True),
        gate_row("m2810-gate-m2807-gate-matrix-pass", "lineage", m2807_gate_rows_pass(source), True),
        gate_row("m2810-gate-execution-row-count", "diagnostic_accounting", len(localization_rows), 12),
        gate_row("m2810-gate-diagnostic-success-count", "diagnostic_accounting", counts["success_count"], 2),
        gate_row("m2810-gate-diagnostic-collision-count", "diagnostic_accounting", counts["collision_count"], 0),
        gate_row("m2810-gate-offtrack-positive-clearance-count", "diagnostic_accounting", counts["offtrack_positive_clearance_count"], 10),
        gate_row("m2810-gate-outcome-bucket-rows", "artifact", len(outcome_rows), 2),
        gate_row("m2810-gate-offtrack-containment-rows", "artifact", len(offtrack_rows), 10),
        gate_row("m2810-gate-stress-axis-context-rows", "artifact", len(stress_rows), 4),
        gate_row("m2810-gate-source-edge-context-rows", "artifact", len(source_edge_rows) > 0, True),
        gate_row("m2810-gate-prior-panel-guardrail-rows", "guardrail", prior_guard_count, 37),
        gate_row("m2810-gate-blocker-guardrail-rows", "guardrail", blocker_guard_count, 7),
        gate_row("m2810-gate-guardrail-context-rows", "guardrail", len(guardrail_rows), 44),
        gate_row("m2810-gate-guardrails-not-executed", "guardrail", guardrails_executed, False),
        gate_row("m2810-gate-guardrails-outside-denominator", "guardrail", guardrails_in_denominator, False),
        gate_row("m2810-gate-guardrail-labels-actor-invisible", "actor_contract", actor_visible, False),
        gate_row("m2810-gate-actor-contract-72-action-3", "actor_contract", actor_contract_preserved(source), True),
        gate_row("m2810-gate-hidden-oracle-actor-input-absent", "actor_contract", hidden_oracle_actor_input_detected(source), False),
        gate_row("m2810-gate-actor-guard-rows-pass", "actor_contract", all(_bool(row["status_pass"]) for row in actor_rows), True),
        gate_row("m2810-gate-follow-up-result-audit-registered", "process", source["source_exists"]["follow_up_manifest"], True),
        gate_row("m2810-gate-claim-boundary-pass", "claim_boundary", claim_boundary_pass(claim_rows), True),
        gate_row("m2810-gate-no-reset-rollout-training-validation", "claim_boundary", False, False),
        gate_row("m2810-gate-no-ranking-promotion-performance", "claim_boundary", False, False),
    ]
...
def build_summary(
    *,
    ...
    return {
        "protocol_version": "engineering_controller_route_a_post_clearance_non_same_repair_offtrack_localization_v0",
        ...
        "offtrack_containment_rows": str(paths["offtrack_containment_rows"]),
        ...
        "offtrack_containment_row_count": len(offtrack_rows),
        ...
        "success_obstacle_pass_count": counts["success_obstacle_pass_count"],
        ...
        "selected_next_action": "m2811_post_clearance_negative_non_same_repair_offtrack_containment_localization_result_audit",
        "claim_scope": CLAIM_SCOPE,
        "forbidden_interpretation": FORBIDDEN_INTERPRETATION,
...
def render_milestone_doc(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- offtrack containment rows: `{summary['offtrack_containment_rows']}`",
            ...
            f"- success obstacle-pass rows: {summary['success_obstacle_pass_count']}",
            ...
            f"- offtrack containment rows: {summary['offtrack_containment_row_count']}",
            ...
            f"- prior-surface guardrail rows: {summary['prior_panel_guardrail_row_count']}",
...
def count_localization_outcomes(rows: list[dict[str, Any]]) -> dict[str, int]:
    return {
        ...
        "success_obstacle_pass_count": sum(1 for row in rows if row.get("failure_family") == "success_obstacle_pass"),
        "collision_negative_clearance_count": sum(
...
def actor_contract_preserved(source: dict[str, Any]) -> bool:
    return (
        ...
        and ACTION_DIM == 3
        and m2807_actor_rows_pass(source)
...
def m2807_actor_rows_pass(source: dict[str, Any]) -> bool:
    return bool(source["m2807_actor_contract_guard_rows"]) and all(
        _bool(row.get("status_pass")) for row in source["m2807_actor_contract_guard_rows"]
    )
...
def hidden_oracle_actor_input_detected(source: dict[str, Any]) -> bool:
    summary_hidden = any(
        _bool(source["m2807_summary"].get(key))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Materialize Route A post-clearance non-same-repair offtrack-containment localization panel."
    )
    parser.add_argument("--m2807-dir", type=Path, default=DEFAULT_M2807_DIR)
    parser.add_argument("--m2809-synthesis", type=Path, default=DEFAULT_M2809_SYNTHESIS)
    parser.add_argument("--m2808-audit", type=Path, default=DEFAULT_M2808_AUDIT)
    ...
    summary = materialize_post_clearance_negative_non_same_repair_offtrack_containment_localization_panel(
        args.output_dir,
        m2807_dir=args.m2807_dir,
        m2809_synthesis=args.m2809_synthesis,
        m2808_audit=args.m2808_audit,
