"""Materialize M2248 offtrack/recovery/corridor reward-extension configs."""
...
DEFAULT_OUTPUT_DIR = Path("runs/m2248_paper_route_current_sim_offtrack_recovery_corridor_reward_extension_materialization")
DEFAULT_TRAINING_OUTPUT_ROOT = Path("runs/m2249_paper_route_current_sim_offtrack_recovery_corridor_training_execution")
DEFAULT_NEXT_BLOCKER = "m2249-paper-route-current-sim-offtrack-recovery-corridor-training-execution-design"
...
def build_repaired_config(
    *,
    ...
    if repaired_track_width != original_track_width:
        raise ValueError("M2248 must not widen track_width as the primary repair")
    build_env_config(env)

    profile = dict(config["controller_profile"])
    profile.update(
        {
            "offtrack_recovery_corridor_reward_extension_config": True,
            "matched_budget_stage": "offtrack_recovery_corridor_reward_v1",
    ...
    config["offtrack_recovery_corridor_repair_protocol"] = {
        "stage": "offtrack_recovery_corridor_reward_v1",
...
def _claim_rows() -> list[dict[str, Any]]:
    return [
        {
            "claim": "offtrack_reward_extension_configs_materialized",
            "admissible": True,
            "reason": "M2248 writes default-preserving reward hooks and config artifacts only",
        },
        {"claim": "training_completed", "admissible": False, "reason": "M2248 does not execute training commands"},
        {"claim": "controller_family_ranking", "admissible": False, "reason": "no repaired checkpoints exist yet"},
        {"claim": "finite_window_vs_gru_conclusion", "admissible": False, "reason": "config materialization is not outcome evidence"},
        {"claim": "level3_self_identification", "admissible": False, "reason": "M2248 runs no history intervention"},
    ]


def materialize_reward_extension_configs(
    *,
    ...
    for profile_name in EXPECTED_PROFILES:
        for seed_id in EXPECTED_SEED_IDS:
            ...
            if not source_path_value.exists():
                ...
                continue
            config = build_repaired_config(profile_name=profile_name, seed_id=seed_id, source_config_root=source_root)
    ...
    result_class = (
        "current_sim_offtrack_recovery_corridor_reward_extension_materialization_pass"
        if (
        ...
        else "current_sim_offtrack_recovery_corridor_reward_extension_materialization_fail"
    )
    summary = {
    ...
    write_run_state(
        ...
        {
            "task_id": "m2248-paper-route-current-sim-offtrack-recovery-corridor-reward-extension-materialization",
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = materialize_reward_extension_configs(
