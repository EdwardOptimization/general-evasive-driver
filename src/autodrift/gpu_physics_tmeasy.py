"""EXACT-TMeasy branchless GPU physics vehicle-dynamics model for AutoDrift (faithful rewrite).

This is the FAITHFUL-rewrite sibling of ``autodrift.gpu_physics``. Where ``gpu_physics``
uses a Magic-Formula (Pacejka) tyre *calibrated* to TMeasy peaks with fudge factors
(``front_grip_scale``/``rear_grip_scale``, ``pac_Dy``/``pac_By`` tuned down) — the tell of a
calibrated approximation — this module replaces the tyre with Chrono's **EXACT TMeasy force
curves**, sampled directly off the running Chrono Sedan TMeasy tyre by
...
The tyre is now a branchless, batched ``torch`` bilinear interpolation of the sampled
``Fx(kappa, Fz)`` and ``Fy(alpha, Fz)`` surfaces (which carry TMeasy's strong degressive
load dependence exactly), combined via the same slip-vector projection the calibrated model
uses (mirroring Chrono's ``tmxy_combined`` + ``CombinedCoulombForces`` friction ellipse).
Terrain friction enters as ``muscale = mu / mu0`` (mu0 = 0.8), exactly as Chrono scales the
TMeasy force caps. There are NO grip fudge factors: ``front_grip_scale = rear_grip_scale =
1.0`` and no ``pac_*`` calibration — the curve IS the Chrono curve.
