"""Run M3067 offtrack-dominant behavior direct-action closed-loop measurement preflight."""
...
MILESTONE_ID = (
    "m3067-engineering-controller-active-safety-driver-v1-offtrack-dominant-behavior-direct-action-"
    "closed-loop-measurement-preflight"
)
NEXT_ID = (
    "m3068-engineering-controller-active-safety-driver-v1-offtrack-dominant-behavior-direct-action-"
    "closed-loop-measurement-result-audit"
)
M3066_ID = (
    "m3066-engineering-controller-active-safety-driver-v1-offtrack-dominant-behavior-bounded-direct-action-"
    "fitting-result-audit"
)
DEFAULT_M3066_AUDIT = Path(f"docs/{M3066_ID}.md")
...
DEFAULT_OUTPUT_DIR = Path(
    "runs/m3067_engineering_controller_active_safety_driver_v1_offtrack_dominant_behavior_direct_action_"
    "closed_loop_measurement_preflight"
)
...
CLAIM_SCOPE = (
    "M3067 Active Safety Driver v1 offtrack-dominant behavior direct-action closed-loop "
    "measurement preflight only; same-denominator current-sim measurement rows and metric "
    "tables may be recorded for the M3065 direct-action candidate. The candidate is executed "
...
class DirectActionActorPolicy:
    """Direct obs72-to-action3 actor for the M3065 fitted reflex artifact."""
...
def load_sources(*, m3066_audit: Path, m3065_dir: Path, m3039_dir: Path, m3037_dir: Path, m3012_dir: Path) -> dict[str, Any]:
    paths = {
        "m3066_audit": m3066_audit,
        "m3065_summary": m3065_dir / "summary.json",
        "m3065_candidate": m3065_dir / "candidate_direct_action_reflex_layer.npz",
        "m3065_gate_matrix": m3065_dir / "gate_matrix.csv",
        "m3065_actor_input_exclusion_rows": m3065_dir / "actor_input_exclusion_rows.csv",
        "m3065_target_quality_boundary_rows": m3065_dir / "target_quality_boundary_rows.csv",
        "m3065_side_effect_rows": m3065_dir / "checkpoint_side_effect_guard_rows.csv",
        "m3065_claim_rows": m3065_dir / "claim_boundary_rows.csv",
    ...
    return {
        ...
        "m3066_audit_text": paths["m3066_audit"].read_text(encoding="utf-8") if exists["m3066_audit"] else "",
        "m3065_summary": read_json(paths["m3065_summary"]) if exists["m3065_summary"] else {},
        "m3065_gate_rows": read_csv_rows(paths["m3065_gate_matrix"]),
        "m3065_actor_input_exclusion_rows": read_csv_rows(paths["m3065_actor_input_exclusion_rows"]),
        "m3065_target_quality_boundary_rows": read_csv_rows(paths["m3065_target_quality_boundary_rows"]),
        "m3065_side_effect_rows": read_csv_rows(paths["m3065_side_effect_rows"]),
        "m3065_claim_rows": read_csv_rows(paths["m3065_claim_rows"]),
...
def workload_plan(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, workload in enumerate(
        ...
        rows.append(
            {
                **dict(workload),
                "measurement_episode_id": f"m3067-measurement-episode-{index:04d}",
                ...
                "direct_action_profile_name": f"{profile_name}+m3065_direct_action",
    ...
    for plan in plan_rows:
        try:
            ...
            if not _bool(direct_action.get("contract_pass", False)):
                raise ValueError("M3065 direct-action artifact failed adapter contract")
            ...
            try:
                ...
                row = run_episode_with_policy(env, policy, "m3065_direct_action", int(plan["eval_seed"]))
            finally:
                env.close()
...
def metric_summary_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for group, group_rows in sorted(groups.items()):
        summaries.append(
            {
                "metric_summary_id": f"m3067-metric-summary-{len(summaries) + 1:04d}",
                "group": group,
...
def claim(claim_id: str, family: str, allowed: bool, made: bool, evidence: str) -> dict[str, Any]:
    return {
        "claim_id": f"m3067-{claim_id}",
        "claim_family": family,
        "allowed_in_m3067": allowed,
        "claim_made": made,
...
def claim_boundary_rows(*, measurement_rows_present: bool, artifacts_present: bool, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3068 audit manifest"),
    ]
    blocked = [
        ...
        ("runtime_base_policy_dependency", "contract", "M3065 direct-action adapter forbids runtime base policy use"),
    ]
...
def gate_matrix_rows(
    *,
    ...
    audit_accepts = (
        "accept_m3065_direct_action_fit_route_to_m3067_closed_loop_measurement_preflight"
        in source["m3066_audit_text"]
    )
    ...
    return [
        ...
        gate("m3066_accepts_m3067_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        gate(
            "m3065_status_pass",
            "lineage",
            _bool(source["m3065_summary"].get("status_pass", False)),
            source["m3065_summary"].get("status_pass"),
            True,
            "lineage_invalid",
        ),
        gate(
            "m3065_gate_matrix_pass",
            "lineage",
            _bool(source["m3065_summary"].get("gate_matrix_pass", False)),
            source["m3065_summary"].get("gate_matrix_pass"),
            True,
            "lineage_invalid",
        ),
        gate(
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path, summary_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": (
            "A bounded result-audit synthesis can accept or reject the M3067 direct-action Active Safety Driver v1 "
        ...
        "lineage": {
            ...
            "derived_from": [MILESTONE_ID, M3066_ID],
            "blocked_by": [
                "M3067 measurement rows require audit before any performance or continuation decision",
                "current-sim measurement rows are not validation or promotion evidence before M3068",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_increment": "audits and synthesizes same-denominator closed-loop measurement rows for the M3065 direct-action candidate",
            ...
            "stop_condition": [
                "stop if M3067 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action adapter contracts were violated",
                "stop if M3067 rows are treated as validation or performance verdicts",
            ],
            "fallback_plan": [
            ...
            "synthesis_trigger": "M3067 completes direct-action closed-loop measurement preflight",
        ...
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit and synthesize M3067 direct-action closed-loop measurement preflight artifacts",
            "admission_evidence": [
                "M3067 summary and gate matrix",
                "M3067 measurement episode metric direct-action adapter actor contract side-effect and claim artifacts",
            ],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3068 status queue scoreboard research log and review",
                "one follow-up manifest only if M3068 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3068 audit accepts or rejects M3067 as complete and claim-safe",
                "next route or stop state is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3068 audits engineering measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": [
                "None in M3068; finite-window and GRU comparison remains a later same-case engineering ablation."
            ],
            "temporal_evidence_window": "M3067 closed-loop measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3067 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3068 audits engineering measurement evidence",
            "must_synthesize_if": [
                "M3068 cannot accept M3067 as complete and claim-safe",
                "M3068 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict or self-ID evidence",
                "M3068 would continue with another materialization-only route before interpreting measurement",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": (
            "Pass only if M3068 audits M3067 artifacts and selects one next route or stop state while preserving "
...
def run_closed_loop_measurement_preflight(
    *,
    ...
    direct_action = load_direct_action_artifact(source["paths"]["m3065_candidate"])
    ...
    summary = {
        ...
        "result_class": (
            "active_safety_driver_v1_offtrack_behavior_direct_action_closed_loop_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_v1_offtrack_behavior_direct_action_closed_loop_measurement_preflight_fail"
        ),
        "status_pass": status_pass,
        ...
        "required_artifacts_present": present,
        "m3065_status_pass": _bool(source["m3065_summary"].get("status_pass", False)),
        "m3065_gate_matrix_pass": _bool(source["m3065_summary"].get("gate_matrix_pass", False)),
        "candidate_direct_action_reflex_layer": str(source["paths"]["m3065_candidate"]),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3066-audit", type=Path, default=DEFAULT_M3066_AUDIT)
