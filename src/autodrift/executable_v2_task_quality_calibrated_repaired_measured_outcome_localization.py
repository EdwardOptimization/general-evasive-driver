"""No-rerun outcome localization for the calibrated repaired measured panel."""
...
DEFAULT_SUMMARY = Path("runs/m1975_executable_v2_task_quality_calibrated_measured_execution_repaired/summary.json")
DEFAULT_EPISODE_ROWS = Path("runs/m1975_executable_v2_task_quality_calibrated_measured_execution_repaired/episode_rows.csv")
DEFAULT_OUTPUT_DIR = Path("runs/m1977_executable_v2_task_quality_calibrated_repaired_measured_outcome_localization")
DEFAULT_NEXT_BLOCKER = "m1978-executable-v2-task-quality-calibrated-repaired-measured-outcome-localization-result-audit"
TARGET_EPISODE_COUNT = 960
TARGET_PROFILE_COUNT = 12
TARGET_SOURCE_KIND_COUNT = 4
TARGET_ROLE_COUNT = 4
TARGET_NORMALIZED_SURFACE_COUNT = 3
TARGET_SAMPLED_LABEL_COUNT = 4
...
AGGREGATE_SPECS: dict[str, tuple[str, ...]] = {
    ...
    "outcome_by_repair_source_kind": ("repair_source_kind",),
    "outcome_by_selection_quota": ("selection_quota_name",),
    "outcome_by_role": ("source_role_semantics",),
    "outcome_by_parent_tier": ("parent_feasibility_tier_id",),
    "outcome_by_parent_surface": ("parent_surface_variant",),
    "outcome_by_normalized_surface": ("normalized_surface_variant",),
    "outcome_by_sampled_label": ("sampled_obstacle_label",),
    "outcome_by_base_geometry": ("base_geometry_source",),
    "outcome_by_profile_source_kind": ("profile_name", "repair_source_kind"),
    "outcome_by_profile_parent_tier": ("profile_name", "parent_feasibility_tier_id"),
    "outcome_by_profile_normalized_surface": ("profile_name", "normalized_surface_variant"),
    "outcome_by_source_kind_role_surface": (
        "repair_source_kind",
        "source_role_semantics",
        "normalized_surface_variant",
    ),
...
def success_source_rows(rows: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return [
        {field: row.get(field, "") for field in SUCCESS_SOURCE_FIELDNAMES}
        for row in rows
        ...
        output.append(
            {
                ...
                "repair_source_kind": scenario_values[0],
                "source_role_semantics": scenario_values[1],
                "parent_feasibility_tier_id": scenario_values[2],
                "normalized_surface_variant": scenario_values[3],
                "sampled_obstacle_label": scenario_values[4],
...
def comparison_support_candidates(rows: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for group_values, group_rows in _group_rows(rows, scenario_keys).items():
        ...
        output.append(
            {
                "repair_source_kind": group_values[0],
                "source_role_semantics": group_values[1],
                "parent_feasibility_tier_id": group_values[2],
                "normalized_surface_variant": group_values[3],
                "sampled_obstacle_label": group_values[4],
                "episode_count": episode_count,
                "success_count": success_count,
                "collision_count": collision_count,
...
def claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "claim": "no_rerun_calibrated_repaired_outcome_localization_completed",
            "admissible": True,
            "reason": "M1977 reads existing M1975 artifacts and writes diagnostic aggregates only",
        },
        {
        ...
        },
        {
            ...
            "admissible": False,
            "reason": "profile patterns require a later controlled comparison",
        },
        {
...
def localize_calibrated_repaired_outcomes(
    *,
    ...
    target_source_kind_count: int = TARGET_SOURCE_KIND_COUNT,
    target_role_count: int = TARGET_ROLE_COUNT,
    target_normalized_surface_count: int = TARGET_NORMALIZED_SURFACE_COUNT,
    target_sampled_label_count: int = TARGET_SAMPLED_LABEL_COUNT,
    ...
    write_csv_rows(output / "comparison_support_candidates.csv", comparison_rows, fieldnames=COMPARISON_CANDIDATE_FIELDNAMES)
    ...
    source_kind_count = len({str(row.get("repair_source_kind", "")) for row in episode_rows})
    role_count = len({str(row.get("source_role_semantics", "")) for row in episode_rows})
    normalized_surface_count = len({str(row.get("normalized_surface_variant", "")) for row in episode_rows})
    sampled_label_count = len({str(row.get("sampled_obstacle_label", "")) for row in episode_rows})
    ...
    result_passes = (
        source_summary.get("result_class") == "task_quality_calibrated_measured_execution_pass"
        and len(episode_rows) == int(target_episode_count)
        and profile_count == int(target_profile_count)
        and source_kind_count == int(target_source_kind_count)
        and role_count == int(target_role_count)
        and normalized_surface_count == int(target_normalized_surface_count)
        and sampled_label_count == int(target_sampled_label_count)
    ...
    summary = {
        "result_class": (
            "task_quality_calibrated_repaired_measured_outcome_localization_pass"
            if result_passes
            else "task_quality_calibrated_repaired_measured_outcome_localization_incomplete_or_fail"
        ),
        ...
        "target_role_count": int(target_role_count),
        "normalized_surface_count": normalized_surface_count,
        "target_normalized_surface_count": int(target_normalized_surface_count),
        ...
        "repair_source_kind_counts": _count_by(episode_rows, "repair_source_kind"),
        "role_counts": _count_by(episode_rows, "source_role_semantics"),
        "parent_tier_counts": _count_by(episode_rows, "parent_feasibility_tier_id"),
        "parent_surface_counts": _count_by(episode_rows, "parent_surface_variant"),
        "normalized_surface_counts": _count_by(episode_rows, "normalized_surface_variant"),
        "sampled_label_counts": _count_by(episode_rows, "sampled_obstacle_label"),
        "base_geometry_source_counts": _count_by(episode_rows, "base_geometry_source"),
...
def main() -> None:
    ...
    parser.add_argument("--target-source-kind-count", type=int, default=TARGET_SOURCE_KIND_COUNT)
    parser.add_argument("--target-role-count", type=int, default=TARGET_ROLE_COUNT)
    parser.add_argument("--target-normalized-surface-count", type=int, default=TARGET_NORMALIZED_SURFACE_COUNT)
    parser.add_argument("--target-sampled-label-count", type=int, default=TARGET_SAMPLED_LABEL_COUNT)
    parser.add_argument("--next-blocker", default=DEFAULT_NEXT_BLOCKER)
    args = parser.parse_args()
    summary = localize_calibrated_repaired_outcomes(
        ...
        target_source_kind_count=int(args.target_source_kind_count),
        target_role_count=int(args.target_role_count),
        target_normalized_surface_count=int(args.target_normalized_surface_count),
        target_sampled_label_count=int(args.target_sampled_label_count),
