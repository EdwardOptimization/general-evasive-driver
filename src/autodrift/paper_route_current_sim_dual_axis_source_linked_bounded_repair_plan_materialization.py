"""Artifact-only source-linked bounded repair-plan materialization for M2417 targets."""
...
from autodrift import (
    paper_route_current_sim_dual_axis_source_linked_offtrack_containment_actionable_target_consolidation as source_tables,
)
...
DEFAULT_TARGET_CONSOLIDATED_ROW_COUNT = 2844
DEFAULT_NEXT_BLOCKER = (
    "m2421-paper-route-current-sim-dual-axis-source-linked-bounded-repair-plan-materialization-result-audit"
)
RESULT_PASS = "current_sim_dual_axis_source_linked_bounded_repair_plan_materialization_pass"
RESULT_FAIL = "current_sim_dual_axis_source_linked_bounded_repair_plan_materialization_incomplete_or_fail"
...
PLAN_FIELDNAMES = [
    ...
    "family_membership_diagnostic_monitoring",
    ...
    "source_linked_repair_plan_materialization",
    "source_linked_family_ranking_claim_made",
...
def _is_family_membership(row: Mapping[str, Any]) -> bool:
    return (
        str(row.get("source_table", "")) == "episode_family_membership_rows"
        or str(row.get("source_consolidated_route", "")) == "source_linked_family_diagnostic_guardrail"
        or str(row.get("consolidated_route", "")) == "source_linked_family_diagnostic_guardrail"
...
def _plan_route(row: Mapping[str, Any]) -> str:
    ...
    if route == "source_linked_family_diagnostic_guardrail" or _is_family_membership(row):
        return "family_membership_diagnostic_monitoring"
    ...
    if route == "max_step_noncompletion_target":
        return "max_step_noncompletion_guardrail"
    if route == "speed_too_low_target":
        return "speed_too_low_guardrail"
...
def _lever_family(row: Mapping[str, Any], *, plan_route: str) -> str:
    ...
    if plan_route == "family_membership_diagnostic_monitoring":
    ...
    if plan_route == "max_step_noncompletion_guardrail":
        return "noncompletion_horizon_guardrail"
    if plan_route == "speed_too_low_guardrail":
        return "low_speed_progress_guardrail"
    ...
    if actionability == "outcome_failure_surface":
        return "outcome_failure_surface_containment"
...
def _candidate_levers(row: Mapping[str, Any], *, plan_route: str, lever_family: str) -> str:
    if plan_route in {"diagnostic_monitoring_only", "family_membership_diagnostic_monitoring"}:
    ...
    if plan_route == "max_step_noncompletion_guardrail":
        return _join(
            [
                "completion-horizon and timeout guardrail",
                "terminal progress audit",
                "do not hide offtrack by timing out",
            ]
        )
    if plan_route == "speed_too_low_guardrail":
        return _join(
            [
                "low-speed progress guardrail",
                "minimum useful maneuver-speed audit",
                "do not solve offtrack by stopping outside task intent",
            ]
        )
    ...
    if lever_family == "outcome_failure_surface_containment":
        return _join(
            [
                "terminal road-departure failure-surface audit",
                "measured noncollision offtrack reduction gate",
...
def _acceptance_gates(row: Mapping[str, Any], *, plan_route: str) -> str:
    if plan_route in {"diagnostic_monitoring_only", "family_membership_diagnostic_monitoring"}:
        return "diagnostic row remains non-ranking; no winner selection; no family or profile tuning"
    if plan_route == "r4_mitigation_semantics_guardrail":
    ...
    if plan_route == "max_step_noncompletion_guardrail":
        return "max-step noncompletion does not increase; timeout is not used to mask offtrack or collision"
    if plan_route == "speed_too_low_guardrail":
        return "speed-too-low rate does not increase; low-speed stopping is not used as a fake avoidance success"
    gates = [
        ...
        "max-step and speed-too-low guardrails do not regress",
...
def _stop_rules(row: Mapping[str, Any], *, plan_route: str) -> str:
    common = [
        "stop if repair requires source-linked family/profile ranking",
    ...
    if plan_route == "max_step_noncompletion_guardrail":
        return _join(["stop if timeouts increase or hide terminal failures", *common])
    if plan_route == "speed_too_low_guardrail":
        return _join(["stop if low-speed stopping replaces evasive behavior", *common])
    return _join(["stop if offtrack improves only by increasing collision, degrading R4, timing out, or stopping", *common])


def _non_regression_guardrails(row: Mapping[str, Any], *, plan_route: str) -> str:
    if plan_route == "family_membership_diagnostic_monitoring":
        return "overlapping family-membership diagnostic boundary; no ranking"
    if plan_route == "diagnostic_monitoring_only":
    ...
    if plan_route == "max_step_noncompletion_guardrail":
        return "max-step noncompletion; timeout rate; terminal progress"
    if plan_route == "speed_too_low_guardrail":
        return "speed-too-low rate; useful progress; terminal maneuver quality"
    guardrails = ["collision guardrails", "R4 mitigation semantics", "max-step", "speed-too-low", "no input contract change"]
    if _bool(row.get("collision_guardrail_required")):
...
def materialize_plan_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    diagnostic = plan_route in {"diagnostic_monitoring_only", "family_membership_diagnostic_monitoring"}
    family_diagnostic = plan_route == "family_membership_diagnostic_monitoring"
    max_step = plan_route == "max_step_noncompletion_guardrail"
    speed_too_low = plan_route == "speed_too_low_guardrail"
    return {
        ...
        "source_priority_score": _float(row.get("source_priority_score")),
...
def claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "claim": "artifact_only_source_linked_bounded_repair_plan_materialization",
            "admissible": True,
            "reason": "M2420 may claim only bounded repair-plan artifact materialization from M2417 targets",
        },
        {
            "claim": "repair_execution",
            "admissible": False,
            "reason": "M2420 names levers and gates but does not execute repair",
        },
        {
            "claim": "scenario_redesign_executed",
            "admissible": False,
            "reason": "M2420 does not modify or execute scenarios",
        },
        {
            "claim": "training_repair_success",
            "admissible": False,
            "reason": "M2420 does not train, replay, or run PPO",
        },
        {
        ...
        },
        {
            ...
            "admissible": False,
            "reason": "profile diagnostics remain non-ranking",
        },
        {
            "claim": "finite_window_vs_gru_conclusion",
            "admissible": False,
            "reason": "M2420 does not run the fair controller matrix",
        },
        {
            "claim": "level3_self_identification",
            "admissible": False,
            "reason": "M2420 does not run history interventions",
        },
        {
            "claim": "current_sim_verdict",
            "admissible": False,
            "reason": "M2420 is planning infrastructure, not a current-sim verdict",
        },
    ]
...
def run_source_linked_bounded_repair_plan_materialization(
    *,
    ...
    max_step_rows = [row for row in plan_rows if str(row.get("plan_route", "")) == "max_step_noncompletion_guardrail"]
    speed_too_low_rows = [row for row in plan_rows if str(row.get("plan_route", "")) == "speed_too_low_guardrail"]
    family_diagnostic_rows = [
        row for row in plan_rows if str(row.get("plan_route", "")) == "family_membership_diagnostic_monitoring"
    ]
    ...
    diagnostic_axis_repair_plan_count = sum(
        1
        for row in plan_rows
        if str(row.get("slice_axis", "")) in DIAGNOSTIC_AXES
        and str(row.get("plan_route", "")).startswith("offtrack_repair_plan")
    )
    family_axis_repair_plan_count = sum(
        1
        for row in plan_rows
        if (
            str(row.get("source_table", "")) == "episode_family_membership_rows"
    ...
    profile_axis_repair_plan_count = sum(
        1
        for row in plan_rows
        if "profile_name" in str(row.get("slice_axis", ""))
        and str(row.get("plan_route", "")).startswith("offtrack_repair_plan")
    )
    ...
    max_step_as_plain_repair_count = sum(
        1
        for row in plan_rows
        if str(row.get("source_consolidated_route", "")) == "max_step_noncompletion_target"
        and str(row.get("plan_route", "")).startswith("offtrack_repair_plan")
    )
    speed_too_low_as_plain_repair_count = sum(
        1
        for row in plan_rows
        if str(row.get("source_consolidated_route", "")) == "speed_too_low_target"
        and str(row.get("plan_route", "")).startswith("offtrack_repair_plan")
    )
    ...
    passes = (
        ...
        and len(max_step_rows) == _int(source_summary.get("max_step_noncompletion_row_count"))
        and len(speed_too_low_rows) == _int(source_summary.get("speed_too_low_row_count"))
        and len(diagnostic_rows) == _int(source_summary.get("diagnostic_guardrail_row_count"))
        and len(family_diagnostic_rows) == _int(source_summary.get("family_membership_diagnostic_row_count"))
        and diagnostic_axis_repair_plan_count == 0
        and family_axis_repair_plan_count == 0
        and profile_axis_repair_plan_count == 0
        and r4_ordinary_repair_plan_count == 0
        and collision_guardrail_as_plain_repair_count == 0
        and max_step_as_plain_repair_count == 0
        and speed_too_low_as_plain_repair_count == 0
    ...
    write_csv_rows(output / "max_step_noncompletion_plan_rows.csv", max_step_rows, fieldnames=PLAN_FIELDNAMES)
    write_csv_rows(output / "speed_too_low_plan_rows.csv", speed_too_low_rows, fieldnames=PLAN_FIELDNAMES)
    write_csv_rows(output / "diagnostic_monitoring_rows.csv", diagnostic_rows, fieldnames=PLAN_FIELDNAMES)
    write_csv_rows(output / "family_membership_diagnostic_rows.csv", family_diagnostic_rows, fieldnames=PLAN_FIELDNAMES)
    ...
    summary = {
        ...
        "max_step_noncompletion_plan_row_count": len(max_step_rows),
        "speed_too_low_plan_row_count": len(speed_too_low_rows),
        ...
        "source_table_counts": _count_by(plan_rows, "source_table"),
        "diagnostic_axis_repair_plan_count": diagnostic_axis_repair_plan_count,
        "family_axis_repair_plan_count": family_axis_repair_plan_count,
        "profile_axis_repair_plan_count": profile_axis_repair_plan_count,
        ...
        "max_step_as_plain_repair_count": max_step_as_plain_repair_count,
        "speed_too_low_as_plain_repair_count": speed_too_low_as_plain_repair_count,
        ...
        "top_max_step_noncompletion_plan_rows": max_step_rows[:10],
        "top_speed_too_low_plan_rows": speed_too_low_rows[:10],
        "top_family_membership_diagnostic_rows": family_diagnostic_rows[:10],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = run_source_linked_bounded_repair_plan_materialization(
    ...
    print(f"max_step_noncompletion_plan_row_count={summary['max_step_noncompletion_plan_row_count']}")
    print(f"speed_too_low_plan_row_count={summary['speed_too_low_plan_row_count']}")
    print(f"diagnostic_monitoring_row_count={summary['diagnostic_monitoring_row_count']}")
    print(f"family_membership_diagnostic_row_count={summary['family_membership_diagnostic_row_count']}")
