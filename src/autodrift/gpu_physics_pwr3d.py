"""PWR3D (APPROACH D): pwr3 + a CLEAN front-axle 4-wheel BRAKE (the braking-phase avoid residual).

This is an exact copy of ``autodrift.gpu_physics_pwr3`` (the FAITHFUL gear-SEED fix on top of the
DRIVE-force front-axle traction cap) with ONE addition: a CLEAN front-axle braking force that closes
the braking-phase avoid residual WITHOUT corrupting the drift.

  MOTIVATION (measured, gpu_pwr3_gate.py): the gear-seed fix closed the avoid ACCEL residual
  (vx_rmse accel 0.909->0.479) but the BRAKE residual is essentially unchanged (0.780->0.784) -- it
  is now the dominant avoid error. The Chrono Sedan brakes ALL FOUR wheels (BrakeSimple, axle0=front
  + axle1=rear both 2000 N.m, no bias; extract_chrono_brake.py). pwr3 brakes ONLY the two REAR wheels
  (via omega_rl/rr), so on low-mu braking the rear axle alone saturates at ~half the friction and the
  model under-decelerates.

  WHY pwr2's 4-wheel brake was REJECTED: pwr2 added the front brake BUT also (a) retroactively reduced
  the front lateral Fy to sqrt((mu*Fz)^2 - Fx_brake^2) -- a combined-slip coupling that stole front
  cornering grip -- and (b) coupled the rear brake into the front via a total-friction headroom scale.
  Because the DRIFT regime trail-brakes HEAVILY (measured: drift brake cmd mean 0.139, 75% of steps
  >0.05, 25% >0.2 -- NOT a no-op), that Fy de-gripping overshot the drift-entry sideslip and broke the
  drift gate.

  THE CLEAN FIX (APPROACH D, this module): apply the front brake as a PURE LONGITUDINAL contact force,
  friction-circle limited with LATERAL RESERVED FIRST, and do NOT touch the front lateral Fy. The front
  lateral force fy_fl/fy_fr is already the TRUE combined-slip force from the EXACT TMeasy curve at the
  actual slip angle; the brake is given only the REMAINING longitudinal friction budget after that
  lateral force is reserved:
      budget   = sqrt((mu*Fz_front)^2 - fy_front^2)          [N]  (friction circle, lateral-reserved)
      demand   = brake * max_brake_torque / r_eff            [N]  (commanded front brake force)
      Fx_brake = -sign(vx_front) * min(demand, budget)
  This keeps the total front force on the friction ellipse (|F| <= mu*Fz) -- the TRUE combined-slip
  constraint -- WITHOUT reducing Fy below its measured value: the brake simply cannot exceed the
  longitudinal headroom, and there is no separate Fy knock-down. There is NO rear<->front coupling and
  NO total-car headroom scale (the rear brake is unchanged through omega_rl/rr; the front brake is an
  independent per-front-wheel contact force). It is gated to GENUINE braking (brake>0 via the demand)
  and the force is added in the BODY longitudinal frame (a sliding brake opposes ground velocity, not
  the wheel heading, so it is NOT rotated by steer). Ablation knob: front_brake_scale (1.0 = on; 0.0 =
  pwr3 exactly). Everything else is IDENTICAL to pwr3.

----------------------------------------------------------------------------------------------------
Original pwr3 docstring (unchanged core, retained for reference):
...
class PhysParams:
    ...
    # 0.0 = ablate the front brake -> pwr3 exactly (rear-only brake). The Sedan brakes all 4 wheels
    ...
    # MEASURED RESULT (gpu_pwr3d_gate.py): raw brake (0.0) closes the brake phase (vx_rmse brake split
    # 0.784->0.32) but OVER-BRAKES the accel phase (0.479->0.667) and the heavily-trail-braked drift
    # (drift vx 0.276->0.672), so overall avoid REGRESSES (0.520->0.639). The NET gate (1.0) protects
    # both: avoid 0.520->0.448 (brake split 0.784->0.124), drift vx 0.276->0.198 -- the only config that
    # is a real avoid improvement. Default 1.0 (net gate). The FWD front axle's net longitudinal contact
    # force IS drive-minus-brake, so the net gate is the physically faithful one for a FRONT brake.
...
def _continuous_derivs(vx, vy, yaw_rate, steer, omega_rl, omega_rr, throttle, brake, gear,
    ...
    # no front/rear bias; extract_chrono_brake.py). pwr3 brakes ONLY the two REAR wheels (via the
