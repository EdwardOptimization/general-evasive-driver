"""Artifact-only localization for M2397 effective-candidate outcomes."""
...
DEFAULT_SOURCE_DIR = Path("runs/m2397_paper_route_current_sim_dual_axis_effective_candidate_measured_validation")
DEFAULT_OUTPUT_DIR = Path("runs/m2399_paper_route_current_sim_dual_axis_effective_candidate_measured_outcome_localization")
DEFAULT_TARGET_EPISODE_COUNT = 30735
...
DEFAULT_NEXT_BLOCKER = "m2400-paper-route-current-sim-dual-axis-effective-candidate-measured-outcome-localization-result-audit"
RESULT_PASS = "current_sim_dual_axis_effective_candidate_measured_outcome_localization_pass"
RESULT_FAIL = "current_sim_dual_axis_effective_candidate_measured_outcome_localization_incomplete_or_fail"
...
COMPOSITE_AXES = (
    ("candidate_id+role_family", ("candidate_id", "role_family")),
    ("candidate_id+profile_name", ("candidate_id", "profile_name")),
    ("candidate_id+pack_id", ("candidate_id", "pack_id")),
    ("candidate_id+role_family+hidden_dynamics_bucket", ("candidate_id", "role_family", "hidden_dynamics_bucket")),
    ("repair_family+role_family", ("repair_family", "role_family")),
    ("repair_family+hidden_dynamics_bucket", ("repair_family", "hidden_dynamics_bucket")),
    ("source_slice_axis+source_slice_value", ("source_slice_axis", "source_slice_value")),
...
EXTRA_FIELDNAMES = [
    "effective_candidate_outcome_localization",
    "effective_candidate_ranking_claim_made",
...
def _augment_slice(row: Mapping[str, Any]) -> dict[str, Any]:
    output = dict(row)
    output.update(
        {
            "effective_candidate_outcome_localization": True,
...
def claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "claim": "artifact_only_effective_candidate_outcome_localization",
            "admissible": True,
            "reason": "M2399 may claim only slice materialization from existing M2397 artifacts",
        },
        {
            "claim": "effective_candidate_ranking",
            "admissible": False,
            "reason": "slice rows are diagnostic and do not rank effective candidates",
        },
        {
        ...
        },
        {
            ...
            "admissible": False,
            "reason": "M2399 does not select or promote a winner",
        },
        {
            "claim": "paper_level_benchmark_result",
            "admissible": False,
            "reason": "M2399 is artifact localization, not a paper-level result",
        },
        {
            "claim": "finite_window_vs_gru_conclusion",
            "admissible": False,
            "reason": "M2399 does not run a finite-window-vs-GRU verdict protocol",
        },
        {
            "claim": "level3_self_identification",
            "admissible": False,
            "reason": "M2399 does not run history interventions",
        },
        {
            "claim": "scenario_redesign_executed",
            "admissible": False,
            "reason": "M2399 localizes existing measured outcomes and does not redesign scenarios",
        },
        {
            "claim": "training_repair_success",
            "admissible": False,
            "reason": "M2399 does not train, repair, replay, or run PPO",
        },
        {
            "claim": "current_sim_verdict",
            "admissible": False,
            "reason": "M2399 localizes one artifact and does not make a current-sim verdict",
        },
    ]
...
def _write_outputs(
    *,
    ...
    write_csv_rows(output / "diagnostic_only_slice_rows.csv", diagnostic_rows, fieldnames=SLICE_FIELDNAMES)
    write_csv_rows(output / "claim_boundary.csv", claim_boundary_rows(), fieldnames=CLAIM_FIELDNAMES)


def run_effective_candidate_measured_outcome_localization(
    *,
    ...
    summary = {
        ...
        "source_candidate_count": _unique_count(episode_rows, "candidate_id"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = run_effective_candidate_measured_outcome_localization(
