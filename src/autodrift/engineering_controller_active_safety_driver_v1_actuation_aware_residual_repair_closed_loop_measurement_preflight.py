"""Run M3050 actuation-aware residual repair closed-loop measurement preflight."""
...
DEFAULT_M3049_AUDIT = Path(f"docs/{M3049_ID}.md")
DEFAULT_M3048_DIR = Path(
    "runs/m3048_engineering_controller_active_safety_driver_v1_actuation_aware_residual_repair_fitting_preflight"
)
...
DEFAULT_OUTPUT_DIR = Path(
    "runs/m3050_engineering_controller_active_safety_driver_v1_actuation_aware_residual_repair_closed_loop_measurement_preflight"
)
...
CLAIM_SCOPE = (
    "M3050 Active Safety Driver v1 actuation-aware residual repair closed-loop "
    "measurement preflight only; "
    "same-denominator current-sim measurement rows and metric tables may be "
    "recorded for the M3048 action-headroom-constrained residual/reflex "
    "candidate. No validation result, driver-performance verdict, current-sim "
    "verdict, ranking, winner selection, checkpoint promotion, checkpoint "
    "mutation, repair success, high-fidelity validation, paper evidence, "
    "finite-window-vs-GRU, full ideal driver, or self-ID claim is made"
)
...
class ResidualActorPolicy:
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        ...
        bounded_residual = np.clip(raw_residual, -self.residual_limit, self.residual_limit).astype(np.float32)
        headroom_low = self.action_low - base_action
        headroom_high = self.action_high - base_action
        residual = np.clip(bounded_residual, headroom_low, headroom_high).astype(np.float32)
        ...
        self.headroom_clip_count += int(np.any(np.abs(residual - bounded_residual) > RESIDUAL_EPS))
    ...
    def telemetry(self) -> dict[str, Any]:
        ...
        return {
            ...
            "headroom_clip_fraction": float(self.headroom_clip_count / steps) if steps else 0.0,
...
def load_sources(*, m3049_audit: Path, m3048_dir: Path, m3039_dir: Path, m3037_dir: Path, m3012_dir: Path) -> dict[str, Any]:
    paths = {
        "m3049_audit": m3049_audit,
        "m3048_summary": m3048_dir / "summary.json",
        "m3048_candidate": m3048_dir / "candidate_residual_reflex_layer.npz",
        "m3048_gate_matrix": m3048_dir / "gate_matrix.csv",
        "m3048_action_saturation_guard_rows": m3048_dir / "action_saturation_guard_rows.csv",
        "m3048_success_guard_rows": m3048_dir / "success_preservation_guard_rows.csv",
        "m3048_side_effect_rows": m3048_dir / "checkpoint_side_effect_guard_rows.csv",
        "m3048_claim_rows": m3048_dir / "claim_boundary_rows.csv",
    ...
    return {
        ...
        "m3049_audit_text": paths["m3049_audit"].read_text(encoding="utf-8") if exists["m3049_audit"] else "",
        "m3048_summary": read_json(paths["m3048_summary"]) if exists["m3048_summary"] else {},
        "m3048_gate_rows": read_csv_rows(paths["m3048_gate_matrix"]),
        "m3048_action_saturation_guard_rows": read_csv_rows(paths["m3048_action_saturation_guard_rows"]),
        "m3048_success_guard_rows": read_csv_rows(paths["m3048_success_guard_rows"]),
        "m3048_side_effect_rows": read_csv_rows(paths["m3048_side_effect_rows"]),
        "m3048_claim_rows": read_csv_rows(paths["m3048_claim_rows"]),
...
def load_residual_artifact(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {
            ...
            "contract_pass": False,
            "action_composition": "",
            "headroom_constraint_applied": False,
        }
    with np.load(path, allow_pickle=False) as data:
        ...
        headroom = (
            bool(np.asarray(data["headroom_constraint_applied"]).reshape(-1)[0])
            if "headroom_constraint_applied" in data.files
            else False
        )
    contract_pass = bool(
        ...
        and composition == "base_action_plus_headroom_constrained_residual_clipped"
        and headroom
    )
    return {
        "weight": weight,
        ...
        "headroom_constraint_applied": headroom,
    }
...
def workload_plan(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, workload in enumerate(
        ...
        rows.append(
            {
                **dict(workload),
                "measurement_episode_id": f"m3050-measurement-episode-{index:04d}",
                ...
                "residual_profile_name": f"{profile_name}+m3048_action_headroom_residual",
    ...
    for plan in plan_rows:
        try:
            ...
            try:
                ...
                row = run_episode_with_policy(env, policy, "m3048_action_headroom_residual", int(plan["eval_seed"]))
            finally:
                env.close()
...
def metric_summary_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for group, group_rows in sorted(groups.items()):
        summaries.append(
            {
                "metric_summary_id": f"m3050-metric-summary-{len(summaries) + 1:04d}",
                "group": group,
                ...
                "headroom_clip_fraction_mean": _mean(_float(row.get("headroom_clip_fraction")) for row in group_rows),
...
def residual_adapter_guard_rows(residual: dict[str, Any], episodes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    action_clip_fraction = max((_float(row.get("action_clip_fraction")) for row in episodes), default=0.0)
    return [
        ...
        guard(
            "residual_action_composition",
            "residual_adapter",
            residual.get("action_composition"),
            "base_action_plus_headroom_constrained_residual_clipped",
        ),
        guard("headroom_constraint_applied", "residual_adapter", residual.get("headroom_constraint_applied"), True),
        ...
        guard("final_action_clip_absent", "residual_adapter", bool(action_clip_fraction <= RESIDUAL_EPS), True),
    ]
...
def claim(claim_id: str, family: str, allowed: bool, made: bool, evidence: str) -> dict[str, Any]:
    return {
        "claim_id": f"m3050-{claim_id}",
        "claim_family": family,
        "allowed_in_m3050": allowed,
        "claim_made": made,
...
def claim_boundary_rows(*, measurement_rows_present: bool, artifacts_present: bool, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3051 audit manifest"),
    ]
    blocked = [
...
def gate_matrix_rows(
    *,
    ...
    audit_accepts = (
        "accept_m3048_action_aware_residual_fit_route_to_m3050_closed_loop_measurement_preflight"
        in source["m3049_audit_text"]
    )
    ...
    return [
        ...
        gate("m3049_accepts_m3050_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        gate("m3048_status_pass", "lineage", _bool(source["m3048_summary"].get("status_pass", False)), source["m3048_summary"].get("status_pass"), True, "lineage_invalid"),
        gate("m3048_gate_matrix_pass", "lineage", _bool(source["m3048_summary"].get("gate_matrix_pass", False)), source["m3048_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path, summary_path: Path) -> dict[str, Any]:
    return {
        ...
        "lineage": {
            ...
            "parent_objective": ["audit M3050 closed-loop measurement artifacts before interpretation"],
            "derived_from": [MILESTONE_ID, M3049_ID],
            "blocked_by": [
                "M3050 measurement rows require audit before any performance or continuation decision",
                "current-sim measurement rows are not validation or promotion evidence before M3051",
            ],
            "supersedes": ["direct interpretation of M3050 measurement rows without audit"],
            "invalidates": [],
        },
        "review_artifact": f"docs/reviews/{NEXT_ID}.md",
        "public_gates": [
            "M3051 must audit M3050 summary measurement metric guard claim and gate artifacts",
            "M3051 must answer evidence_summary supported_claims falsified_claims failure_taxonomy_summary public_gate_overfit_risk and next_branch_decision",
            "M3051 must preserve actor 72/action 3 and claim boundaries",
            "M3051 must reject validation ranking promotion high-fidelity paper finite-window-vs-GRU full-driver and self-ID claims unless separately routed",
            "M3051 must select exactly one next route or stop state",
        ],
        "private_holdout_policy": "not_used",
        "forbidden_shortcuts": [
            "do not rerun rollout validate rank promote tune or mutate checkpoints",
            "do not convert M3050 rows into performance current-sim high-fidelity paper finite-window-vs-GRU full-driver or self-ID claims",
            "do not change actor input or action contract",
        ],
        "workflow_synthesis": {
            "branch": "active_safety_driver_v1_engineering_mainline",
            "evidence_axis": "active_safety_driver_v1_actuation_aware_closed_loop_measurement_result_audit",
            "evidence_increment": "audits and synthesizes the first same-denominator closed-loop measurement rows for the M3048 action-headroom-constrained residual candidate",
            ...
            "stop_condition": [
                "stop if M3050 artifacts are missing or gate matrix fails",
                "stop if actor or residual adapter contracts were violated",
                "stop if M3050 rows are treated as validation or performance verdicts",
            ],
            "fallback_plan": [
            ...
            "synthesis_trigger": "M3050 completes action-aware closed-loop measurement preflight",
        ...
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit and synthesize M3050 closed-loop measurement preflight artifacts",
            "admission_evidence": [
                "M3050 summary and gate matrix",
                "M3050 measurement episode metric residual adapter actor contract side-effect and claim artifacts",
            ],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3051 status queue scoreboard research log and review",
                "one follow-up manifest only if M3051 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3051 audit accepts or rejects M3050 as complete and claim-safe",
                "next route or stop state is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3051 audits engineering measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": [
                "None in M3051; finite-window and GRU comparison remains a later same-case engineering ablation."
            ],
            "temporal_evidence_window": "M3050 closed-loop measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3050 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits and synthesizes newly generated action-aware Active Safety Driver v1 closed-loop measurement rows before selecting the next branch route",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3051 audits engineering measurement evidence",
            "must_synthesize_if": [
                "M3051 cannot accept M3050 as complete and claim-safe",
                "M3051 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict or self-ID evidence",
                "M3051 would continue with another materialization-only route before interpreting measurement",
            ],
        },
        "success_criteria": [
            f"docs/{NEXT_ID}.md exists",
            "M3051 audits M3050 row counts gates actor residual side-effect and claim boundaries",
            "M3051 answers all synthesis_questions",
            "M3051 selects exactly one next route or stop state",
        ...
        "failure_criteria": [
            "M3051 hides M3050 failures or missing artifacts",
            "M3051 treats M3050 measurements as validation or performance verdict",
            "M3051 changes actor input or action contract",
            "M3051 leaves next route ambiguous",
        ],
        "decision_rule": "Pass only if M3051 audits M3050 artifacts and selects one next route or stop state while preserving actor residual and claim boundaries without overclaiming.",
        "commands": [{"name": "active_safety_driver_v1_actuation_aware_closed_loop_measurement_result_audit_doc", "command": "true"}],
...
def render_doc(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- headroom clip fraction mean: {summary['measurement_headroom_clip_fraction_mean']}",
...
def run_closed_loop_measurement_preflight(
    *,
    ...
    residual = load_residual_artifact(source["paths"]["m3048_candidate"])
    ...
    summary = {
        ...
        "result_class": (
            "active_safety_driver_v1_actuation_aware_closed_loop_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_v1_actuation_aware_closed_loop_measurement_preflight_fail"
        ),
        "status_pass": status_pass,
        ...
        "measurement_headroom_clip_fraction_mean": _mean(_float(row.get("headroom_clip_fraction")) for row in episodes),
        ...
        "required_artifacts_present": present,
        "m3048_status_pass": _bool(source["m3048_summary"].get("status_pass", False)),
        "m3048_gate_matrix_pass": _bool(source["m3048_summary"].get("gate_matrix_pass", False)),
        "candidate_residual_reflex_layer": str(source["paths"]["m3048_candidate"]),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3049-audit", type=Path, default=DEFAULT_M3049_AUDIT)
    parser.add_argument("--m3048-dir", type=Path, default=DEFAULT_M3048_DIR)
...
def main() -> None:
    ...
    summary = run_closed_loop_measurement_preflight(
        m3049_audit=args.m3049_audit,
        m3048_dir=args.m3048_dir,
