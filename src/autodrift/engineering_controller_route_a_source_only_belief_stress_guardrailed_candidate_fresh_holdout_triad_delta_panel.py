"""M2793 source-only guardrailed candidate fresh-holdout triad delta panel.

This preflight runs a bounded source/base/candidate closed-loop diagnostic
panel after M2792 accepts M2791 artifact safety. It writes triad execution rows
and candidate-minus-source/base delta rows for audit only. It does not train,
validate, rank, select a winner, promote a checkpoint, compute a success-rate
...
DEFAULT_MILESTONE = (
    "m2793-engineering-controller-route-a-source-only-belief-stress-guardrailed-"
    "candidate-fresh-holdout-triad-delta-panel-preflight"
)
...
DEFAULT_DOC_PATH = Path(
    "docs/m2793-engineering-controller-route-a-source-only-belief-stress-guardrailed-"
    "candidate-fresh-holdout-triad-delta-panel-preflight.md"
)
DEFAULT_M2792_AUDIT = Path(
    "docs/m2792-engineering-controller-route-a-source-only-belief-stress-guardrailed-"
    "multi-objective-training-result-audit.md"
)
...
DEFAULT_FOLLOW_UP_MANIFEST = Path(
    "experiments/manifests/m2794-engineering-controller-route-a-source-only-belief-"
    "stress-guardrailed-candidate-fresh-holdout-triad-delta-panel-result-audit.json"
)
DEFAULT_NEXT_BLOCKER = (
    "m2794-engineering-controller-route-a-source-only-belief-stress-guardrailed-"
    "candidate-fresh-holdout-triad-delta-panel-result-audit"
)
DEFAULT_SEED_START_INDEX = 8
DEFAULT_SEED_COUNT = 4
DEFAULT_HORIZON_STEPS = 140

TRIAD_SUBJECTS = ("source", "base_candidate", "candidate")
CLAIM_SCOPE = (
    "Route A source-only belief-stress guardrailed candidate fresh-holdout triad "
    "closed-loop diagnostic preflight only"
)
...
RESULT_CLASS_PASS = (
    "engineering_controller_route_a_source_only_belief_stress_guardrailed_candidate_"
    "fresh_holdout_triad_delta_panel_preflight_pass"
)
RESULT_CLASS_FAIL = (
    "engineering_controller_route_a_source_only_belief_stress_guardrailed_candidate_"
    "fresh_holdout_triad_delta_panel_preflight_failed"
)
...
TRIAD_EXECUTION_FIELDNAMES = PAIRED_EXECUTION_FIELDNAMES
TRIAD_DELTA_FIELDNAMES = [
    ...
    "candidate_minus_reference_final_road_margin_m",
    "candidate_minus_reference_final_speed_mps",
    "candidate_minus_reference_max_abs_yaw_rate",
    "candidate_minus_reference_max_abs_y",
    "candidate_minus_reference_throttle_brake_conflict_proxy",
    "candidate_minus_reference_mean_throttle",
    "candidate_minus_reference_mean_brake",
    "candidate_minus_reference_mean_action_l1",
    "candidate_minus_reference_command_response_proxy",
...
def run_guardrailed_candidate_fresh_holdout_triad_delta_panel(
    output_dir: Path | str,
    *,
    m2792_audit: Path | str = DEFAULT_M2792_AUDIT,
    ...
    if int(seed_start_index) < DEFAULT_SEED_START_INDEX:
        raise ValueError("M2793 requires seed_start_index outside M2784 0..3 and M2787 4..7")
    if int(seed_count) < 2:
        raise ValueError("M2793 requires at least two fresh holdout seeds")
    ...
    objective_rows = select_objective_rows(read_csv_rows(source_paths["training_objective_rows"]))
    ...
    source_delta_rows = build_triad_delta_rows(execution_rows, subject_registry, reference_subject="source")
    base_delta_rows = build_triad_delta_rows(execution_rows, subject_registry, reference_subject="base_candidate")
    ...
    proof_gate_rows = build_proof_gate_rows(
        ...
        checkpoint_manifest=m2791_checkpoint_manifest,
    ...
    summary = build_summary(
        ...
        m2782_summary=m2782_summary,
        checkpoint_manifest=m2791_checkpoint_manifest,
        m2787_delta_rows=m2787_delta_rows,
        subject_registry=subject_registry,
        objective_rows=objective_rows,
        execution_rows=execution_rows,
        source_delta_rows=source_delta_rows,
    ...
    write_json(paths["follow_up_manifest"], build_m2794_manifest(summary))
    write_doc(paths["doc"], summary)

    summary = {
        **summary,
        "required_artifacts_present": required_artifacts_present(paths),
        "m2794_follow_up_manifest_registered": paths["follow_up_manifest"].exists(),
    }
...
def _source_paths(
    ...
    return {
        ...
        "training_objective_rows": m2791_dir / "training_objective_rows.csv",
        "m2791_gate_matrix": m2791_dir / "gate_matrix.csv",
        "m2791_behavior_retention_gate_rows": m2791_dir / "behavior_retention_gate_rows.csv",
        "mitigation_reference_guard_rows": m2791_dir / "mitigation_reference_guard_rows.csv",
...
def _require_sources(paths: dict[str, Path]) -> None:
    ...
    if missing:
        raise FileNotFoundError(f"M2793 missing required source artifacts: {missing}")
...
def select_objective_rows(rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    if len(selected) != expected_count:
        raise RuntimeError(f"expected {expected_count} M2791 ordinary objective rows, got {len(selected)}")
    return sorted(
        selected,
...
def load_triad_subject_registry(
    source_checkpoint: Path,
    base_candidate_checkpoint: Path,
    candidate_checkpoint: Path,
    *,
    ...
    for subject, path in (
        ...
        ("base_candidate", base_candidate_checkpoint),
    ...
    *,
    ...
    for objective in objective_rows:
        for seed_index in range(int(seed_start_index), int(seed_start_index) + int(seed_count)):
            ...
            pair_id = (
                f"m2793_triad_{objective['role_family']}_{objective['dynamics_axis']}_"
...
def build_triad_delta_rows(
    ...
    *,
    ...
    delta_family = "candidate_minus_source" if reference_subject == "source" else "candidate_minus_base"
    for pair_id in sorted(by_pair):
        pair = by_pair[pair_id]
        reference = pair.get(reference_subject)
        candidate = pair.get("candidate")
        paired_complete = reference is not None and candidate is not None
        if not paired_complete:
            continue
        reference_action_terms = [
            float(reference["mean_abs_steer"]),
            float(reference["mean_throttle"]),
            float(reference["mean_brake"]),
        ]
        ...
        delta_values = {
            "candidate_minus_reference_minimum_obstacle_clearance_m": _delta(
                candidate,
                reference,
                "minimum_obstacle_clearance_m",
            ),
            "candidate_minus_reference_minimum_road_margin_m": _delta(candidate, reference, "minimum_road_margin_m"),
            "candidate_minus_reference_final_road_margin_m": _delta(candidate, reference, "final_road_margin_m"),
            "candidate_minus_reference_final_speed_mps": _delta(candidate, reference, "final_speed_mps"),
            "candidate_minus_reference_max_abs_yaw_rate": _delta(candidate, reference, "max_abs_yaw_rate"),
            "candidate_minus_reference_max_abs_y": _delta(candidate, reference, "max_abs_y"),
            "candidate_minus_reference_throttle_brake_conflict_proxy": _delta(
                candidate,
                reference,
                "throttle_brake_conflict_proxy",
            ),
            "candidate_minus_reference_mean_throttle": _delta(candidate, reference, "mean_throttle"),
            "candidate_minus_reference_mean_brake": _delta(candidate, reference, "mean_brake"),
            "candidate_minus_reference_mean_action_l1": _delta(candidate, reference, "mean_action_l1"),
            "candidate_minus_reference_command_response_proxy": _delta(
                candidate,
                reference,
        ...
        rows.append(
            {
                ...
                "pair_id": pair_id,
                "role_family": reference["role_family"],
                "dynamics_axis": reference["dynamics_axis"],
                "stress_family": reference["stress_family"],
                "seed_index": reference["seed_index"],
                "seed": reference["seed"],
                "reference_subject": reference_subject,
                "candidate_subject": "candidate",
                "reference_execution_row_id": reference["execution_row_id"],
                "candidate_execution_row_id": candidate["execution_row_id"],
                "reference_checkpoint": str(subject_registry[reference_subject]["checkpoint_path"]),
                "candidate_checkpoint": str(subject_registry["candidate"]["checkpoint_path"]),
                "reference_checkpoint_hash": subject_registry[reference_subject]["checkpoint_hash"],
                "candidate_checkpoint_hash": subject_registry["candidate"]["checkpoint_hash"],
                "reference_steps_executed": reference["steps_executed"],
                "candidate_steps_executed": candidate["steps_executed"],
                **delta_values,
                "mean_action_delta_l1": float(
                    np.mean(np.abs(np.asarray(candidate_action_terms) - np.asarray(reference_action_terms)))
                ),
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    rows = [
        ("validation_result", "validation", False, False, "M2793 does not run measured validation"),
        ("ranking_result", "ranking", False, False, "M2793 does not rank checkpoints"),
        ("winner_selection", "promotion", False, False, "M2793 selects no winner"),
        ("checkpoint_promotion", "promotion", False, False, "M2793 promotes no checkpoint"),
        ("success_rate_verdict", "metric_artifact", False, False, "M2793 emits no success-rate verdict"),
        ("driver_performance", "performance", False, False, "M2793 is diagnostic delta evidence only"),
        ("paper_result", "paper", False, False, "M2793 is not paper evidence"),
        ("current_sim_verdict", "current_sim", False, False, "M2793 is not a current-sim verdict"),
        ("high_fidelity_validation", "high_fidelity", False, False, "M2793 does not run HF validation"),
        ("level3_self_id", "self_id", False, False, "M2793 is not self-ID evidence"),
        (
            "triad_delta_artifacts_complete",
            "allowed_artifact_completion",
            True,
            True,
            "M2793 may claim triad diagnostic artifacts were written",
        ),
    ]
    return [
        {
...
def build_proof_gate_rows(
    *,
    ...
    lineage_pass = (
        ...
        == 3
        and checkpoint_manifest.get("source_reference_checkpoint_hash")
        == subject_registry["source"]["checkpoint_hash"]
        and checkpoint_manifest.get("base_candidate_checkpoint_hash")
        == subject_registry["base_candidate"]["checkpoint_hash"]
        and checkpoint_manifest.get("candidate_checkpoint_hash")
        == subject_registry["candidate"]["checkpoint_hash"]
    )
    return [
        gate(
            "proof_m2792_audit_present",
            "proof",
            "lineage",
            source_paths["m2792_audit"].exists(),
            str(source_paths["m2792_audit"]),
            "M2792 audit artifact exists",
            1,
            "lineage_invalid",
        ),
        gate(
            "proof_m2791_status_pass",
            "proof",
            "lineage",
            bool(m2791_summary.get("status_pass", False)),
            str(bool(m2791_summary.get("status_pass", False))),
            "true",
            1,
            "lineage_invalid",
        ),
        ...
        gate(
            ...
            "source base candidate hashes match checkpoint manifest",
            1,
            "lineage_invalid",
        ),
        gate(
        ...
        gate(
            "proof_triad_execution_row_count",
            "proof",
            "artifact",
        ...
        gate(
            "proof_candidate_minus_source_delta_row_count",
            "proof",
            "artifact",
            len(source_delta_rows) == expected_delta_rows,
            str(len(source_delta_rows)),
            str(expected_delta_rows),
            len(source_delta_rows),
            "metric_artifact",
        ),
        gate(
            "proof_candidate_minus_base_delta_row_count",
            "proof",
            "artifact",
            len(base_delta_rows) == expected_delta_rows,
            str(len(base_delta_rows)),
            str(expected_delta_rows),
            len(base_delta_rows),
            "metric_artifact",
        ),
        gate(
            ...
            "proof",
            "artifact",
            bool(all_delta_rows) and all(as_bool(row["paired_row_complete"]) for row in all_delta_rows),
            "all pairs complete",
            "all pairs complete",
            len(all_delta_rows),
            "metric_artifact",
        ...
        gate(
            ...
            "proof",
            "claim_boundary",
            not any(as_bool(row["ranking_admissible"]) or as_bool(row["winner_selected"]) for row in all_delta_rows)
            and not any(as_bool(row["success_rate_verdict_computed"]) for row in all_delta_rows),
            "false",
            "false",
            len(all_delta_rows),
            "objective_overfit",
...
def build_generalization_holdout_gate_rows(
    ...
    source_delta_rows: list[dict[str, Any]],
    base_delta_rows: list[dict[str, Any]],
    m2787_delta_rows: list[dict[str, str]],
    m2787_summary: dict[str, Any],
    *,
    seed_start_index: int,
    seed_count: int,
    ...
    previous_seeds = set(int(index) for index in m2787_summary.get("fresh_holdout_seed_indices", []))
    previous_seeds.update(int(row["seed_index"]) for row in m2787_delta_rows if str(row.get("seed_index", "")))
    previous_seeds.update(int(index) for index in m2787_summary.get("m2784_seed_indices", []))
    ...
    return [
        gate(
            ...
            "generalization",
            "scenario_sampling",
            {row["role_family"] for row in source_delta_rows} == set(ORDINARY_ROLE_FAMILIES),
            ";".join(sorted({row["role_family"] for row in source_delta_rows})),
        ...
        gate(
            ...
            "generalization",
            "scenario_sampling",
            {row["dynamics_axis"] for row in source_delta_rows} == set(DYNAMICS_AXES),
            ";".join(sorted({row["dynamics_axis"] for row in source_delta_rows})),
            ";".join(DYNAMICS_AXES),
            len(source_delta_rows),
            "scenario_sampling_failure",
        ),
        gate(
            ...
            "generalization",
            "scenario_sampling",
            {row["stress_family"] for row in source_delta_rows} == set(STRESS_FAMILIES),
            ";".join(sorted({row["stress_family"] for row in source_delta_rows})),
        ...
        gate(
            "generalization_seed_indices_outside_previous_surfaces",
            "generalization",
            "seed_split",
            int(seed_start_index) >= DEFAULT_SEED_START_INDEX and holdout_seed_indices.isdisjoint(previous_seeds),
            ",".join(str(index) for index in sorted(holdout_seed_indices)),
            "disjoint from M2784 0..3 and M2787 4..7",
            len(source_delta_rows),
            "scenario_sampling_failure",
        ),
        gate(
            ...
            "generalization",
            "seed_split",
            _each_bucket_holdout_seed_coverage(source_delta_rows, int(seed_start_index), int(seed_count)),
            "complete",
            "complete",
            len(source_delta_rows),
            "seed_fragility",
        ...
        gate(
            "generalization_triad_subject_coverage",
            "generalization",
            "artifact",
            {row["checkpoint_subject"] for row in execution_rows} == set(TRIAD_SUBJECTS)
            and len(execution_rows) == len(objective_rows) * int(seed_count) * len(TRIAD_SUBJECTS),
        ...
        gate(
            "generalization_dual_delta_family_coverage",
            "generalization",
            "artifact",
            {row["delta_family"] for row in delta_rows} == {"candidate_minus_base", "candidate_minus_source"},
            ";".join(sorted({row["delta_family"] for row in delta_rows})),
            "candidate_minus_base;candidate_minus_source",
            len(delta_rows),
            "metric_artifact",
        ),
        gate(
            "generalization_horizon_longer_than_m2787",
            "generalization",
            "horizon",
            int(horizon_steps) > int(m2787_summary.get("horizon_steps", 0)),
            str(int(horizon_steps)),
            f">{int(m2787_summary.get('horizon_steps', 0))}",
            len(execution_rows),
            "scenario_sampling_failure",
        ),
    ]
...
def build_behavior_retention_gate_rows(
    ...
    return [
        gate(
            ...
            "behavior_retention",
            "lineage",
            int(m2787_summary.get("paired_delta_row_count", 0)) >= 72
            and bool(m2787_summary.get("gate_matrix_pass", False)),
            str(m2787_summary.get("paired_delta_row_count", 0)),
            "M2787 delta rows and gates present",
            int(m2787_summary.get("paired_delta_row_count", 0)),
            "lineage_invalid",
        ),
        gate(
            "behavior_candidate_minus_source_obstacle_clearance_counted",
            "behavior_retention",
            "behavior_regression",
            source_obstacle["count"] == expected_count,
            f"positive={source_obstacle['positive']} negative={source_obstacle['negative']}",
            "obstacle-clearance deltas counted",
            source_obstacle["count"],
            "behavior_regression",
        ),
        gate(
            "behavior_candidate_minus_base_obstacle_clearance_counted",
            "behavior_retention",
            "behavior_regression",
            base_obstacle["count"] == expected_count,
            f"positive={base_obstacle['positive']} negative={base_obstacle['negative']}",
            "obstacle-clearance deltas counted",
            base_obstacle["count"],
            "behavior_regression",
        ),
        ...
        gate(
            "behavior_throttle_brake_conflict_delta_counted",
            "behavior_retention",
            "behavior_regression",
            source_conflict["count"] == expected_count and base_conflict["count"] == expected_count,
            f"source_zero={source_conflict['zero']} base_zero={base_conflict['zero']}",
            "conflict deltas counted",
            expected_count * 2,
            "behavior_regression",
        ),
        gate(
            "behavior_candidate_update_not_interpreted_as_winner",
            "behavior_retention",
            "objective_overfit",
            not any(as_bool(row["ranking_admissible"]) or as_bool(row["winner_selected"]) for row in source_delta_rows + base_delta_rows),
            "false",
            "false",
            len(source_delta_rows) + len(base_delta_rows),
            "objective_overfit",
        ),
    ]
...
def build_summary(
    *,
    ...
    previous_seed_indices = sorted(
        set(int(index) for index in m2787_summary.get("fresh_holdout_seed_indices", []))
        | set(int(index) for index in m2787_summary.get("m2784_seed_indices", []))
        | {int(row["seed_index"]) for row in m2787_delta_rows if str(row.get("seed_index", ""))}
    )
    status_pass = bool(
        gate_matrix_pass
        and actor_contract_shape
        and all(as_bool(row["status_pass"]) for row in mitigation_guard_rows)
        and bool(m2791_summary.get("status_pass", False))
        and bool(m2787_summary.get("status_pass", False))
    ...
    return {
        ...
        "doc": str(paths["doc"]),
        "next_blocker": next_blocker,
        "m2792_audit": str(source_paths["m2792_audit"]),
        ...
        "m2787_horizon_steps": int(m2787_summary.get("horizon_steps", 0)),
        "m2787_paired_delta_row_count": int(m2787_summary.get("paired_delta_row_count", len(m2787_delta_rows))),
        ...
        "base_candidate_model_state_hash": subject_registry["base_candidate"]["model_state_hash"],
        "candidate_model_state_hash": subject_registry["candidate"]["model_state_hash"],
        "checkpoint_manifest_candidate_hash": checkpoint_manifest.get("candidate_checkpoint_hash", ""),
        "checkpoint_manifest_base_hash": checkpoint_manifest.get("base_candidate_checkpoint_hash", ""),
        ...
        "m2794_follow_up_manifest_registered": False,
        ...
        "horizon_steps": int(horizon_steps),
        "horizon_longer_than_m2787": int(horizon_steps) > int(m2787_summary.get("horizon_steps", 0)),
        ...
        "candidate_minus_source_obstacle_clearance_positive_count": source_metrics[
            "candidate_minus_reference_minimum_obstacle_clearance_m"
        ]["positive"],
        "candidate_minus_source_obstacle_clearance_negative_count": source_metrics[
            "candidate_minus_reference_minimum_obstacle_clearance_m"
        ]["negative"],
        "candidate_minus_base_obstacle_clearance_positive_count": base_metrics[
            "candidate_minus_reference_minimum_obstacle_clearance_m"
        ]["positive"],
        "candidate_minus_base_obstacle_clearance_negative_count": base_metrics[
            "candidate_minus_reference_minimum_obstacle_clearance_m"
        ]["negative"],
...
def build_run_state(
    ...
    return {
        "run_state_id": "m2793_guardrailed_candidate_fresh_holdout_triad_delta_panel_state_v0",
        "generated_at_utc": utc_timestamp(),
...
def build_m2794_manifest(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "lineage": {
            ...
            "parent_config": [
                "experiments/manifests/m2793-engineering-controller-route-a-source-only-belief-stress-guardrailed-candidate-fresh-holdout-triad-delta-panel-preflight.json",
                "experiments/manifests/m2792-engineering-controller-route-a-source-only-belief-stress-guardrailed-multi-objective-training-result-audit.json",
            ...
            "parent_objective": [
                "audit the M2793 fresh-holdout source/base/candidate triad delta panel before interpretation"
            ],
            "derived_from": [
                DEFAULT_MILESTONE,
            ...
            "blocked_by": [
                "M2793 triad deltas must be audited before any validation ranking promotion performance or self-ID interpretation",
                "M2793 remains source-only and cannot resolve the M2638 high-fidelity source dependency",
                "Obstacle-clearance retention must stay hard before road-margin or yaw-rate interpretation",
            ],
            "supersedes": [
                "direct interpretation of M2793 deltas without result audit",
        ...
        "workflow_synthesis": {
            "branch": "engineering_controller_route_a_source_only_belief_stress_guardrailed_candidate_fresh_holdout_triad_delta_panel",
            "evidence_axis": "source_only_belief_stress_guardrailed_candidate_triad_delta_panel_result_audit",
            "evidence_increment": "audits M2793 fresh-holdout triad closed-loop diagnostic delta artifacts before interpretation",
            ...
            "stop_condition": [
                "stop if M2793 required artifacts are incomplete",
                "stop if actor or claim boundaries fail",
                "stop if holdout seed indices overlap earlier surfaces",
                "stop if obstacle-clearance guard is weakened or hidden",
            ...
            "fallback_plan": [
                ...
                "pivot to broader architecture or scenario-distribution change if fresh holdout fails to show robust candidate signal",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M2793 writes fresh-holdout source/base/candidate closed-loop delta artifacts",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "audit M2793 fresh-holdout source-only triad delta panel artifacts",
            "admission_evidence": [
                "M2793 summary and gate artifacts exist",
                "M2793 writes triad execution and delta rows with source base candidate lineage",
                "M2793 is not validated ranked or promoted before this audit",
            ],
            "blocked_shortcuts": [
                "no new execution or training in M2794",
            ...
            "allowed_updates": [
                f"docs/{task_id}.md",
                "M2794 status queue scoreboard research log and review",
            ],
            "next_stage_criteria": [
                "M2793 artifacts are complete and claim-safe or failure is classified",
                "one bounded follow-up or stop decision is registered",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M2794 audits source-only M2793 artifacts and cannot establish self-ID.",
            "history_necessity_tests": [
                "M2794 may check that M2793 covered history stress rows but runs no self-ID comparison."
            ],
            "temporal_evidence_window": "M2790-M2794 source-only guardrailed belief-stress branch.",
            "negative_result_policy": "If M2793 artifacts fail or deltas are negative, preserve failure and route to synthesis or repair rather than weakening gates.",
            "allowed_claims": [
                "M2793 fresh-holdout triad delta artifacts are accepted or rejected as complete and claim-safe",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits fresh M2793 holdout triad closed-loop delta artifacts before any extension",
            "paper_verdict_delta": "no paper verdict; audit decides whether M2793 can be used as future engineering evidence",
            "must_synthesize_if": [
                "M2794 finds incomplete artifacts or claim-boundary failure",
                "M2794 finds obstacle-clearance guard weakening",
                "another process-only milestone is proposed after M2794 without fresh evidence or synthesis",
            ],
        },
        "hypothesis": "M2793 fresh-holdout triad closed-loop delta artifacts can be audited for completeness and claim safety before interpretation.",
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M2794 writes a claim-safe audit of M2793 artifacts and routes before interpretation.",
...
def write_doc(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"m2787_horizon_steps: {summary['m2787_horizon_steps']}",
...
def _delta(candidate: dict[str, Any], reference: dict[str, Any], field: str) -> float:
    return float(candidate[field]) - float(reference[field])


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--m2792-audit", type=Path, default=DEFAULT_M2792_AUDIT)
