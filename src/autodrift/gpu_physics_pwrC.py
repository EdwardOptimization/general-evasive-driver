"""PWR-C: pwr3 (gear-SEED fix) + APPROACH C cornering longitudinal coupling / induced drag.

This is gpu_physics_pwr3 (the carried planar FWD-kludge model: EXACT Chrono TMeasy tyre + physical
slip-relaxation + the FAITHFUL highest-in-band gear seed + the measured front-axle traction cap) with
ONE added physics term: the cornering-induced LONGITUDINAL DRAG that pwr3 omits.

  APPROACH C (cornering longitudinal coupling / induced drag): the avoid is a lane-change. pwr3
  resolves the front tyre force into the BODY (heading) frame via the steer rotation -Fy_f*sin(steer)
  and applies the rear force directly in body-x; the physically-real combined-slip longitudinal
  coupling -- each tyre operating at a slip angle alpha has its lateral force projected onto the
  axle VELOCITY direction, -|Fy*sin(alpha)|, a pure dissipative drag -- is MISSING on BOTH axles.

  MEASURED MOTIVATION (probes on surrogate_avoid_labels.npz / surrogate_drift_data.npz):
    - The pwr3 avoid over-speed (model vx > Chrono vx) grows MONOTONICALLY with |steer|: mean error
      +0.17 m/s at |steer|<0.05 -> +0.50 at [0.15,0.30) -> +0.66 at |steer|>0.3. It is a CORNERING
      phenomenon.
    - Chrono DECELERATES through the turn: body ax -0.52 m/s^2 at moderate steer, -0.80 m/s^2 at
      large steer (~1.16 kN), at the SAME ~0.15 throttle as straight cruise. pwr3 (front free-rolling,
      drive on the un-coupled rear) does not, so it carries too much speed -> the +0.66 m/s over-speed.
    - During the avoid corner the FRONT axle spends ~83% of its friction budget (4.09 of 4.93 kN) on
      lateral force -- the FWD-reality combined-slip coupling pwr3's rear-drive kludge cannot see.

  THE TERM:  dFx = -corner_drag_scale * gate(|steer|) * ( |Fy_f*sin(alpha_f)| +
                     corner_drag_rear_scale*|Fy_r*sin(alpha_r)| ) * sign(vx)
  added to the body-x force (alpha_f/alpha_r are the lagged per-axle slip angles; this is DISTINCT
  from, not double-counting, the existing front heading projection -Fy_f*sin(steer)). A smoothstep
  STEER gate over [0.15,0.25] rad confines the drag to the avoid cornering (drift's beta-gate step
  sits at |steer| ~0.08 rad; drift |steer| p90 ~0.14), so the drift regime is left EXACTLY untouched.

  MEASURED RESULT (sigma_scale 0.165; defaults scale 1.5, rear 1.0, gate [0.15,0.25]):
    AVOID vx_rmse 0.520 -> 0.4765 (accel 0.479->0.441, brake 0.784->0.710); closes 15.4% of the
      pwr3 -> 0.235 floor gap. STABLE (finite, 0 NaN, max|vx| 11.8 m/s, deterministic).
    DRIFT beta@24 p90 0.0323 -> 0.0324, true-vx 0.0368 -> 0.0369, drift vx_rmse 0.2756 -> 0.2736 --
      i.e. DRIFT IS PRESERVED (within noise) by the steer gate.
  HONEST VERDICT: Approach C is a REAL, drift-safe, measured avoid-faithful improvement, but a PARTIAL
  one -- it recovers only ~15% of the residual. The probe stands: most of the +660 N is the FWD-vs-rear
  drive DISTRIBUTION x combined-slip coupling, which a planar rear-drive kludge cannot fully reproduce
  with an additive body-x drag. C is a viable INCREMENTAL T2 rung, not a full close.

----------------------------------------------------------------------------------------------------
Inherited pwr3 docstring (the gear-SEED fix + traction cap; unchanged core):
...
class PhysParams:
    ...
    # APPROACH C: scale on the cornering-induced longitudinal drag (velocity-frame projection of
    # the lateral tyre force, -Fy*sin(alpha) per axle). 1.0 = the full physical induced drag the
    # TMeasy tyre carries; 0.0 reproduces pwr3 exactly. NOT fit -- the term itself is the measured
    # tyre geometry (Fy and alpha are both model state); the scale is a single sensitivity knob.
    # DEFAULT 1.5: the measured velocity-frame induced drag, scaled to the avoid-faithful operating
    # point (the saturation of the avoid-vx gain; see the sweep in the module header). The drift
    # regime is held EXACTLY by the steer gate below, so this scale only sets the avoid magnitude.
    corner_drag_scale: float = 1.5
    # REAR-axle induced-drag scale. Physically both axles dissipate cornering drag; with the steer
    # gate active the rear contribution only fires in the high-steer avoid cornering (NOT in drift,
    # where the rear runs at huge slip), so the full physical front+rear drag is safe. Default 1.0.
    corner_drag_rear_scale: float = 1.0
    # STEER gate on the induced drag: smoothstep that ramps the drag in over [steer_lo, steer_hi] of
    # |steer| (rad). The avoid lane-change corners at |steer| 0.15-0.42 rad; the drift beta-gate
    # step sits at |steer| ~0.08 rad (drift |steer| p90 ~0.14), so a gate over [0.15, 0.25] lets the
    # induced drag fire in the avoid cornering while leaving the drift regime AND straight-line
    # cruise UNTOUCHED. This is the ONE non-pure-measurement choice -- a regime SELECTOR (where the
    # measured tyre induced drag applies), not a fit magnitude. lo=hi=0 disables the gate.
    corner_drag_steer_lo: float = 0.15
    corner_drag_steer_hi: float = 0.25
...
_PARAM_KEYS = (
    ...
    "rolling_resist_coeff", "drag_coeff", "h_cg_scale", "drive_scale", "corner_drag_scale",
    "corner_drag_rear_scale", "corner_drag_steer_lo", "corner_drag_steer_hi",
)
...
def _accel_from_forces(vx, vy, yaw_rate, steer, Fx_f, Fy_f, Fx_r, Fy_r, P: PhysParamBatch,
    ...
    APPROACH C (cornering longitudinal coupling / induced drag): pwr3 resolves the front tyre
    force into the BODY (heading) frame via the steer rotation and applies the rear force directly
    in body-x, so the only cornering-longitudinal term it carries is the front HEADING projection
    -Fy_f*sin(steer). The physically-real cornering-INDUCED DRAG -- the lateral tyre force
    projected onto the axle's VELOCITY direction (each tyre operates at a slip angle alpha, so its
    lateral force has a component -Fy*sin(alpha) opposing travel) -- is MISSING. This is the
    combined-slip longitudinal coupling Chrono's TMeasy tyre carries on BOTH axles: measured on the
    avoid down-ramp Chrono decelerates ~0.8 m/s^2 (~1.16 kN) MORE than the model at large steer
    (the over-speed that grows monotonically with |steer|: +0.17 m/s straight -> +0.66 m/s at
    |steer|>0.3). We add the velocity-frame induced drag of each axle's lateral force:

        dFx_induced = -corner_drag_scale * ( Fy_f*sin(alpha_f) + Fy_r*sin(alpha_r) )

    where alpha_f/alpha_r are the (lagged) per-axle slip angles. This is ZERO at zero slip (drift
    powertrain and straight-line cruise unchanged) and OPPOSES forward motion in the turn (it is a
    drag, always >= 0 power dissipation). It is a body-x force added alongside the existing front
    heading projection -Fy_f*sin(steer); the two are distinct (heading rotation vs slip-angle
    velocity-frame drag) so this is NOT double counting. corner_drag_scale defaults to 1.0 (the
    full measured velocity-frame induced drag)."""
    cs = torch.cos(steer)
    sn = torch.sin(steer)
    ...
    # APPROACH C: cornering-induced longitudinal drag (velocity-frame projection of the lateral
    # tyre force on each axle). Magnitude per axle = |Fy*sin(alpha)| (lateral force projected onto
    # the slip-angle velocity direction); it ALWAYS opposes the direction of travel (a pure
    # dissipative drag), so we sign it by -sign(vx). Guarded to zero if slip angles were not
    # supplied (signature parity / quasi-static call sites).
    if alpha_f is None or alpha_r is None:
        Fx_induced = torch.zeros_like(vx)
    else:
        cd = P["corner_drag_scale"]
        cdr = P["corner_drag_rear_scale"]
        drag_mag = (Fy_f * torch.sin(alpha_f)).abs() + cdr * (Fy_r * torch.sin(alpha_r)).abs()
        # optional smoothstep STEER gate (regime selector): 0 below lo, 1 above hi, smooth between.
        lo = P["corner_drag_steer_lo"]
        hi = P["corner_drag_steer_hi"]
        gate_on = (hi > lo + 1e-6)
        u = ((steer.abs() - lo) / (hi - lo).clamp_min(1e-6)).clamp(0.0, 1.0)
        gate = torch.where(gate_on, u * u * (3.0 - 2.0 * u), torch.ones_like(u))
        Fx_induced = -cd * gate * drag_mag * torch.sign(vx)

    Fx_body = Fx_f_body + Fx_r - drag - rolling + Fx_induced
...
def _continuous_derivs(vx, vy, yaw_rate, steer, omega_rl, omega_rr, throttle, brake, gear,
    ...
    # APPROACH C: pass the (advanced, lagged) per-axle slip angles so the cornering-induced
    # longitudinal drag -Fy*sin(alpha) is added in the velocity frame (see _accel_from_forces).
    vx_dot, vy_dot, yaw_dot, ax_body, ay_body = _accel_from_forces(
        vx, vy, yaw_rate, steer, Fx_f, Fy_f, Fx_r, Fy_r, P,
        alpha_f=alpha_f_lag, alpha_r=alpha_r_lag,
    )
