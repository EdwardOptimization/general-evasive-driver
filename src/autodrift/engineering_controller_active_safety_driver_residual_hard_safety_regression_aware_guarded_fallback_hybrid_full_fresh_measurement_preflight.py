"""Run M3137 residual regression-aware guarded fallback hybrid full-fresh measurement."""
...
from autodrift.engineering_controller_active_safety_driver_residual_hard_safety_regression_aware_guarded_fallback_hybrid_materialization_preflight import (
    ACTION_COMPONENTS,
    OUTPUT_SEMANTICS,
    POLICY_CONFIG,
    POLICY_ID,
    regression_aware_guarded_fallback_hybrid_action,
)
from autodrift.high_fidelity_interface import ACTION_DIM, P0_OBSERVATION_DIM


MILESTONE_ID = (
    "m3137-engineering-controller-active-safety-driver-residual-hard-safety-regression-aware-"
    "guarded-fallback-hybrid-full-fresh-measurement-preflight"
)
NEXT_ID = (
...
M3136_ID = (
    "m3136-engineering-controller-active-safety-driver-residual-hard-safety-regression-aware-"
    "guarded-fallback-hybrid-materialization-result-audit"
)
M3135_ID = (
    "m3135-engineering-controller-active-safety-driver-residual-hard-safety-regression-aware-"
    "guarded-fallback-hybrid-materialization-preflight"
)
M3105_ID = (
...
DEFAULT_M3136_AUDIT = Path(f"docs/{M3136_ID}.md")
DEFAULT_M3135_DIR = Path(
...
DEFAULT_OUTPUT_DIR = Path(
    "runs/m3137_engineering_controller_active_safety_driver_residual_hard_safety_"
    "regression_aware_guarded_fallback_hybrid_full_fresh_measurement_preflight"
)
...
CLAIM_SCOPE = (
    "M3137 Active Safety Driver residual regression-aware guarded fallback hybrid full-fresh "
    "measurement preflight only; the complete M3084 fresh current-sim denominator may be "
    "executed through the M3135 direct-action function as the full obs72-to-action3 action "
    "source and measurement, same-row comparison against M3105 M3095 M3100 and M3090, "
    "contract, claim, gate, doc, and M3138 audit artifacts may be written. No validation, "
...
class M3137RepairMeasurementPolicy:
    """Policy adapter that calls the M3135 direct-action repair function."""
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        del info
        action = regression_aware_guarded_fallback_hybrid_action(observation, POLICY_CONFIG)
        self.step_count += 1
...
def load_sources(
    *,
    ...
    paths = {
        "m3136_audit": m3136_audit,
        "m3135_summary": m3135_dir / "summary.json",
        "m3135_policy_config": m3135_dir / "direct_action_policy_config.json",
        "m3135_gate_rows": m3135_dir / "gate_matrix.csv",
        "m3135_guarded_hybrid_rule_rows": m3135_dir / "guarded_hybrid_rule_rows.csv",
        "m3135_runtime_contract_rows": m3135_dir / "runtime_contract_rows.csv",
        "m3135_actor_input_exclusion_rows": m3135_dir / "actor_input_exclusion_rows.csv",
    ...
    return {
        ...
        "m3136_audit_text": paths["m3136_audit"].read_text(encoding="utf-8") if exists["m3136_audit"] else "",
        "m3135_summary": read_json(paths["m3135_summary"]) if exists["m3135_summary"] else {},
        "m3135_policy_config": read_json(paths["m3135_policy_config"]) if exists["m3135_policy_config"] else {},
        "m3135_gate_rows": read_csv_rows(paths["m3135_gate_rows"]),
        "m3135_guarded_hybrid_rule_rows": read_csv_rows(paths["m3135_guarded_hybrid_rule_rows"]),
        "m3135_runtime_contract_rows": read_csv_rows(paths["m3135_runtime_contract_rows"]),
        "m3135_actor_input_exclusion_rows": read_csv_rows(paths["m3135_actor_input_exclusion_rows"]),
...
def full_fresh_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(plan, start=1):
        row["runtime_smoke_episode_id"] = f"m3137-measurement-episode-{index:04d}"
    return plan


def _with_scope(rows: list[dict[str, Any]], *, id_prefix: str = "m3137") -> list[dict[str, Any]]:
    updated = []
    for row in rows:
        item = dict(row)
        ...
        if "policy" in item:
            item["policy"] = "active_safety_driver_residual_hard_safety_regression_aware_guarded_fallback_hybrid_measurement"
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3137 plan row failed pre-execution guards")
            ...
            policy = M3137RepairMeasurementPolicy()
            try:
                ...
                row = m3088.run_episode_with_policy(
                    env,
                    policy,
                    "active_safety_driver_residual_hard_safety_regression_aware_guarded_fallback_hybrid_measurement",
                    int(plan["eval_seed"]),
                )
            finally:
                env.close()
...
def same_row_comparison_rows(
    ...
    for episode in episodes:
        ...
        for baseline_id in BASELINE_IDS:
            ...
            rows.append(
                {
                    "comparison_id": f"m3137-same-row-comparison-{len(rows) + 1:04d}",
                    ...
                    "m3137_outcome_bucket": episode.get("outcome_bucket", ""),
                    ...
                    "m3137_min_clearance_margin": episode.get("min_clearance_margin", ""),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3137 residual regression-aware guarded fallback hybrid full-fresh measurement artifacts before any validation ranking promotion driver-performance current-sim high-fidelity full-driver repair-success robustness-result or self-ID claim.",
        "lineage": {
            "parent_checkpoint": [
                str(doc_path),
                "runs/m3135_engineering_controller_active_safety_driver_residual_hard_safety_regression_aware_guarded_fallback_hybrid_materialization_preflight/direct_action_policy_config.json",
            ],
            "parent_dataset": [
            ...
            "parent_objective": [
                "audit full-fresh M3135 residual regression-aware guarded fallback hybrid repair measurement before broader interpretation"
            ],
            "derived_from": [MILESTONE_ID, M3136_ID, M3135_ID, M3105_ID, M3095_ID, M3100_ID],
            "blocked_by": [
                "M3137 full-fresh measurement rows require audit before any validation or repair-success route",
                "same-row comparison against M3105 M3095 M3100 and M3090 is measurement context and not a performance verdict before M3138",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "residual_hard_safety_regression_aware_guarded_fallback_hybrid_full_fresh_measurement_result_audit",
            "evidence_increment": "audits full-fresh current-sim measurement rows and same-row comparison from the M3135 residual regression-aware guarded fallback hybrid direct-action repair",
            ...
            "stop_condition": [
                "stop if M3137 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "synthesize if M3138 cannot select one next route",
            ],
            "fallback_plan": [
                "route to M3135 artifact repair if artifacts are incomplete or contract-unsafe",
                "route to behavior repair synthesis if measurement is behavior-negative but contract-safe",
                "route to validation planning only after M3138 accepts artifact completeness and claim boundaries",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3137 completes full-fresh M3135 residual repair measurement preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3137 full-fresh residual regression-aware guarded fallback hybrid repair measurement artifacts",
            "admission_evidence": ["M3137 summary gate matrix comparison measurement metric contract and claim artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3138 status queue scoreboard research log and review",
                "one follow-up manifest only if M3138 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3138 accepts or rejects M3137 as complete and claim-safe",
                "next behavior synthesis validation-planning stop or repair route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3138 audits engineering runtime-measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3138; self-ID/GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3137 full-fresh residual repair measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3137 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits complete fresh-denominator environment-loop evidence through the M3135 residual regression-aware guarded fallback hybrid direct-action repair",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3138 audits engineering runtime measurement evidence",
            "must_synthesize_if": [
                "M3138 cannot accept M3137 as complete and claim-safe",
                "M3138 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict robustness-result or self-ID evidence",
                "M3138 cannot select behavior synthesis validation planning stop or next repair route",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M3138 audits M3137 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
        "commands": [
            {
                "name": "active_safety_driver_regression_aware_guarded_fallback_hybrid_full_fresh_measurement_result_audit_doc",
                "command": "true",
            }
        ],
...
def gate_matrix_rows(
    *,
    ...
    config = source["m3135_policy_config"]
    audit_accepts = "accept_m3135_guarded_fallback_hybrid_materialization_route_to_m3137_full_fresh_measurement" in str(
        source.get("m3136_audit_text", "")
    )
    ...
    return [
        ...
        gate("m3136_accepts_m3137_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        gate("m3135_status_pass", "lineage", _bool(source["m3135_summary"].get("status_pass", False)), source["m3135_summary"].get("status_pass"), True, "lineage_invalid"),
        gate("m3135_gate_matrix_pass", "lineage", _bool(source["m3135_summary"].get("gate_matrix_pass", False)), source["m3135_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
        gate("m3135_policy_config_status", "lineage", str(config.get("policy_id", "")) == POLICY_ID, config.get("policy_id"), POLICY_ID, "lineage_invalid"),
        gate("m3135_gate_rows_pass", "lineage", all(_bool(row.get("status_pass", False)) for row in source.get("m3135_gate_rows", [])), len(source.get("m3135_gate_rows", [])), "all pass", "lineage_invalid"),
        gate(
            "m3135_guarded_hybrid_rule_rows_present",
            "lineage",
            len(source.get("m3135_guarded_hybrid_rule_rows", [])) >= 8,
            len(source.get("m3135_guarded_hybrid_rule_rows", [])),
            ">=8",
            "lineage_invalid",
        ),
        gate(
            "m3135_runtime_contract_rows_pass",
            "lineage",
            len(source.get("m3135_runtime_contract_rows", [])) >= 4
            and all(_bool(row.get("status_pass", False)) for row in source.get("m3135_runtime_contract_rows", [])),
            len(source.get("m3135_runtime_contract_rows", [])),
            ">=4 rows all pass",
            "lineage_invalid",
        ),
        gate(
            "m3135_actor_input_exclusion_rows_pass",
            "lineage",
            len(source.get("m3135_actor_input_exclusion_rows", [])) >= 10
            and all(_bool(row.get("status_pass", False)) for row in source.get("m3135_actor_input_exclusion_rows", [])),
            len(source.get("m3135_actor_input_exclusion_rows", [])),
            ">=10 rows all pass",
            "lineage_invalid",
        ),
...
def contract_guard_rows(
    *,
    ...
    sample_action = regression_aware_guarded_fallback_hybrid_action(
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3138 audit manifest"),
    ]
    blocked = [
    ...
    rows = [
        {
            "claim_id": f"m3137-{claim_id}",
            "claim_family": family,
            "allowed_in_m3137": True,
            "claim_made": made,
            "status_pass": made,
...
def run_full_fresh_measurement_preflight(
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "result_class": (
            "active_safety_driver_residual_hard_safety_regression_aware_guarded_fallback_hybrid_full_fresh_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_residual_hard_safety_regression_aware_guarded_fallback_hybrid_full_fresh_measurement_preflight_fail"
        ),
        ...
        "required_artifacts_present": present,
        "m3135_status_pass": _bool(source["m3135_summary"].get("status_pass", False)),
        "m3135_gate_matrix_pass": _bool(source["m3135_summary"].get("gate_matrix_pass", False)),
        ...
        "checkpoint_model_required": False,
        "recurrent_hidden_state_required": False,
        "direct_action_formula": "action = regression_aware_guarded_fallback_hybrid_action(obs72) -> [steer, throttle, brake]",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3136-audit", type=Path, default=DEFAULT_M3136_AUDIT)
    parser.add_argument("--m3135-dir", type=Path, default=DEFAULT_M3135_DIR)
...
def main() -> None:
    ...
    summary = run_full_fresh_measurement_preflight(
        m3136_audit=args.m3136_audit,
        m3135_dir=args.m3135_dir,
