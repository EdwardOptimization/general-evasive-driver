"""Run M3120 residual trajectory authority and stability recovery repair full-fresh measurement."""
...
from autodrift.engineering_controller_active_safety_driver_residual_trajectory_authority_stability_recovery_repair_materialization_preflight import (
    M3118_POLICY_CONFIG,
    POLICY_ID,
    residual_trajectory_authority_stability_recovery_direct_action,
)
...
NEXT_ID = (
    "m3121-engineering-controller-active-safety-driver-residual-trajectory-authority-"
    "stability-recovery-repair-full-fresh-measurement-result-audit"
)
M3119_ID = (
...
DEFAULT_M3119_AUDIT = Path(f"docs/{M3119_ID}.md")
...
CLAIM_SCOPE = (
    "M3120 Active Safety Driver residual trajectory authority and stability recovery repair full-fresh "
    "measurement preflight only; the complete M3084 fresh current-sim denominator may be "
    "executed through the M3118 direct-action function as the full obs72-to-action3 action "
    "source and measurement, same-row comparison against M3105 M3095 M3100 and M3090, "
    "contract, claim, gate, doc, and M3121 audit artifacts may be written. No validation, "
...
class M3120RepairMeasurementPolicy:
    """Policy adapter that calls the M3118 direct-action repair function."""
    ...
    def act(self, observation: np.ndarray, info: dict[str, Any]) -> np.ndarray:
        del info
        action = residual_trajectory_authority_stability_recovery_direct_action(observation, M3118_POLICY_CONFIG)
        self.step_count += 1
...
def load_sources(
    *,
    ...
    paths = {
        "m3119_audit": m3119_audit,
        "m3118_summary": m3118_dir / "summary.json",
        "m3118_policy_config": m3118_dir / "direct_action_policy_config.json",
        "m3118_gate_rows": m3118_dir / "gate_matrix.csv",
        "m3118_safety_reflex_rule_rows": m3118_dir / "safety_reflex_rule_rows.csv",
        "m3118_residual_trace_requirement_rows": m3118_dir / "residual_trace_requirement_rows.csv",
        "m3118_actor_input_exclusion_rows": m3118_dir / "actor_input_exclusion_rows.csv",
    ...
    return {
        ...
        "m3119_audit_text": paths["m3119_audit"].read_text(encoding="utf-8") if exists["m3119_audit"] else "",
        "m3118_summary": read_json(paths["m3118_summary"]) if exists["m3118_summary"] else {},
        "m3118_policy_config": read_json(paths["m3118_policy_config"]) if exists["m3118_policy_config"] else {},
        "m3118_gate_rows": read_csv_rows(paths["m3118_gate_rows"]),
        "m3118_safety_reflex_rule_rows": read_csv_rows(paths["m3118_safety_reflex_rule_rows"]),
        "m3118_residual_trace_requirement_rows": read_csv_rows(paths["m3118_residual_trace_requirement_rows"]),
        "m3118_actor_input_exclusion_rows": read_csv_rows(paths["m3118_actor_input_exclusion_rows"]),
...
def full_fresh_plan(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(plan, start=1):
        row["runtime_smoke_episode_id"] = f"m3120-measurement-episode-{index:04d}"
    return plan


def _with_scope(rows: list[dict[str, Any]], *, id_prefix: str = "m3120") -> list[dict[str, Any]]:
    updated = []
    for row in rows:
        item = dict(row)
        ...
        if "policy" in item:
            item["policy"] = "active_safety_driver_residual_trajectory_authority_stability_recovery_repair_measurement"
    ...
    for plan in plan_rows:
        try:
            if not _bool(plan.get("status_pass", False)):
                raise ValueError("M3120 plan row failed pre-execution guards")
            ...
            policy = M3120RepairMeasurementPolicy()
            try:
                ...
                row = m3088.run_episode_with_policy(
                    env,
                    policy,
                    "active_safety_driver_residual_trajectory_authority_stability_recovery_repair_measurement",
                    int(plan["eval_seed"]),
                )
            finally:
                env.close()
...
def same_row_comparison_rows(
    ...
    for episode in episodes:
        ...
        for baseline_id in BASELINE_IDS:
            ...
            rows.append(
                {
                    "comparison_id": f"m3120-same-row-comparison-{len(rows) + 1:04d}",
                    ...
                    "m3120_outcome_bucket": episode.get("outcome_bucket", ""),
                    ...
                    "m3120_min_clearance_margin": episode.get("min_clearance_margin", ""),
...
def build_follow_up_manifest(*, output_dir: Path, doc_path: Path) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "A bounded result audit can accept or reject the M3120 residual trajectory authority and stability recovery full-fresh measurement artifacts before any validation ranking promotion driver-performance current-sim high-fidelity full-driver repair-success robustness-result or self-ID claim.",
        "lineage": {
            "parent_checkpoint": [
                str(doc_path),
                "runs/m3118_engineering_controller_active_safety_driver_residual_trajectory_authority_stability_recovery_repair_materialization_preflight/direct_action_policy_config.json",
            ],
            "parent_dataset": [
            ...
            "parent_objective": [
                "audit full-fresh M3118 residual trajectory-authority stability-recovery repair measurement before broader interpretation"
            ],
            "derived_from": [MILESTONE_ID, M3119_ID, M3118_ID, M3105_ID, M3095_ID, M3100_ID],
            "blocked_by": [
                "M3120 full-fresh measurement rows require audit before any validation or repair-success route",
                "same-row comparison against M3105 M3095 M3100 and M3090 is measurement context and not a performance verdict before M3121",
            ],
        ...
        "workflow_synthesis": {
            ...
            "evidence_axis": "residual_trajectory_authority_stability_recovery_full_fresh_measurement_result_audit",
            "evidence_increment": "audits full-fresh current-sim measurement rows and same-row comparison from the M3118 residual trajectory-authority stability-recovery direct-action repair",
            ...
            "stop_condition": [
                "stop if M3120 artifacts are missing or gate matrix fails",
                "stop if actor or direct-action contracts were violated",
                "synthesize if M3121 cannot select one next route",
            ],
            "fallback_plan": [
                ...
                "route to validation planning only after M3121 accepts artifact completeness and claim boundaries",
            ],
            "synthesis_cadence": 10,
            "synthesis_trigger": "M3120 completes full-fresh M3118 residual repair measurement preflight",
            "synthesis_decision": "not_applicable",
        },
        "training_stage": {
            "stage": "process",
            "stage_objective": "Audit M3120 full-fresh residual trajectory-authority stability-recovery repair measurement artifacts",
            "admission_evidence": ["M3120 summary gate matrix comparison measurement metric contract and claim artifacts"],
            "blocked_shortcuts": [
            ...
            "allowed_updates": [
                ...
                "M3121 status queue scoreboard research log and review",
                "one follow-up manifest only if M3121 selects exactly one next route",
            ],
            "next_stage_criteria": [
                "M3121 accepts or rejects M3120 as complete and claim-safe",
                "next behavior synthesis validation-planning stop or repair route is explicit",
            ],
        },
        "self_id_evidence_discipline": {
            "claim_level": "not_applicable",
            "current_frame_substitution_risk": "M3121 audits engineering runtime-measurement artifacts and cannot infer history necessity or self-ID.",
            "history_necessity_tests": ["None in M3121; self-ID/GRU comparisons remain auxiliary diagnostics only."],
            "temporal_evidence_window": "M3120 full-fresh residual repair measurement artifacts only.",
            ...
            "allowed_claims": [
                "M3120 artifact completeness and claim-safety audit",
        ...
        "local_search_guard": {
            ...
            "evidence_expansion": "audits complete fresh-denominator environment-loop evidence through the M3118 residual trajectory-authority stability-recovery direct-action repair",
            "paper_verdict_delta": "paper and self-ID remain diagnostic; M3121 audits engineering runtime measurement evidence",
            "must_synthesize_if": [
                "M3121 cannot accept M3120 as complete and claim-safe",
                "M3121 would claim validation driver-performance paper high-fidelity finite-window-vs-GRU current-sim verdict robustness-result or self-ID evidence",
                "M3121 cannot select behavior synthesis validation planning stop or next repair route",
            ],
        },
        "success_criteria": [
        ...
        "decision_rule": "Pass only if M3121 audits M3120 artifacts and selects one next route or stop state while preserving actor direct-action and claim boundaries without overclaiming.",
...
def gate_matrix_rows(
    *,
    ...
    config = source["m3118_policy_config"]
    audit_accepts = "accept_m3118_materialization_route_to_m3120_full_fresh_measurement" in str(
        source.get("m3119_audit_text", "")
    )
    ...
    return [
        ...
        gate("m3119_accepts_m3120_route", "lineage", audit_accepts, "route marker", "present", "lineage_invalid"),
        gate("m3118_status_pass", "lineage", _bool(source["m3118_summary"].get("status_pass", False)), source["m3118_summary"].get("status_pass"), True, "lineage_invalid"),
        gate("m3118_gate_matrix_pass", "lineage", _bool(source["m3118_summary"].get("gate_matrix_pass", False)), source["m3118_summary"].get("gate_matrix_pass"), True, "lineage_invalid"),
        gate("m3118_policy_config_status", "lineage", str(config.get("policy_id", "")) == POLICY_ID, config.get("policy_id"), POLICY_ID, "lineage_invalid"),
        gate("m3118_gate_rows_pass", "lineage", all(_bool(row.get("status_pass", False)) for row in source.get("m3118_gate_rows", [])), len(source.get("m3118_gate_rows", [])), "all pass", "lineage_invalid"),
        gate(
            "m3118_safety_reflex_rule_rows_present",
            "lineage",
            len(source.get("m3118_safety_reflex_rule_rows", [])) >= 6,
            len(source.get("m3118_safety_reflex_rule_rows", [])),
            ">=6",
            "lineage_invalid",
        ),
        gate(
            "m3118_residual_trace_requirement_rows_pass",
            "lineage",
            len(source.get("m3118_residual_trace_requirement_rows", [])) == 7
            and all(_bool(row.get("status_pass", False)) for row in source.get("m3118_residual_trace_requirement_rows", [])),
            len(source.get("m3118_residual_trace_requirement_rows", [])),
            "7 rows all pass",
            "lineage_invalid",
        ),
        gate(
            "m3118_actor_input_exclusion_rows_pass",
            "lineage",
            len(source.get("m3118_actor_input_exclusion_rows", [])) >= 10
            and all(_bool(row.get("status_pass", False)) for row in source.get("m3118_actor_input_exclusion_rows", [])),
            len(source.get("m3118_actor_input_exclusion_rows", [])),
            ">=10 rows all pass",
            "lineage_invalid",
        ),
...
def render_doc(summary: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            "# M3120 Residual Trajectory-Authority Stability-Recovery Full-Fresh Measurement Preflight",
            "",
            "## Summary",
            "",
            ...
            "## Interpretation",
            "",
            "M3120 records full-fresh current-sim rows through the M3118 residual trajectory authority and stability recovery direct-action repair function and writes same-row deltas against M3105, M3095, M3100, and M3090. These are measurement and audit-input artifacts for M3121 only. They are not validation, ranking, promotion, repair-success, robustness-result, driver-performance, current-sim verdict, high-fidelity, paper, finite-window-vs-GRU, full-driver, or self-ID evidence.",
            "",
            "Rejected claims:",
            "",
            "```text",
...
def contract_guard_rows(
    *,
    ...
    sample_action = residual_trajectory_authority_stability_recovery_direct_action(
...
def claim_boundary_rows(*, follow_up_manifest_registered: bool) -> list[dict[str, Any]]:
    allowed = [
        ...
        ("follow_up_result_audit_registered", "follow_up_route", follow_up_manifest_registered, "M3121 audit manifest"),
    ]
    blocked = [
    ...
    rows = [
        {
            "claim_id": f"m3120-{claim_id}",
            "claim_family": family,
            "allowed_in_m3120": True,
            "claim_made": made,
            "status_pass": made,
...
def run_full_fresh_measurement_preflight(
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "result_class": (
            "active_safety_driver_residual_trajectory_authority_stability_recovery_repair_full_fresh_measurement_preflight_pass"
            if status_pass
            else "active_safety_driver_residual_trajectory_authority_stability_recovery_repair_full_fresh_measurement_preflight_fail"
        ),
        ...
        "required_artifacts_present": present,
        "m3118_status_pass": _bool(source["m3118_summary"].get("status_pass", False)),
        "m3118_gate_matrix_pass": _bool(source["m3118_summary"].get("gate_matrix_pass", False)),
        ...
        "checkpoint_model_required": False,
        "recurrent_hidden_state_required": False,
        "direct_action_formula": "action = residual_trajectory_authority_stability_recovery_direct_action(obs72) -> [steer, throttle, brake]",
        ...
        "decision": "active_safety_driver_residual_trajectory_authority_stability_recovery_repair_full_fresh_measurement_route_to_m3121_result_audit",
        "next_blocker": NEXT_ID,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m3119-audit", type=Path, default=DEFAULT_M3119_AUDIT)
