...
- source payload: `runs/m1816_executable_v2_stable_source_reset_validation_adapter/targeted_reset_executable_v2_panel_specs.json`
- reset run: `false`
- rollout/training/replay/PPO: `false`

## Purpose

M1818 promoted the source-label compatibility branch into targeted reset
validation. M1819 pre-registers the exact M1792 reset-only command over the
M1816 converted payload. This milestone does not run reset.
...
```text
runs/m1820_executable_v2_stable_source_targeted_reset_feasibility_preflight
```
...
python -m autodrift.executable_v2_reset_feasibility_preflight \
  --executable-v2-panel-specs runs/m1816_executable_v2_stable_source_reset_validation_adapter/targeted_reset_executable_v2_panel_specs.json \
  ...
  --next-blocker m1821-executable-v2-stable-source-targeted-reset-feasibility-result-audit
```
...
M1820 passes if the adapter result class is:

```text
executable_v2_reset_feasibility_preflight_pass
```

and all expected counts match. A pass would support only targeted reset
feasibility for these 36 rows; it still would not admit measured execution or
controller-family ranking without a result audit and execution-design milestone.

## Failure Handling

If M1820 fails, the result should be audited rather than repaired in-place.
Likely failure classes:

- `scenario_sampling_failure`: some patched stable source still cannot sample
  the requested label under reset.
- `metric_artifact`: metadata/count mismatch or guardrail inconsistency.

## Guardrails

- environment reset started: `false` in M1819 design;
...
- exact targeted reset-only command and expected counts;
- M1820 execution is admitted.
