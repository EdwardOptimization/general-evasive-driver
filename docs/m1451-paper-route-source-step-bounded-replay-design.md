...
M1451 designs the first bounded replay smoke admitted by M1450.

Decision:

```text
source_step_bounded_replay_design_admit_smoke
```

M1451 does not run replay, train, run PPO, promote, use private holdout, export
...
```text
When the same source-step relocated scene is replayed from normal history and
from selected history interventions, do any M1450 rows produce terminal outcome
sensitivity?
```
...
M1452 should pass the smoke-run gate if:
