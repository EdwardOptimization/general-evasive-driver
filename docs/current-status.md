...
```text
stage4 repeat evidence from M201 seed 5204 before any further continuation
```

## Near-Term Rule

M202 is positive as one short guarded stage4, but this is only single-seed
evidence. Repeat the same stage4 recipe from M201 seed `5204` on fresh seeds.
Do not chain from M202 or run a longer PPO continuation until stage4 repeats
