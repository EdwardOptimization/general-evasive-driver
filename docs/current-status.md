...
```text
residual rows: 48
R4 proxy semantics / post-collision blocked: 12
...
M2342 designs and M2343 implements an artifact-only consolidation over the 26
redesign-related rows:

```text
original redesign rows: 12
remapped redesign rows: 14
combined redesign-related rows: 26 unique scenarios
secondary coverage-materialization rows: 9
recommended redesign split: geometry/timing 13, hidden-dynamics range 13
needs user review: 0
guardrail violations: 0
```
...
overwrite the active config or run reset/rollout validation. M2350 is now
complete:
...
G/H/G+H/GH selections: 13 / 13 / 26 / 26
...
M2351 should synthesize the dual-axis calibration branch because the harness
local-search guard triggered after M2350. The synthesis must interpret the
...
M2354 audits the M2353 failure without rerun. Classification:

```text
primary failure: scenario_sampling_failure
subtype: sampling_incompatible_candidate_transform
baseline_reference_pack failures: 0
failure by axis: G 18, GH 11, H 3
...
M2357 audits M2356 and accepts the repaired artifacts as reset-validation
design input:
...
M2302 materializes the guarded-v2 config pack:
