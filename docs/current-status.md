...
artifacts. It found `2304` input episodes, `212` grouped support rows, `13`
...
M2221 implements that artifact-only diagnosis: `2304` existing episode rows,
`9` scene candidates, profile/history/pair/L3 rows `54/36/45/9`,
...
now materialize medium-v1 configs and command matrix without training. M2233
passes that materialization: `15` configs, `15` matrix rows, `32768` total
...
candidates. M2244 runs this diagnostic localization and completes cleanly:
`480` episode rows, `15` complete profile-seed groups, missing inputs `0`, and
...
`480` public episode protocol used by M2244. M2252 freezes that protocol:
evaluate exactly the `15` M2250 selected checkpoints, `32` episodes each, using
...
synthesis, or stop. M2256 implements that no-rerun diagnosis and finds the
...
so the M1990 fail artifacts remain unchanged; it must not run reset. M1995
freezes the M1996 repaired reset-only command using output
