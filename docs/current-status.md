...
benchmark rows without training or promotion. M446 implements the miner and
...
configs to test whether they are useful self-identification diagnostics. M450
finds both M449 configs fail seed-block robustness at seed `9900`: ablation
...
M467 implements and runs that selector. It classifies `199` wrong-history rows:
`35` near-boundary candidates, `0` proof candidates, `35` near-boundary
no-effect rows, and `7` high-slack diagnostics. Near-boundary rows cover
`drift_required` and `unavoidable`, but wrong history does not degrade them;
...
M470 completes that design. The next step is an expanded same-window
matched-current mining run on seeds `10200`, `10300`, and `10400`, increasing
...
the M474 adversarial pairs. M475 rejects proof expansion: all `197`
...
clamped hidden state as deployable proof. M479 selects `wrong_late_2_once`,
`wrong_late_4_once`, `wrong_late_8_once`, and `wrong_late_12_once`; the next
blocker is M480 implementation and smoke.
