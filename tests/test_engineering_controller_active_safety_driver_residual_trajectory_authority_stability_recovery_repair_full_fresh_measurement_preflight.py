...
import autodrift.engineering_controller_active_safety_driver_residual_trajectory_authority_stability_recovery_repair_full_fresh_measurement_preflight as m3120


def test_m3120_policy_adapter_telemetry_preserves_direct_action_contract():
    policy = m3120.M3120RepairMeasurementPolicy()
    action = policy.act(np.zeros(m3120.P0_OBSERVATION_DIM, dtype=np.float32), {})
    telemetry = policy.telemetry()

    assert action.shape == (m3120.ACTION_DIM,)
    assert np.all(np.isfinite(action))
    assert np.max(np.abs(action)) <= 1.0
    assert telemetry["runtime_driver_id"] == m3120.POLICY_ID
    assert telemetry["candidate_output_semantics"] == m3120.OUTPUT_SEMANTICS
...
def test_same_row_comparison_records_four_baselines_without_claims():
    episode = {
        "runtime_smoke_episode_id": "m3120-measurement-episode-0001",
    ...
    rows = m3120.same_row_comparison_rows(
        [episode],
        [m3105_baseline],
        [m3095_baseline],
        [m3100_baseline],
    ...
    assert [row["comparison_id"] for row in rows] == [
        "m3120-same-row-comparison-0001",
        "m3120-same-row-comparison-0002",
        "m3120-same-row-comparison-0003",
        "m3120-same-row-comparison-0004",
    ]
...
def test_claim_boundary_blocks_repair_success_and_requires_m3121_audit():
    rows = m3120.claim_boundary_rows(follow_up_manifest_registered=True)
    by_id = {row["claim_id"]: row for row in rows}

    assert by_id["m3120-follow_up_result_audit_registered"]["allowed_in_m3120"] is True
    assert by_id["m3120-follow_up_result_audit_registered"]["claim_made"] is True
    assert by_id["m3120-repair_success"]["allowed_in_m3120"] is False
    assert by_id["m3120-repair_success"]["claim_made"] is False
    assert by_id["m3120-driver_performance_verdict"]["claim_made"] is False
    assert all(row["status_pass"] for row in rows)


def test_follow_up_manifest_is_result_audit_not_validation(tmp_path):
    manifest = m3120.build_follow_up_manifest(
        output_dir=tmp_path / "m3120",
        doc_path=tmp_path / "m3120.md",
    )

    assert manifest["id"] == m3120.NEXT_ID
