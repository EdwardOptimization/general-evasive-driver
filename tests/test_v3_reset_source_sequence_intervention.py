...
from autodrift.v3_reset_source_sequence_intervention import (
    classify_v3_reset_sequence_result,
    load_v3_reset_source_rows,
)
...
def test_load_v3_reset_source_rows_selects_primary_and_sentinel(tmp_path):
    reset_rows = tmp_path / "reset.csv"
    rejected_rows = tmp_path / "rejected.csv"
    _write(reset_rows, [_row(pair_id=1), _row(pair_id=2, seed=73001, reset_action_l2_gap=0.001)])
    _write(
        rejected_rows,
        [
            _row(
    ...
    rows = load_v3_reset_source_rows(
    ...
    assert rows[1]["source_pool"] == "m740_history_insensitive"


def test_classify_v3_reset_sequence_result_maps_base_classes():
    assert (
        classify_v3_reset_sequence_result(result_class="sequence_outcome_positive")
        == "v3_reset_sequence_outcome_positive"
    )
    assert classify_v3_reset_sequence_result(result_class="sequence_action_only") == "v3_reset_sequence_action_only"
