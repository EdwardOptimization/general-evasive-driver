...
from autodrift.source_history_directional_feasibility_probe import run_directional_feasibility_probe
from test_source_history_policy_gate import _history_frame, _write_checkpoint, _write_csv


def test_run_directional_feasibility_probe_writes_artifacts(tmp_path):
    ...
    assert (run_dir / "directional_feasibility_rows.csv").exists()
    assert (run_dir / "train_trace.csv").exists()
    assert (run_dir / "checkpoints" / "base_init_directional_candidate.pt").exists()
    assert (run_dir / "checkpoints" / "m1288_init_directional_candidate.pt").exists()
