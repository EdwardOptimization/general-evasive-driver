...
import autodrift.engineering_controller_active_safety_driver_v1_offtrack_dominant_behavior_direct_action_closed_loop_measurement_preflight as m3067
...
def test_direct_action_policy_outputs_clipped_action_without_base_policy() -> None:
    ...
    policy = m3067.DirectActionActorPolicy(
        weight=weight,
        bias=bias,
        action_low=np.full((3,), -1.0, dtype=np.float32),
        action_high=np.full((3,), 1.0, dtype=np.float32),
...
def test_load_direct_action_artifact_rejects_runtime_base_policy_dependency(tmp_path: Path) -> None:
    artifact_path = tmp_path / "candidate_direct_action_reflex_layer.npz"
    _write_direct_action_artifact(artifact_path, base_policy_required=True)

    artifact = m3067.load_direct_action_artifact(artifact_path)

    assert artifact["contract_pass"] is False
    assert artifact["base_policy_required_at_runtime"] is True


def test_load_direct_action_artifact_accepts_m3065_contract(tmp_path: Path) -> None:
    artifact_path = tmp_path / "candidate_direct_action_reflex_layer.npz"
    _write_direct_action_artifact(artifact_path)

    artifact = m3067.load_direct_action_artifact(artifact_path)
    guards = m3067.direct_action_adapter_guard_rows(artifact, [])
    ...
    assert all(m3067._bool(row["status_pass"]) for row in guards)
...
def test_workload_plan_does_not_require_checkpoint_path(tmp_path: Path) -> None:
    ...
    plan = m3067.workload_plan(source)

    assert len(plan) == 1
    assert plan[0]["status_pass"] is True
    assert plan[0]["runtime_base_policy_required"] is False
    assert plan[0]["direct_action_profile_name"] == "route_a_candidate+m3065_direct_action"
