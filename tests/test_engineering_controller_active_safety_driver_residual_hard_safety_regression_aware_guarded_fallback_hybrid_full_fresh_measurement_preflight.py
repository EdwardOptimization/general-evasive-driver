...
import autodrift.engineering_controller_active_safety_driver_residual_hard_safety_regression_aware_guarded_fallback_hybrid_full_fresh_measurement_preflight as m3137


def test_m3137_policy_adapter_telemetry_preserves_direct_action_contract():
    policy = m3137.M3137RepairMeasurementPolicy()
    action = policy.act(np.zeros(m3137.P0_OBSERVATION_DIM, dtype=np.float32), {})
    telemetry = policy.telemetry()

    assert action.shape == (m3137.ACTION_DIM,)
    assert np.all(np.isfinite(action))
    assert np.max(np.abs(action)) <= 1.0
    assert telemetry["runtime_driver_id"] == m3137.POLICY_ID
    assert telemetry["candidate_output_semantics"] == m3137.OUTPUT_SEMANTICS
...
def test_same_row_comparison_records_four_baselines_without_claims():
    episode = {
        "runtime_smoke_episode_id": "m3137-measurement-episode-0001",
    ...
    rows = m3137.same_row_comparison_rows(
        [episode],
        [m3105_baseline],
        [m3095_baseline],
        [m3100_baseline],
    ...
    assert [row["comparison_id"] for row in rows] == [
        "m3137-same-row-comparison-0001",
        "m3137-same-row-comparison-0002",
        "m3137-same-row-comparison-0003",
        "m3137-same-row-comparison-0004",
    ]
...
def test_claim_boundary_blocks_repair_success_and_requires_m3138_audit():
    rows = m3137.claim_boundary_rows(follow_up_manifest_registered=True)
    by_id = {row["claim_id"]: row for row in rows}

    assert by_id["m3137-follow_up_result_audit_registered"]["allowed_in_m3137"] is True
    assert by_id["m3137-follow_up_result_audit_registered"]["claim_made"] is True
    assert by_id["m3137-repair_success"]["allowed_in_m3137"] is False
    assert by_id["m3137-repair_success"]["claim_made"] is False
    assert by_id["m3137-driver_performance_verdict"]["claim_made"] is False
    assert all(row["status_pass"] for row in rows)


def test_follow_up_manifest_is_result_audit_not_validation(tmp_path):
    manifest = m3137.build_follow_up_manifest(
        output_dir=tmp_path / "m3137",
        doc_path=tmp_path / "m3137.md",
    )

    assert manifest["id"] == m3137.NEXT_ID
    assert manifest["gate_tier"] == "process"
    assert manifest["training_stage"]["stage"] == "process"
    assert manifest["workflow_synthesis"]["branch"] == "active_safety_driver_residual_regression_aware_guarded_fallback_hybrid"
...
def test_gate_matrix_accepts_m3136_guarded_fallback_hybrid_route():
    source = {
        "source_exists": {"m3136_audit": True},
        "m3136_audit_text": "decision: accept_m3135_guarded_fallback_hybrid_materialization_route_to_m3137_full_fresh_measurement",
        "m3135_summary": {"status_pass": True, "gate_matrix_pass": True},
        "m3135_policy_config": {
            "policy_id": m3137.POLICY_ID,
            "observation_shape": m3137.P0_OBSERVATION_DIM,
            "action_shape": m3137.ACTION_DIM,
            "output_semantics": m3137.OUTPUT_SEMANTICS,
            "runtime_base_policy_required": False,
        },
        "m3135_gate_rows": [{"status_pass": True}],
        "m3135_guarded_hybrid_rule_rows": [{} for _ in range(8)],
        "m3135_runtime_contract_rows": [{"status_pass": True} for _ in range(4)],
        "m3135_actor_input_exclusion_rows": [{"status_pass": True} for _ in range(10)],
    ...
    assert by_id["m3137-m3136_accepts_m3137_route"]["status_pass"] is True
