...
from autodrift import engineering_controller_route_a_post_clearance_negative_non_same_repair_cross_axis_bounded_execution_preflight as m2807
...
def _write_source_artifacts(root: Path) -> dict[str, Path]:
    ...
    workload_rows = []
    for task_source_id, task_family, source_edge, _axis in m2807.SELECTED_TASK_SOURCES:
    ...
    m2753_prior_rows = [
        {
            ...
            "workload_id": f"m1680-spec-{index:04d}::L3_online_gru",
            "profile_name": "L3_online_gru",
            "termination_reason": "off_track",
        }
        for index in (1, 3, 8, 10, 37, 39, 42, 43, 44, 45, 46, 47)
    ]
    write_csv_rows(m2753_dir / "candidate_execution_rows.csv", m2753_prior_rows)
    write_csv_rows(
        m2804_dir / "blocker_matrix.csv",
        [
            {
                "blocker_id": "m2804_blocker_protected_mitigation",
                "route": "Route A",
            ...
            {
                "blocker_id": "m2804_blocker_hf3_source_dependency_unavailable",
                "route": "Route C",
    ...
    write_csv_rows(
        m2804_dir / "next_action_admission_rows.csv",
    ...
    design = root / "m2806.md"
    design.write_text("M2806 design\n", encoding="utf-8")
    follow_up = root / "m2808.json"
    follow_up.write_text('{"id": "m2808"}\n', encoding="utf-8")
    return {
        "m1690": m1690,
        "m2746_dir": m2746_dir,
        "m2737_dir": m2737_dir,
...
def test_m2807_selects_non_same_panel_cross_axis_surface_and_blocks_overclaims(
    monkeypatch,
    tmp_path: Path,
) -> None:
    paths = _write_source_artifacts(tmp_path)
    output_dir = tmp_path / "m2807"
    doc_path = tmp_path / "m2807.md"
    ...
    def fake_execution(**kwargs: object) -> dict[str, object]:
        ...
        write_csv_rows(output / "candidate_execution_failure_rows.csv", [], fieldnames=m2807.FAILURE_FIELDNAMES)
        write_json(output / "run_state.json", {"complete": True, "accounted_count": len(rows)})
        return {
            "result_class": "engineering_controller_route_a_post_clearance_negative_non_same_repair_cross_axis_bounded_execution_pass",
            "all_selected_metrics_finite": True,
        }

    monkeypatch.setattr(m2807, "run_candidate_execution", fake_execution)
    summary = m2807.run_post_clearance_negative_non_same_repair_cross_axis_bounded_execution_preflight(
        m1690_workload=paths["m1690"],
        m2746_dir=paths["m2746_dir"],
        m2737_dir=paths["m2737_dir"],
        m2753_dir=paths["m2753_dir"],
        m2804_dir=paths["m2804_dir"],
        m2806_design=paths["design"],
    ...
    assert summary["prior_panel_unique_task_source_count"] == 21
    ...
    candidate_rows = _read_csv(output_dir / "non_same_repair_candidate_rows.csv")
    ...
    blocked_claims = [row for row in claim_rows if row["allowed_in_m2807"] == "False"]
