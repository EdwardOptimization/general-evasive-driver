...
from autodrift import engineering_controller_route_a_post_clearance_negative_non_same_repair_offtrack_containment_localization_panel as m2810
...
def _write_source_artifacts(root: Path) -> dict[str, Path]:
        ...
        execution_rows.append(
            {
                ...
                "min_clearance_margin": 2.0 + index,
                "return": 10.0 + index,
                "time_to_first_off_track_s": "" if success else 1.0 + index / 10.0,
    ...
    write_csv_rows(m2807_dir / "non_same_repair_candidate_rows.csv", candidate_rows)
    write_csv_rows(m2807_dir / "candidate_execution_rows.csv", execution_rows)
    write_csv_rows(
        m2807_dir / "stress_axis_aggregate_rows.csv",
        [
            {
    ...
    write_csv_rows(
        m2807_dir / "prior_surface_exclusion_rows.csv",
        [
            {
                "exclusion_id": f"m2807-prior-panel-exclusion-{index + 1:04d}",
    ...
    write_csv_rows(
        m2807_dir / "blocker_guard_rows.csv",
        [
            {
                "guard_id": f"m2807-blocker-guard-{index + 1:04d}",
    ...
    write_csv_rows(
        m2807_dir / "actor_contract_guard_rows.csv",
        [
    ...
    write_csv_rows(
        m2807_dir / "claim_boundary_rows.csv",
        [{"claim_id": "m2807-claim", "allowed_in_m2807": True, "claim_made": True, "status_pass": True}],
    )
    write_csv_rows(
        m2807_dir / "gate_matrix.csv",
        [{"gate_id": "m2807-gate", "gate_family": "artifact", "status_pass": True}],
    )
    docs = {}
    for name in ["m2809.md", "m2808.md", "route.md", "m2811.json"]:
        path = root / name
    ...
    return {
        ...
        "follow_up": docs["m2811.json"],
    }


def test_m2810_materializes_negative_failure_localization_without_overclaims(tmp_path: Path) -> None:
    paths = _write_source_artifacts(tmp_path)
    output_dir = tmp_path / "m2810"
    doc_path = tmp_path / "m2810.md"

    summary = m2810.materialize_post_clearance_negative_non_same_repair_offtrack_containment_localization_panel(
        output_dir,
        m2807_dir=paths["m2807_dir"],
        m2809_synthesis=paths["m2809"],
        m2808_audit=paths["m2808"],
    ...
    assert summary["success_obstacle_pass_count"] == 2
    assert summary["collision_negative_clearance_count"] == 0
    assert summary["offtrack_positive_clearance_count"] == 10
    assert summary["offtrack_containment_row_count"] == 10
    assert summary["guardrail_context_row_count"] == 44
    assert summary["prior_panel_guardrail_row_count"] == 37
    assert summary["blocker_guardrail_row_count"] == 7
    ...
    blocked_claims = [row for row in claim_rows if row["allowed_in_m2810"] == "False"]
    ...
    offtrack_rows = _read_csv(output_dir / "offtrack_containment_rows.csv")
    assert len(offtrack_rows) == 10
    assert {row["offtrack_noncollision"] for row in offtrack_rows} == {"True"}
    assert {row["ranking_claim_made"] for row in offtrack_rows} == {"False"}
