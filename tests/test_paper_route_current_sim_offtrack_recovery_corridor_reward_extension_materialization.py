...
from autodrift.paper_route_current_sim_offtrack_recovery_corridor_reward_extension_materialization import (
    build_repaired_config,
    materialize_reward_extension_configs,
)
...
def test_build_repaired_config_preserves_track_width_and_contract(tmp_path: Path) -> None:
    source_root = _source_configs(tmp_path)

    config = build_repaired_config(
    ...
    assert config["env"]["track_cost_scale"] == 2.8
    assert config["env"]["road_margin_cost_scale"] == 1.2
    assert config["env"]["off_track_penalty"] == 6.0
...
def test_materialize_reward_extension_configs_writes_matched_matrix(tmp_path: Path) -> None:
    source_root = _source_configs(tmp_path)
    ...
    assert summary["result_class"] == "current_sim_offtrack_recovery_corridor_reward_extension_materialization_pass"
    ...
    assert generated["offtrack_recovery_corridor_repair_protocol"]["stage"] == "offtrack_recovery_corridor_reward_v1"
