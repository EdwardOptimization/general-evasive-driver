...
def _write_trace(path: Path, *, steps: int, scale: float) -> np.ndarray:
...
def _write_source_artifacts(root: Path, *, actor_visible_label: bool = False) -> dict[str, Path]:
        ...
        target_rows.append(
            {
                ...
                "target_labels_actor_visible": actor_visible_label and index == 1,
        ...
        mask_rows.append(
            {
                ...
                "target_action_delta_abs_max": 0.03,
                "target_quality_validated": False,
                "target_labels_actor_visible": actor_visible_label and index == 1,
                "target_provenance_actor_visible": False,
                "fit_candidate_after_audit": True,
                "status_pass": not (actor_visible_label and index == 1),
            }
        )

    success_raw = raw_dir / "success-trace.npz"
    success_target = tensor_dir / "success-target.npz"
    success_obs = _write_trace(success_raw, steps=3, scale=0.0)
    ...
    return {"m2983_dir": m2983_dir, "m2987_dir": m2987_dir, "audit": audit, "design": design}


def test_run_m2990_writes_bounded_fitting_artifacts_and_followup(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(m2990, "EXPECTED_CANDIDATE_COUNT", 2)
    monkeypatch.setattr(m2990, "EXPECTED_SUCCESS_GUARD_COUNT", 1)
    monkeypatch.setattr(m2990, "EXPECTED_STALE_EXCLUSION_COUNT", 1)
    paths = _write_source_artifacts(tmp_path)

    summary = m2990.run_bounded_fitting_preflight(
        m2987_dir=paths["m2987_dir"],
        m2988_audit=paths["audit"],
        m2989_design=paths["design"],
        m2983_dir=paths["m2983_dir"],
        output_dir=tmp_path / "m2990",
        doc_path=tmp_path / "m2990.md",
        follow_up_manifest=tmp_path / "m2991.json",
    )

    assert summary["status_pass"] is True
    assert summary["gate_matrix_pass"] is True
    assert summary["bounded_offline_fitting_run"] is True
    assert summary["validation_run"] is False
    assert summary["ranking_run"] is False
    assert summary["checkpoint_mutated"] is False
    assert summary["fitting_dataset_row_count"] == 2
    assert summary["success_guard_loss_row_count"] == 1
    assert summary["stale_exclusion_audit_row_count"] == 1
    assert summary["final_weighted_mse"] <= summary["initial_weighted_mse"]
    assert Path(summary["candidate_residual_head_artifact"]).exists()
    assert (tmp_path / "m2991.json").exists()
    loss_rows = _read_csv(tmp_path / "m2990" / "fitting_loss_trace_rows.csv")
    assert loss_rows[-1]["status_pass"] == "True"
    assert read_json(tmp_path / "m2990" / "summary.json")["selected_next_action"] == m2990.NEXT_ID


def test_run_m2990_fails_closed_when_target_label_is_actor_visible(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(m2990, "EXPECTED_CANDIDATE_COUNT", 2)
    monkeypatch.setattr(m2990, "EXPECTED_SUCCESS_GUARD_COUNT", 1)
    monkeypatch.setattr(m2990, "EXPECTED_STALE_EXCLUSION_COUNT", 1)
    paths = _write_source_artifacts(tmp_path, actor_visible_label=True)
    ...
    assert summary["bounded_offline_fitting_run"] is False
    assert not Path(summary["candidate_residual_head_artifact"]).exists()
    dataset_rows = _read_csv(tmp_path / "m2990" / "fitting_dataset_rows.csv")
    assert dataset_rows[0]["target_labels_actor_visible"] == "True"
