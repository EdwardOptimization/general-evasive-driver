...
from autodrift import paper_route_outcome_supported_decisive_comparison_support_outcome_localization as loc
...
def _episode(
    *,
    ...
    return {
        ...
        "candidate_id": f"{task_source_id}_candidate",
        ...
        "target_support_tier": "multi_profile_easy_support",
...
def _write_inputs(tmp_path: Path, rows: list[dict[str, object]], *, mismatch: bool = False) -> tuple[Path, Path]:
    ...
    write_json(summary_path, {"result_class": "comparison_support_measured_execution_pass", "outcome_counts": counts})
    write_csv_rows(episode_rows_path, rows)
    return summary_path, episode_rows_path


def test_comparison_support_localization_writes_aggregates_and_reproduces_counts(tmp_path: Path) -> None:
    ...
    summary = loc.localize_comparison_support_outcomes(
        ...
        target_support_tier_count=1,
    )

    assert summary["result_class"] == "comparison_support_outcome_localization_pass"
    ...
    assert (tmp_path / "out" / "outcome_by_profile_intent.csv").exists()
    assert (tmp_path / "out" / "comparison_ready_candidates.csv").exists()
...
def test_comparison_support_localization_fails_closed_on_source_count_mismatch(tmp_path: Path) -> None:
    rows = [
        _episode(
    ...
    assert summary["result_class"] == "comparison_support_outcome_localization_incomplete_or_fail"
...
def test_comparison_support_localization_fails_closed_on_missing_schema(tmp_path: Path) -> None:
    row = _episode(
        workload_id="success_0",
        task_source_id="source_0",
        profile_name="L3_online_gru",
