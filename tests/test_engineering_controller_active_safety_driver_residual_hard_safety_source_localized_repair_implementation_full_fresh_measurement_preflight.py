...
import autodrift.engineering_controller_active_safety_driver_residual_hard_safety_source_localized_repair_implementation_full_fresh_measurement_preflight as m3172


def test_m3172_policy_adapter_telemetry_preserves_direct_action_contract():
    policy = m3172.M3172RepairMeasurementPolicy()
    action = policy.act(np.zeros(m3172.P0_OBSERVATION_DIM, dtype=np.float32), {})
    telemetry = policy.telemetry()

    assert action.shape == (m3172.ACTION_DIM,)
    assert np.all(np.isfinite(action))
    assert np.max(np.abs(action)) <= 1.0
    assert telemetry["runtime_driver_id"] == m3172.POLICY_ID
    assert telemetry["candidate_output_semantics"] == m3172.OUTPUT_SEMANTICS
...
def test_same_row_comparison_records_four_baselines_without_claims():
    episode = {
        "runtime_smoke_episode_id": "m3172-measurement-episode-0001",
    ...
    rows = m3172.same_row_comparison_rows(
        [episode],
        [m3105_baseline],
        [m3095_baseline],
        [m3100_baseline],
    ...
    assert [row["comparison_id"] for row in rows] == [
        "m3172-same-row-comparison-0001",
        "m3172-same-row-comparison-0002",
        "m3172-same-row-comparison-0003",
        "m3172-same-row-comparison-0004",
    ]
...
def test_claim_boundary_blocks_repair_success_and_requires_m3173_audit():
    rows = m3172.claim_boundary_rows(follow_up_manifest_registered=True)
    by_id = {row["claim_id"]: row for row in rows}

    assert by_id["m3172-follow_up_result_audit_registered"]["allowed_in_m3172"] is True
    assert by_id["m3172-follow_up_result_audit_registered"]["claim_made"] is True
    assert by_id["m3172-repair_success"]["allowed_in_m3172"] is False
    assert by_id["m3172-repair_success"]["claim_made"] is False
    assert by_id["m3172-driver_performance_verdict"]["claim_made"] is False
    assert all(row["status_pass"] for row in rows)


def test_follow_up_manifest_is_result_audit_not_validation(tmp_path):
    manifest = m3172.build_follow_up_manifest(
        output_dir=tmp_path / "m3172",
        doc_path=tmp_path / "m3172.md",
    )

    assert manifest["id"] == m3172.NEXT_ID
    assert manifest["priority"] == 31730
    assert manifest["gate_tier"] == "process"
    assert manifest["training_stage"]["stage"] == "process"
    assert (
        manifest["workflow_synthesis"]["branch"]
        == "active_safety_driver_residual_hard_safety_source_localized_repair_measurement"
    )
...
def test_gate_matrix_accepts_m3171_source_localized_route_and_m3170_artifacts():
    source = {
        "source_exists": {
            "m3171_audit": True,
            "m3170_summary": True,
            ...
            "m3170_runtime_contract_rows": True,
            "m3170_action_probe_rows": True,
        },
        "m3171_audit_text": (
            "decision: "
            "accept_m3170_source_localized_repair_implementation_route_to_m3172_full_fresh_measurement_preflight"
        ),
        "m3170_summary": {"status_pass": True, "gate_matrix_pass": True},
        "m3170_policy_config": {
            "policy_id": m3172.POLICY_ID,
            "observation_shape": m3172.P0_OBSERVATION_DIM,
            "action_shape": m3172.ACTION_DIM,
            "output_semantics": m3172.OUTPUT_SEMANTICS,
            "runtime_base_policy_required": False,
        },
        "m3170_gate_rows": [{"status_pass": True}],
        "m3170_source_localized_rule_rows": [{} for _ in range(4)],
        "m3170_runtime_contract_rows": [{"status_pass": True}],
        "m3170_action_probe_rows": [
    ...
    assert by_id["m3172-m3171_accepts_m3172_route"]["status_pass"] is True
    assert by_id["m3172-m3170_status_pass"]["status_pass"] is True
    assert by_id["m3172-m3170_source_localized_rule_rows_present"]["status_pass"] is True
    assert by_id["m3172-m3170_runtime_contract_rows_pass"]["status_pass"] is True
    assert by_id["m3172-m3170_action_probe_rows_pass"]["status_pass"] is True
