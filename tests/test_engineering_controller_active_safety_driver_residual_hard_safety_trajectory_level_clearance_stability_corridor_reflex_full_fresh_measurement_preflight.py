...
import autodrift.engineering_controller_active_safety_driver_residual_hard_safety_trajectory_level_clearance_stability_corridor_reflex_full_fresh_measurement_preflight as m3131


def test_m3131_policy_adapter_telemetry_preserves_direct_action_contract():
    policy = m3131.M3131RepairMeasurementPolicy()
    action = policy.act(np.zeros(m3131.P0_OBSERVATION_DIM, dtype=np.float32), {})
    telemetry = policy.telemetry()

    assert action.shape == (m3131.ACTION_DIM,)
    assert np.all(np.isfinite(action))
    assert np.max(np.abs(action)) <= 1.0
    assert telemetry["runtime_driver_id"] == m3131.POLICY_ID
    assert telemetry["candidate_output_semantics"] == m3131.OUTPUT_SEMANTICS
...
def test_same_row_comparison_records_four_baselines_without_claims():
    episode = {
        "runtime_smoke_episode_id": "m3131-measurement-episode-0001",
    ...
    rows = m3131.same_row_comparison_rows(
        [episode],
        [m3105_baseline],
        [m3095_baseline],
        [m3100_baseline],
    ...
    assert [row["comparison_id"] for row in rows] == [
        "m3131-same-row-comparison-0001",
        "m3131-same-row-comparison-0002",
        "m3131-same-row-comparison-0003",
        "m3131-same-row-comparison-0004",
    ]
...
def test_claim_boundary_blocks_repair_success_and_requires_m3132_audit():
    rows = m3131.claim_boundary_rows(follow_up_manifest_registered=True)
    by_id = {row["claim_id"]: row for row in rows}

    assert by_id["m3131-follow_up_result_audit_registered"]["allowed_in_m3131"] is True
    assert by_id["m3131-follow_up_result_audit_registered"]["claim_made"] is True
    assert by_id["m3131-repair_success"]["allowed_in_m3131"] is False
    assert by_id["m3131-repair_success"]["claim_made"] is False
    assert by_id["m3131-driver_performance_verdict"]["claim_made"] is False
    assert all(row["status_pass"] for row in rows)


def test_follow_up_manifest_is_result_audit_not_validation(tmp_path):
    manifest = m3131.build_follow_up_manifest(
        output_dir=tmp_path / "m3131",
        doc_path=tmp_path / "m3131.md",
    )

    assert manifest["id"] == m3131.NEXT_ID
