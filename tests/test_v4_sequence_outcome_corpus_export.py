...
from autodrift.v4_sequence_outcome_corpus_export import (
    V4_ROLLOUT_FIELDS,
    classify_v4_sequence_outcome_corpus,
    export_v4_sequence_outcome_corpus,
)


def _row(**overrides):
    row = {field: "" for field in V4_ROLLOUT_FIELDS}
    row.update(
        {
            "source_index": "1",
            "source_role": "primary",
...
def _write_csv(path, rows):
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=V4_ROLLOUT_FIELDS, lineterminator="\n")
        writer.writeheader()
...
def test_export_filters_sentinel_and_preserves_v4_metadata(tmp_path):
    normal = _row()
    positive = _row(
    ...
    source_summary.write_text('{"result_class":"v4_reset_sequence_outcome_positive"}\n', encoding="utf-8")

    result = export_v4_sequence_outcome_corpus(
    ...
    assert result["result_class"] == "v4_sequence_outcome_corpus_exported"
    ...
    assert positives[0]["source_kind"] == "v4_reset_source"
    assert positives[0]["source_pool"] == "m749_v4_reset_only"
    assert positives[0]["claim_boundary_level"] == "current_model_or_proxy"
    ...
    assert summary["positive_claim_boundary_levels"] == ["current_model_or_proxy"]
...
def test_classify_v4_sequence_outcome_corpus_artifact_before_balance():
    assert (
        classify_v4_sequence_outcome_corpus(
            positive_rows=1000,
            ...
            positive_rows_missing_v4_metadata=1,
...
def test_classify_v4_sequence_outcome_corpus_hard_negative_sparse():
    assert (
        classify_v4_sequence_outcome_corpus(
            positive_rows=1000,
