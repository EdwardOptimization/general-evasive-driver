import autodrift.engineering_controller_active_safety_driver_residual_hard_safety_action_authority_effectiveness_candidate_residual_trace_measurement_preflight as m3205
...
def _execution(binding_id, trace_execution_id, *, collision=False, offtrack=False, success=False, margin=0.0):
    return {
        ...
        "output_semantics": m3205.OUTPUT_SEMANTICS,
        ...
        "claim_boundary": m3205.CLAIM_SCOPE,
    }
...
def _step(binding_id, trace_execution_id, step_index, *, steer=0.0, throttle=0.0, brake=0.0, obs_hash="hash"):
    return {
        ...
        "task_family": "T5",
        "eval_seed": 100,
        "obs72_dim": m3205.P0_OBSERVATION_DIM,
...
def _source(tmp_path):
    ...
    for index in range(1, 8):
        ...
        m3194_rows.append(
            {
                **_execution(binding_id, f"m3194-{index}", collision=index <= 5, offtrack=index > 5),
                "evidence_axis": evidence_axis,
            }
        )
    ...
    return {
        "source_exists": {
            "m3204_audit": True,
            "m3203_summary": True,
            "m3203_gate_rows": True,
            "m3203_runtime_contract_rows": True,
            "m3199_summary": True,
        ...
        "m3204_audit_text": "routes to M3205 residual-trace measurement",
        "m3203_summary": {"status_pass": True, "gate_matrix_pass": True},
        "m3203_gate_rows": [{"status_pass": "True"}],
        "m3203_runtime_contract_rows": [{"status_pass": "True"}],
        ...
        "m3189_trace_execution_rows": rows,
...
def test_trace_execution_plan_preserves_seven_bindings(tmp_path):
    plan = m3205.trace_execution_plan(_source(tmp_path))
...
def test_same_trace_comparison_rows_compute_outcome_and_action_deltas():
    candidate = [_execution("binding-1", "m3205-1", success=True, margin=1.0)]
    ...
    candidate_steps = [
        ...
        _step("binding-1", "m3205-1", 1, steer=0.2, throttle=-0.3, brake=0.4),
    ]
    m3194_steps = [
        _step("binding-1", "m3194-1", 0, steer=0.0, throttle=0.0, brake=0.0),
        _step("binding-1", "m3194-1", 1, steer=0.1, throttle=-0.1, brake=0.1),
    ]
    incumbent_steps = [
        _step("binding-1", "inc-1", 0, steer=0.0, throttle=0.0, brake=0.0),
        _step("binding-1", "inc-1", 1, steer=0.0, throttle=0.0, brake=0.0),
    ]

    rows = m3205.same_trace_comparison_rows(
        m3205_executions=candidate,
        m3205_steps=candidate_steps,
        m3194_executions=m3194,
        m3194_steps=m3194_steps,
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["outcome_changed_vs_m3194"] is True
    assert row["outcome_changed_vs_incumbent"] is True
    assert row["hard_safety_improved_vs_m3194"] is True
    assert row["hard_safety_improved_vs_incumbent"] is True
    assert row["meaningful_action_delta_step_count_vs_m3194"] == 1
    assert row["meaningful_action_delta_step_count_vs_incumbent"] == 1


def test_gates_contracts_and_claims_accept_complete_pack(tmp_path):
    source = _source(tmp_path)
    plan = m3205.trace_execution_plan(source)
    executions = []
    steps = []
    ...
    comparisons = m3205.same_trace_comparison_rows(
    ...
    guards = m3205.contract_guard_rows(
        source=source,
        plan_rows=plan,
    ...
    claims = m3205.claim_boundary_rows(follow_up_manifest_registered=True)
    gates = m3205.gate_matrix_rows(
        source=source,
        plan_rows=plan,
        executions=executions,
        steps=steps,
...
def test_follow_up_manifest_is_m3206_result_audit(tmp_path):
    manifest = m3205.build_follow_up_manifest(output_dir=tmp_path / "m3205", doc_path=tmp_path / "m3205.md")

    assert manifest["id"] == m3205.NEXT_ID
