from __future__ import annotations

from pathlib import Path

from autodrift.fusion_actor_proposal_repair import run_fusion_actor_proposal_repair


def _write_candidate_summary(path: Path) -> None:
    rows = [
        "candidate_id,alpha,checkpoint,selected_repair_candidate",
        f"alpha02,0.2,{path.parent / 'alpha02.pt'},True",
        f"alpha04,0.4,{path.parent / 'alpha04.pt'},True",
        f"alpha10,1.0,{path.parent / 'alpha10.pt'},True",
    ]
    for filename in ("alpha02.pt", "alpha04.pt", "alpha10.pt"):
        (path.parent / filename).write_text("checkpoint", encoding="utf-8")
    path.write_text("\n".join(rows) + "\n", encoding="utf-8")


def _write_materialization(path: Path) -> None:
    path.mkdir(parents=True)
    (path / "diagnostic_policy_guardrail_rows.csv").write_text("role\n", encoding="utf-8")


def _fake_candidate_summary(*, alpha: float, pass_candidate: bool, restored: bool = True) -> dict[str, object]:
    initial = 0.01 * float(alpha)
    ratio = 0.5 if pass_candidate else 0.0
    repaired = initial * (1.0 - ratio)
    return {
        "candidate_id": f"alpha{alpha}",
        "proposal_source_type": "same_line_interpolation",
        "alpha": float(alpha),
        "proposal_checkpoint": f"alpha{alpha}.pt",
        "feature_mode": "differentiable_feature",
        "trainable_scope": "fusion_actor",
        "initial_positive_exact_residual_mean": initial,
        "repaired_positive_exact_residual_mean": repaired,
        "positive_exact_residual_reduction": initial - repaired,
        "positive_exact_residual_reduction_ratio": ratio,
        "initial_positive_action_l2_max": initial * 10.0,
        "repaired_positive_action_l2_max": repaired * 10.0,
        "initial_fusion_actor_l2_to_base": alpha,
        "repaired_fusion_actor_l2_to_base": alpha * 0.9,
        "excluded_parameter_delta_max": 0.0,
        "model_restored_after_probe": restored,
        "accepted_backtracking_step_count": 1 if pass_candidate else 0,
        "projection_stop_reason": "target_reduction_reached" if pass_candidate else "no_backtracking_candidate_accepted",
        "passes_candidate_gate": pass_candidate and restored,
        "null_result_classification": "fusion_actor_candidate_repair_public_pass"
        if pass_candidate and restored
        else "residual_not_reduced",
        "guardrail_violation_count": 0,
    }


def _run(tmp_path: Path, *, primary_pass: bool = True, restored: bool = True):
    candidate_summary = tmp_path / "candidate_summary.csv"
    materialization = tmp_path / "materialization"
    _write_candidate_summary(candidate_summary)
    _write_materialization(materialization)

    def repair_fn(**kwargs):
        run_dir = Path(kwargs["run_dir"])
        run_dir.mkdir(parents=True)
        alpha = float(kwargs["alpha"])
        return _fake_candidate_summary(alpha=alpha, pass_candidate=primary_pass or alpha != 0.2, restored=restored)

    return run_fusion_actor_proposal_repair(
        base_checkpoint=tmp_path / "base.pt",
        candidate_summary=candidate_summary,
        materialization_run_dir=materialization,
        run_dir=tmp_path / "run",
        repair_fn=repair_fn,
    )


def test_fusion_actor_repair_aggregates_public_pass(tmp_path: Path) -> None:
    summary = _run(tmp_path)

    assert summary["passes_public_smoke_gates"] is True
    assert summary["selected_candidate_count"] == 3
    assert summary["candidate_public_pass_count"] == 3
    assert summary["primary_alpha_0_2_pass"] is True
    assert summary["model_restored_after_probe_count"] == 3
    assert summary["null_result_classification"] == "fusion_actor_proposal_repair_public_pass"
    assert (tmp_path / "run" / "summary.json").exists()
    assert (tmp_path / "run" / "candidate_summary.csv").exists()
    assert (tmp_path / "run" / "guardrail_summary.csv").exists()


def test_fusion_actor_repair_fails_when_primary_fails(tmp_path: Path) -> None:
    summary = _run(tmp_path, primary_pass=False)

    assert summary["passes_public_smoke_gates"] is False
    assert summary["primary_alpha_0_2_pass"] is False
    assert summary["null_result_classification"] == "primary_alpha_0_2_not_repaired"


def test_fusion_actor_repair_fails_when_model_not_restored(tmp_path: Path) -> None:
    summary = _run(tmp_path, restored=False)

    assert summary["passes_public_smoke_gates"] is False
    assert summary["model_restored_after_probe_count"] == 0
    assert summary["null_result_classification"] == "model_restore_guardrail_failure"
