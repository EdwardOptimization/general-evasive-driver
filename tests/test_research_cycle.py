...
def test_plan_next_task_reports_planned_when_no_pending_task(tmp_path):
    queue_path = tmp_path / "queue.csv"
    status_path = tmp_path / "status.json"
    status_path.write_text(
        json.dumps({"last_result": {"task_id": "previous", "status": "completed", "returncode": 0}}),
    ...
    queue_path.write_text(
        "\n".join(
            [
                "id,priority,status,kind,hypothesis,command,success_artifact,notes",
                "done,1,completed,benchmark,done echo,echo done,,",
                "planned-later,30,planned,training,later echo,,,",
                "planned-next,10,planned,gate,next echo,,,",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert task is not None
    assert task.id == "planned-next"
    assert status["next_task"]["id"] == "planned-next"
    assert status["counts"]["planned"] == 2
...
def test_run_next_task_idle_reports_planned_next_task(tmp_path):
    queue_path = tmp_path / "queue.csv"
    status_path = tmp_path / "status.json"
    log_path = tmp_path / "research-log.md"
    queue_path.write_text(
