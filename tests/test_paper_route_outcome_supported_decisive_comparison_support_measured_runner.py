...
from autodrift import paper_route_outcome_supported_decisive_comparison_support_measured_runner as runner
...
def _spec(task_id: str, *, generated: bool = False) -> dict[str, object]:
    return {
        ...
        "scenario_redesign_branch": "paper_route_outcome_supported_decisive_comparison_support_scenario_redesign",
        "comparison_support_intent": "support_ladder_easy" if not generated else "discriminative_boundary",
        "target_support_tier": "multi_profile_easy_support" if not generated else "profile_discriminative_support",
        ...
        "dynamics_band": "nominal_mu",
        "obstacle_timing_band": "early",
...
def test_comparison_support_measured_runner_preserves_metadata(tmp_path: Path) -> None:
    specs_path, workload_path = _write_inputs(tmp_path)

    summary = runner.run_comparison_support_measured_execution(
    ...
    assert summary["result_class"] == "comparison_support_measured_execution_pass"
    assert summary["episode_count"] == 4
    assert summary["failure_count"] == 0
    assert summary["metadata_missing_count"] == 0
    assert summary["intent_quota_pass"] is True
    assert summary["target_support_tier_quota_pass"] is True
    ...
    assert "comparison_support_intent" in episode_rows
    assert "same_current_brake_authority_older_history_proxy" in episode_rows
    assert "generated=true|semantics=comparison_support_smoke_proxy|paper_claim=false" in summary["generated_proxy_counts"]
    assert (tmp_path / "out" / "intent_aggregate.csv").exists()
    assert (tmp_path / "out" / "target_support_tier_aggregate.csv").exists()


def test_comparison_support_measured_runner_preserves_failures(tmp_path: Path) -> None:
    ...
    assert summary["result_class"] == "comparison_support_measured_execution_incomplete_or_fail"
...
def test_comparison_support_measured_runner_uses_optional_eval_seed_override(tmp_path: Path) -> None:
    specs_path, workload_path = _write_inputs(tmp_path)
    ...
    assert seen[sorted_ids[0]] == 212300
    assert seen[sorted_ids[1]] == 999001
    assert seen[sorted_ids[2]] == 212302


def test_comparison_support_measured_runner_fails_closed_on_schema_mismatch(tmp_path: Path) -> None:
    specs_path = tmp_path / "specs.json"
    workload_path = tmp_path / "workload.csv"
